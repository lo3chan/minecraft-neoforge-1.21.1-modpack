...
public class VillagerBrainEventInternal {
   ...
   public void addSensor(SensorType<? extends Sensor<Villager>> newSensor) {
      try {
         ...
         Sensor<Villager> sensorInstance = newSensor.create();
         ...
         Moonlight.LOGGER.warn("failed to register pumpkin sensor type for villagers: {}", String.valueOf(var7));
      }
   }
   ...
   public <P extends Pair<Integer, ? extends Behavior<Villager>>> boolean addTaskToActivity(Activity activity, P task) {
      try {
         ...
         Map<Activity, Set<Behavior<Villager>>> tasksWithSamePriority = map.computeIfAbsent((Integer)task.getFirst(), m -> Maps.newHashMap());
         Set<Behavior<Villager>> activityTaskSet = tasksWithSamePriority.computeIfAbsent(activity, a -> Sets.newLinkedHashSet());
         activityTaskSet.add((Behavior<Villager>)task.getSecond());
         return true;
