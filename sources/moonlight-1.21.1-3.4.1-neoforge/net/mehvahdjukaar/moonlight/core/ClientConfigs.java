...
public class ClientConfigs {
   ...
   public static final Supplier<ClientConfigs.TooltipMode> TAGS_TOOLTIP;
   public static final Supplier<Boolean> CUSTOM_CONFIG_SCREEN;
   public static final Supplier<Boolean> SHOW_ALL_MOD_CONFIGS;
   public static final Supplier<ClientConfigs.ForeignConfigMode> CONVERT_FOREIGN_CONFIGS;
   ...
   static {
      ...
      LAZY_MAP_DATA = builder.comment("Prevents map texture from being upladed to GPU when only map markers have changed.Could increase performance")
         .define("lazy_map_upload", true);
      MAPS_MIPMAP = builder.comment(
      ...
      FIX_SHADE = builder.comment(
         ...
         .define("consistent_entity_renderer_shading", ClientConfigs.ShadeFix.NO_GUI);
      ...
      TAGS_TOOLTIP = builder.comment("Show Item and Block tags on item tooltip")
         .define("tags_tooltips", PlatHelper.isDev() ? ClientConfigs.TooltipMode.ON : ClientConfigs.TooltipMode.OFF);
      builder.pop();
      builder.push("config_screen");
      CUSTOM_CONFIG_SCREEN = builder.comment(
            "Use Moonlight's config screen. When off, configs open the loader's screen instead: NeoForge's own (or Configured), Cloth Config or YACL on Fabric"
         )
         .define("custom_config_screen", true);
      SHOW_ALL_MOD_CONFIGS = builder.comment(
            "Give every installed mod with a config screen a tile, not just the ones using Moonlight. Their own screen opens when clicked"
         )
         .define("show_all_mod_configs", false);
      CONVERT_FOREIGN_CONFIGS = builder.comment(
            "Draw other mods' configs inside Moonlight's screen instead of theirs (NeoForge only). GENERIC_ONLY covers just the mods that never wrote a screen of their own, ALWAYS covers every mod. Per world server configs are only shown while a world is open. Best effort: options we can't show are left as they are"
         )
         .define("convert_foreign_configs", ClientConfigs.ForeignConfigMode.GENERIC_ONLY);
      CONFIG_ITEM_CAROUSEL = builder.comment("Show a slowly panning strip of a mod's items on its config screen").define("config_item_carousel", true);
      builder.pop();
      CONFIG = builder.build();
      CONFIG.forceLoad();
   }

   public static enum ForeignConfigMode {
      NEVER,
      GENERIC_ONLY,
      ALWAYS;
