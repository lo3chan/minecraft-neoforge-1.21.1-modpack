...
public class DynamicResourcesInternals {
   ...
   public static void init() {
      MoonlightEventsHelper.addListener(
         earlyPackReloadEvent -> {
            ...
            List<DynResourceGenerator<?>> validGen = GENERATORS.stream().filter(genx -> genx.dynamicPack.getPackType() == type).toList();
            if (!validGen.isEmpty()) {
               List<String> modIds = validGen.stream().map(DynResourceGenerator::getModId).toList();
               Moonlight.LOGGER.info("Starting Legacy Runtime Resource Generation for pack type {} with generators from mods {}: {}", type, modIds, validGen);
               Stopwatch stopwatch = Stopwatch.createStarted();
               IProgressTracker reporter = earlyPackReloadEvent.progress();

               for (DynResourceGenerator<?> gen : validGen) {
                  gen.onEarlyReload(earlyPackReloadEvent, reporter);
               }

               Moonlight.LOGGER
                  .info("Finished Legacy Runtime Resources Generation for {} packs in a total of {} ms", GENERATORS.size(), stopwatch.elapsed().toMillis());
            }
         },
      ...
      MoonlightEventsHelper.addListener(earlyPackReloadEvent -> {
         ...
         if (!validGen.isEmpty()) {
            Collection<PackResources> selectedPacks = earlyPackReloadEvent.selectedPacks();
            GlobalCachedStrategy.refreshState(type, selectedPacks);

            for (DynamicResourcesProvider p : PROVIDERS.get(type)) {
               p.prepare();
            }

            List<ResourceLocation> modIds = validGen.stream().map(DynamicResourcesProvider::getName).toList();
            Moonlight.LOGGER.info("Starting runtime resource generation for pack type {} with generators {}", type, modIds);
            ...
            for (DynamicResourcesProvider gen : validGen) {
               if (!gen.canUseExternalResourcePacks()) {
                  if (vanillaManager == null) {
                     vanillaManager = FilteredResManager.vanilla(manager, type);
                  }

                  gen.reload(vanillaManager, reporter);
               } else {
                  gen.reload(manager, reporter);
               }
            }

            Moonlight.LOGGER.info("Finished runtime resources generation for {} packs in a total of {} ", validGen.size(), stopwatch);
   ...
   public static void registerProvider(DynamicResourcesProvider provider) {
      ...
      for (DynamicResourcesProvider p : PROVIDERS.get(packType)) {
