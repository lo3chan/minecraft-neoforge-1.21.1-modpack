...
public class CommonConfigs {
   ...
   private static final SchemaCodec<CommonConfigs.Nested> NESTED_SCHEMA = SchemaRecord.create(
      CommonConfigs.Nested.class,
      i -> i.group(
            i.field("x", SchemaCodecs.intRange(-16, 16), CommonConfigs.Nested::x),
            i.field("weight", SchemaCodecs.doubleRange(0.0, 1.0), CommonConfigs.Nested::weight),
            i.field("speed", SchemaCodecs.floatRange(0.0F, 10.0F), CommonConfigs.Nested::speed)
         )
         .apply(i, CommonConfigs.Nested::new)
   );
   private static final SchemaCodec<CommonConfigs.SchemaTest> SCHEMA_TEST = SchemaRecord.create(
      CommonConfigs.SchemaTest.class,
      i -> i.group(
            i.field("name", SchemaCodecs.STRING, CommonConfigs.SchemaTest::name),
            i.field("enabled", SchemaCodecs.BOOL, CommonConfigs.SchemaTest::enabled),
            i.field("level", SchemaCodecs.intRange(0, 100), CommonConfigs.SchemaTest::level),
            i.field(
               "facing", SchemaCodecs.enumeration(Direction.CODEC, List.of(Direction.values()), Direction::getSerializedName), CommonConfigs.SchemaTest::facing
            ),
            i.field("color", SchemaCodecs.colorArgb(Codec.INT), CommonConfigs.SchemaTest::color),
            i.field("item", SchemaCodecs.registryEntry(Registries.ITEM, BuiltInRegistries.ITEM.byNameCodec()), CommonConfigs.SchemaTest::item),
            i.field("tag", SchemaCodecs.tag(Registries.ITEM), CommonConfigs.SchemaTest::tag),
            i.field("tags", SchemaCodecs.list(SchemaCodecs.STRING), CommonConfigs.SchemaTest::tags),
            i.field("nested", NESTED_SCHEMA, CommonConfigs.SchemaTest::nested),
            i.field("entries", SchemaCodecs.list(NESTED_SCHEMA), CommonConfigs.SchemaTest::entries),
            i.field("extra", SchemaCodecs.map(SchemaCodecs.STRING, SchemaCodecs.STRING), CommonConfigs.SchemaTest::extra)
         )
         .apply(i, CommonConfigs.SchemaTest::new)
   );
   ...
   static {
      ...
      MULTI_THREADED_GENERATION = builder.comment(
      ...
      EXTRA_CHILDREN_DEBUG = builder.comment(
      ...
      FASTER_CACHE_SEARCH = builder.comment(
      ...
      GLOBAL_DATAPACKS_DIR = builder.comment(
      ...
      if (PlatHelper.isDev()) {
         ...
         builder.icon("minecraft:lever")
            .comment("A boolean feature, drawn as a check/cross toggle with its icon next to the symbol")
         ...
         builder.comment("A double, edited as a text field").define("test_double", 2.0, 0.0, 22.0);
         ...
         builder.comment("A min/max range shown as two fields on one row").defineRange("test_range", Range.of(2.0, 8.0), 0.0, 10.0);
         builder.comment("A Vec3 shown as three x/y/z fields on one row").defineVec3("test_vec3", new Vec3(0.5, 1.0, -0.5), -10.0, 10.0);
         ...
         builder.comment("An ARGB color, edited as a hex field").defineColor("test_color", -43691);
         builder.comment("An RGB color with no alpha channel").defineColor("test_color_no_alpha", 2783210, false);
         ...
         builder.comment("An item list, each entry picked from an item dropdown with icons")
            .defineItemList("test_item_list", List.of(ResourceLocation.parse("minecraft:diamond"), ResourceLocation.parse("minecraft:emerald")));
         ...
         builder.comment("A plain Java bean (no codec needed), stored and edited as JSON").defineBean("test_bean", new CommonConfigs.TestBean());
         builder.comment("A record bean, also round-tripped through Gson").defineBean("test_record_bean", new CommonConfigs.TestRecordBean("world", 7));
         builder.comment(
               "A codec object with a declared CodecUI schema, edited via a generated form: records and lists become navigable sub pages, ids and tags get registry dropdowns, and the map falls back to the JSON editor"
            )
            .defineObject(
               "test_schema",
               () -> new CommonConfigs.SchemaTest(
                  "hello",
                  true,
                  5,
                  Direction.NORTH,
                  ...
                  ItemTags.LOGS,
                  List.of("alpha", "beta"),
                  new CommonConfigs.Nested(1, 0.5, 1.0F),
                  List.of(new CommonConfigs.Nested(2, 0.25, 2.0F), new CommonConfigs.Nested(3, 0.75, 4.0F)),
         ...
         builder.comment("A codec object LIST: the config's own page is the list, so entries are added and removed right there")
            .defineObjectList("test_schema_list", () -> List.of(new CommonConfigs.Nested(4, 0.5, 1.0F)), NESTED_SCHEMA);
         builder.icon("minecraft:oak_log").push("nested");
         builder.comment("A float value living in a nested sub category").define("nested_float", 0.5F, 0.0F, 1.0F);
         builder.pop();
         builder.icon("minecraft:redstone").pushFeature("test_feature", true);
         builder.comment("Only meaningful while the feature is on").define("feature_speed", 1.0, 0.0, 10.0);
   ...
   public record SchemaTest(
      ...
      TagKey<Item> tag,
      List<String> tags,
      CommonConfigs.Nested nested,
      List<CommonConfigs.Nested> entries,
