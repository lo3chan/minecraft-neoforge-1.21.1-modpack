...
public interface SelfCustomBakedModel extends IDynamicBakedModel, CustomBakedModel {
   ...
   default ModelData getModelData(@NotNull BlockAndTintGetter level, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData modelData) {
      ...
      ExtraModelDataImpl wrapped = (ExtraModelDataImpl)this.getModelData(
         (ExtraModelData)Preconditions.checkNotNull(d, "tile entity model data was null.How?"), pos, state, level
      );
      return ((ExtraModelDataImpl)Preconditions.checkNotNull(wrapped, "getModelData returned null. How?")).data();
   }
}
