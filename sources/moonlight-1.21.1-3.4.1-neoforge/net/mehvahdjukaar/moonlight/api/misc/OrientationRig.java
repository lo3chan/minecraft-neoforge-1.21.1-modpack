...
public class OrientationRig {
   ...
   public Quaternionf getRotation(float partialTicks) {
      return new Quaternionf(this.prevRotation).slerp(this.rotation, partialTicks);
   }

   public Quaternionf getWantedRotation() {
      return new Quaternionf(this.wantedRotation);
   }

   public void pointToward(Vec3 target) {
      Vector3f t = target.toVector3f();
      t.normalize();
      Quaternionf targetRotation = new Quaternionf().lookAlong(new Vector3f(t).negate(), new Vector3f(0.0F, 1.0F, 0.0F));
