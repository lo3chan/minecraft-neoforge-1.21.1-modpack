...
public abstract class BaseColor<T extends BaseColor<T>> {
   ...
   public float distTo(T other) {
      return (float)Math.sqrt(
         (this.v0 - other.v0) * (this.v0 - other.v0) + (this.v1 - other.v1) * (this.v1 - other.v1) + (this.v2 - other.v2) * (this.v2 - other.v2)
      );
   }
   ...
   public static <C extends BaseColor<C>> C mixColors(List<C> colors) {
      ...
      C mixed = (C)colors.getFirst();

      for (int i = 1; i < size; i++) {
         mixed = mixed.mixWith(colors.get(i), 1.0F / (i + 1.0F));
      }
   ...
   protected static float averageAngles(Float... angles) {
      ...
      for (int var5 = 0; var5 < var4; var5++) {
         ...
         x += Mth.cos((float)(ax * 3.141592653589793 * 2.0));
         y += Mth.sin((float)(ax * 3.141592653589793 * 2.0));
      }

      double ax = Math.atan2(y, x) / 6.283185307179586;
      return (float)ax;
   }
