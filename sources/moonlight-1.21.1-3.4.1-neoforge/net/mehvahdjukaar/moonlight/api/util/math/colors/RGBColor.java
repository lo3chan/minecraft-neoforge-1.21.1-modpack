...
public class RGBColor extends BaseColor<RGBColor> {
   public RGBColor(int value) {
      this(getR(value) / 255.0F, getG(value) / 255.0F, getB(value) / 255.0F, getA(value) / 255.0F);
   }
   ...
   public RGBColor multiply(float red, float green, float blue, float alpha) {
      return new RGBColor(
         Mth.clamp(red * this.red(), 0.0F, 1.0F),
         Mth.clamp(green * this.green(), 0.0F, 1.0F),
         Mth.clamp(blue * this.blue(), 0.0F, 1.0F),
