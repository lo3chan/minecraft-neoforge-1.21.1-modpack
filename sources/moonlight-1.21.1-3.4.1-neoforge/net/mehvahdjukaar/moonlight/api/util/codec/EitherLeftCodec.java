...
public final class EitherLeftCodec<A, B> implements Codec<Either<A, B>> {
   ...
   public <T> DataResult<Pair<Either<A, B>, T>> decode(DynamicOps<T> dynamicOps, T t) {
      return this.leftCodec.decode(dynamicOps, t).map(pair -> pair.mapFirst(Either::left));
   }

   public <T> DataResult<T> encode(Either<A, B> abEither, DynamicOps<T> dynamicOps, T t) {
      return abEither.left().map(a -> this.leftCodec.encode(a, dynamicOps, t)).orElseGet(() -> DataResult.error(() -> "Expected left value"));
   }

   @Override
   public String toString() {
      return "EitherLeftCodec[" + this.leftCodec + "]";
   }
}
