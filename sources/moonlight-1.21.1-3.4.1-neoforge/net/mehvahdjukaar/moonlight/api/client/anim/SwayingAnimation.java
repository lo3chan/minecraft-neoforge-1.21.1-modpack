...
public class SwayingAnimation extends SwingAnimation {
   ...
   public void tick(boolean inWater) {
      ...
      float k = 0.01F;
      if (timer < 800.0) {
         a = (float)Math.max(this.maxSwingAngle * Math.exp(-(timer / this.angleDamping)), (double)this.minSwingAngle);
         k = (float)Math.max(6.283185307179586 * (float)Math.exp(-(timer / this.periodDamping)), 0.009999999776482582);
      }

      this.angle = a * Mth.cos((float)(timer / this.maxPeriod - k));
   ...
   public boolean hit(Vec3 mot, double eMass) {
      if (mot.length() > 0.05) {
         ...
         Vec3 vec = new Vec3(this.rotationAxis.get());
