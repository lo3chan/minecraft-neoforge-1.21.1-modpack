...
public class ParticleUtil {
   public static void spawnParticleInASphere(
      Level level, double x, double y, double z, Supplier<ParticleOptions> type, int amount, float speed, float angleVariation, float speedVariation
   ) {
      double azimuthIncrement = 3.141592653589793 * (3.0 - Math.sqrt(5.0));

      for (int i = 0; i < amount; i++) {
         double inclination = Math.acos(1.0 - 2.0 * (i + 0.5) / amount);
         double azimuth = azimuthIncrement * i;
         if (angleVariation != 0.0F) {
            inclination += level.random.nextFloat() * angleVariation - angleVariation / 2.0F;
            azimuth += level.random.nextFloat() * angleVariation - angleVariation / 2.0F;
         }

         float s = speed;
         if (speedVariation != 0.0F) {
            s = speed + (level.random.nextFloat() * speedVariation - speedVariation / 2.0F);
         }
   ...
   public static void spawnParticleOnBlockShape(Level level, BlockPos pos, ParticleOptions particleOptions, UniformInt uniformInt, float maxSpeed) {
      spawnParticleOnBoundingBox(level.getBlockState(pos).getShape(level, pos).bounds().move(pos), level, particleOptions, uniformInt, maxSpeed);
   }
   ...
   public static void spawnParticleOnBoundingBox(AABB bb, Level level, ParticleOptions particleOptions, UniformInt uniformInt, float maxSpeed) {
      ...
      Vec3 blockCenter = new Vec3(
         bb.minX - 0.5 + (bb.maxX - bb.minX) / 2.0, bb.minY - 0.5 + (bb.maxY - bb.minY) / 2.0, bb.minZ - 0.5 + (bb.maxZ - bb.minZ) / 2.0
      );
   ...
   public static void spawnParticlesOnBlockFaces(
      Level level, BlockPos pos, ParticleOptions particleOptions, UniformInt uniformInt, float minSpeed, float maxSpeed, boolean perpendicular
   ) {
   ...
   public static void spawnParticleOnFace(
      Level level, BlockPos pos, Direction direction, ParticleOptions particleOptions, float minSpeed, float maxSpeed, boolean perpendicular
   ) {
      Vec3 vec3 = Vec3.atCenterOf(pos);
      int i = direction.getStepX();
      int j = direction.getStepY();
      int k = direction.getStepZ();
      double d0 = vec3.x + (i == 0 ? Mth.nextDouble(level.random, -0.5, 0.5) : i * 0.6);
      double d1 = vec3.y + (j == 0 ? Mth.nextDouble(level.random, -0.5, 0.5) : j * 0.6);
      double d2 = vec3.z + (k == 0 ? Mth.nextDouble(level.random, -0.5, 0.5) : k * 0.6);
      double dx;
      double dy;
      double dz;
      if (perpendicular) {
         ...
         dx = i == 0 ? minSpeed + d * level.random.nextDouble() : 0.0;
         dy = j == 0 ? minSpeed + d * level.random.nextDouble() : 0.0;
         dz = k == 0 ? minSpeed + d * level.random.nextDouble() : 0.0;
      }
            ...
            for (int l = 0; l < i; l++) {
               for (int i1 = 0; i1 < j; i1++) {
                  for (int j1 = 0; j1 < k; j1++) {
                     ...
                     particleEngine.add(
                        new TerrainParticle(
                           (ClientLevel)level, pPos.getX() + d7, pPos.getY() + d8, pPos.getZ() + d9, d4 - 0.5, d5 - 0.5, d6 - 0.5, pState, pPos
                        )
                     );
                  }
               }
