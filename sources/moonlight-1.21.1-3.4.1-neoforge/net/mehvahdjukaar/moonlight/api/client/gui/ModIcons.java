...
public final class ModIcons {
   private static final Map<String, Optional<ModIcons.Icon>> CACHE = new HashMap<>();
   private static final List<String> GUESSED_ICON_PATHS = List.of(
      "icon.png", "logo.png", "%s.png", "%s-icon.png", "%s_icon.png", "%s-logo.png", "%s_logo.png", "assets/%s/icon.png", "assets/%s/logo.png", "pack.png"
   );

   @Nullable
   public static ModIcons.Icon get(String modId) {
      return CACHE.computeIfAbsent(modId, ModIcons::load).orElse(null);
   }

   private static Optional<ModIcons.Icon> load(String modId) {
      try {
         Path path = PlatHelper.getModIcon(modId);
         if (path == null) {
            path = guessIcon(modId);
         }
         ...
         if (path == null) {
            ...
            return Optional.of(new ModIcons.Icon(id, image.getWidth(), image.getHeight()));
         }
      } catch (Exception var4) {
         Moonlight.LOGGER.warn("Failed to load mod icon for {}", modId, var4);
         return Optional.empty();
      }
   }
