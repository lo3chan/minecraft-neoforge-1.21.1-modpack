...
public class BakedQuadsTransformerImpl implements BakedQuadsTransformer {
   ...
   public BakedQuad transform(BakedQuad quad) {
      ...
      BakedQuad newQuad = new BakedQuad(v, tint, this.directionRemap.apply(quad.getDirection()), sprite, shade, ambientOcclusion);
   ...
   private IQuadTransformer applyingSpriteInplace(TextureAtlasSprite sprite) {
      return q -> {
         ...
         int[] v = q.getVertices();
         float segmentWScale = (float)sprite.contents().width() / oldSprite.contents().width();
         float segmentHScale = (float)sprite.contents().height() / oldSprite.contents().height();
