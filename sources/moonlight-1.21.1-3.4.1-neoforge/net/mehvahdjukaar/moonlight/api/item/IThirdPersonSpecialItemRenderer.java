...
public interface IThirdPersonSpecialItemRenderer {
   ...
   static IThirdPersonSpecialItemRenderer get(Item target) {
      if (target instanceof IThirdPersonSpecialItemRenderer p) {
         return p;
      } else {
         return ((IExtendedItem)target).moonlight$getClientAnimationExtension() instanceof IThirdPersonSpecialItemRenderer p ? p : null;
      }
   }
}
