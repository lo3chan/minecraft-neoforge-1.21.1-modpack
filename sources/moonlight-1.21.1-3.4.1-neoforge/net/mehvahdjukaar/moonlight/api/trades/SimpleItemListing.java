...
public record SimpleItemListing(
   ItemCost price, Optional<ItemCost> price2, ItemStack offer, int maxTrades, int xp, float priceMult, int level, LootItemFunction func
) implements ModItemListing {
   public static final MapCodec<SimpleItemListing> CODEC = RecordCodecBuilder.mapCodec(
   ...
   public static SimpleItemListing createDefault(
      ItemCost price,
      Optional<ItemCost> price2,
      ItemStack offer,
      int maxTrades,
      Optional<Integer> xp,
      float priceMult,
      int level,
      Optional<LootItemFunction> func
   ) {
      boolean buying = offer.is(Items.EMERALD);
      return new SimpleItemListing(price, price2, offer, maxTrades, xp.orElse(ModItemListing.defaultXp(buying, level)), priceMult, level, func.orElse(null));
   }

   public MerchantOffer getOffer(Entity entity, RandomSource randomSource) {
      Codec<? extends LootItemFunction> TYPED_CODEC = BuiltInRegistries.LOOT_FUNCTION_TYPE
         .byNameCodec()
         .dispatch("function", LootItemFunction::getType, LootItemFunctionType::codec);
      ...
      if (this.func != null && entity.level() instanceof ServerLevel serverLevel) {
         ...
         LootContext context = new net.minecraft.world.level.storage.loot.LootContext.Builder(lootParams)
      ...
      return new MerchantOffer(this.price, this.price2, stack.get(), this.maxTrades, this.xp, this.priceMult);
   }
