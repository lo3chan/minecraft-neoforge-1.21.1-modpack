...
abstract class AbstractOptionRow extends BaseWidget implements ContainerEventHandler, OptionRow, ControlGuideProvider {
   ...
   private static final int SEARCH_RESULT_MARKER = 1728053247;
   ...
   public ComponentPath nextFocusPath(FocusNavigationEvent navigation) {
      ...
      if (result.handled()) {
         ...
         return this.getOption().isEnabled() && !this.isFocused() ? ComponentPath.leaf(this) : null;
      }
   }
   ...
   public void updateNarration(NarrationElementOutput builder) {
      ...
      Component title = (Component)(value == null ? this.getOption().getName() : CommonComponents.optionNameValue(this.getOption().getName(), value));
   ...
   protected boolean isMouseOverRow(double mouseX, double mouseY) {
      return mouseX >= this.getX() && mouseX < this.controlLimitX() && mouseY >= this.getY() && mouseY < this.getLimitY();
   }

   protected int centeredTextY() {
      return (int)(this.getY() + Math.ceil((this.getHeight() - 9) / 2.0));
   }
   ...
   protected MutableComponent formatDisabledControlValue(Component value) {
      return value.copy().withStyle(Style.EMPTY.withColor(ChatFormatting.GRAY).withItalic(true));
   }
   ...
   private void renderRow(GuiGraphics guiGraphics, int mouseX, int mouseY) {
      ...
      this.drawRect(guiGraphics, this.getX(), this.getY(), rowLimitX, this.getLimitY(), !this.hovered && !selectedSearchResult ? 1073741824 : -536870912);
   ...
   private void renderSearchResultMarker(GuiGraphics guiGraphics, @Nullable OptionUiState optionUiState) {
      if (optionUiState != null && optionUiState.isHighlighted() && !optionUiState.isSelected()) {
         this.drawRect(guiGraphics, this.getX(), this.getY(), this.getX() + 2, this.getLimitY(), 1728053247);
      }
   }
   ...
   private OptionUiState optionUiState() {
      return this.option instanceof OptionExtended optionExtended ? this.optionStateStore.optionUiState(optionExtended.rso$getId()) : null;
   }
   ...
   private String formatLabel(Option option, String label) {
      ...
      if (option.isEnabled()) {
         formattedLabel = option.hasChanged() ? ChatFormatting.ITALIC + label : ChatFormatting.WHITE + label;
      } else {
