...
final class OptionRowFactory {
   ...
   private OptionRow createOptionRow(Option option, LayoutBounds dim) {
      ...
      return (OptionRow)(switch (option) {
         ...
         case IntegerOption integerOption when control instanceof SliderControl -> new IntegerSliderOptionRow(
            dim, this.theme, this.optionStateStore, integerOption
         );
         ...
         case ExternalButtonOption externalButtonOption when control instanceof ExternalButtonControl -> new ExternalButtonOptionRow(
            this.screen, dim, this.theme, this.optionStateStore, externalButtonOption
         );
   ...
   private OptionRow createSodiumFallbackRow(Option option, LayoutBounds dim) {
      try {
         ...
         LOGGER.warn("Could not create Sodium fallback row for option type {}; rendering it as unsupported", option.getClass().getName(), var4);
