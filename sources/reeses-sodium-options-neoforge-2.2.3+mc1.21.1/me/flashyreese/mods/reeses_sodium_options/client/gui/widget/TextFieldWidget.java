...
public class TextFieldWidget extends BaseWidget implements ControlGuideProvider {
   ...
   public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
      if (this.isVisible()) {
         ...
         this.drawRect(
            ...
            this.getLimitX(),
            this.getLimitY(),
            this.isFocused() ? GuiThemes.DEFAULT_BUTTON.bgHighlight : GuiThemes.DEFAULT_BUTTON.bgDefault
         );
         ...
         if (!displayedText.isEmpty()) {
            ...
            guiGraphics.drawString(this.font, formatted(preCursorText), textStartX, textStartY, -1);
            textEndX = textStartX + this.font.width(formatted(preCursorText));
         }
         ...
         if (!displayedText.isEmpty() && isCursorWithinDisplayedText && selectionStartOffset < displayedText.length()) {
            guiGraphics.drawString(this.font, formatted(displayedText.substring(selectionStartOffset)), textEndX, textStartY, -1);
         }

         if (this.isFocused()) {
            int color = (int)(this.currentCursorAlpha * 255.0F) << 24 | 13684944;
   ...
   public void setSelectionEnd(int index) {
      ...
      if (this.selectionEnd == this.firstCharacterIndex) {
         this.firstCharacterIndex = this.firstCharacterIndex - this.font.plainSubstrByWidth(this.text, innerWidth, true).length();
      }

      if (this.selectionEnd > endIndex) {
         this.firstCharacterIndex = this.firstCharacterIndex + (this.selectionEnd - endIndex);
   ...
   private void updateCursorAlpha() {
      if (ReeseSodiumOptionsConfig.config().isReducedMotion()) {
         ...
         float cursorAlpha = (float)(this.nextCursorUpdate - currentTimeMillis) / 750.0F;
         if (cursorAlpha <= 0.25F) {
            cursorAlpha *= 4.0F;
         } else if (cursorAlpha >= 0.75F) {
            cursorAlpha = (1.0F - cursorAlpha) * 4.0F;
         } else {
            cursorAlpha = 1.0F;
         }
   ...
   public ComponentPath nextFocusPath(FocusNavigationEvent navigation) {
      return !this.visible ? null : super.nextFocusPath(navigation);
   }

   @Override
   public void updateNarration(NarrationElementOutput builder) {
      Component label = (Component)(this.placeholder == null ? Component.empty() : this.placeholder);
      builder.add(NarratedElementType.TITLE, Component.translatable("gui.narrate.editBox", new Object[]{label, this.text}));
   }
}
