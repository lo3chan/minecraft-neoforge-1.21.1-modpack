...
final class TabRail {
   ...
   void updateScrollState(LayoutBounds frameDim) {
      ...
      if (!this.canScroll) {
         ...
         this.scrollBar = new ScrollBarWidget(
            new LayoutBounds(this.dim.getLimitX() - 11, frameDim.y(), 10, frameDim.height()),
   ...
   private void addTabButton(List<GuiEventListener> children, Tab<?> tab, LayoutBounds tabDim, Optional<Tab<?>> selectedTab, Consumer<Tab<?>> tabSelector) {
      ...
      TabButtonWidget button = this.tabButtonWidgets
         .computeIfAbsent(tab, key -> new TabButtonWidget(tabDim, (Tab<?>)key, selected, () -> tabSelector.accept(key)));
   ...
   private void addTabHeader(
      ...
      TabGroup group,
      LayoutBounds tabHeaderDim,
      Optional<Tab<?>> selectedTab,
      Consumer<Tab<?>> tabSelector,
      Runnable rebuildFrame
   ) {
      boolean selected = this.groupModel.isHeaderSelected(group, selectedTab);
      TabHeaderWidget header = this.tabHeaderWidgets
         .computeIfAbsent(
            group.id(), unused -> new TabHeaderWidget(tabHeaderDim, group.modOptions(), () -> this.activateHeader(group, tabSelector, rebuildFrame), selected)
         );
   ...
   private int calculateWidth(LayoutBounds frameDim) {
      ...
      int maximumWidth = Math.max(120, Math.min(220, (int)(frameDim.width() * 0.35)));
      return Math.clamp(preferredWidth, 120, maximumWidth);
   }
