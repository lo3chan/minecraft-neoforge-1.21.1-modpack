...
public final class SearchIndex<T> implements SearchIndexContext<T> {
   ...
   private SearchIndex(SearchIndex.Builder<T> builder) {
      ...
      this.build(builder.items, builder.extractSearchableText);
   }

   public static <T> SearchIndex.Builder<T> builder(Function<T, String> extractSearchableText) {
      return new SearchIndex.Builder<>(extractSearchableText);
   }
   ...
   private void build(List<T> sourceItems, Function<T, String> extractSearchableText) {
      for (T item : sourceItems) {
         ...
         if (!normalized.isEmpty()) {
            ...
            for (String gramx : unique) {
               this.idfWeights.merge(gramx, 1.0, Double::sum);
            }
         }
      ...
      for (Entry<String, Double> entry : this.idfWeights.entrySet()) {
   ...
   public static final class Builder<T> {
      ...
      public SearchIndex.Builder<T> add(T item) {
         this.items.add(item);
         return this;
      }

      public SearchIndex.Builder<T> addAll(List<T> items) {
         this.items.addAll(items);
         return this;
      }

      public SearchIndex.Builder<T> foldDiacritics(boolean foldDiacritics) {
         this.foldDiacritics = foldDiacritics;
         return this;
      }

      public SearchIndex.Builder<T> maxResults(int maxResults) {
         this.maxResults = Math.max(1, maxResults);
         return this;
      }

      public SearchIndex.Builder<T> minScore(double minScore) {
         this.minScore = Math.max(0.0, minScore);
         return this;
      }

      public SearchIndex.Builder<T> rerankWithEditDistance(boolean rerankWithEditDistance) {
         this.rerankWithEditDistance = rerankWithEditDistance;
         return this;
      }

      public SearchIndex.Builder<T> rerankLimit(int rerankLimit) {
         this.rerankLimit = Math.max(1, rerankLimit);
         return this;
      }

      public SearchIndex.Builder<T> rerankWeight(double rerankWeight) {
