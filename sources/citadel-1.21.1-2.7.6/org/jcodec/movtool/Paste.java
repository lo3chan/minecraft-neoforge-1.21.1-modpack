...
public class Paste {
   ...
   public void paste(MovieBox to, MovieBox from, double sec) {
      ...
      for (int ix = 0; ix < matches[1].length; ix++) {
         if (matches[1][ix] == -1) {
            Util.spread(to, toTracks[ix], displayTv, to.rescale(from.getDuration(), from.getTimescale()));
         }
      }
   ...
   private boolean matches(TrakBox trakBox1, TrakBox trakBox2) {
      return trakBox1.getHandlerType().equals(trakBox2.getHandlerType())
         && this.matchHeaders(trakBox1, trakBox2)
         && this.matchSampleSizes(trakBox1, trakBox2)
         && this.matchMediaHeader(trakBox1, trakBox2)
         && this.matchClip(trakBox1, trakBox2)
         && this.matchLoad(trakBox1, trakBox2);
   }
   ...
   private boolean matchMediaHeader(TrakBox trakBox1, TrakBox trakBox2) {
      ...
      if ((vmhd1 == null || vmhd2 != null) && (vmhd1 != null || vmhd2 == null)) {
         if (vmhd1 != null && vmhd2 != null) {
            return vmhd1.getGraphicsMode() == vmhd2.getGraphicsMode()
               && vmhd1.getbOpColor() == vmhd2.getbOpColor()
               && vmhd1.getgOpColor() == vmhd2.getgOpColor()
               && vmhd1.getrOpColor() == vmhd2.getrOpColor();
         } else {
   ...
   private boolean matchHeaders(TrakBox trakBox1, TrakBox trakBox2) {
      ...
      return "vide".equals(trakBox1.getHandlerType())
            && Platform.arrayEqualsInt(th1.getMatrix(), th2.getMatrix())
            && th1.getLayer() == th2.getLayer()
            && th1.getWidth() == th2.getWidth()
            && th1.getHeight() == th2.getHeight()
   ...
   private boolean matchLoad(TrakBox trakBox1, TrakBox trakBox2) {
      ...
      return load1 != null && load2 != null
         ? load1.getPreloadStartTime() == load2.getPreloadStartTime()
   ...
   private boolean matchClip(TrakBox trakBox1, TrakBox trakBox2) {
      ...
      return crgn1 != null && crgn2 != null
         ? crgn1.getRgnSize() == crgn2.getRgnSize()
            && crgn1.getX() == crgn2.getX()
            && crgn1.getY() == crgn2.getY()
            && crgn1.getWidth() == crgn2.getWidth()
            && crgn1.getHeight() == crgn2.getHeight()
         : crgn1 == null && crgn2 == null;
   }
}
