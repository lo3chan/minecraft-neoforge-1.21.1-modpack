...
public class Util {
   ...
   private static void appendSampleToChunk(TrakBox trakBox1, TrakBox trakBox2, int off) {
      ...
      for (int i = 0; i < orig.length; i++) {
         shifted[i] = new SampleToChunkBox.SampleToChunkEntry(
            orig[i].getFirst() + stsc1.getSampleToChunk().length, orig[i].getCount(), orig[i].getEntry() + off
         );
      }

      NodeBox.findFirstPath(trakBox1, NodeBox.class, Box.path("mdia.minf.stbl"))
         .replace(
            "stsc", SampleToChunkBox.createSampleToChunkBox((SampleToChunkBox.SampleToChunkEntry[])ArrayUtil.addAllObj(stsc1.getSampleToChunk(), shifted))
         );
   }
   ...
   private static void appendTimeToSamples(TrakBox trakBox1, TrakBox trakBox2) {
      ...
      TimeToSampleBox sttsNew = TimeToSampleBox.createTimeToSampleBox(
         (TimeToSampleBox.TimeToSampleEntry[])ArrayUtil.addAllObj(stts1.getEntries(), stts2.getEntries())
      );
   ...
   private static void appendChunkOffsets(TrakBox trakBox1, TrakBox trakBox2) {
      ...
      stbl1.add(
         (Box)(co641 == null && co642 == null
            ? ChunkOffsetsBox.createChunkOffsetsBox(ArrayUtil.addAllLong(off1, off2))
            : ChunkOffsets64Box.createChunkOffsets64Box(ArrayUtil.addAllLong(off1, off2)))
      );
   }
   ...
   public static void forceEditList(MovieBox movie, TrakBox trakBox) {
      ...
      if (edits == null || edits.size() == 0) {
         ...
         var4.add(new Edit((int)mvhd.getDuration(), 0L, 1.0F));
         trakBox.setEdits(var4);
      }
   }
