...
public class Transcoder {
   ...
   private Transcoder(
      Source[] source,
      Sink[] sink,
      Transcoder.Mapping[] videoMappings,
      Transcoder.Mapping[] audioMappings,
   ...
   public void transcode() throws IOException {
      ...
      for (int s = 0; s < this.sinks.length; s++) {
         Transcoder.Stream stream = new Transcoder.Stream(
            this.sinks[s], this.videoMappings[s].copy, this.audioMappings[s].copy, this.extraFilters[s], pixelStore
         );
         allStreams[s] = stream;
   ...
   public static class TranscoderBuilder {
      ...
      public Transcoder create() {
         return new Transcoder(
            this.source.toArray(new Source[0]),
            this.sink.toArray(new Sink[0]),
            this.videoMappings.toArray(new Transcoder.Mapping[0]),
            this.audioMappings.toArray(new Transcoder.Mapping[0]),
            this.filters.toArray(new List[0]),
            this.seekFrames.toArray(),
            this.maxFrames.toArray()
         );
      }
   }
}
