         ...
         for (int i = 0; i < color.nComp; i++) {
            ...
            if ((width & mask) != 0) {
               throw new IllegalArgumentException(
                  "Component " + i + " width should be a multiple of " + (1 << color.compWidth[i]) + " for colorspace: " + color
               );
            }

            if (crop != null && (crop.getWidth() & mask) != 0) {
               throw new IllegalArgumentException(
                  "Component " + i + " cropped width should be a multiple of " + (1 << color.compWidth[i]) + " for colorspace: " + color
               );
            }
            ...
            if ((height & mask) != 0) {
               throw new IllegalArgumentException(
                  "Component " + i + " height should be a multiple of " + (1 << color.compHeight[i]) + " for colorspace: " + color
               );
            }

            if (crop != null && (crop.getHeight() & mask) != 0) {
               throw new IllegalArgumentException(
                  "Component " + i + " cropped height should be a multiple of " + (1 << color.compHeight[i]) + " for colorspace: " + color
               );
            }
         }
      }
      ...
      if (!this.compatible(src)) {
         ...
         for (int plane = 0; plane < this.color.nComp; plane++) {
            if (this.data[plane] != null) {
               System.arraycopy(
                  src.data[plane], 0, this.data[plane], 0, (this.width >> this.color.compWidth[plane]) * (this.height >> this.color.compHeight[plane])
               );
            }
         }
      }
         ...
         if (this.color.planar) {
            for (int plane = 0; plane < this.data.length; plane++) {
               if (this.data[plane] != null) {
                  this.cropSub(
                     this.data[plane],
                     this.crop.getX() >> this.color.compWidth[plane],
                     this.crop.getY() >> this.color.compHeight[plane],
                     this.crop.getWidth() >> this.color.compWidth[plane],
                     this.crop.getHeight() >> this.color.compHeight[plane],
                     this.width >> this.color.compWidth[plane],
                     this.crop.getWidth() >> this.color.compWidth[plane],
                     result.data[plane]
                  );
               }
            ...
            this.cropSub(
               ...
               this.crop.getWidth(),
               this.crop.getHeight(),
               this.width * this.color.nComp,
               this.crop.getWidth() * this.color.nComp,
               result.data[0]
            );
         }
