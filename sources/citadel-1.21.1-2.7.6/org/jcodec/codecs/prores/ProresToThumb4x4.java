...
public class ProresToThumb4x4 extends ProresDecoder {
   ...
   public Picture decodeFrameHiBD(ByteBuffer data, byte[][] target, byte[][] lowBits) {
      ...
      if (target != null && target[0].length >= lumaSize && target[1].length >= chromaSize && target[2].length >= chromaSize) {
         if (fh.frameType == 0) {
            this.decodePicture(
               data, target, lowBits, codedWidth, codedHeight, codedWidth >> 3, fh.qMatLuma, fh.qMatChroma, progressive_scan_4x4, 0, fh.chromaType
            );
         } else {
            this.decodePicture(
               data,
         ...
         return new Picture(
            ...
            color,
            lowBits == null ? 0 : 2,
            new Rect(0, 0, fh.width >> 1 & color.getWidthMask(), fh.height >> 1 & color.getHeightMask())
         );
      } else {
