...
public class ProresEncoder extends VideoEncoder {
   ...
   protected void encodePicture(ByteBuffer out, int[][] scaledLuma, int[][] scaledChroma, int[] scan, Picture picture, int vStep, int vOffset) {
      ...
      for (int mbY = 0; mbY < mbHeight; mbY++) {
         ...
         for (int sliceMbCount = 8; mbX < mbWidth; mbX += sliceMbCount) {
            ...
            qp = this.encodeSlice(
               out, scaledLuma, scaledChroma, scan, sliceMbCount, mbX, mbY, picture, qp, mbWidth, mbHeight, unsafeBottom || unsafeRight, vStep, vOffset
            );
   ...
   public VideoEncoder.EncodedFrame encodeFrame(Picture pic, ByteBuffer buffer) {
      ...
      writeFrameHeader(
         out,
         new ProresConsts.FrameHeader(
            0, pic.getCroppedWidth(), pic.getCroppedHeight(), this.interlaced ? 1 : 0, true, scan, this.profile.qmatLuma, this.profile.qmatChroma, 2
         )
      );
   ...
   public static final class Profile {
      public static final ProresEncoder.Profile PROXY = new ProresEncoder.Profile(
         "PROXY", ProresConsts.QMAT_LUMA_APCO, ProresConsts.QMAT_CHROMA_APCO, "apco", 1000, 4, 8
      );
      public static final ProresEncoder.Profile LT = new ProresEncoder.Profile(
         "LT", ProresConsts.QMAT_LUMA_APCS, ProresConsts.QMAT_CHROMA_APCS, "apcs", 2100, 1, 9
      );
      public static final ProresEncoder.Profile STANDARD = new ProresEncoder.Profile(
         "STANDARD", ProresConsts.QMAT_LUMA_APCN, ProresConsts.QMAT_CHROMA_APCN, "apcn", 3500, 1, 6
      );
      public static final ProresEncoder.Profile HQ = new ProresEncoder.Profile(
