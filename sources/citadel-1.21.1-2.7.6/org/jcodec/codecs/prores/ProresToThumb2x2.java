...
public class ProresToThumb2x2 extends ProresDecoder {
   ...
   public Picture decodeFrameHiBD(ByteBuffer data, byte[][] target, byte[][] lowBits) {
      ...
      if (target != null && target[0].length >= lumaSize && target[1].length >= chromaSize && target[2].length >= chromaSize) {
         if (fh.frameType == 0) {
            this.decodePicture(
               data, target, lowBits, codedWidth, codedHeight, codedWidth >> 2, fh.qMatLuma, fh.qMatChroma, new int[]{0, 1, 2, 3}, 0, fh.chromaType
            );
         } else {
            this.decodePicture(
               data,
         ...
         return new Picture(
            ...
            color,
            lowBits == null ? 0 : 2,
            new Rect(0, 0, fh.width >> 2 & color.getWidthMask(), fh.height >> 2 & color.getHeightMask())
         );
      } else {
