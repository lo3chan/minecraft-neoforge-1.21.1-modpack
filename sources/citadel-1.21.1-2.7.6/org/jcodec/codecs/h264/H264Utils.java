...
public class H264Utils {
   ...
   public static final void skipToNALUnit(ByteBuffer buf) {
      if (buf.hasRemaining()) {
         ...
         while (buf.hasRemaining()) {
            ...
            val |= buf.get() & 255;
            if ((val & 16777215) == 1) {
               ((Buffer)buf).position(buf.position());
               break;
            }
         }
      }
   ...
   public static final ByteBuffer gotoNALUnit(ByteBuffer buf) {
      if (!buf.hasRemaining()) {
         ...
         while (buf.hasRemaining()) {
            ...
            val |= buf.get() & 255;
            if ((val & 16777215) == 1) {
               ((Buffer)buf).position(buf.position() - (val == 1 ? 4 : 3));
               ((Buffer)result).limit(buf.position() - from);
               break;
            }
         }
         ...
         for (int lim = buf.limit() + buf.arrayOffset(); pos < lim; pos += 3) {
            ...
            if ((b & 254) == 0) {
               ...
               if (b == 1 && pos - posFrom >= 2 && arr[pos - 1] == 0 && arr[pos - 2] == 0) {
                  ...
                  ((Buffer)buf).position(pos + 1 - buf.arrayOffset() - lenSize);
                  ((Buffer)result).limit(buf.position() - from);
                  return result;
               }
            }
   ...
   public static final void unescapeNAL(ByteBuffer _buf) {
      if (_buf.remaining() >= 2) {
         ...
         ((Buffer)_buf).limit(out.position());
      }
   }
   ...
   public static final void escapeNALinplace(ByteBuffer src) {
      ...
      int old = src.limit();
      ((Buffer)src).limit(src.limit() + loc.length);
   ...
   public static void decodeMOVPacketInplace(ByteBuffer result, AvcCBox avcC) {
      if (avcC.getNalLengthSize() != 4) {
         ...
         while (dup.remaining() >= 4) {
            int size = dup.getInt();
            ((Buffer)dup).position(dup.position() - 4);
            dup.putInt(1);
            ((Buffer)dup).position(dup.position() + size);
         }
      }
   }
   ...
   public static void wipePSinplace(ByteBuffer _in, Collection<ByteBuffer> spsList, Collection<ByteBuffer> ppsList) {
      ...
      while (dup.hasRemaining()) {
         ...
         if (nu.type == NALUnitType.PPS) {
            ...
            ((Buffer)_in).position(dup.position());
         } else if (nu.type == NALUnitType.SPS) {
            if (spsList != null) {
               spsList.add(NIOUtils.duplicate(buf));
            }
   ...
   public abstract static class SliceHeaderTweaker {
      ...
      private SliceHeader part2(
         ByteBuffer is, ByteBuffer os, NALUnit nu, SeqParameterSet sps, PictureParameterSet pps, ByteBuffer nal, BitReader reader, SliceHeader sh
      ) {
         ...
         ((Buffer)nal).limit(os.position());
         H264Utils.escapeNALinplace(nal);
         ((Buffer)os).position(nal.limit());
         return sh;
      }
