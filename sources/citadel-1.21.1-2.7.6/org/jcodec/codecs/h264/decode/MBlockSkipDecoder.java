...
public class MBlockSkipDecoder extends MBlockDecoderBase {
   ...
   public void decodeSkip(MBlock mBlock, Frame[][] refs, Picture mb, SliceType sliceType) {
      ...
      if (sliceType == SliceType.P) {
         this.predictPSkip(
            ...
            this.mapper.topLeftAvailable(mBlock.mbIdx),
            this.mapper.topRightAvailable(mBlock.mbIdx),
            mBlock.x,
            mb
         );
   ...
   public void predictPSkip(Frame[][] refs, int mbX, int mbY, boolean lAvb, boolean tAvb, boolean tlAvb, boolean trAvb, H264Utils.MvList x, Picture mb) {
      ...
      if (lAvb && tAvb) {
         ...
         if (a != 0 && b != 0) {
            mvX = MBlockDecoderUtils.calcMVPredictionMedian(
               a, b, this.s.mvTop.getMv((mbX << 2) + 4, 0), this.s.mvTopLeft.getMv(0, 0), lAvb, tAvb, trAvb, tlAvb, 0, 0
            );
            mvY = MBlockDecoderUtils.calcMVPredictionMedian(
               a, b, this.s.mvTop.getMv((mbX << 2) + 4, 0), this.s.mvTopLeft.getMv(0, 0), lAvb, tAvb, trAvb, tlAvb, 0, 1
            );
         }
      }
