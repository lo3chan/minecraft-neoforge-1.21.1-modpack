...
public class MBlockDecoderBDirect extends MBlockDecoderBase {
   ...
   private void predictBTemporalDirect(
      Frame[][] refs,
      int mbX,
      int mbY,
      boolean lAvb,
      ...
      for (int i = 0; i < blocks8x8.length; i++) {
         ...
         if (!this.sh.sps.direct8x8InferenceFlag) {
            ...
            for (int j = 0; j < js.length; j++) {
               ...
               this.interpolator
                  .getBlockLuma(
                     refs[0][x.mv0R(blk4x4)], this.mbb[0], H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4], blkPredX + x.mv0X(blk4x4), blkPredY + x.mv0Y(blk4x4), 4, 4
                  );
               this.interpolator
                  .getBlockLuma(refs[1][0], this.mbb[1], H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4], blkPredX + x.mv1X(blk4x4), blkPredY + x.mv1Y(blk4x4), 4, 4);
            }
         } else {
            ...
            this.interpolator
               .getBlockLuma(
                  refs[0][x.mv0R(blk4x4_0)],
                  this.mbb[0],
                  H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4_0],
                  blkPredX + x.mv0X(blk4x4_0),
                  blkPredY + x.mv0Y(blk4x4_0),
                  8,
                  8
               );
            this.interpolator
               .getBlockLuma(refs[1][0], this.mbb[1], H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4_0], blkPredX + x.mv1X(blk4x4_0), blkPredY + x.mv1Y(blk4x4_0), 8, 8);
         }
   ...
   private void predTemp4x4(Frame[][] refs, int mbX, int mbY, H264Utils.MvList x, int blk4x4) {
      ...
      if (refL0.isShortTerm() && td != 0) {
         ...
         x.setPair(
            blk4x4,
            H264Utils.Mv.packMv(dsf * H264Utils.Mv.mvX(mvCol) + 128 >> 8, dsf * H264Utils.Mv.mvY(mvCol) + 128 >> 8, refIdxL0),
   ...
   private void predictBSpatialDirect(
      Frame[][] refs,
      int mbX,
      int mbY,
      boolean lAvb,
      ...
      if (refIdxL0 < 0 && refIdxL1 < 0) {
         for (int i = 0; i < blocks8x8.length; i++) {
            ...
            PredictionMerger.mergePrediction(
               ...
               this.mbb[0].getPlaneData(0),
               this.mbb[1].getPlaneData(0),
               H264Const.BLK_8x8_MB_OFF_LUMA[blk8x8],
               16,
               8,
               8,
               mb.getPlaneData(0),
         ...
         for (int i = 0; i < blocks8x8.length; i++) {
            ...
            if (!this.sh.sps.direct8x8InferenceFlag) {
               ...
               for (int j = 0; j < js.length; j++) {
                  ...
                  int blkIndY = blk4x4 >> 2;
                  MBlockDecoderUtils.debugPrint(
                     "DIRECT_4x4 [%d, %d]: (%d,%d,%d), (%d,%d," + refIdxL1 + ")",
                     blkIndY,
                     blkIndX,
                     x.mv0X(blk4x4),
                     x.mv0Y(blk4x4),
                  ...
                  if (refIdxL0 >= 0) {
                     this.interpolator
                        .getBlockLuma(
                           refs[0][refIdxL0], this.mbb[0], H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4], blkPredX + x.mv0X(blk4x4), blkPredY + x.mv0Y(blk4x4), 4, 4
                        );
                  }
                  ...
                  if (refIdxL1 >= 0) {
                     this.interpolator
                        .getBlockLuma(
                           refs[1][refIdxL1], this.mbb[1], H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4], blkPredX + x.mv1X(blk4x4), blkPredY + x.mv1Y(blk4x4), 4, 4
                        );
                  }
               }
            } else {
               ...
               if (refIdxL0 >= 0) {
                  this.interpolator
                     .getBlockLuma(
                        ...
                        this.mbb[0],
                        H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4_0],
                        blkPredXx + x.mv0X(blk4x4_0),
                        blkPredYx + x.mv0Y(blk4x4_0),
                        8,
                        8
                     );
               }
               ...
               if (refIdxL1 >= 0) {
                  this.interpolator
                     .getBlockLuma(
                        ...
                        this.mbb[1],
                        H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4_0],
                        blkPredXx + x.mv1X(blk4x4_0),
                        blkPredYx + x.mv1Y(blk4x4_0),
                        8,
                        8
                     );
               }
            ...
            PredictionMerger.mergePrediction(
               ...
               x.mv0R(blk4x4_0),
               x.mv1R(blk4x4_0),
               refIdxL0 >= 0 ? (refIdxL1 >= 0 ? H264Const.PartPred.Bi : H264Const.PartPred.L0) : H264Const.PartPred.L1,
               0,
               this.mbb[0].getPlaneData(0),
               this.mbb[1].getPlaneData(0),
               H264Const.BLK_4x4_MB_OFF_LUMA[blk4x4_0],
   ...
   private int calcRef(int a0, int b0, int c0, int d0, boolean lAvb, boolean tAvb, boolean tlAvb, boolean trAvb, int mbX) {
      return this.minPos(
         this.minPos(lAvb ? H264Utils.Mv.mvRef(a0) : -1, tAvb ? H264Utils.Mv.mvRef(b0) : -1),
         trAvb ? H264Utils.Mv.mvRef(c0) : (tlAvb ? H264Utils.Mv.mvRef(d0) : -1)
      );
   }
   ...
   private void pred4x4(
      ...
      boolean colZero = col.isShortTerm()
         && H264Utils.Mv.mvRef(mvCol) == 0
         && MathUtil.abs(H264Utils.Mv.mvX(mvCol)) >> 1 == 0
