...
public class MBlockDecoderBase {
   ...
   protected static int[][] initScalingMatrix(SliceHeader sh2) {
      if (sh2.sps.scalingMatrix != null || sh2.pps.extended != null && sh2.pps.extended.scalingMatrix != null) {
         int[][] merged = new int[][]{
            H264Const.defaultScalingList4x4Intra,
            null,
            null,
            H264Const.defaultScalingList4x4Inter,
            null,
            null,
            H264Const.defaultScalingList8x8Intra,
            H264Const.defaultScalingList8x8Inter,
            null,
            null,
            null,
            null
   ...
   public void decodeChroma(MBlock mBlock, int mbX, int mbY, boolean leftAvailable, boolean topAvailable, Picture mb, int qp) {
      if (this.s.chromaFormat == ColorSpace.MONO) {
         ...
         ChromaPredictionBuilder.predictWithMode(
            mBlock.ac[1],
            mBlock.chromaPredictionMode,
            mbX,
            leftAvailable,
            topAvailable,
            this.s.leftRow[1],
   ...
   private void chromaAC(
      boolean leftAvailable, boolean topAvailable, int mbX, int mbY, int[] dc, int comp, int crQp, MBType curMbType, boolean codedAC, int[][] residualOut
   ) {
   ...
   public void predictChromaInter(Frame[][] refs, H264Utils.MvList vectors, int x, int y, int comp, Picture mb, H264Const.PartPred[] predType) {
      for (int blk8x8 = 0; blk8x8 < 4; blk8x8++) {
         for (int list = 0; list < 2; list++) {
            if (H264Const.usesList(predType[blk8x8], list)) {
               for (int blk4x4 = 0; blk4x4 < 4; blk4x4++) {
                  ...
                  BlockInterpolator.getBlockChroma(
                     ref.getPlaneData(comp),
                     ref.getPlaneWidth(comp),
                     ref.getPlaneHeight(comp),
                     this.mbb[list].getPlaneData(comp),
                     blkPoy * mb.getPlaneWidth(comp) + blkPox,
                     mb.getPlaneWidth(comp),
                     xx,
                     yy,
                     2,
                     2
         ...
         PredictionMerger.mergePrediction(
            ...
            predType[blk8x8],
            comp,
            this.mbb[0].getPlaneData(comp),
            this.mbb[1].getPlaneData(comp),
            H264Const.BLK_8x8_MB_OFF_CHROMA[blk8x8],
            mb.getPlaneWidth(comp),
            4,
            4,
            mb.getPlaneData(comp),
            refs,
            this.poc
         );
      }
