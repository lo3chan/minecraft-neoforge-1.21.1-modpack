...
public class SliceReader {
   ...
   public SliceReader(
      PictureParameterSet activePps, CABAC cabac, CAVLC[] cavlc, MDecoder mDecoder, BitReader reader, Mapper mapper, SliceHeader sh, NALUnit nalUnit
   ) {
   ...
   public boolean readMacroblock(MBlock mBlock) {
      ...
      if ((!this.endOfData || this.mbSkipRun != 0) && this.mbIdx < mbWidth * mbHeight) {
         ...
         if (!this.sh.sliceType.isIntra()
            && this.activePps.entropyCodingModeFlag
            && this.readMBSkipFlag(this.sh.sliceType, this.mapper.leftAvailable(this.mbIdx), this.mapper.topAvailable(this.mbIdx), mbX)) {
            this.prevMBType = null;
            this.prevMbSkipped = true;
            mBlock.skipped = true;
            int blk8x8X = mbX << 1;
         ...
         this.endOfData = this.activePps.entropyCodingModeFlag && this.mDecoder.decodeFinalBin() == 1
            || !this.activePps.entropyCodingModeFlag && !CAVLCReader.moreRBSPData(this.reader);
         this.mbIdx++;
   ...
   boolean readTransform8x8Flag(boolean leftAvailable, boolean topAvailable, MBType leftType, MBType topType, boolean is8x8Left, boolean is8x8Top) {
      return !this.activePps.entropyCodingModeFlag
         ? CAVLCReader.readBool(this.reader, "transform_size_8x8_flag")
         : this.cabac.readTransform8x8Flag(this.mDecoder, leftAvailable, topAvailable, leftType, topType, is8x8Left, is8x8Top);
   }
   ...
   protected int readCodedBlockPatternIntra(boolean leftAvailable, boolean topAvailable, int leftCBP, int topCBP, MBType leftMB, MBType topMB) {
      return !this.activePps.entropyCodingModeFlag
         ? H264Const.CODED_BLOCK_PATTERN_INTRA_COLOR[CAVLCReader.readUEtrace(this.reader, "coded_block_pattern")]
         : this.cabac.codedBlockPatternIntra(this.mDecoder, leftAvailable, topAvailable, leftCBP, topCBP, leftMB, topMB);
   }
   ...
   int readRefIdx(
      ...
      return !this.activePps.entropyCodingModeFlag
         ? CAVLCReader.readTE(this.reader, this.numRef[list] - 1)
         : this.cabac
            .readRefIdx(this.mDecoder, leftAvailable, topAvailable, leftType, topType, leftPred, topPred, curPred, mbX, partX, partY, partW, partH, list);
   }
   ...
   int readMVD(
      ...
      return !this.activePps.entropyCodingModeFlag
         ? CAVLCReader.readSE(this.reader, "mvd_l0_x")
         : this.cabac
            .readMVD(this.mDecoder, comp, leftAvailable, topAvailable, leftType, topType, leftPred, topPred, curPred, mbX, partX, partY, partW, partH, list);
   }
   ...
   int rem4x4PredMode() {
      return !this.activePps.entropyCodingModeFlag
         ? CAVLCReader.readNBit(this.reader, 3, "MB: rem_intra4x4_pred_mode")
         : this.cabac.rem4x4PredMode(this.mDecoder);
   }

   boolean prev4x4PredMode() {
      return !this.activePps.entropyCodingModeFlag
         ? CAVLCReader.readBool(this.reader, "MBP: prev_intra4x4_pred_mode_flag")
         : this.cabac.prev4x4PredModeFlag(this.mDecoder);
   }
   ...
   void read16x16DC(boolean leftAvailable, boolean topAvailable, int mbX, int[] dc) {
      ...
      } else if (this.cabac.readCodedBlockFlagLumaDC(this.mDecoder, mbX, this.leftMBType, this.topMBType[mbX], leftAvailable, topAvailable, MBType.I_16x16)
         == 1) {
         this.cabac
            .readCoeffs(
               this.mDecoder, CABAC.BlockType.LUMA_16_DC, dc, 0, 16, CoeffTransformer.zigzag4x4, H264Const.identityMapping16, H264Const.identityMapping16
            );
      }
   }
   ...
   int read16x16AC(boolean leftAvailable, boolean topAvailable, int mbX, int cbpLuma, int[] ac, int blkOffLeft, int blkOffTop, int blkX, int blkY) {
      if (this.activePps.entropyCodingModeFlag) {
         return this.cabac
            ...
            ? this.cabac
               .readCoeffs(
                  this.mDecoder, CABAC.BlockType.LUMA_15_AC, ac, 1, 15, CoeffTransformer.zigzag4x4, H264Const.identityMapping16, H264Const.identityMapping16
               )
            : 0;
      } else {
         return this.cavlc[0]
            .readACBlock(
               ...
               blkX,
               blkOffTop,
               blkOffLeft != 0 || leftAvailable,
               blkOffLeft == 0 ? this.leftMBType : MBType.I_16x16,
               blkOffTop != 0 || topAvailable,
               blkOffTop == 0 ? this.topMBType[mbX] : MBType.I_16x16,
               1,
               15,
   ...
   int readResidualAC(
      boolean leftAvailable, boolean topAvailable, int mbX, MBType curMbType, int cbpLuma, int blkOffLeft, int blkOffTop, int blkX, int blkY, int[] ac
   ) {
      if (!this.activePps.entropyCodingModeFlag) {
         return this.reader.remaining() <= 0
            ...
            : this.cavlc[0]
               .readACBlock(
                  ...
                  blkOffTop,
                  blkOffLeft != 0 || leftAvailable,
                  blkOffLeft == 0 ? this.leftMBType : curMbType,
                  blkOffTop != 0 || topAvailable,
                  blkOffTop == 0 ? this.topMBType[mbX] : curMbType,
                  0,
                  16,
                  CoeffTransformer.zigzag4x4
               );
         ...
         return this.cabac
            ...
            ? this.cabac
               .readCoeffs(
                  this.mDecoder, CABAC.BlockType.LUMA_16, ac, 0, 16, CoeffTransformer.zigzag4x4, H264Const.identityMapping16, H264Const.identityMapping16
               )
            : 0;
      }
   }
   ...
   public int readLumaAC8x8(int blkX, int blkY, int[] ac) {
      int readCoeffs = this.cabac
         .readCoeffs(
            this.mDecoder, CABAC.BlockType.LUMA_64, ac, 0, 64, CoeffTransformer.zigzag8x8, H264Const.sig_coeff_map_8x8, H264Const.last_sig_coeff_map_8x8
         );
      ...
      if (!this.activePps.entropyCodingModeFlag) {
         this.cavlc[comp].readChromaDCBlock(this.reader, dc, leftAvailable, topAvailable);
            ...
            .readCodedBlockFlagChromaDC(
               ...
               topAvailable,
               this.leftCBPChroma,
               this.topCBPChroma[mbX],
               curMbType
            )
         == 1) {
         this.cabac
            .readCoeffs(
               this.mDecoder, CABAC.BlockType.CHROMA_DC, dc, 0, 4, H264Const.identityMapping16, H264Const.identityMapping16, H264Const.identityMapping16
            );
      }
   }
   ...
   public void readChromaAC(boolean leftAvailable, boolean topAvailable, int mbX, int comp, MBType curMbType, int[] ac, int blkOffLeft, int blkOffTop, int blkX) {
      if (!this.activePps.entropyCodingModeFlag) {
         ...
         this.cabac
            .readCoeffs(
               this.mDecoder, CABAC.BlockType.CHROMA_AC, ac, 1, 15, CoeffTransformer.zigzag4x4, H264Const.identityMapping16, H264Const.identityMapping16
            );
      }
   }
   ...
   public void readMBlockBDirect(MBlock mBlock) {
      ...
      mBlock._cbp = this.readCodedBlockPatternInter(
         lAvb, tAvb, this.leftCBPLuma | this.leftCBPChroma << 4, this.topCBPLuma[mbX] | this.topCBPChroma[mbX] << 4, this.leftMBType, this.topMBType[mbX]
      );
   ...
   public void readInter16x16(H264Const.PartPred p0, MBlock mBlock) {
      ...
      for (int list = 0; list < 2; list++) {
         if (H264Const.usesList(p0, list) && this.numRef[list] > 1) {
            mBlock.pb16x16.refIdx[list] = this.readRefIdx(
               leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[mbX << 1], p0, mbX, 0, 0, 4, 4, list
            );
         }
      }
   ...
   private void readPredInter8x16(MBlock mBlock, int mbX, boolean leftAvailable, boolean topAvailable, int list, H264Const.PartPred p0, H264Const.PartPred p1) {
      ...
      if (H264Const.usesList(p0, list)) {
         mBlock.pb168x168.mvdX1[list] = this.readMVD(
            0, leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[blk8x8X], p0, mbX, 0, 0, 2, 4, list
         );
         mBlock.pb168x168.mvdY1[list] = this.readMVD(
            1, leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[blk8x8X], p0, mbX, 0, 0, 2, 4, list
         );
      }

      if (H264Const.usesList(p1, list)) {
         mBlock.pb168x168.mvdX2[list] = this.readMVD(
            0, true, topAvailable, MBType.P_8x16, this.topMBType[mbX], p0, this.predModeTop[blk8x8X + 1], p1, mbX, 2, 0, 2, 4, list
         );
         mBlock.pb168x168.mvdY2[list] = this.readMVD(
            1, true, topAvailable, MBType.P_8x16, this.topMBType[mbX], p0, this.predModeTop[blk8x8X + 1], p1, mbX, 2, 0, 2, 4, list
         );
      }
   }

   private void readPredictionInter16x8(
      MBlock mBlock, int mbX, boolean leftAvailable, boolean topAvailable, H264Const.PartPred p0, H264Const.PartPred p1, int list
   ) {
      int blk8x8X = mbX << 1;
      if (H264Const.usesList(p0, list)) {
         mBlock.pb168x168.mvdX1[list] = this.readMVD(
            0, leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[blk8x8X], p0, mbX, 0, 0, 4, 2, list
         );
         mBlock.pb168x168.mvdY1[list] = this.readMVD(
            1, leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[blk8x8X], p0, mbX, 0, 0, 4, 2, list
         );
      }

      if (H264Const.usesList(p1, list)) {
         mBlock.pb168x168.mvdX2[list] = this.readMVD(
            0, leftAvailable, true, this.leftMBType, MBType.P_16x8, this.predModeLeft[1], p0, p1, mbX, 0, 2, 4, 2, list
         );
         mBlock.pb168x168.mvdY2[list] = this.readMVD(
            1, leftAvailable, true, this.leftMBType, MBType.P_16x8, this.predModeLeft[1], p0, p1, mbX, 0, 2, 4, 2, list
         );
      }
   }
   ...
   public void readInter16x8(H264Const.PartPred p0, H264Const.PartPred p1, MBlock mBlock) {
      ...
      for (int list = 0; list < 2; list++) {
         if (H264Const.usesList(p0, list) && this.numRef[list] > 1) {
            mBlock.pb168x168.refIdx1[list] = this.readRefIdx(
               leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[mbX << 1], p0, mbX, 0, 0, 4, 2, list
            );
         }

         if (H264Const.usesList(p1, list) && this.numRef[list] > 1) {
            mBlock.pb168x168.refIdx2[list] = this.readRefIdx(
               leftAvailable, true, this.leftMBType, mBlock.curMbType, this.predModeLeft[1], p0, p1, mbX, 0, 2, 4, 2, list
            );
         }
      }
   ...
   public void readIntra8x16(H264Const.PartPred p0, H264Const.PartPred p1, MBlock mBlock) {
      ...
      for (int list = 0; list < 2; list++) {
         if (H264Const.usesList(p0, list) && this.numRef[list] > 1) {
            mBlock.pb168x168.refIdx1[list] = this.readRefIdx(
               leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.predModeLeft[0], this.predModeTop[mbX << 1], p0, mbX, 0, 0, 2, 4, list
            );
         }

         if (H264Const.usesList(p1, list) && this.numRef[list] > 1) {
            mBlock.pb168x168.refIdx2[list] = this.readRefIdx(
               true, topAvailable, mBlock.curMbType, this.topMBType[mbX], p0, this.predModeTop[(mbX << 1) + 1], p1, mbX, 2, 0, 2, 4, list
            );
         }
      }
   ...
   private void readPredictionInter16x16(MBlock mBlock, int mbX, boolean leftAvailable, boolean topAvailable, int list, H264Const.PartPred curPred) {
      ...
      if (H264Const.usesList(curPred, list)) {
         mBlock.pb16x16.mvdX[list] = this.readMVD(
            0,
            leftAvailable,
            topAvailable,
            this.leftMBType,
         ...
         mBlock.pb16x16.mvdY[list] = this.readMVD(
            1,
            leftAvailable,
            topAvailable,
            this.leftMBType,
   ...
   private void readResidualInter(MBlock mBlock, boolean leftAvailable, boolean topAvailable, int mbX, int mbY) {
      mBlock._cbp = this.readCodedBlockPatternInter(
         leftAvailable,
         topAvailable,
         this.leftCBPLuma | this.leftCBPChroma << 4,
         this.topCBPLuma[mbX] | this.topCBPChroma[mbX] << 4,
         this.leftMBType,
         this.topMBType[mbX]
      );
      mBlock.transform8x8Used = false;
      if (mBlock.cbpLuma() != 0 && this.transform8x8) {
         mBlock.transform8x8Used = this.readTransform8x8Flag(
            leftAvailable, topAvailable, this.leftMBType, this.topMBType[mbX], this.tf8x8Left, this.tf8x8Top[mbX]
         );
      }
   ...
   public void readMBlock8x8(MBlock mBlock) {
      ...
      if (mBlock.curMbType != MBType.P_8x8 && mBlock.curMbType != MBType.P_8x8ref0) {
         ...
         noSubMBLessThen8x8 = H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[0]] == 0
            && H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[1]] == 0
            && H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[2]] == 0
            && H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[3]] == 0;
      } else {
         this.readPrediction8x8P(mBlock, mbX, leftAvailable, topAvailable);
         noSubMBLessThen8x8 = mBlock.pb8x8.subMbTypes[0] == 0
            && mBlock.pb8x8.subMbTypes[1] == 0
            && mBlock.pb8x8.subMbTypes[2] == 0
            && mBlock.pb8x8.subMbTypes[3] == 0;
      }
   ...
   private void readPrediction8x8P(MBlock mBlock, int mbX, boolean leftAvailable, boolean topAvailable) {
      ...
      if (this.numRef[0] > 1 && mBlock.curMbType != MBType.P_8x8ref0) {
         mBlock.pb8x8.refIdx[0][0] = this.readRefIdx(
            leftAvailable,
            topAvailable,
            this.leftMBType,
            this.topMBType[mbX],
            H264Const.PartPred.L0,
            H264Const.PartPred.L0,
            H264Const.PartPred.L0,
            mbX,
            0,
         ...
         mBlock.pb8x8.refIdx[0][1] = this.readRefIdx(
            true, topAvailable, MBType.P_8x8, this.topMBType[mbX], H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, mbX, 2, 0, 2, 2, 0
         );
         mBlock.pb8x8.refIdx[0][2] = this.readRefIdx(
            leftAvailable, true, this.leftMBType, MBType.P_8x8, H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, mbX, 0, 2, 2, 2, 0
         );
         mBlock.pb8x8.refIdx[0][3] = this.readRefIdx(
            true, true, MBType.P_8x8, MBType.P_8x8, H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, mbX, 2, 2, 2, 2, 0
         );
      }
      ...
      for (int list = 0; list < 2; list++) {
         if (this.numRef[list] > 1) {
            if (H264Const.usesList(p[0], list)) {
               mBlock.pb8x8.refIdx[list][0] = this.readRefIdx(
                  leftAvailable,
                  topAvailable,
                  this.leftMBType,
                  this.topMBType[mbX],
                  this.predModeLeft[0],
                  this.predModeTop[mbX << 1],
                  p[0],
                  mbX,
                  0,
                  0,
            ...
            if (H264Const.usesList(p[1], list)) {
               mBlock.pb8x8.refIdx[list][1] = this.readRefIdx(
                  true, topAvailable, MBType.B_8x8, this.topMBType[mbX], p[0], this.predModeTop[(mbX << 1) + 1], p[1], mbX, 2, 0, 2, 2, list
               );
            }

            if (H264Const.usesList(p[2], list)) {
               mBlock.pb8x8.refIdx[list][2] = this.readRefIdx(
                  leftAvailable, true, this.leftMBType, MBType.B_8x8, this.predModeLeft[1], p[0], p[2], mbX, 0, 2, 2, 2, list
               );
            }
      ...
      for (int listx = 0; listx < 2; listx++) {
         if (H264Const.usesList(p[0], listx)) {
            this.readSubMb8x8(
               mBlock,
               0,
               H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[0]],
               topAvailable,
               leftAvailable,
               0,
               0,
         ...
         if (H264Const.usesList(p[1], listx)) {
            this.readSubMb8x8(
               mBlock,
               1,
               H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[1]],
               topAvailable,
               true,
               2,
               0,
         ...
         if (H264Const.usesList(p[2], listx)) {
            this.readSubMb8x8(
               mBlock,
               2,
               H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[2]],
               true,
               leftAvailable,
               0,
               2,
         ...
         if (H264Const.usesList(p[3], listx)) {
            this.readSubMb8x8(
               mBlock,
               3,
               H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[3]],
               true,
               true,
               2,
               2,
   ...
   private void readSub4x4(
      ...
      mBlock.pb8x8.mvdX4[list][partNo] = this.readMVD(
         0, true, true, curMBType, curMBType, partPred, partPred, partPred, mbX, blk8x8X + 1, blk8x8Y + 1, 1, 1, list
      );
      mBlock.pb8x8.mvdY4[list][partNo] = this.readMVD(
         1, true, true, curMBType, curMBType, partPred, partPred, partPred, mbX, blk8x8X + 1, blk8x8Y + 1, 1, 1, list
      );
   }
   ...
   public void readIntraNxN(MBlock mBlock) {
      ...
      mBlock.chromaPredictionMode = this.readChromaPredMode(mbX, leftAvailable, topAvailable);
      mBlock._cbp = this.readCodedBlockPatternIntra(
         leftAvailable,
         topAvailable,
         this.leftCBPLuma | this.leftCBPChroma << 4,
         this.topCBPLuma[mbX] | this.topCBPChroma[mbX] << 4,
   ...
   private void readLuma(MBlock mBlock, boolean leftAvailable, boolean topAvailable, int mbX, int mbY) {
      for (int i = 0; i < 16; i++) {
         ...
         if ((mBlock.cbpLuma() & 1 << (i >> 2)) == 0) {
            ...
            mBlock.nCoeff[i] = this.readResidualAC(
               leftAvailable, topAvailable, mbX, mBlock.curMbType, mBlock.cbpLuma(), blkOffLeft, blkOffTop, blkX, blkY, mBlock.ac[0][i]
            );
         }
      }
   ...
   private void readMBlockI(MBlock mBlock) {
      mBlock.mbType = this.decodeMBTypeI(
         mBlock.mbIdx,
         this.mapper.leftAvailable(mBlock.mbIdx),
         this.mapper.topAvailable(mBlock.mbIdx),
         this.leftMBType,
         this.topMBType[this.mapper.getMbX(mBlock.mbIdx)]
      );
   ...
   private void readMBlockB(MBlock mBlock) {
      mBlock.mbType = this.readMBTypeB(
         mBlock.mbIdx,
         this.mapper.leftAvailable(mBlock.mbIdx),
         this.mapper.topAvailable(mBlock.mbIdx),
         this.leftMBType,
