...
public class CABAC {
   ...
   private int readMBType16x16(MDecoder decoder) {
      ...
      return decoder.decodeBin(7) == 0
         ? type + (decoder.decodeBin(9) << 1) + decoder.decodeBin(10)
         : type + (decoder.decodeBin(8) << 2) + (decoder.decodeBin(9) << 1) + decoder.decodeBin(10) + 4;
   }
   ...
   private int readMBType16x16P(MDecoder decoder, int ctxOff) {
      ...
      ctxOff++;
      return decoder.decodeBin(ctxOff) == 0
         ? type + (decoder.decodeBin(++ctxOff) << 1) + decoder.decodeBin(ctxOff)
         : type + (decoder.decodeBin(ctxOff) << 2) + (decoder.decodeBin(ctxOff + 1) << 1) + decoder.decodeBin(ctxOff + 1) + 4;
   }
   ...
   public int readCodedBlockFlagChromaDC(
      MDecoder decoder,
      int mbX,
      int comp,
      MBType left,
   ...
   public int readCodedBlockFlagLumaAC(
      MDecoder decoder,
      ...
      if (blkOffLeft == 0) {
         tLeft = this.condTerm(
            cur, leftAvailable, left, left != null && left != MBType.I_PCM && this.cbp(leftCBPLuma, 3, blkOffTop), this.codedBlkLeft[comp][blkOffTop]
         );
      } else {
   ...
   public int readCodedBlockFlagLuma64(
      MDecoder decoder,
      int blkX,
      int blkY,
      int comp,
      ...
      if (blkOffLeft == 0) {
         tLeft = this.condTerm(
            ...
            leftAvailable,
            left,
            left != null && left != MBType.I_PCM && is8x8Left && this.cbp(leftCBPLuma, 3, blkOffTop),
            this.codedBlkLeft[comp][blkOffTop]
         );
      } else {
         tLeft = this.condTerm(cur, true, cur, this.cbp(curCBPLuma, blkOffLeft - 1, blkOffTop), this.codedBlkLeft[comp][blkOffTop]);
      }
      ...
      if (blkOffTop == 0) {
         tTop = this.condTerm(
            cur, topAvailable, top, top != null && top != MBType.I_PCM && is8x8Top && this.cbp(topCBPLuma, blkOffLeft, 3), this.codedBlkTop[comp][blkX]
         );
      } else {
         tTop = this.condTerm(cur, true, cur, this.cbp(curCBPLuma, blkOffLeft, blkOffTop - 1), this.codedBlkTop[comp][blkX]);
      }
   ...
   public int readCodedBlockFlagChromaAC(
      MDecoder decoder,
      int blkX,
      int blkY,
      int comp,
   ...
   public int codedBlockPatternIntra(MDecoder mDecoder, boolean leftAvailable, boolean topAvailable, int cbpLeft, int cbpTop, MBType mbLeft, MBType mbTop) {
      ...
      int cr1 = cr0 != 0
         ? mDecoder.decodeBin(81 + this.condTermCr1(leftAvailable, mbLeft, cbpLeft >> 4) + 2 * this.condTermCr1(topAvailable, mbTop, cbpTop >> 4))
         : 0;
   ...
   public int readMVD(
      ...
      boolean predEqA = leftPred != null
         && leftPred != H264Const.PartPred.Direct
         && (leftPred == H264Const.PartPred.Bi || leftPred == curPred || curPred == H264Const.PartPred.Bi && H264Const.usesList(leftPred, list));
      boolean predEqB = topPred != null
         && topPred != H264Const.PartPred.Direct
   ...
   public boolean readTransform8x8Flag(
      MDecoder mDecoder, boolean leftAvailable, boolean topAvailable, MBType leftType, MBType topType, boolean is8x8Left, boolean is8x8Top
   ) {
   ...
   public static final class BlockType {
      ...
      private BlockType(
         int codecBlockCtxOff,
         int sigCoeffCtxOff,
         int lastSigCoeffCtxOff,
         int sigCoeffFlagFldCtxOff,
         int lastSigCoeffFldCtxOff,
         int coeffAbsLevelCtxOff,
         int coeffAbsLevelAdjust
      ) {
