...
public class VP8Decoder extends VideoDecoder {
   ...
   public Picture decodeFrame(ByteBuffer frame, byte[][] buffer) {
      ...
      if (!keyFrame) {
         ...
         ((Buffer)tokenBuffer).position(partitionSize + headerOffset);
               ...
               if (mb.lumaMode == 4) {
                  for (int sbRow = 0; sbRow < 4; sbRow++) {
                     for (int sbCol = 0; sbCol < 4; sbCol++) {
                        ...
                        sb.mode = headerDecoder.readTree(
                           VP8Util.SubblockConstants.subblockModeTree, VP8Util.SubblockConstants.keyFrameSubblockModeProb[A.mode][L.mode]
                        );
                     }
                  }
               } else {
