...
public class JpegDecoder extends VideoDecoder {
   ...
   private void decodeMCU(
      BitReader bits, int[] dcPredictor, int[][] quant, VLC[] huff, Picture result, int bx, int by, int blockH, int blockV, int field, int step
   ) {
      int sx = bx << blockH - 1;
      int sy = by << blockV - 1;
   ...
   void decodeBlock(
      BitReader bits, int[] dcPredictor, int[][] quant, VLC[] huff, Picture result, int[] buf, int blkX, int blkY, int plane, int chroma, int field, int step
   ) {
      Arrays.fill(buf, 0);
   ...
   private static ByteBuffer readToMarker(ByteBuffer data) {
      ...
      while (data.hasRemaining()) {
         ...
         if (b0 == -1) {
            byte b1 = data.get();
            if (b1 != 0) {
               ((Buffer)data).position(data.position() - 2);
               break;
            }
