...
public class MPEGDecoder extends VideoDecoder {
   ...
   public Picture decodeFrame(ByteBuffer buffer, byte[][] buf) {
      ...
      if ((this.refFrames[0] != null || ph.picture_coding_type <= 1) && (this.refFrames[1] != null || ph.picture_coding_type <= 2)) {
         ...
         Picture pic = new Picture(
            context.codedWidth, context.codedHeight, buf, (byte[][])null, context.color, 0, new Rect(0, 0, context.picWidth, context.picHeight)
         );
   ...
   private PictureHeader readHeader(ByteBuffer buffer) {
      ...
      ByteBuffer segment;
      for (ByteBuffer fork = buffer.duplicate(); (segment = MPEGUtil.nextSegment(fork)) != null; ((Buffer)buffer).position(fork.position())) {
   ...
   public Picture decodePicture(MPEGDecoder.Context context, PictureHeader ph, ByteBuffer buffer, byte[][] buf, int vertOff, int vertStep) {
      ...
      if (buf.length >= 3 && buf[0].length >= planeSize && buf[1].length >= planeSize && buf[2].length >= planeSize) {
         try {
            ...
            if ((ph.picture_coding_type == 1 || ph.picture_coding_type == 2)
               && ph.pictureCodingExtension != null
               && ph.pictureCodingExtension.picture_structure != 3) {
               this.refFields[ph.pictureCodingExtension.picture_structure - 1] = this.copyAndCreateIfNeeded(
                  pic, this.refFields[ph.pictureCodingExtension.picture_structure - 1]
               );
            }
   ...
   public void decodeSlice(PictureHeader ph, int verticalPos, MPEGDecoder.Context context, byte[][] buf, BitReader _in, int vertOff, int vertStep) throws IOException {
      ...
      MPEGPred pred = new MPEGPred(
         ph.pictureCodingExtension != null
            ? ph.pictureCodingExtension.f_code
            : new int[][]{{ph.forward_f_code, ph.forward_f_code}, {ph.backward_f_code, ph.backward_f_code}},
         this.sh.sequenceExtension != null ? this.sh.sequenceExtension.chroma_format : 1,
   ...
   public int decodeMacroblock(
      ...
      if (mbType.spatial_temporal_weight_code_flag == 1
         && ph.pictureSpatialScalableExtension != null
      ...
      if (mbType.macroblock_intra == 1) {
         ...
         if (ph.pictureCodingExtension == null || ph.pictureCodingExtension.picture_structure == 3) {
            ...
            pred.predictInField(
               new Picture[]{this.refFrames[refIdx], this.refFrames[refIdx]},
               mbX << 4,
               mbY << 4,
               predFwd,
               bits,
      ...
      if (mbType.macroblock_motion_backward != 0) {
         ...
         if (ph.pictureCodingExtension != null && ph.pictureCodingExtension.picture_structure != 3) {
            pred.predictInField(
               new Picture[]{this.refFrames[0], this.refFrames[0]},
               mbX << 4,
               mbY << 4,
               predBack,
               bits,
         ...
         if ((cbp & cbpMask) != 0) {
            ...
            if (mbType.macroblock_intra == 1) {
               this.blockIntra(
                  ...
                  vlcCoeff,
                  block,
                  context.intra_dc_predictor,
                  i,
                  context.scan,
                  !this.sh.hasExtensions() && !ph.hasExtensions() ? 8 : 12,
                  intra_dc_mult,
                  qScale,
                  qmat
               );
   ...
   private void mvZero(MPEGDecoder.Context context, PictureHeader ph, MPEGPred pred, int mbX, int mbY, int[][] mbPix) {
      if (ph.picture_coding_type == 2) {
         pred.predict16x16NoMV(
            this.refFrames[0], mbX << 4, mbY << 4, ph.pictureCodingExtension == null ? 3 : ph.pictureCodingExtension.picture_structure, 0, mbPix
         );
      } else {
         int[][] pp = mbPix;
         if (context.lastPredB.macroblock_motion_backward == 1) {
            pred.predict16x16NoMV(
               this.refFrames[0], mbX << 4, mbY << 4, ph.pictureCodingExtension == null ? 3 : ph.pictureCodingExtension.picture_structure, 1, mbPix
            );
         ...
         if (context.lastPredB.macroblock_motion_forward == 1) {
            pred.predict16x16NoMV(
               this.refFrames[1], mbX << 4, mbY << 4, ph.pictureCodingExtension == null ? 3 : ph.pictureCodingExtension.picture_structure, 0, pp
            );
            if (mbPix != pp) {
               avgPred(mbPix, pp);
            }
   ...
   protected void put(int[][] mbPix, byte[][] buf, int stride, int chromaFormat, int mbX, int mbY, int width, int height, int vertOff, int vertStep) {
      ...
      this.putSub(
         buf[1],
         (mbY << chromaMBH) * (chromaStride << vertStep) + vertOff * chromaStride + (mbX << chromaMBW),
         chromaStride << vertStep,
         mbPix[1],
         chromaMBW,
         chromaMBH
      );
   ...
   protected void blockIntra(
      BitReader bits, VLC vlcCoeff, int[] block, int[] intra_dc_predictor, int blkIdx, int[] scan, int escSize, int intra_dc_mult, int qScale, int[] qmat
   ) {
