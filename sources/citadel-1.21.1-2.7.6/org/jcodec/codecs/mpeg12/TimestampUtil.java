...
public class TimestampUtil {
   ...
   private abstract static class BaseCommand extends FixTimestamp {
      ...
      protected long doWithTimestamp(int streamId, long pts, boolean isPts) {
         return !"all".equals(this.streamSelector)
               && (!"video".equals(this.streamSelector) || !this.isVideo(streamId))
               && (!"audio".equals(this.streamSelector) || !this.isAudio(streamId))
            ? pts
            : this.withTimestamp(pts, isPts);
      }
