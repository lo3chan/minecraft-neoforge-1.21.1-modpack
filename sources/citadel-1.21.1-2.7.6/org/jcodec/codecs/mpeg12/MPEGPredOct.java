...
public class MPEGPredOct extends MPEGPred {
   ...
   private static final int[][] COEFF = new int[][]{
      {0, 0, 128, 0, 0, 0},
      {0, -6, 123, 12, -1, 0},
      {2, -11, 108, 36, -8, 1},
      {0, -9, 93, 50, -6, 0},
   ...
   protected int getPix6Vert(byte[] ref, int refW, int refH, int x, int y, int refVertStep, int refVertOff, int[] coeff) {
      ...
      return ref[y0 * refW + x] * coeff[0]
         + ref[y1 * refW + x] * coeff[1]
         + ref[y2 * refW + x] * coeff[2]
         + ref[y3 * refW + x] * coeff[3]
         + ref[y4 * refW + x] * coeff[4]
         + ref[y5 * refW + x] * coeff[5]
         + 16384;
   }

   private void predictSubXSubYUnSafe(
      byte[] ref, int rx, int ix, int ry, int iy, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep
   ) {
      ...
      for (int offTmp = dblTgtW; i < tgtH; i++) {
         for (int j = 0; j < tgtW; offTgt++) {
            tgt[offTgt] = this.tmp[offTmp - dblTgtW] * coeff[0]
                  + this.tmp[offTmp - tgtW] * coeff[1]
                  + this.tmp[offTmp] * coeff[2]
                  + this.tmp[offTmp + tgtW] * coeff[3]
                  + this.tmp[offTmp + dblTgtW] * coeff[4]
                  + this.tmp[offTmp + tripleTgtW] * coeff[5]
                  + 8192
               >> 14;
            j++;
            offTmp++;
   ...
   private void predictSubXSubYSafe(
      byte[] ref, int rx, int ix, int ry, int iy, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep
   ) {
      int[] coeff = COEFF[ix];
      ...
      for (int offTmp = 0; i < tgtH + 5; i++) {
         for (int j = 0; j < tgtW; offRef++) {
            this.tmp[offTmp] = ref[offRef - 2] * coeff[0]
               + ref[offRef - 1] * coeff[1]
               + ref[offRef] * coeff[2]
               + ref[offRef + 1] * coeff[3]
               + ref[offRef + 2] * coeff[4]
               + ref[offRef + 3] * coeff[5];
            j++;
            offTmp++;
         }
      ...
      for (int offTmp = dblTgtW; i < tgtH; i++) {
         for (int j = 0; j < tgtW; offTgt++) {
            tgt[offTgt] = (
                  this.tmp[offTmp - dblTgtW] * coeff[0]
                        + this.tmp[offTmp - tgtW] * coeff[1]
                        + this.tmp[offTmp] * coeff[2]
                        + this.tmp[offTmp + tgtW] * coeff[3]
                        + this.tmp[offTmp + dblTgtW] * coeff[4]
   ...
   private void predictSubXFullYUnSafe(
      byte[] ref, int rx, int ix, int ry, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep
   ) {
      int[] coeff = COEFF[ix];
   ...
   private void predictSubXFullYSafe(
      byte[] ref, int rx, int ix, int ry, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep
   ) {
      int[] coeff = COEFF[ix];
   ...
   private void predictFullXSubYUnSafe(
      byte[] ref, int rx, int ry, int iy, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep
   ) {
      int[] coeff = COEFF[iy];
      int tgtOff = tgtW * tgtY;
      int lfTgt = tgtVertStep * tgtW;
   ...
   private void predictFullXSubYSafe(
      byte[] ref, int rx, int ry, int iy, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep
   ) {
      int[] coeff = COEFF[iy];
      int offTgt = tgtW * tgtY;
      ...
      for (int i = 0; i < tgtH; i++) {
         for (int j = 0; j < tgtW; offRef++) {
            tgt[offTgt] = (
                  ref[offRef - dblRefW] * coeff[0]
                        + ref[offRef - singleRefW] * coeff[1]
                        + ref[offRef] * coeff[2]
                        + ref[offRef + singleRefW] * coeff[3]
                        + ref[offRef + dblRefW] * coeff[4]
                        + ref[offRef + tripleRefW] * coeff[5]
                        + 64
                     >> 7
               )
               + 128;
