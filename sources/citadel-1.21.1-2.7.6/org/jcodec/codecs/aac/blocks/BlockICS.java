...
public class BlockICS extends Block {
                  ...
                  while (i < run_end) {
                     ...
                     if (offset[1] != clipped_offset) {
                        System.out
                           .println(
                              String.format(
                                 "Noise gain clipped (%d -> %d).\nIf you heard an audible artifact, there may be a bug _in the decoder. ",
                                 offset[1],
                                 clipped_offset
                              )
                           );
               ...
               while (i < run_end) {
                  ...
                  if (offset[2] != clipped_offset) {
                     System.out
                        .println(
                           String.format(
                              "Intensity stereo position clipped (%d -> %d).\nIf you heard an audible artifact, there may be a bug _in the decoder. ",
                              offset[2],
                              clipped_offset
                           )
                        );
            ...
            if (cb_idx != 0) {
               ...
               for (int j = 0; j < 2; j++) {
                  if ((nzt & 1 << j) != 0) {
                     ...
                     coef[cf++] = MathUtil.cubeRoot(n) | bits & -2147483648;
                     bits <<= 1;
                  } else {
                     int v = (int)vq[cb_idx & 15];
                     coef[cf++] = bits & -2147483648 | v;
                  }
   ...
   static {
      for (int i = 0; i < 428; i++) {
         ff_aac_pow2sf_tab[i] = (float)Math.pow(2.0, (i - 200) / 4.0);
      }
   }
