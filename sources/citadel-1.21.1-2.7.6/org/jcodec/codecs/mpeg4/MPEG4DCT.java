   ...
   public static void idctColumnsPut(short[] block, byte[] dst, int dstOffset, int stride) {
      for (int i = 0; i < 8; i++) {
         ...
         if ((
            ...
            == 0) {
            dst[offset + stride * 0] = dst[offset + stride * 1] = dst[offset + stride * 2] = dst[offset + stride * 3] = dst[offset + stride * 4] = dst[offset
               + stride * 5] = dst[offset + stride * 6] = dst[offset + stride * 7] = clamp255(block[i + 0] + 32 >> 6);
         } else {
   ...
   public static void idctRows(short[] block) {
      for (int i = 0; i < 8; i++) {
         ...
         if ((
            ...
            == 0) {
            block[offset] = block[offset + 1] = block[offset + 2] = block[offset + 3] = block[offset + 4] = block[offset + 5] = block[offset + 6] = block[offset
               + 7] = (short)(block[offset] << 3);
         } else {
