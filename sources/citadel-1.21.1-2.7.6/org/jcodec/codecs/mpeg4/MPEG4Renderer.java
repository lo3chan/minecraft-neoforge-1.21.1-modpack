...
public class MPEG4Renderer {
   ...
   static void renderMBInter(MPEG4DecodingContext ctx, Picture[] refs, Macroblock mb, int ref, boolean bvop) {
      ...
      if (mb.mode == 2 && !bvop) {
         ...
         if (ctx.quarterPel) {
            MPEG4Interpolator.interpolate16x16QP(
               mb.pred[0], backward.getPlaneData(0), mbX, mbY, codedW, codedH, mv[0].x, mv[0].y, backward.getWidth(), ctx.rounding
            );
         } else {
            MPEG4Interpolator.interpolate16x16Planar(
               mb.pred[0], backward.getPlaneData(0), mbX, mbY, codedW, codedH, mv[0].x, mv[0].y, backward.getWidth(), ctx.rounding
            );
         }
      }
      ...
      MPEG4Interpolator.interpolate8x8Planar(
         mb.pred[1], 0, 8, refs[ref].getPlaneData(1), 8 * mb.x, 8 * mb.y, codedWcr, codedHcr, uv_dx, uv_dy, refs[ref].getPlaneWidth(1), ctx.rounding
      );
      MPEG4Interpolator.interpolate8x8Planar(
         mb.pred[2], 0, 8, refs[ref].getPlaneData(2), 8 * mb.x, 8 * mb.y, codedWcr, codedHcr, uv_dx, uv_dy, refs[ref].getPlaneWidth(2), ctx.rounding
      );
      if (mb.cbp != 0) {
         for (int i = 0; i < 6; i++) {
