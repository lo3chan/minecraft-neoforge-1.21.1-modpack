...
public class MPEG4Interpolator {
   ...
   public static final void interpolate8x8Planar(
      byte[] dst, int dstOff, int dstStride, byte[] refn, int x, int y, int w, int h, int dx, int dy, int stride, boolean rounding
   ) {
      int x_ = x + (dx >> 1);
      int y_ = y + (dy >> 1);
      switch (((dx & 1) << 1) + (dy & 1)) {
   ...
   private static final void interpolate8x8PlanarHor(
      byte[] dst, int dstOffset, int dstStride, byte[] src, int x, int y, int w, int h, int stride, boolean rounding
   ) {
   ...
   private static final void interpolate8x8PlanarBoth(
      byte[] dst, int dstOff, int dstStride, byte[] src, int x, int y, int w, int h, int stride, boolean rounding
   ) {
      ...
      if (x >= 0 && y >= 0 && x <= w - 9 && y <= h - 9) {
         ...
         while (j < 8 * stride) {
            for (int i = 0; i < 8; i++) {
               dst[d + i] = (byte)(
                  src[srcOffset + j + i] + src[srcOffset + j + i + 1] + src[srcOffset + j + stride + i] + src[srcOffset + j + stride + i + 1] + rnd >> 2
               );
            }
