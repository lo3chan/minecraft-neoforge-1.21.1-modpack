...
public class EbmlMaster extends EbmlBase {
   ...
   public ByteBuffer getData() {
      ...
      if (size > 2147483647L) {
         System.out
            .println("EbmlMaster.getData: id.length " + this.id.length + "  EbmlUtil.ebmlLength(" + size + "): " + EbmlUtil.ebmlLength(size) + " size: " + size);
      }

      ByteBuffer bb = ByteBuffer.allocate((int)(this.id.length + EbmlUtil.ebmlLength(size) + size));
      bb.put(this.id);
