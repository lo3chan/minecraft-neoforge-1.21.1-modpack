...
public class CuesFactory {
   ...
   public EbmlMaster createCues() {
      ...
      for (CuesFactory.CuePointMock cpm : this.a) {
         ...
         if (cueClusterPosition.data.limit() != cpm.cueClusterPositionSize) {
            System.err
               .println(
                  "estimated size of CueClusterPosition differs from the one actually used. ElementId: "
                     + EbmlUtil.toHexString(cpm.id)
                     + " "
                     + cueClusterPosition.getData().limit()
                     + " vs "
                     + cpm.cueClusterPositionSize
               );
         }
   ...
   public int estimateFixedSize(int numberOfClusters) {
      ...
      return s + MKVType.Cues.id.length + EbmlUtil.ebmlLength(s);
   }
   ...
   public int estimateSize() {
      ...
      for (CuesFactory.CuePointMock cpm : this.a) {
         s += estimateCuePointSize(
            EbmlUint.calculatePayloadSize(cpm.timecode), EbmlUint.calculatePayloadSize(this.videoTrackNr), EbmlUint.calculatePayloadSize(cpm.elementOffset)
         );
      }
   ...
   public static int estimateCuePointSize(int timecodeSizeInBytes, int trackNrSizeInBytes, int clusterPositionSizeInBytes) {
      ...
      int cueTrackPositionSize = MKVType.CueTrack.id.length
         + EbmlUtil.ebmlLength(trackNrSizeInBytes)
         + trackNrSizeInBytes
         + MKVType.CueClusterPosition.id.length
         + EbmlUtil.ebmlLength(clusterPositionSizeInBytes)
         + clusterPositionSizeInBytes;
      cueTrackPositionSize += MKVType.CueTrackPositions.id.length + EbmlUtil.ebmlLength(cueTrackPositionSize);
      return MKVType.CuePoint.id.length + EbmlUtil.ebmlLength(cueTimeSize + cueTrackPositionSize) + cueTimeSize + cueTrackPositionSize;
   }
