...
public class MTSDump extends MPSDump {
   ...
   public MTSDump(ReadableByteChannel ch, int targetGuid) {
      ...
      ((Buffer)this.buf).position(this.buf.limit());
      ((Buffer)this.tsBuf).position(this.tsBuf.limit());
   }
   ...
   private static void dumpProgramPids(ReadableByteChannel readableFileChannel) throws IOException {
      ...
      ((Buffer)buf).flip();
      ((Buffer)buf).limit(buf.limit() - buf.limit() % 188);
      int pmtPid = -1;
      ...
      try {
         ...
         while (dst.hasRemaining()) {
            if (!this.buf.hasRemaining()) {
               ...
               ((Buffer)dub).flip();
               ((Buffer)dub).limit(dub.limit() - dub.limit() % 188);
               this.buf = dub;
            }
