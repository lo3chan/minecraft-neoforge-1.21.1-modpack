...
public class MPSDump {
   ...
   protected void logPes(PESPacket pkt, int hdrSize, ByteBuffer payload) {
      System.out
         .println(
            pkt.streamId
               + "("
               + (pkt.streamId >= 224 ? "video" : "audio")
               + ") ["
               + pkt.pos
               + ", "
               + (payload.remaining() + hdrSize)
               + "], pts: "
               + pkt.pts
               + ", dts: "
               + pkt.dts
   ...
   private static ByteBuffer getPesPayload(ByteBuffer buffer) {
      ...
      while (copy.hasRemaining()) {
         ...
         if (marker >= 441) {
            ((Buffer)result).limit(copy.position());
            ((Buffer)buffer).position(copy.position());
            return result;
         }
      ...
      private void dumpPictureHeader(ByteBuffer b) {
         ...
         System.out
            .print(
               MainUtils.colorBright(
                  "picture header <type:"
                     + (this.picHeader.picture_coding_type == 1 ? "I" : (this.picHeader.picture_coding_type == 2 ? "P" : "B"))
                     + ", temp_ref:"
                     + this.picHeader.temporal_reference
                     + ">",
                  MainUtils.ANSIColor.BROWN,
                  true
               )
            );
      }
