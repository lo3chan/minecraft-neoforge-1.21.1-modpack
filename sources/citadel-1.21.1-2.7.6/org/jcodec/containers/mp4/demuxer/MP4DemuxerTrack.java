...
public class MP4DemuxerTrack extends AbstractMP4DemuxerTrack {
      ...
      if (this.curFrame >= this.sizes.length) {
         ...
         if (storage != null && storage.remaining() < size) {
            ...
            if (result != null && result.remaining() < size) {
               ...
               if (this.syncSamples != null && this.ssOff < this.syncSamples.length && this.curFrame + 1L == this.syncSamples[this.ssOff]) {
                  sync = true;
                  this.ssOff++;
               }

               boolean psync = false;
               if (this.partialSync != null && this.psOff < this.partialSync.length && this.curFrame + 1L == this.partialSync[this.psOff]) {
                  psync = true;
                  this.psOff++;
               }
   ...
   protected void seekPointer(long frameNo) {
      ...
      for (int i = 0; i < this.noInChunk; i++) {
         this.offInChunk = this.offInChunk + this.sizes[(int)frameNo - this.noInChunk + i];
      }

      if (this.syncSamples != null) {
         this.ssOff = 0;

         while (this.ssOff < this.syncSamples.length && this.syncSamples[this.ssOff] < this.curFrame + 1L) {
            this.ssOff++;
         }
      }
      ...
      if (this.partialSync != null) {
         ...
         while (this.psOff < this.partialSync.length && this.partialSync[this.psOff] < this.curFrame + 1L) {
            this.psOff++;
         }
      }
   }
