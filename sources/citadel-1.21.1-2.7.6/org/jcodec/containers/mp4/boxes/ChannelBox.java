...
public class ChannelBox extends FullBox {
   ...
   public void parse(ByteBuffer input) {
      ...
      for (int i = 0; i < numDescriptions; i++) {
         this.descriptions[i] = new ChannelBox.ChannelDescription(
            input.getInt(),
            input.getInt(),
            new float[]{Float.intBitsToFloat(input.getInt()), Float.intBitsToFloat(input.getInt()), Float.intBitsToFloat(input.getInt())}
         );
      }
   }
