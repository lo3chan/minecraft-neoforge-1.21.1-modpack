...
public class FLVTool {
   ...
   public static void main1(String[] args) throws IOException {
      if (args.length < 1) {
         ...
         FLVTool.PacketProcessorFactory processorFactory = processors.get(command);
         if (processorFactory == null) {
            ...
            if (cmd.args.length < 1) {
               ...
               int maxPackets = cmd.getIntegerFlagD(FLAG_MAX_PACKETS, 2147483647);
   ...
   public static class ClipPacketProcessor implements FLVTool.PacketProcessor {
      ...
      public boolean processPacket(FLVTag pkt, FLVWriter writer) throws IOException {
         if (pkt.getType() == FLVTag.Type.VIDEO
            && pkt.getTagHeader().getCodec() == Codec.H264
      ...
      public static class Factory implements FLVTool.PacketProcessorFactory {
         ...
         public FLVTool.PacketProcessor newPacketProcessor(MainUtils.Cmd flags) {
            return new FLVTool.ClipPacketProcessor(
               flags.getDoubleFlag(FLVTool.ClipPacketProcessor.FLAG_FROM), flags.getDoubleFlag(FLVTool.ClipPacketProcessor.FLAG_TO)
            );
         }
   ...
   public static class FixPtsProcessor implements FLVTool.PacketProcessor {
      ...
      private void processOneTag(FLVWriter writer) throws IOException {
         ...
         if (tag.getType() == FLVTag.Type.AUDIO) {
            ...
            double duration = 1024.0 * this.audioTagsInQueue / (48000 * this.videoTagsInQueue);
            tag.setPts((int)Math.round(this.lastPtsVideo * 1000.0));
            this.lastPtsVideo = this.lastPtsVideo
               + Math.min(
                  1.33 * duration,
                  Math.max(
                     0.6699999999999999 * duration,
                     duration + Math.min(1.0, Math.abs(this.lastPtsAudio - this.lastPtsVideo)) * (this.lastPtsAudio - this.lastPtsVideo)
                  )
               );
            this.videoTagsInQueue--;
      ...
      private double audioFrameDuration(FLVTag.AudioTagHeader audioTagHeader) {
         if (Codec.AAC == audioTagHeader.getCodec()) {
            return 1024.0 / audioTagHeader.getAudioFormat().getSampleRate();
         } else if (Codec.MP3 == audioTagHeader.getCodec()) {
            return 1152.0 / audioTagHeader.getAudioFormat().getSampleRate();
         } else {
   ...
   public static class InfoPacketProcessor implements FLVTool.PacketProcessor {
      ...
      private void dumpOnePacket(FLVTag pkt, int duration) {
         ...
         if (pkt.getTagHeader() instanceof FLVTag.VideoTagHeader) {
            ...
            System.out
               .print("|C=" + at.getCodec() + "|SR=" + format.getSampleRate() + "|SS=" + (format.getSampleSizeInBits() >> 3) + "|CH=" + format.getChannels());
      ...
      public static class Factory implements FLVTool.PacketProcessorFactory {
         private static final MainUtils.Flag FLAG_CHECK = MainUtils.Flag.flag(
            "check", null, "Check sanity and report errors only, no packet dump will be generated."
         );
   ...
   public static class ShiftPtsProcessor implements FLVTool.PacketProcessor {
      ...
      @Override
      public boolean processPacket(FLVTag pkt, FLVWriter writer) throws IOException {
         boolean avcPrivatePacket = pkt.getType() == FLVTag.Type.VIDEO
            && ((FLVTag.VideoTagHeader)pkt.getTagHeader()).getCodec() == Codec.H264
            && ((FLVTag.AvcVideoTagHeader)pkt.getTagHeader()).getAvcPacketType() == 0;
         boolean aacPrivatePacket = pkt.getType() == FLVTag.Type.AUDIO
            && ((FLVTag.AudioTagHeader)pkt.getTagHeader()).getCodec() == Codec.AAC
            && ((FLVTag.AacAudioTagHeader)pkt.getTagHeader()).getPacketType() == 0;
         boolean validPkt = pkt.getType() != FLVTag.Type.SCRIPT && !avcPrivatePacket && !aacPrivatePacket;
         if (this.expectWrapAround && validPkt && pkt.getPts() < this.prevPts && (long)this.prevPts - pkt.getPts() > 1073741824L) {
         ...
         if (this.firstPtsSeen) {
            ...
            if (this.shiftBy != null) {
               ...
               if (this.ptsDelta + pkt.getPts() < 0L) {
      ...
      private void writePacket(FLVTag pkt, FLVWriter writer) throws IOException {
         long newPts = pkt.getPts() + this.ptsDelta;
         if (newPts < 0L) {
      ...
      public static class Factory implements FLVTool.PacketProcessorFactory {
         @Override
         public FLVTool.PacketProcessor newPacketProcessor(MainUtils.Cmd flags) {
            return new FLVTool.ShiftPtsProcessor(
               flags.getIntegerFlagD(FLVTool.ShiftPtsProcessor.FLAG_TO, 0),
               flags.getIntegerFlag(FLVTool.ShiftPtsProcessor.FLAG_BY),
               flags.getBooleanFlagD(FLVTool.ShiftPtsProcessor.FLAG_WRAP_AROUND, false)
            );
         }
