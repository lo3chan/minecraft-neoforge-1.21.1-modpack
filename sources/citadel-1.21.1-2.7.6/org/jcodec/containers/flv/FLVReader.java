...
public class FLVReader {
   ...
   public static Codec[] videoCodecMapping = new Codec[]{
      null, null, Codec.SORENSON, Codec.FLASH_SCREEN_VIDEO, Codec.VP6, Codec.VP6, Codec.FLASH_SCREEN_V2, Codec.H264
   };
   ...
   public FLVReader(SeekableByteChannel ch) throws IOException {
      ...
      if (!readHeader(this.readBuf)) {
         ((Buffer)this.readBuf).position(0);
         ...
         Logger.warn(
            String.format(
               "Parsing a corrupt FLV file, first tag found at %d. %s",
               this.readBuf.position(),
   ...
   public FLVTag readPrevPacket() throws IOException {
      ...
      ((Buffer)this.readBuf).position(this.readBuf.position() - 4);
      if (this.readBuf.position() > startOfLastPacket) {
         ((Buffer)this.readBuf).position(this.readBuf.position() - startOfLastPacket);
         return this.parsePacket(this.readBuf);
      } else {
         long oldPos = this.ch.position() - this.readBuf.remaining();
         if (oldPos <= 9L) {
            return null;
         } else {
            long newPos = Math.max(0L, oldPos - this.readBuf.capacity() / 2);
            this.ch.setPosition(newPos);
            ((Buffer)this.readBuf).clear();
            this.ch.read(this.readBuf);
            ((Buffer)this.readBuf).flip();
            ((Buffer)this.readBuf).position((int)(oldPos - newPos));
      ...
      while (readBuf.remaining() >= 15) {
         ...
         if (readBuf.remaining() >= payloadSize + 4) {
            ...
            if (thisPacketSize != payloadSize + 11) {
               ((Buffer)readBuf).position(readBuf.position() - 15);
               if (!this.repositionFile()) {
   ...
   public static boolean positionAtPacket(ByteBuffer readBuf) {
      ...
      while (dup.hasRemaining()) {
         ...
         if (dup.position() >= 8 && pointerPos < dup.limit() - 4 && dup.getInt(pointerPos) - payloadSize == 11) {
            ((Buffer)readBuf).position(dup.position() - 8);
            return true;
         }
      }
      ...
      for (int i = 0; i < 10; i++) {
         while (this.readBuf.hasRemaining()) {
            ...
            if (this.readBuf.position() >= 8 && pointerPos < this.readBuf.limit() - 4 && this.readBuf.getInt(pointerPos) - payloadSize == 11) {
               ((Buffer)this.readBuf).position(this.readBuf.position() - 8);
               return true;
            }
         }
