...
public class MXFDemuxer {
   ...
   public MXFDemuxer.MXFDemuxerTrack[] findTracks() throws IOException {
      ...
      return tracks.values().toArray(new MXFDemuxer.MXFDemuxerTrack[tracks.size()]);
   }
   ...
   public static List<MXFMetadata> readPartitionMeta(SeekableByteChannel ff, MXFPartition header) throws IOException {
      ...
      KLV kl;
      while (metaBuffer.hasRemaining() && (kl = KLV.readKLFromBuffer(metaBuffer, basePos)) != null && metaBuffer.remaining() >= kl.len) {
   ...
   private void findIndex() {
      ...
      for (MXFMetadata meta : this.metadata) {
         if (meta instanceof IndexSegment) {
            ...
            this.duration = this.duration + (double)is.getIndexEditRateDen() * is.getIndexDuration() / is.getIndexEditRateNum();
         }
      }
   }
   ...
   public MXFDemuxer.MXFDemuxerTrack[] getAudioTracks() {
      ...
      return audio.toArray(new MXFDemuxer.MXFDemuxerTrack[0]);
   }
   ...
   public static class MXFDemuxerTrack implements SeekableDemuxerTrack {
      ...
      public Packet nextFrame() throws IOException {
         if (this.indexSegmentIdx >= this.demuxer.indexSegments.size()) {
            ...
            boolean kf;
            for (kf = seg.getIe().getKeyFrameOff()[this.indexSegmentSubIdx] == 0;
               frameEssenceOffset >= this.partEssenceOffset + this.demuxer.partitions.get(this.partIdx).getEssenceLength()
                  && this.partIdx < this.demuxer.partitions.size() - 1;
               this.partIdx++
            ) {
               this.partEssenceOffset = this.partEssenceOffset + this.demuxer.partitions.get(this.partIdx).getEssenceLength();
            }
            ...
            if (!this.audio) {
               ...
               this.pts = this.pts + this.audioFrameDuration;
            }
      ...
      public MXFDemuxer.MXFPacket readPacket(long off, int len, long pts, int timescale, int duration, int frameNo, boolean kf) throws IOException {
         ...
         synchronized (ch) {
            ...
            return kl != null && this.essenceUL.equals(kl.key)
               ? new MXFDemuxer.MXFPacket(
                  NIOUtils.fetchFromChannel(ch, (int)kl.len),
                  pts,
                  timescale,
                  duration,
                  frameNo,
                  kf ? Packet.FrameType.KEY : Packet.FrameType.INTER,
                  null,
                  off,
                  len
               )
      ...
      public boolean gotoFrame(long frameNo) {
         if (frameNo == this.frameNo) {
            ...
            for (this.indexSegmentIdx = 0;
               this.indexSegmentIdx < this.demuxer.indexSegments.size()
      ...
      public DemuxerTrackMeta getMeta() {
         ...
         return new DemuxerTrackMeta(
            ...
            null,
            this.demuxer.totalFrames,
            null,
            VideoCodecMeta.createSimpleVideoCodecMeta(size, ColorSpace.YUV420),
            null
         );
      }
   ...
   public static class MXFPacket extends Packet {
      ...
      public MXFPacket(
         ByteBuffer data, long pts, int timescale, long duration, long frameNo, Packet.FrameType frameType, TapeTimecode tapeTimecode, long offset, int len
      ) {
