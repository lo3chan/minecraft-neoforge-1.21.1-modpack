...
public class LanczosInterpolator implements AudioFilter {
   ...
   public static double lanczos(double x, int a) {
      return x < -a ? 0.0 : (x > a ? 0.0 : a * Math.sin(3.141592653589793 * x) * Math.sin(3.141592653589793 * x / a) / (9.869604401089358 * x * x));
   }

   public LanczosInterpolator(int fromRate, int toRate) {
      this.rateStep = (double)fromRate / toRate;
   }
   ...
   public void filter(FloatBuffer[] _in, long[] pos, FloatBuffer[] out) {
      if (_in.length != 1) {
         ...
         if (out0.remaining() < (in0.remaining() - 6) / this.rateStep) {
            ...
            while (true) {
               double inSample = 3.0 + outSample * this.rateStep + Math.ceil(pos[0] / this.rateStep) * this.rateStep - pos[0];
               int p0i = (int)Math.floor(inSample);
               int q0i = (int)Math.ceil(inSample);
               if (p0i >= in0.limit() - 3) {
                  ((Buffer)in0).position(p0i - 3);
                  return;
               }
