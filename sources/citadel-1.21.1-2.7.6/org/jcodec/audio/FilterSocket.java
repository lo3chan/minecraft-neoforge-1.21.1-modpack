...
public class FilterSocket {
   ...
   public void allocateBuffers(int bufferSize) {
      for (int i = 0; i < this.totalInputs; i++) {
         ...
         ((Buffer)this.buffers[i]).position(this.delays[i]);
      }
   }
   ...
   public void filter(FloatBuffer[] outputs) {
      if (outputs.length != this.totalOutputs) {
         ...
         for (int oi = 0; i < this.filters.length; i++) {
            this.filters[i]
               .filter(
                  Platform.copyOfRangeO(this.buffers, ii, this.filters[i].getNInputs() + ii),
                  Platform.copyOfRangeL(this.positions, ii, this.filters[i].getNInputs() + ii),
                  Platform.copyOfRangeO(outputs, oi, this.filters[i].getNOutputs() + oi)
               );
   ...
   public void rotate() {
      for (int i = 0; i < this.buffers.length; i++) {
         this.positions[i] = this.positions[i] + this.buffers[i].position();
