...
public class LanczosResampler extends BaseResampler {
   ...
   public LanczosResampler(Size from, Size to) {
      super(from, to);
      this._scaleFactorX = (double)to.getWidth() / from.getWidth();
      this._scaleFactorY = (double)to.getHeight() / from.getHeight();
   ...
   private static void buildTaps(int nTaps, int precision, double scaleFactor, short[][] tapsOut) {
      ...
      for (int i = 0; i < precision; i++) {
         ...
         for (int t = 0; j < nTaps / 2 + 1; t++) {
            double x = -o + j;
            double sinc_val = scaleFactor * sinc(scaleFactor * x * 3.141592653589793);
            double wnd_val = Math.sin(x * 3.141592653589793 / (nTaps - 1) + 1.5707963267948966);
   ...
   protected short[] getTapsX(int dstX) {
      int oi = (int)(dstX * this.precision / this._scaleFactorX);
   ...
   protected short[] getTapsY(int dstY) {
      int oy = (int)(dstY * this.precision / this._scaleFactorY);
