   ...
   private void doIt(String _in) throws Exception {
      ...
      try {
         ...
         for (int i = sf; (inFrame = inTrack.nextFrame()) != null; i++) {
            ...
            decodedPics.add(
               decoder.decodeFrameFromNals(nalUnits, Picture.create(size.getWidth() + 15 & -16, size.getHeight() + 15 & -16, ColorSpace.YUV420).getData())
            );
            if (i % 500 == 0) {
      ...
      try {
         ...
         for (Picture pic : decodedPics) {
            ...
            boolean equals = Platform.arrayEqualsByte(
               ArrayUtil.toByteArrayShifted(JCodecUtil2.getAsIntArray(yuv, pic.getPlaneWidth(0) * pic.getPlaneHeight(0))), pic.getPlaneData(0)
            );
            equals &= Platform.arrayEqualsByte(
               ArrayUtil.toByteArrayShifted(JCodecUtil2.getAsIntArray(yuv, pic.getPlaneWidth(1) * pic.getPlaneHeight(1))), pic.getPlaneData(1)
            );
            equals &= Platform.arrayEqualsByte(
               ArrayUtil.toByteArrayShifted(JCodecUtil2.getAsIntArray(yuv, pic.getPlaneWidth(2) * pic.getPlaneHeight(2))), pic.getPlaneData(2)
            );
            if (!equals) {
               this.diff(seqNo);
            }
