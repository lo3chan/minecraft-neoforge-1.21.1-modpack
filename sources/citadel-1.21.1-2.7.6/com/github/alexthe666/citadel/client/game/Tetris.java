...
public class Tetris {
   ...
   private boolean groundedTetromino() {
      for (Vec3i vec : this.fallingShape.getRelativePositions()) {
         ...
         if (x >= 0 && x < 10 && y < HEIGHT && (y == 0 || this.settledBlocks[x][y - 1] != null)) {
            return true;
         }
      }
   ...
   private void groundTetromino() {
      ...
      Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(this.fallingBlock.getSoundType().getPlaceSound(), 1.0F));
   }
   ...
   private void generateNextTetromino() {
      ...
      for (int tries = 0; tries < 5; tries++) {
         try {
            BlockState block = ((Block)((Reference)BuiltInRegistries.BLOCK.getAny().get()).getDelegate().value()).defaultBlockState();
            BakedModel blockModel = Minecraft.getInstance().getBlockRenderer().getBlockModel(block);
            if (!block.is(Blocks.GLOWSTONE)
               && !blockModel.isCustomRenderer()
               && blockModel.getRenderTypes(block, this.random, ModelData.EMPTY).contains(RenderType.solid())) {
               randomState = block;
               break;
            }
   ...
   private void renderTetromino(
      TetrominoShape shape, BlockState blockState, Rotation fallingRotation, float x, float y, float scale, float offsetX, float offsetY
   ) {
   ...
   public void render(TitleScreen screen, GuiGraphics guiGraphics, float partialTick) {
      float scale = Math.min(screen.width / 15.0F, (float)screen.height / HEIGHT);
      float offsetX = screen.width / 2.0F - scale * 5.0F;
      float offsetY = scale * 0.5F;
      if (this.started) {
         guiGraphics.fill(
            RenderType.guiOverlay(),
            (int)(screen.width * 0.05F),
            (int)(screen.height * 0.3F),
            (int)(screen.width * 0.05F) + 70,
            (int)(screen.height * 0.5F),
            -1873784752
         );
         guiGraphics.fill(
            RenderType.guiOverlay(),
            (int)(screen.width * 0.7F),
            (int)(screen.height * 0.3F),
            (int)(screen.width * 0.7F) + 130,
            (int)(screen.height * 0.84F),
            -1873784752
         );
         ...
         if (this.nextShape != null) {
            this.renderTetromino(this.nextShape, this.nextBlock, Rotation.NONE, 0.0F, 0.0F, scale, screen.width * 0.85F, screen.height * 0.4F);
         }
         ...
         int var10003 = (int)(screen.width * 0.065F);
         guiGraphics.drawCenteredString(Minecraft.getInstance().font, "SCORE", var10003, (int)(screen.height * 0.175F), rainbow);
         guiGraphics.drawCenteredString(
            Minecraft.getInstance().font, this.score + "", (int)(screen.width * 0.065F), (int)(screen.height * 0.175F) + 10, rainbow
         );
         guiGraphics.pose().popPose();
         var10003 = (int)(screen.width * 0.71F);
         guiGraphics.drawString(Minecraft.getInstance().font, "[LEFT ARROW] move left", var10003, (int)(screen.height * 0.55F), rainbow);
         var10003 = (int)(screen.width * 0.71F);
         guiGraphics.drawString(Minecraft.getInstance().font, "[RIGHT ARROW] move right", var10003, (int)(screen.height * 0.55F) + 10, rainbow);
         var10003 = (int)(screen.width * 0.71F);
         guiGraphics.drawString(Minecraft.getInstance().font, "[UP ARROW] rotate", var10003, (int)(screen.height * 0.55F) + 20, rainbow);
         var10003 = (int)(screen.width * 0.71F);
         guiGraphics.drawString(Minecraft.getInstance().font, "[DOWN ARROW] quick drop", var10003, (int)(screen.height * 0.55F) + 30, rainbow);
         var10003 = (int)(screen.width * 0.71F);
         guiGraphics.drawString(Minecraft.getInstance().font, "[T] start over", var10003, (int)(screen.height * 0.55F) + 50, rainbow);
         ...
         if (this.gameOver) {
            ...
            guiGraphics.pose()
               .scale(
                  3.0F + (float)Math.sin(hue * 3.141592653589793) * 0.4F,
                  3.0F + (float)Math.sin(hue * 3.141592653589793) * 0.4F,
                  3.0F + (float)Math.sin(hue * 3.141592653589793) * 0.4F
               );
            guiGraphics.pose().mulPose(Axis.ZP.rotationDegrees((float)Math.sin(hue * 3.141592653589793) * 10.0F));
