...
public class BasicModelPart {
   ...
   public void render(PoseStack matrixStackIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, int color) {
      if (this.showModel && (!this.cubeList.isEmpty() || !this.childModels.isEmpty())) {
         ...
         ObjectListIterator var6 = this.childModels.iterator();

         while (var6.hasNext()) {
            BasicModelPart BasicModelPart = (BasicModelPart)var6.next();
   ...
   private void doRender(Pose matrixEntryIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, int color) {
      Matrix4f matrix4f = matrixEntryIn.pose();
      Matrix3f matrix3f = matrixEntryIn.normal();
      ObjectListIterator var8 = this.cubeList.iterator();

      while (var8.hasNext()) {
         BasicModelPart.ModelBox BasicModelPart$modelbox = (BasicModelPart.ModelBox)var8.next();
   ...
   public BasicModelPart.ModelBox getRandomCube(RandomSource randomIn) {
      return !this.cubeList.isEmpty() ? (BasicModelPart.ModelBox)this.cubeList.get(randomIn.nextInt(this.cubeList.size())) : null;
   }
