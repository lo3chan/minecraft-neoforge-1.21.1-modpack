   ...
   public static class CitadelThreadFactory implements ThreadFactory {
      ...
      public Thread newThread(@NotNull Runnable runnable) throws RuntimeException {
         ...
         if (workqueue.isSameThread()) {
            ...
            classLoader = CompletableFuture.<ClassLoader>supplyAsync(() -> Thread.currentThread().getContextClassLoader(), workqueue)
               .orTimeout(10L, TimeUnit.SECONDS)
               .exceptionally(
                  ex -> {
                     throw new RuntimeException(
                        String.format(
                           "Couldn't join threads within timeout range. Tried joining '%s' on '%s'", Thread.currentThread().getName(), workqueue.name()
                        )
                     );
                  }
               )
