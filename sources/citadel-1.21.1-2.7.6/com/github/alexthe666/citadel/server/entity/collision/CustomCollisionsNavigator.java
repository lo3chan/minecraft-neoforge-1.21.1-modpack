...
public class CustomCollisionsNavigator extends GroundPathNavigation {
   ...
   public static boolean isClearForMovementBetweenRespectCustomCollisions(Mob mob, Vec3 pos1, Vec3 pos2, boolean allowSwimming) {
      ...
      BlockHitResult hitResult = mob.level().clip(new ClipContext(pos1, vec3, Block.COLLIDER, allowSwimming ? Fluid.ANY : Fluid.NONE, mob));
      if (hitResult.getType() == Type.MISS) {
         return true;
      } else {
         BlockPos hitPos = hitResult.getBlockPos();
         BlockState state = mob.level().getBlockState(hitPos);
         return mob instanceof ICustomCollisions customCollisions
            && customCollisions.canPassThrough(hitPos, state, state.getCollisionShape(mob.level(), hitPos));
      }
   }
