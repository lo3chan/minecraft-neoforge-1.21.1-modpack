         ...
         for (int k = 0; k < sbr.N_L[sbr.bs_limiter_bands]; k++) {
            ...
            for (int var29 = ml1; var29 < ml2; var29++) {
               ...
               if ((l >= sbr.l_A[ch] || sbr.bs_add_harmonic_prev[ch][current_hi_res_band] != 0 && sbr.bs_add_harmonic_flag_prev[ch])
                  && var29 + sbr.kx == sbr.f_table_res[1][current_hi_res_band + 1] + sbr.f_table_res[1][current_hi_res_band] >> 1) {
            ...
            for (int var30 = ml1; var30 < ml2; var30++) {
               adj.G_lim_boost[l][var30] = (float)Math.sqrt(G_lim[var30] * G_boost);
               adj.Q_M_lim_boost[l][var30] = (float)Math.sqrt(Q_M_lim[var30] * G_boost);
               if (S_M[var30] != 0.0F) {
                  adj.S_M_boost[l][var30] = (float)Math.sqrt(S_M[var30] * G_boost);
               } else {
