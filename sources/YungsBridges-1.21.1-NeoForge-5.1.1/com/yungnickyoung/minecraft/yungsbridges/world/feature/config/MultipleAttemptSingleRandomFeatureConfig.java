...
public class MultipleAttemptSingleRandomFeatureConfig implements FeatureConfiguration {
   public static final Codec<MultipleAttemptSingleRandomFeatureConfig> CODEC = RecordCodecBuilder.create(
      builder -> builder.group(PlacedFeature.LIST_CODEC.fieldOf("features").forGetter(config -> config.features))
   ...
   public Stream<ConfiguredFeature<?, ?>> getFeatures() {
      return this.getPlacedFeatures().stream().flatMap(placedFeatureSupplier -> ((PlacedFeature)placedFeatureSupplier.value()).getFeatures());
   }
}
