...
public class SusBlockEntityRenderer implements BlockEntityRenderer<SusBlockEntity> {
   ...
   public SusBlockEntityRenderer(Context p_277899_) {
      this.itemRenderer = p_277899_.getItemRenderer();
   }

   public void render(
      SusBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay
   ) {
      if (pBlockEntity.getLevel() != null) {
         int i = (Integer)pBlockEntity.getBlockState().getValue(BlockStateProperties.DUSTED);
         if (i > 0) {
            ...
            if (direction != null) {
               ...
               if (!itemstack.isEmpty()) {
                  ...
                  pPoseStack.mulPose(Axis.YP.rotationDegrees((flag ? 90 : 0) + 11));
