...
public class BiomeModificationsImpl {
   ...
   @Nullable
   private static MapCodec<BiomeModificationsImpl.BiomeModifierImpl> noneBiomeModCodec = null;
      ...
      EventBusesHooks.whenAvailable(
         ...
         bus -> bus.addListener(
            event -> event.register(
               Keys.BIOME_MODIFIER_SERIALIZERS,
               registry -> registry.register(
                  ResourceLocation.fromNamespaceAndPath("architectury", "none_biome_mod_codec"),
                  noneBiomeModCodec = MapCodec.unit(BiomeModificationsImpl.BiomeModifierImpl.INSTANCE)
               )
            )
         )
      );
   ...
   private static BiomeModifications.BiomeContext wrapSelectionContext(Optional<ResourceKey<Biome>> biomeResourceKey, Builder event) {
         ...
         public boolean hasTag(TagKey<Biome> tag) {
            ...
            if (server != null) {
               ...
               if (registry.isPresent()) {
                  Optional<Reference<Biome>> holder = registry.get().getHolder(biomeResourceKey.get());
                  if (holder.isPresent()) {
   ...
   private static class BiomeModifierImpl implements BiomeModifier {
      ...
      public void modify(Holder<Biome> arg, Phase phase, Builder builder) {
         List<Pair<Predicate<BiomeModifications.BiomeContext>, BiConsumer<BiomeModifications.BiomeContext, BiomeProperties.Mutable>>> list = switch (phase) {
            case ADD -> BiomeModificationsImpl.ADDITIONS;
            case REMOVE -> BiomeModificationsImpl.REMOVALS;
            case MODIFY -> BiomeModificationsImpl.REPLACEMENTS;
            case AFTER_EVERYTHING -> BiomeModificationsImpl.POST_PROCESSING;
            default -> null;
         };
         if (list != null) {
      ...
      public MapCodec<? extends BiomeModifier> codec() {
         return BiomeModificationsImpl.noneBiomeModCodec != null ? BiomeModificationsImpl.noneBiomeModCodec : MapCodec.unit(INSTANCE);
      }
   }

   public static class BiomeWrapped implements BiomeProperties {
      protected final Builder event;
      ...
      public BiomeWrapped(Builder event) {
         this(
            event,
            new BiomeModificationsImpl.MutableClimatePropertiesWrapped(event.getClimateSettings()),
            new BiomeModificationsImpl.MutableEffectsPropertiesWrapped(event.getSpecialEffects()),
            new BiomeModificationsImpl.GenerationSettingsBuilderWrapped(event.getGenerationSettings()),
            new BiomeModificationsImpl.SpawnSettingsBuilderWrapped(event.getMobSpawnSettings())
         );
      }
   ...
   private static class GenerationSettingsBuilderWrapped implements GenerationProperties {
      ...
      @Override
      public Iterable<Holder<ConfiguredWorldCarver<?>>> getCarvers(Carving carving) {
         return this.generation.getCarvers(carving);
      }

      @Override
      public Iterable<Holder<PlacedFeature>> getFeatures(Decoration decoration) {
         return this.generation.getFeatures(decoration);
      }

      @Override
      public List<Iterable<Holder<PlacedFeature>>> getFeatures() {
         return this.generation.features;
      }
   }

   public static class MutableBiomeWrapped extends BiomeModificationsImpl.BiomeWrapped implements BiomeProperties.Mutable {
      public MutableBiomeWrapped(Builder event) {
         super(
            event,
            new BiomeModificationsImpl.MutableClimatePropertiesWrapped(event.getClimateSettings()),
            new BiomeModificationsImpl.MutableEffectsPropertiesWrapped(event.getSpecialEffects()),
            new BiomeModificationsImpl.MutableGenerationSettingsBuilderWrapped(event.getGenerationSettings()),
            new BiomeModificationsImpl.MutableSpawnSettingsBuilderWrapped(event.getMobSpawnSettings())
         );
      }
   ...
   public static class MutableEffectsPropertiesWrapped implements EffectsProperties.Mutable {
      public net.minecraft.world.level.biome.BiomeSpecialEffects.Builder builder;

      public MutableEffectsPropertiesWrapped(net.minecraft.world.level.biome.BiomeSpecialEffects.Builder builder) {
         this.builder = builder;
      }
      ...
      public OptionalInt getFoliageColorOverride() {
         return this.builder.foliageColorOverride.map(OptionalInt::of).orElseGet(OptionalInt::empty);
      }

      @Override
      public OptionalInt getGrassColorOverride() {
         return this.builder.grassColorOverride.map(OptionalInt::of).orElseGet(OptionalInt::empty);
      }
   ...
   private static class MutableGenerationSettingsBuilderWrapped
      extends BiomeModificationsImpl.GenerationSettingsBuilderWrapped
      ...
      @Override
      public GenerationProperties.Mutable addFeature(Decoration decoration, Holder<PlacedFeature> feature) {
         this.generation.addFeature(decoration, feature);
         return this;
      }

      @Override
      public GenerationProperties.Mutable addFeature(Decoration decoration, ResourceKey<PlacedFeature> feature) {
         MinecraftServer server = GameInstance.getServer();
         if (server != null) {
            Optional<? extends Registry<PlacedFeature>> registry = server.registryAccess().registry(Registries.PLACED_FEATURE);
            if (registry.isPresent()) {
               Optional<Reference<PlacedFeature>> holder = registry.get().getHolder(feature);
               if (holder.isPresent()) {
      ...
      @Override
      public GenerationProperties.Mutable addCarver(Carving carving, Holder<ConfiguredWorldCarver<?>> feature) {
         this.generation.addCarver(carving, feature);
         return this;
      }

      @Override
      public GenerationProperties.Mutable addCarver(Carving carving, ResourceKey<ConfiguredWorldCarver<?>> feature) {
         MinecraftServer server = GameInstance.getServer();
         if (server != null) {
            Optional<? extends Registry<ConfiguredWorldCarver<?>>> registry = server.registryAccess().registry(Registries.CONFIGURED_CARVER);
            if (registry.isPresent()) {
               Optional<Reference<ConfiguredWorldCarver<?>>> holder = registry.get().getHolder(feature);
               if (holder.isPresent()) {
      ...
      @Override
      public GenerationProperties.Mutable removeFeature(Decoration decoration, ResourceKey<PlacedFeature> feature) {
         this.generation.getFeatures(decoration).removeIf(supplier -> supplier.is(feature));
         return this;
      }

      @Override
      public GenerationProperties.Mutable removeCarver(Carving carving, ResourceKey<ConfiguredWorldCarver<?>> feature) {
   ...
   private static class MutableSpawnSettingsBuilderWrapped extends BiomeModificationsImpl.SpawnSettingsBuilderWrapped implements SpawnProperties.Mutable {
      ...
      @Override
      public SpawnProperties.Mutable addSpawn(MobCategory category, SpawnerData data) {
      ...
      @Override
      public SpawnProperties.Mutable setSpawnCost(EntityType<?> entityType, MobSpawnCost cost) {
         this.builder.addMobCharge(entityType, cost.charge(), cost.energyBudget());
         return this;
      }
   ...
   private static class SpawnSettingsBuilderWrapped implements SpawnProperties {
      ...
      @Override
      public Map<MobCategory, List<SpawnerData>> getSpawners() {
         return this.builder.spawners;
      }

      @Override
      public Map<EntityType<?>, MobSpawnCost> getMobSpawnCosts() {
