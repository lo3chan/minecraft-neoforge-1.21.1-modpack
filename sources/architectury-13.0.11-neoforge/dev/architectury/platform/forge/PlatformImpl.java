...
public class PlatformImpl {
   ...
   public static Collection<String> getModIds() {
      return ModList.get().getMods().stream().<String>map(IModInfo::getModId).collect(Collectors.toList());
   }
   ...
   private static class ModImpl implements Mod {
      ...
      public Optional<Path> findResource(String... path) {
         return Optional.of(this.info.getOwningFile().getFile().findResource(path)).filter(x$0 -> Files.exists(x$0));
      }

      @Override
      public Collection<String> getAuthors() {
         Optional<String> optional = this.info.getConfig().getConfigElement(new String[]{"authors"}).map(String::valueOf);
         return (Collection<String>)(optional.isPresent() ? Collections.singleton(optional.get()) : Collections.emptyList());
      }
      ...
      public void registerConfigurationScreen(Mod.ConfigurationScreenProvider configurationScreenProvider) {
         this.container
            .registerExtensionPoint(IConfigScreenFactory.class, (IConfigScreenFactory)(container, screen) -> configurationScreenProvider.provide(screen));
      }
   }
}
