...
public class JigsawStructureAssembler {
   ...
   private void addChildrenForPiece(PieceEntry pieceEntry) {
      ...
      List<StructureBlockInfo> pieceJigsawBlocks = piece.getElement()
         .getShuffledJigsawBlocks(this.settings.structureTemplateManager, piece.getPosition(), piece.getRotation(), this.settings.rand);
      ...
      for (StructureBlockInfo jigsawBlockInfo : pieceJigsawBlocks) {
         ...
         if (optionalPoolHolder.isEmpty()) {
            ...
            if (targetPool.size() == 0 && !targetPoolHolder.is(Pools.EMPTY)) {
               ...
               Holder<StructureTemplatePool> fallbackPoolHolder = ((StructureTemplatePool)targetPoolHolder.value()).getFallback();
               ...
               if (fallbackPool.size() == 0 && !fallbackPoolHolder.is(Pools.EMPTY)) {
                  YungsApiCommon.LOGGER
                     .warn(
                        "Empty or nonexistent fallback pool: {}", fallbackPoolHolder.unwrapKey().map(key -> key.location().toString()).orElse("<unregistered>")
                     );
               } else {
   ...
   private PieceContext createPieceContextForJigsawBlock(StructureBlockInfo jigsawBlockInfo, PieceEntry pieceEntry, MutableObject<BoxOctree> parentOctree) {
      ...
      return new PieceContext(
         null, jigsawBlockInfo, jigsawBlockTargetPos, pieceBoundingBox.minY(), jigsawBlockInfo.pos(), pieceOctree, pieceEntry, pieceEntry.getDepth()
      );
   }
   ...
   private Optional<StructurePoolElement> chooseCandidateFromPool(PieceContext context) {
      ...
      boolean isPieceRigid = piece.getElement().getProjection() == Projection.RIGID;
      ...
      while (candidatePoolElements.size() > 0 && totalWeightSum > 0) {
         ...
         if (chosenPoolElementx instanceof YungJigsawPoolElement yungElementx && !yungElementx.isAtValidDepth(context.depth)) {
            ...
            while (true) {
               ...
               List<StructureBlockInfo> candidateJigsawBlocks = chosenPoolElementx.getShuffledJigsawBlocks(
                  this.settings.structureTemplateManager, BlockPos.ZERO, rotation, this.settings.rand
               );
                     ...
                     .mapToInt(
                        pieceCandidateJigsawBlock -> {
                           if (!tempCandidateBoundingBox.isInside(
                              ...
                              Optional<? extends Holder<StructureTemplatePool>> candidateTargetPool = this.settings
                                 .poolRegistry
                                 .getHolder(candidateTargetPoolKey);
                              Optional<Holder<StructureTemplatePool>> candidateFallbackPool = candidateTargetPool.map(
                                 poolHolder -> ((StructureTemplatePool)poolHolder.value()).getFallback()
                              );
                              int candidateMaxSize = candidateTargetPool.<Integer>map(
                                    poolHolder -> ((StructureTemplatePool)poolHolder.value()).getMaxSize(this.settings.structureTemplateManager)
                                 )
                                 .orElse(0);
                              int candidateFallbackMaxSize = candidateFallbackPool.<Integer>map(
                                    poolHolder -> ((StructureTemplatePool)poolHolder.value()).getMaxSize(this.settings.structureTemplateManager)
                                 )
                                 .orElse(0);
               ...
               for (StructureBlockInfo candidateJigsawBlock : candidateJigsawBlocks) {
                  if (JigsawBlock.canAttach(context.jigsawBlock, candidateJigsawBlock)) {
                     ...
                     BoundingBox rotatedCandidateBoundingBox = chosenPoolElementx.getBoundingBox(
                        this.settings.structureTemplateManager, candidateJigsawBlockRelativePos, rotation
                     );
                     Projection candidateProjection = chosenPoolElementx.getProjection();
                     boolean isCandidateRigid = candidateProjection == Projection.RIGID;
                     candidateJigsawBlockRelativeY = candidateJigsawBlockPos.getY();
                     candidateJigsawYOffsetNeeded = jigsawBlockRelativeY
                        - candidateJigsawBlockRelativeY
                        + JigsawBlock.getFrontFacing(context.jigsawBlock.state()).getStepY();
   ...
   private void applyModifications() {
      for (PieceEntry pieceEntry : this.pieces) {
         if (pieceEntry.getPiece().getElement() instanceof YungJigsawSinglePoolElement yungElement && yungElement.hasModifiers()) {
            PoolElementStructurePiece piece = pieceEntry.getPiece();
   ...
   public static class Settings {
      ...
      public JigsawStructureAssembler.Settings dimensionPadding(DimensionPadding dimensionPadding) {
         this.dimensionPadding = dimensionPadding;
         return this;
      }

      public JigsawStructureAssembler.Settings liquidSettings(LiquidSettings liquidSettings) {
         this.liquidSettings = liquidSettings;
         return this;
      }

      public JigsawStructureAssembler.Settings biomeSource(BiomeSource biomeSource) {
      ...
      public boolean isInBounds(int y) {
         if (this.maxY.isPresent() && y > this.maxY.get()) {
            return false;
         } else if (this.minY.isPresent() && y < this.minY.get()) {
            return false;
         } else {
            return y < this.levelHeightAccessor.getMinBuildHeight() + this.dimensionPadding.bottom()
               ? false
               : y <= this.levelHeightAccessor.getMaxBuildHeight() - this.dimensionPadding.top();
         }
      }
   }
}
