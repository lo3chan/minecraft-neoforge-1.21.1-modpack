...
public class BlockStateRandomizer {
   public static final Codec<BlockStateRandomizer> CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
            BlockStateRandomizer.Entry.CODEC.listOf().fieldOf("entries").forGetter(randomizer -> randomizer.entries),
   ...
   public CompoundTag saveTag() {
      ...
      ListTag entriesTag = (ListTag)Util.make(new ListTag(), tag -> this.entries.forEach(entry -> {
   ...
   public BlockStateRandomizer(CompoundTag compoundTag) {
      this.defaultBlockState = (BlockState)Block.BLOCK_STATE_REGISTRY.byId(compoundTag.getInt("defaultBlockStateId"));
      ...
      entriesTag.forEach(entryTag -> {
         ...
         BlockState blockState = (BlockState)Block.BLOCK_STATE_REGISTRY.byId(entryCompoundTag.getInt("entryBlockStateId"));
   ...
   public static BlockStateRandomizer from(BlockState... blockStates) {
      ...
      float chance = 1.0F / blockStates.length;
   ...
   public BlockStateRandomizer addBlock(BlockState blockState, float chance) {
      if (this.entries.stream().anyMatch(entry -> entry.blockState.equals(blockState))) {
         ...
         float currTotal = this.entries.stream().map(entry -> entry.probability).reduce(Float::sum).orElse(0.0F);
