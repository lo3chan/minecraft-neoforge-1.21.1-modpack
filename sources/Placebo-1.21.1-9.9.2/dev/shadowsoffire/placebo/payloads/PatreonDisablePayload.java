...
public record PatreonDisablePayload(PatreonDisablePayload.CosmeticType cosmetic, UUID id) implements CustomPacketPayload {
   ...
   public static final StreamCodec<FriendlyByteBuf, PatreonDisablePayload> CODEC = StreamCodec.composite(
      PatreonDisablePayload.CosmeticType.STREAM_CODEC,
      PatreonDisablePayload::cosmetic,
      UUIDUtil.STREAM_CODEC,
      PatreonDisablePayload::id,
   ...
   public static enum CosmeticType {
      ...
      public static final IntFunction<PatreonDisablePayload.CosmeticType> BY_ID = ByIdMap.continuous(Enum::ordinal, values(), OutOfBoundsStrategy.ZERO);
      public static final StreamCodec<ByteBuf, PatreonDisablePayload.CosmeticType> STREAM_CODEC = ByteBufCodecs.idMapper(BY_ID, Enum::ordinal);
   }
   ...
   public static class Provider implements PayloadProvider<PatreonDisablePayload> {
      ...
      public void handle(PatreonDisablePayload msg, IPayloadContext ctx) {
         if (ctx.flow() == PacketFlow.SERVERBOUND) {
            PacketDistributor.sendToAllPlayers(new PatreonDisablePayload(msg.cosmetic(), ctx.player().getUUID()), new CustomPacketPayload[0]);
         } else {
