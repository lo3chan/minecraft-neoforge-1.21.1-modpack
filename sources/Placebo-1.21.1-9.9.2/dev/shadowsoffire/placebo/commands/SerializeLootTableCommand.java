...
public class SerializeLootTableCommand {
   ...
   public static void register(LiteralArgumentBuilder<CommandSourceStack> builder) {
      builder.then(
         ((LiteralArgumentBuilder)Commands.literal("serialize_loot_table").requires(s -> s.hasPermission(2)))
            .then(
               Commands.argument("loot_table", ResourceLocationArgument.id())
                  ...
                  .executes(
                     ctx -> {
                        ...
                        RegistryAccess access = ((CommandSourceStack)ctx.getSource()).getServer().reloadableRegistries().get();
                        LootTable table = (LootTable)access.registryOrThrow(Registries.LOOT_TABLE).get(id);
                        if (table == LootTable.EMPTY) {
                           throw NOT_FOUND.create(id);
                        } else {
                           RuntimeDatagenHelpers.write(
                              (JsonElement)LootTable.DIRECT_CODEC.encodeStart(RegistryOps.create(JsonOps.INSTANCE, access), table).getOrThrow(),
                              "loot_table",
                              id
                           );
                           String path = "datagen/" + id.getNamespace() + "/loot_table/" + id.getPath() + ".json";
                           ((CommandSourceStack)ctx.getSource())
                              .sendSuccess(() -> Component.translatable("placebo.cmd.serialize_success", new Object[]{id.toString(), path}), true);
                           return 0;
                        }
                     }
                  )
