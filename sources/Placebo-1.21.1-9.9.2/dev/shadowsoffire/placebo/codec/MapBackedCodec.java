...
public class MapBackedCodec<V extends CodecProvider<? super V>> implements Codec<V> {
   ...
   public <T> DataResult<Pair<V, T>> decode(DynamicOps<T> ops, T input) {
      ...
      Optional<ResourceLocation> key = type.map(
         t -> (ResourceLocation)((Pair)ResourceLocation.CODEC.decode(ops, t).resultOrPartial(Placebo.LOGGER::error).get()).getFirst()
      );
      Codec codec = key.map(this.registry::get).orElse((Codec)this.defaultCodec.get());
      return codec == null
         ? DataResult.error(() -> "Failure when parsing a " + this.name + ". Unrecognized type: " + key.<String>map(ResourceLocation::toString).orElse("null"))
   ...
   public <T> DataResult<T> encode(V input, DynamicOps<T> ops, T prefix) {
      ...
      if (key == null) {
         ...
         T encodedKey = (T)ResourceLocation.CODEC.encodeStart(ops, key).getOrThrow(IllegalStateException::new);
         T encodedObj = (T)codec.encode(input, ops, prefix).getOrThrow(IllegalStateException::new);
