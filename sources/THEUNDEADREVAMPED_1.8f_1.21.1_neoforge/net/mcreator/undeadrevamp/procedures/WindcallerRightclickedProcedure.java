...
public class WindcallerRightclickedProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
      if (entity != null) {
         if (world instanceof ServerLevel projectileLevel) {
            Projectile _entityToSpawn = (new Object() {
               public Projectile getArrow(Level level, Entity shooter, float damage, final int knockback, final byte piercing) {
                  AbstractArrow entityToSpawn = new WINDTEXTProjectileEntity((EntityType)UndeadRevamp2ModEntities.WINDTEXT_PROJECTILE.get(), level) {
            ...
            }).getArrow(projectileLevel, entity, 3.0F, 0, (byte)30);
         ...
         if (entity instanceof Player _player) {
            _player.getCooldowns().addCooldown(itemstack.getItem(), 359);
         }

         if (world instanceof ServerLevel _levelx) {
            itemstack.hurtAndBreak(3, _levelx, null, _stkprov -> {});
         }
         ...
         entity.setDeltaMovement(
            new Vec3(
               Math.sin(Math.toRadians(entity.getYRot() + 180.0F)) * 1.25 * -2.0,
               (Math.sin(Math.toRadians(0.0F - entity.getXRot())) + 0.5) * 1.5,
               Math.cos(Math.toRadians(entity.getYRot())) * 1.25 * -2.0
            )
         );
      }
   }
