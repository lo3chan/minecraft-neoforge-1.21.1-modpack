...
import net.mcreator.undeadrevamp.procedures.SuckerEntityIsHurtProcedure;
import net.mcreator.undeadrevamp.procedures.SuckerOnEntityTickUpdateProcedure;
import net.mcreator.undeadrevamp.procedures.SuckerOnInitialEntitySpawnProcedure;
...
public class SuckerEntity extends Monster implements GeoEntity {
   ...
   protected void defineSynchedData(Builder builder) {
      ...
      builder.define(TEXTURE, "thesucker");
   }
   ...
   protected void registerGoals() {
      super.registerGoals();
      this.targetSelector.addGoal(1, new NearestAttackableTargetGoal(this, BigsuckerEntity.class, false, false) {
      ...
      this.goalSelector.addGoal(2, new Goal() {
         ...
         public boolean canUse() {
            return SuckerEntity.this.getTarget() != null && !SuckerEntity.this.getMoveControl().hasWanted();
         }

         public boolean canContinueToUse() {
            return SuckerEntity.this.getMoveControl().hasWanted() && SuckerEntity.this.getTarget() != null && SuckerEntity.this.getTarget().isAlive();
         }

         public void start() {
            LivingEntity livingentity = SuckerEntity.this.getTarget();
            Vec3 vec3d = livingentity.getEyePosition(1.0F);
            SuckerEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1.2);
         }

         public void tick() {
            LivingEntity livingentity = SuckerEntity.this.getTarget();
            if (SuckerEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
               SuckerEntity.this.doHurtTarget(livingentity);
            } else {
               double d0 = SuckerEntity.this.distanceToSqr(livingentity);
               if (d0 < 16.0) {
                  Vec3 vec3d = livingentity.getEyePosition(1.0F);
                  SuckerEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1.2);
               }
      ...
      this.goalSelector.addGoal(4, new LookAtPlayerGoal(this, BigsuckerEntity.class, 8.0F));
      this.goalSelector.addGoal(5, new RandomStrollGoal(this, 0.8, 20) {
         protected Vec3 getPosition() {
            RandomSource random = SuckerEntity.this.getRandom();
            double dir_x = SuckerEntity.this.getX() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double dir_y = SuckerEntity.this.getY() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double dir_z = SuckerEntity.this.getZ() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
   ...
   public boolean hurt(DamageSource source, float amount) {
      SuckerEntityIsHurtProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
   ...
   public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata) {
      ...
      SuckerOnInitialEntitySpawnProcedure.execute(world, this.getX(), this.getY(), this.getZ(), this);
      return retval;
   }
   ...
   public void baseTick() {
      super.baseTick();
      SuckerOnEntityTickUpdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
   ...
   public static void init(RegisterSpawnPlacementsEvent event) {
      event.register(
         (EntityType)UndeadRevamp2ModEntities.SUCKER.get(),
