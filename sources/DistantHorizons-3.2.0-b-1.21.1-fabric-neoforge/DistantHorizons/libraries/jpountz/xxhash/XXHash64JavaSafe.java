...
import DistantHorizons.libraries.jpountz.util.SafeUtils;
...
final class XXHash64JavaSafe extends XXHash64 {
   ...
   public long hash(byte[] buf, int off, int len, long seed) {
      ...
      while (off < end) {
         h64 ^= (SafeUtils.readByte(buf, off) & 255) * 2870177450012600261L;
   ...
   public long hash(ByteBuffer buf, int off, int len, long seed) {
      if (buf.hasArray()) {
         ...
         while (off < end) {
            h64 ^= (ByteBufferUtils.readByte(buf, off) & 255) * 2870177450012600261L;
            h64 = Long.rotateLeft(h64, 11) * -7046029288634856825L;
            off++;
         }
