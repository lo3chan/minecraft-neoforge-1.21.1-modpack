...
public class GlDhFogRenderer_fabric implements IDhFogRenderer {
   public static GlDhFogRenderer_fabric INSTANCE = new GlDhFogRenderer_fabric();
   ...
   private GlDhFogRenderer_fabric() {
   }

   public void init() {
      if (!this.init) {
         this.init = true;
         GlDhFogShader_fabric.INSTANCE.init();
         GlDhFogApplyShader_fabric.INSTANCE.init();
      }
   }
   ...
   public void render(RenderParams renderParams, DhApiFogRenderParam fogRenderParams) {
      try (GLState state = new GLState()) {
         ...
         GlDhFogShader_fabric.INSTANCE.frameBuffer = this.fogFramebuffer;
         GlDhFogShader_fabric.INSTANCE.prepUniformObjects(renderParams.dhMvmProjMatrix, fogRenderParams);
         GlDhFogShader_fabric.INSTANCE.render(renderParams);
         GlDhFogApplyShader_fabric.INSTANCE.fogTexture = this.fogTexture;
         GlDhFogApplyShader_fabric.INSTANCE.render(renderParams);
      }
   }

   public void free() {
      GlDhFogShader_fabric.INSTANCE.free();
      GlDhFogApplyShader_fabric.INSTANCE.free();
   }
}
