...
public class GlGenericObjectShaderProgram extends GlShaderProgram implements IDhApiGenericObjectShaderProgram {
   ...
   public final GlAbstractVertexAttribute va = GlAbstractVertexAttribute.create();
   ...
   public GlGenericObjectShaderProgram(boolean useInstancedRendering) {
      super(
         useInstancedRendering ? "assets/distanthorizons/shaders/generic/gl/instanced/vert.vert" : "assets/distanthorizons/shaders/generic/gl/direct/vert.vert",
         useInstancedRendering ? "assets/distanthorizons/shaders/generic/gl/instanced/frag.frag" : "assets/distanthorizons/shaders/generic/gl/direct/frag.frag",
         "vPosition"
      );
      this.va.bind();
   ...
   public void bindVertexBuffer(int vbo) {
      this.va.bindBufferToAllBindingPoints(vbo);
   }

   @Override
   public void fillIndirectUniformData(
      DhApiRenderParam renderParameters, DhApiRenderableBoxGroupShading shading, IDhApiRenderableBoxGroup boxGroup, DhApiVec3d camPos
   ) {
      ...
      super.bind();
      this.setUniform(
         this.instancedShaderOffsetChunkUniform,
         new DhApiVec3i(
      ...
      this.setUniform(
         this.instancedShaderOffsetSubChunkUniform,
         new DhVec3f(
      ...
      this.setUniform(
         ...
         new DhApiVec3i(LodUtil.getChunkPosFromDouble(camPos.x), LodUtil.getChunkPosFromDouble(camPos.y), LodUtil.getChunkPosFromDouble(camPos.z))
      );
      this.setUniform(
         this.instancedShaderCameraSubChunkPosUniform,
         new DhVec3f(LodUtil.getSubChunkPosFromDouble(camPos.x), LodUtil.getSubChunkPosFromDouble(camPos.y), LodUtil.getSubChunkPosFromDouble(camPos.z))
      );
   ...
   @Override
   public void fillDirectUniformData(DhApiRenderParam renderParameters, IDhApiRenderableBoxGroup boxGroup, DhApiRenderableBox box, DhApiVec3d camPos) {
      DhMat4f projectionMvmMatrix = new DhMat4f(renderParameters.dhProjectionMatrix);
      projectionMvmMatrix.multiply(renderParameters.dhModelViewMatrix);
      ...
      boxTransform.multiply(
         DhMat4f.createScaleMatrix((float)(box.maxPos.x - box.minPos.x), (float)(box.maxPos.y - box.minPos.y), (float)(box.maxPos.z - box.minPos.z))
      );
