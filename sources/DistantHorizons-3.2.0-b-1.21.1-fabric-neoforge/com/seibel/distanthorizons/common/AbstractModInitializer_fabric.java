...
import com.seibel.distanthorizons.common.commands.CommandInitializer_fabric;
import com.seibel.distanthorizons.common.wrappers.DependencySetup_fabric;
...
public abstract class AbstractModInitializer_fabric {
   protected static final DhLogger LOGGER = new DhLoggerBuilder().build();
   private CommandInitializer_fabric commandInitializer;

   protected abstract void createInitialSharedBindings();

   protected abstract void createInitialClientBindings();

   protected abstract AbstractModInitializer$IEventProxy_fabric createClientProxy();

   protected abstract AbstractModInitializer$IEventProxy_fabric createServerProxy(boolean bl);
   ...
   public void onInitializeClient() {
      DependencySetup_fabric.createClientBindings();
   ...
   public void onInitializeServer() {
      DependencySetup_fabric.createServerBindings();
      ...
      this.commandInitializer = new CommandInitializer_fabric();
      this.subscribeRegisterCommandsEvent(dispatcher -> this.commandInitializer.initCommands(dispatcher));
      this.subscribeServerStartingEvent(server -> {
         MinecraftServerWrapper_fabric.INSTANCE.dedicatedServer = (class_3176)server;
         this.initConfig();
         Initializer.postConfigInit();
         this.postInit();
         this.postServerInit();
         ...
         String serverFolderPath = server.method_3831() + "";
         LOGGER.info("Distant Horizons server Initialized at " + serverFolderPath);
      });
   }

   private void startup() {
      DependencySetup_fabric.createSharedBindings();
   ...
   private void postClientInit() {
      ...
      Thread dhSetupThread = new Thread(() -> {
         try {
            DependencySetup_fabric.setRenderingApiBindings();
         } catch (Exception var5) {
            NativeDialogUtil.showDialog("Distant Horizons", var5.getMessage(), "ok", "error");
            MinecraftClientWrapper_fabric.INSTANCE.crashMinecraft(var5.getMessage(), var5);
