...
public class ServerPlayerStateManager {
   ...
   private final ConcurrentMap<IServerPlayerWrapper, ServerPlayerStateManager.MessageQueueState> messageQueueByPlayerWrapper = new ConcurrentHashMap<>();
   ...
   public void handlePluginMessage(IServerPlayerWrapper player, AbstractNetworkMessage message) {
      if (player != null && message != null) {
         ServerPlayerStateManager.MessageQueueState messageQueue = this.messageQueueByPlayerWrapper
            .computeIfAbsent(player, k -> new ServerPlayerStateManager.MessageQueueState());
   ...
   public void handlePluginMessagesFromQueue(ServerPlayerState playerState) {
      ServerPlayerStateManager.MessageQueueState messageQueue = this.messageQueueByPlayerWrapper
         .computeIfAbsent(playerState.getServerPlayer(), k -> new ServerPlayerStateManager.MessageQueueState());
      this.handlePluginMessagesFromQueue(playerState, messageQueue);
   }

   private void handlePluginMessagesFromQueue(ServerPlayerState playerState, ServerPlayerStateManager.MessageQueueState messageQueueState) {
   ...
   public ServerPlayerState getConnectedPlayer(IServerPlayerWrapper player) {
      return this.connectedPlayerStateByPlayerWrapper.get(player);
   }
   ...
   public Iterable<ServerPlayerState> getReadyPlayers() {
      return this.getConnectedPlayers().stream().filter(ServerPlayerState::isReady)::iterator;
   }
