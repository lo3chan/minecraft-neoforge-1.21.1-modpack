...
public class FullDataPayloadSender implements AutoCloseable {
   ...
   private final ConcurrentLinkedQueue<FullDataPayloadSender.PendingTransfer> transferQueue = new ConcurrentLinkedQueue<>();
   ...
   public void sendInChunks(FullDataPayload payload, Runnable sendFinalMessage) {
      this.transferQueue.add(new FullDataPayloadSender.PendingTransfer(payload, sendFinalMessage));
   }
   ...
   private void tick() {
      ...
      int bytesToSend = maxPlayerRate > 0 ? maxPlayerRate * 1000 / 20 + 1 : 2147483647;
      this.sharedBandwidthLimit.setSenderActive(this, bytesToSend > 0);

      while (bytesToSend > 0) {
         FullDataPayloadSender.PendingTransfer pendingTransfer = this.transferQueue.peek();
         if (pendingTransfer == null) {
            return;
         }

         int chunkSize = Math.min(Math.min(bytesToSend, 1048000), pendingTransfer.buffer.readableBytes());
         boolean isFirstChunk = pendingTransfer.buffer.readerIndex() == 0;
         FullDataSplitMessage chunkMessage = new FullDataSplitMessage(
            pendingTransfer.bufferId, pendingTransfer.buffer.readSlice(chunkSize).retain(), isFirstChunk
         );
