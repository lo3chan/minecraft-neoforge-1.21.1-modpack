...
public class ArrayGridList<T> extends ArrayList<T> {
   ...
   public final T get(MovableGridRingList.Pos2D pos) {
      return this.get(pos.getX(), pos.getY());
   }

   public final T set(MovableGridRingList.Pos2D pos, T e) {
      return this.set(pos.getX(), pos.getY(), e);
   }

   public T get(int x, int y) {
      return !this.inRange(x, y) ? null : this.get(this.getIndexOf(x, y));
   }
   ...
   public T getLast() {
      return this.get(this.gridSize - 1, this.gridSize - 1);
   }

   public T set(int x, int y, T e) {
      return !this.inRange(x, y) ? null : this.set(this.getIndexOf(x, y), e);
   }

   public T setFirst(T e) {
      return this.set(0, 0, e);
   }

   public T setLast(T e) {
      return this.set(this.gridSize - 1, this.gridSize - 1, e);
   }
   ...
   public final void fill(Consumer<? super T> dealloc, BiFunction<Integer, Integer, T> filler) {
      this.forEachPos((x, y) -> {
   ...
   public String toString() {
      return this.getClass().toString() + " " + this.gridSize + "*" + this.gridSize + "[" + this.size() + "]";
   }
