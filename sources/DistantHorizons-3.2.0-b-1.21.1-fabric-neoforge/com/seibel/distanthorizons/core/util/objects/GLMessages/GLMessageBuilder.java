...
public class GLMessageBuilder {
   ...
   public GLMessageBuilder(
      Function<EGLMessageType, Boolean> typeFilter, Function<EGLMessageSeverity, Boolean> severityFilter, Function<EGLMessageSource, Boolean> sourceFilter
   ) {
   ...
   private boolean doesMessagePassFilters(GLMessage msg) {
      if (this.sourceFilter != null && !this.sourceFilter.apply(msg.source)) {
         ...
         return this.typeFilter != null && !this.typeFilter.apply(msg.type) ? false : this.severityFilter == null || this.severityFilter.apply(msg.severity);
      }
   }
   ...
   private boolean runNextParserStage(String str) {
      if (this.parserStage == 0) {
         ...
         return this.legacyMessage ? this.runNextLegacyParserStage(str) : this.runNextNewParserStage(str);
      }
   }
   ...
   private static String trySubstring(String string, int beginIndex) {
      return beginIndex > string.length() ? "" : string.substring(beginIndex);
   }
}
