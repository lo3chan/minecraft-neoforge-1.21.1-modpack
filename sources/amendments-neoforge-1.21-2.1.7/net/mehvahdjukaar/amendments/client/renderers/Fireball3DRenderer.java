...
public class Fireball3DRenderer<E extends Entity> extends ThrownProjectile3DRenderer<E> {
   ...
   protected int getBlockLightLevel(E entity, BlockPos pos) {
      return entity.getRemainingFireTicks() <= 0 && this.noOverlayTexture != null
         ? super.getBlockLightLevel(entity, pos)
         : entity.level().getBrightness(LightLayer.BLOCK, pos);
   }

   @Override
   public void renderBall(E entity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
      boolean hasOverlay = entity.getRemainingFireTicks() > 0 || this.noOverlayTexture == null;
      ResourceLocation mainTexture = hasOverlay ? this.getTextureLocation(entity) : this.noOverlayTexture;
      ...
      this.cube.render(poseStack, vertexConsumer, packedLight, OverlayTexture.NO_OVERLAY);
      this.cubeEmissive.render(poseStack, vertexConsumer, 15728880, OverlayTexture.NO_OVERLAY);
      if (hasOverlay) {
         float f = entity.tickCount + partialTick;
         RenderType fireRenderType = RenderType.energySwirl(this.overlayTexture, this.fireXOffset(f) % 1.0F, f * 0.01F % 1.0F);
         VertexConsumer outlineVertexConsumer = bufferSource.getBuffer(fireRenderType);
         this.overlay.render(poseStack, outlineVertexConsumer, 15728880, OverlayTexture.NO_OVERLAY);
      }
   }
