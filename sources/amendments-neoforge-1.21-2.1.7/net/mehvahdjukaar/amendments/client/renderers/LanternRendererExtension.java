...
public class LanternRendererExtension implements IThirdPersonAnimationProvider, IThirdPersonSpecialItemRenderer, IFirstPersonSpecialItemRenderer {
   ...
   public <T extends Player, M extends EntityModel<T> & ArmedModel & HeadedModel> void renderThirdPersonItem(
      ...
      if (!stack.isEmpty()) {
         ...
         poseStack.translate(shoulderOffset + (left ? 1 : -1) / 16.0F, 0.625F, 0.0625F);
         HumanoidModel<?> model = (HumanoidModel<?>)parentModel;
         if (left) {
            poseStack.mulPose(Axis.YP.rotationDegrees(model.leftArm.zRot * 57.295776F));
            poseStack.mulPose(Axis.XP.rotationDegrees(-model.leftArm.xRot * 57.295776F));
         } else {
            poseStack.mulPose(Axis.YP.rotationDegrees(model.rightArm.zRot * 57.295776F));
            poseStack.mulPose(Axis.XP.rotationDegrees(-model.rightArm.xRot * 57.295776F));
         }

         float scale = (float)ClientConfigs.LANTERN_HOLDING_SIZE.get().doubleValue();
   ...
   public boolean renderFirstPersonItem(
      ...
      poseStack.pushPose();
      poseStack.translate(-0.025 * f, 0.125, 0.0);
   ...
   private static void renderLanternModel(LivingEntity entity, ItemStack itemStack, PoseStack poseStack, MultiBufferSource buffer, int light, boolean left) {
      ...
      if (CompatHandler.THIN_AIR) {
         BlockState newState = ThinAirCompat.maybeSetAirQuality(state, entity.getEyePosition(), entity.level());
         if (newState != null) {
            state = newState;
         }
      }
