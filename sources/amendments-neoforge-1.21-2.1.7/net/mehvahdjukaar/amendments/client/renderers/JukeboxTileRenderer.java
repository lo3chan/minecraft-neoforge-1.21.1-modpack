...
public class JukeboxTileRenderer implements BlockEntityRenderer<JukeboxBlockEntity> {
   public JukeboxTileRenderer(Context context) {
   }

   public void render(
      JukeboxBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay
   ) {
      ItemStack item = blockEntity.getTheItem();
      if (!item.isEmpty() && (Boolean)blockEntity.getBlockState().getValue(JukeboxBlock.HAS_RECORD)) {
