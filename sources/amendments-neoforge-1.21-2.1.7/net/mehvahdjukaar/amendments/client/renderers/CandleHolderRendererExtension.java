...
public class CandleHolderRendererExtension implements IThirdPersonAnimationProvider, IThirdPersonSpecialItemRenderer, IFirstPersonSpecialItemRenderer {
   ...
   private static final ResourceLocation ENDER_FLAME_TEXTURE = ResourceLocation.fromNamespaceAndPath("endergetic", "textures/particle/ender_fire_flame.png");
   private static final ResourceLocation CUPRIC_FLAME_TEXTURE = ResourceLocation.fromNamespaceAndPath(
      "caverns_and_chasms", "textures/particle/cupric_fire_flame.png"
   );
   ...
   public CandleHolderRendererExtension(BlockItem blockItem) {
      ...
      this.litState = (BlockState)this.offState.setValue(CandleBlock.LIT, true);
   ...
   public <T extends Player, M extends EntityModel<T> & ArmedModel & HeadedModel> void renderThirdPersonItem(
      ...
      if (!stack.isEmpty()) {
         ...
         ItemTransform transform = new ItemTransform(
            new Vector3f(55.0F, -180.0F, 0.0F), new Vector3f(0.0F, 2.0F, 1.25F).mul(0.0625F), new Vector3f(0.375F, 0.375F, 0.375F)
         );
   ...
   public boolean renderFirstPersonItem(
      ...
      poseStack.scale(-scale, scale, -scale);
      this.renderBlockModel(player, stack, poseStack, buffer, light, left);
      if (!player.isInWater()) {
         poseStack.translate(f * 0.03, 0.0, -0.03999999910593033);
