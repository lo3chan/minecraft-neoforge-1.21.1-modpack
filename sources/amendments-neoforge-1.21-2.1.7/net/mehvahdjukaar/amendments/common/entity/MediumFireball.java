...
public class MediumFireball extends ImprovedProjectileEntity implements IVisualTransformationProvider {
   ...
   public void spawnTrailParticles() {
      ...
      if (this.level().isClientSide) {
         this.trailEmitter.tick(this, (p, v) -> {
            if (!this.isExtinguished) {
               this.level().addParticle((ParticleOptions)ModRegistry.FIREBALL_TRAIL_PARTICLE.get(), p.x, p.y, p.z, this.getBbWidth(), 0.0, 0.0);
            }
         });
   ...
   public boolean displayFireAnimation() {
      return this.level().isClientSide && ClientConfigs.FIREBALL_3D.get() ? false : super.displayFireAnimation();
   }
   ...
   protected void onHit(HitResult result) {
      ...
      if (!this.level().isClientSide) {
         if (!this.isExtinguished) {
            ...
            FireballExplosion.explodeServer(
               this.level(), this, null, null, this.getX(), this.getY(), this.getZ(), 1.0F, bl, ExplosionInteraction.NONE, settings
            );
         }
   ...
   protected void onHitEntity(EntityHitResult result) {
      ...
      if (this.level() instanceof ServerLevel serverLevel) {
         ...
         DamageSource source = this.fireballDamage(fireballOwner);
         if (this.isExtinguished) {
            entity.hurt(source, ProjectileStats.PLAYER_FIREBALL.damageOnHit());
         } else {
            int fireTick = entity.getRemainingFireTicks();
            entity.igniteForSeconds(ProjectileStats.PLAYER_FIREBALL.directHitFireSeconds());
   ...
   private DamageSource fireballDamage(@Nullable Entity thrower) {
      DamageSources sources = this.damageSources();
      return thrower == null ? sources.source(DamageTypes.UNATTRIBUTED_FIREBALL, this) : sources.source(DamageTypes.FIREBALL, this, thrower);
   }
