...
public class DyeBottleItem extends Item {
   protected static final HashBiMap<DyeColor, Integer> COLOR_TO_DIFFUSE = Arrays.stream(DyeColor.values())
      .collect(Collectors.toMap(Function.identity(), DyeColor::getTextureDiffuseColor, (color, color2) -> color2, HashBiMap::create));
   ...
   public static ItemStack fromFluidStack(SoftFluidStack stack) {
      ...
      item.set(DataComponents.DYED_COLOR, (DyedItemColor)stack.getOrDefault(DataComponents.DYED_COLOR, DEFAULT_COLOR));
      return item;
   }

   public static int mixColor(int oldColor, int newColor, int oldAmount, int newAmount) {
      return new RGBColor(oldColor).asHCL().mixWith(new RGBColor(newColor).asHCL(), (float)newAmount / (oldAmount + newAmount)).asRGB().toInt();
   }
   ...
   public static DyeColor getClosestDye(SoftFluidStack stack) {
      return getClosestDye(((DyedItemColor)stack.getOrDefault(DataComponents.DYED_COLOR, DEFAULT_COLOR)).rgb());
   }

   public static DyeColor getClosestDye(ItemStack stack) {
      return getClosestDye(((DyedItemColor)stack.get(DataComponents.DYED_COLOR)).rgb());
   }
   ...
   public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> list, TooltipFlag tooltipFlag) {
      ...
      if (color != null) {
         DyeColor dye = (DyeColor)COLOR_TO_DIFFUSE.inverse().get(color.rgb());
         if (dye != null) {
            ...
            list.add(Component.translatable("item.color", new Object[]{String.format(Locale.ROOT, "#%06X", color.rgb())}).withStyle(ChatFormatting.GRAY));
         } else {
