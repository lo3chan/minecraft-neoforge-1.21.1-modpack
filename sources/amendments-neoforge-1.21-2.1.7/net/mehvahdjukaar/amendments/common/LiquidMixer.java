...
public class LiquidMixer {
   ...
   public static SoftFluidStack mixPotions(SoftFluidStack firstFluid, SoftFluidStack secondFluid) {
      PotionContents tankFluidContent = (PotionContents)firstFluid.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY);
      PotionContents newFluidContent = (PotionContents)secondFluid.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY);
      ...
      if (newEffects.equals(existingEffects)) {
         ...
         float oldMult = (float)oldCount / newCount;
         ...
         Map<Holder<MobEffect>, MobEffectInstance> mergedMap = combinedEffects.stream()
            .collect(Collectors.toMap(MobEffectInstance::getEffect, effectInstance -> (MobEffectInstance)effectInstance, LiquidMixer::mergeEffects));
