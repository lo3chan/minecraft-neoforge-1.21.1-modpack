...
public class ServerBoundSyncLecternBookMessage implements Message {
   ...
   public ServerBoundSyncLecternBookMessage(FriendlyByteBuf buffer) {
      ...
      this.pages = (List<String>)buffer.readCollection(FriendlyByteBuf.limitValue(Lists::newArrayListWithCapacity, 200), b -> b.readUtf(8192));
   ...
   public void handle(Context context) {
      ...
      if (level.getBlockEntity(this.pos) instanceof LecternBlockEntity be) {
         ...
         if (book.getItem() instanceof WritableBookItem) {
            ...
            Consumer<List<FilteredText>> consumer = this.title.isPresent()
               ? l -> this.signBook(be, player, book, l.get(0), l.subList(1, l.size()))
               : l -> this.updateBookContents(be, player, book, l);
            this.<List<String>, List<FilteredText>>filterTextPacket(player, list, TextFilter::processMessageBundle)
   ...
   private void updateBookContents(LecternBlockEntity be, ServerPlayer player, ItemStack itemstack, List<FilteredText> pages) {
      if (itemstack.is(Items.WRITABLE_BOOK)) {
         List<Filterable<String>> list = pages.stream().map(t -> this.filterableFromOutgoing(t, player)).toList();
         itemstack.set(DataComponents.WRITABLE_BOOK_CONTENT, new WritableBookContent(list));
         be.setChanged();
         be.getLevel().sendBlockUpdated(be.getBlockPos(), be.getBlockState(), be.getBlockState(), 3);
      }

      if (this.takeBook && player.containerMenu instanceof LecternEditMenu m) {
   ...
   private void signBook(LecternBlockEntity be, ServerPlayer player, ItemStack book, FilteredText title, List<FilteredText> pages) {
      ...
      List<Filterable<Component>> filteredPages = pages.stream().map(t -> this.filterableFromOutgoing(t, player).map(s -> Component.literal(s))).toList();
      signedBook.set(
         DataComponents.WRITTEN_BOOK_CONTENT,
         new WrittenBookContent(this.filterableFromOutgoing(title, player), player.getName().getString(), 0, filteredPages, true)
      );
