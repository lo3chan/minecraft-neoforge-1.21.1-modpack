...
public class PendulumAnimation extends SwingAnimation {
   ...
   public PendulumAnimation(Supplier<PendulumAnimation.Config> config, Function<BlockState, Vector3f> axisGetter) {
      super(axisGetter);
      this.config = config;
      PendulumAnimation.Config c = config.get();
      if (c != null) {
   ...
   public float getAngle(float partialTicks) {
      return 57.295776F * Mth.lerp(partialTicks, this.prevAngle, this.angle);
   }

   @Override
   public void setAngle(float angle) {
      this.angle = 0.017453292F * angle;
   }
   ...
   public void reset() {
      this.angle = 0.0F;
      this.prevAngle = 0.0F;
      PendulumAnimation.Config c = this.config.get();
      this.angularVel = capVelocity(c.k, 1000.0F, this.angle, c.minAngleEnergy);
   }
   ...
   public void tick(Level level, BlockPos pos, BlockState state) {
      ...
      float dt = 0.05F;
      float energy = 0.0F;
      PendulumAnimation.Config config = this.config.get();
      float k = config.k;
      ...
      if (this.hasDrag && !hasAcc) {
         if (energy > config.minAngleEnergy) {
            ...
            float drag = (float)(damping * this.angularVel);
            acc -= drag;
         } else {
            this.hasDrag = false;
         }
   ...
   public void addImpulse(double vel) {
      this.angularVel = (float)(this.angularVel + vel);
      this.hasDrag = true;
   }

   private static float capVelocity(float k, float currentVel, float angle, float targetEnergy) {
      float newVel = (float)Math.sqrt(Math.max(0.0F, 2.0F * (targetEnergy - k * (1.0F - Mth.cos(angle)))));
      if (currentVel < 0.0F) {
         newVel *= -1.0F;
      }
   ...
   public boolean hitByEntity(Entity entity, BlockState state, BlockPos pos) {
      if (this.immunity != 0) {
         ...
         if (eVel.length() < 0.01) {
            ...
            PendulumAnimation.Config config = ClientConfigs.HANGING_SIGN_CONFIG.get();
            double eMass;
            ...
            Vec3 signVel = new Vec3(0.0, Mth.sin(this.angle), 0.0).add(normalVec.scale(Mth.cos(this.angle)));
            ...
            if (eRelVel * eRelVel < 1.0E-4) {
               ...
               double y1 = (-g + delta) / (2.0 * f);
               double y2 = (-g - delta) / (2.0 * f);
               double x1 = magnitude + eMass * eRelVel - eMass * y1;
               double x2 = magnitude + eMass * eRelVel - eMass * y2;
               double x;
               if (Mth.abs((float)(x2 - magnitude)) < 1.0E-4) {
                  x = x1;
               } else {
                  x = x2;
               }

               float dW = (float)(x / radius) - this.angularVel;
   ...
   public static class Config {
      private static final Codec<Double> POSITIVE_DOUBLE = floatRangeMinExclusiveWithMessage(
         0.0, 1.7976931348623157E308, float_ -> "Value must be positive: " + float_
      );
      public static final Codec<PendulumAnimation.Config> CODEC = RecordCodecBuilder.create(
         instance -> instance.group(
               Codec.doubleRange(0.0, 360.0).fieldOf("min_angle").forGetter(c -> (double)(57.295776F * c.minAngle)),
               Codec.doubleRange(0.0, 360.0).fieldOf("max_angle").forGetter(c -> (double)(57.295776F * c.maxAngle)),
            ...
            .apply(instance, PendulumAnimation.Config::new)
      );
      ...
      public Config(double minAngle, double maxAngle, double damping, double frequency, boolean hitbox, double mass, double force) {
         this.minAngle = (float)(minAngle * 0.01745329238474369);
         this.maxAngle = (float)(maxAngle * 0.01745329238474369);
         this.damping = (float)damping;
         this.frequency = (float)frequency;
         this.k = (float)Math.pow(6.283185307179586 * frequency, 2.0);
      ...
      public Config() {
         this(0.8, 60.0, 0.525, 0.6, true, 1.0, 15.0);
      }
   }
}
