...
class DirectionalCakeConversion implements BlockUse {
   ...
   public InteractionResult tryPerformingAction(
      ...
      if (player.isSecondaryUseActive()) {
      ...
      } else if (state.is(BlockTags.CANDLE_CAKES) && stack.is(ItemTags.CANDLES)) {
         return InteractionResult.PASS;
      } else if (!state.is(Blocks.CAKE)
         || !stack.is(ItemTags.CANDLES) && player.getDirection() != Direction.EAST && (Integer)state.getValue(CakeBlock.BITES) == 0) {
         if (CommonConfigs.DOUBLE_CAKES.get() && stack.is(Items.CAKE)) {
            ...
            BlockHitResult raytrace = new BlockHitResult(new Vec3(pos.getX(), pos.getY(), pos.getZ()), hit.getDirection(), pos, false);
