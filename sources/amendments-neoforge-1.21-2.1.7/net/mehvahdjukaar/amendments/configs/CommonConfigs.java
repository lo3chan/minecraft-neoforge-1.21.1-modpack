...
public class CommonConfigs {
   ...
   private static Map<MobEffect, MobEffect> getInverseEffects() {
      ...
      map.put((MobEffect)MobEffects.DIG_SPEED.value(), (MobEffect)MobEffects.DIG_SLOWDOWN.value());
      map.put((MobEffect)MobEffects.DIG_SLOWDOWN.value(), (MobEffect)MobEffects.DIG_SPEED.value());
      map.put((MobEffect)MobEffects.MOVEMENT_SPEED.value(), (MobEffect)MobEffects.MOVEMENT_SLOWDOWN.value());
      map.put((MobEffect)MobEffects.MOVEMENT_SLOWDOWN.value(), (MobEffect)MobEffects.MOVEMENT_SPEED.value());
      map.put((MobEffect)MobEffects.DAMAGE_BOOST.value(), (MobEffect)MobEffects.WEAKNESS.value());
      map.put((MobEffect)MobEffects.WEAKNESS.value(), (MobEffect)MobEffects.DAMAGE_BOOST.value());
      map.put((MobEffect)MobEffects.HARM.value(), (MobEffect)MobEffects.HEAL.value());
      map.put((MobEffect)MobEffects.HEAL.value(), (MobEffect)MobEffects.HARM.value());
      map.put((MobEffect)MobEffects.LUCK.value(), (MobEffect)MobEffects.UNLUCK.value());
      map.put((MobEffect)MobEffects.UNLUCK.value(), (MobEffect)MobEffects.LUCK.value());
      return map;
   }
   ...
   public static boolean isFlagOn(String s) {
      if (s.equals("dye_water")) {
         ...
         return s.equals("dragon_charge") ? DRAGON_CHARGE.get() : false;
      }
   }

   static {
      ConfigBuilder builder = ConfigBuilder.create("amendments", ConfigType.COMMON_SYNCED);
      ...
      CHARGES_COOLDOWN = builder.comment("Cooldown for fire & dragon charges in ticks").define("thrown_cooldown", 10, 0, 1000);
      DRAGON_CHARGE = builder.comment("Adds dragons charge item").define("dragon_charge", true);
      FIRE_CHARGE_GRAVITY = builder.comment("Makes fire & dragon charges have gravity").define("gravity", true);
      DEFLECT_FIRE_CHARGES = builder.comment("Makes fire charges deflectable by punching").define("deflectable", false);
      THROWABLE_FIRE_CHARGES = builder.comment("Allows throwing fire & dragon charges").define("fire_charges_throwable", true);
      FIRE_CHARGE_DISPENSER = builder.comment(
      ...
      CAULDRON_HAND_CRAFTING = builder.comment("Allows crafting items using cauldrons by clicking on them").define("hand_crafting", true);
      CAULDRON_IN_WORLD_CRAFTING = builder.comment("Allows crafting items using cauldrons by throwing items in them").define("in_world_crafting", true);
      DYE_WATER = builder.comment("Allows dying cauldron water bedrock style and mixing them too").define("dye_water", true);
      DYE_RECIPES_PER_LAYER = builder.comment(
      ...
      POTION_RECIPES_PER_LAYER = builder.comment(
      ...
      INVERSE_POTIONS = builder.comment("Map of potion ids to their inverse ids. Used for potion mixing")
         .defineObject(
            "inverse_potions",
            CommonConfigs::getInverseEffects,
            (Codec)CodecUtils.optionalMapCodec(BuiltInRegistries.MOB_EFFECT.byNameCodec(), BuiltInRegistries.MOB_EFFECT.byNameCodec())
         );
      CONNECT_TO_FENCES = builder.comment("Makes cauldrons connect to fences").define("connect_to_fences", true);
      LAVA_LAYERS = builder.comment(
      ...
      WALL_LANTERN_HIGH_PRIORITY = builder.comment(
      ...
      FALLING_LANTERNS = builder.comment(
