...
public abstract class SpiderDungeonPiece extends StructurePiece {
   protected static final Set<Block> BLOCK_BLACKLIST = Sets.newHashSet(
      new Block[]{
            ...
            mask -> {
               if (decoRand.nextFloat() < 0.15F) {
                  ...
                  if ((
                     ...
                     && !BLOCK_BLACKLIST.contains(this.getBlock(world, globalX, y, globalZ, box).getBlock())) {
   ...
   protected void placeSphereRandomized(
      WorldGenLevel world,
      BoundingBox box,
      BlockPos center,
      float radius,
      RandomSource rand,
      float chance,
      BlockStateRandomizer blockSelector,
         ...
         for (float z = -radius; z <= radius; z++) {
            for (float y = -radius; y <= radius; y++) {
               if (x * x + y * y + z * z < radius * radius
                  && (!replaceOnlyAir || this.getBlock(world, (int)x + center.getX(), (int)y + center.getY(), (int)z + center.getZ(), box).isAir())
