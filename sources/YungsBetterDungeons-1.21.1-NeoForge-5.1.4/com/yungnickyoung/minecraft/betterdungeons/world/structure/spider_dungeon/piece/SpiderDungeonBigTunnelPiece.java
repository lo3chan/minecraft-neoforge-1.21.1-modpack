...
public class SpiderDungeonBigTunnelPiece extends SpiderDungeonPiece {
   ...
   protected void addAdditionalSaveData(StructurePieceSerializationContext structurePieceSerializationContext, CompoundTag compoundTag) {
      ...
      for (int i = 0; i < 30; i++) {
         yawListNBT.add(FloatTag.valueOf(this.yaws[i]));
      }
   ...
   public void addChildren(StructurePiece structurePiece, StructurePieceAccessor structurePieceAccessor, RandomSource randomSource) {
      if (this.pitch == 0.0F) {
         this.pitch = Mth.clamp(randomSource.nextFloat() * -3.1415927F, -2.6F, -0.6F);
      }
      ...
      if (this.yaws[0] == 0.0F) {
         this.yaws[0] = randomSource.nextFloat() * 6.2831855F;
      }
      ...
      if (caveStartX - 2.5F - 4.0F < minX) {
         minX = (int)caveStartX - 2 - 4;
      }

      if (caveStartX + 2.5F + 4.0F > maxX) {
         maxX = (int)caveStartX + 2 + 4;
      }

      if (caveStartY - 2.5F - 4.0F < minY) {
         minY = (int)caveStartY - 2 - 4;
      }

      if (caveStartY + 2.5F + 4.0F > maxY) {
         maxY = (int)caveStartY + 2 + 4;
      }

      if (caveStartZ - 2.5F - 4.0F < minZ) {
         minZ = (int)caveStartZ - 2 - 4;
      }

      if (caveStartZ + 2.5F + 4.0F > maxZ) {
         maxZ = (int)caveStartZ + 2 + 4;
      }

      for (int i = 1; i < 30; i++) {
         float var21 = yawModifier * 0.75F;
         yawModifier = var21 + randomSource.nextFloat() * randomSource.nextFloat();
      ...
      if (this.genDepth == 0) {
         StructurePiece nextBigTunnelPiece = new SpiderDungeonBigTunnelPiece(this.endPos, this.genDepth + 1, 0.0F, this.yaws[29]);
      ...
      StructurePiece smallTunnelPiece1 = new SpiderDungeonSmallTunnelPiece(this.endPos, smallTunnelAngle, this.genDepth + 1);
      ...
      smallTunnelAngle += randomSource.nextFloat() * 0.4F + 0.9F;
      StructurePiece smallTunnelPiece2 = new SpiderDungeonSmallTunnelPiece(this.endPos, smallTunnelAngle, this.genDepth + 1);
      ...
      smallTunnelAngle += randomSource.nextFloat() * 0.4F + 0.9F;
      StructurePiece smallTunnelPiece3 = new SpiderDungeonSmallTunnelPiece(this.endPos, smallTunnelAngle, this.genDepth + 1);
      ...
      if (randomSource.nextFloat() < 0.5F) {
         StructurePiece smallTunnelPiece4 = new SpiderDungeonSmallTunnelPiece(this.endPos, smallTunnelAngle, this.genDepth + 1);
         ...
         StructurePiece extraBigTunnelPiece = new SpiderDungeonBigTunnelPiece(
            this.endPos, this.genDepth + 1, randomSource.nextFloat() * 3.1415927F / 6.0F - 0.5235988F, smallTunnelAngle
         );
      ...
      StructurePiece nestPiece = new SpiderDungeonNestPiece(this.endPos, this.genDepth + 1);
   ...
   public void postProcess(
      ...
      for (int i = 0; i < 30; i++) {
         ...
         float xRadius = Mth.lerp(Mth.sin(i * 3.1415927F / 30.0F), 2.0F, 2.5F);
         float yRadius = Mth.lerp(Mth.sin(i * 3.1415927F / 30.0F), 2.0F, 2.5F);
         float zRadius = Mth.lerp(Mth.sin(i * 3.1415927F / 30.0F), 2.0F, 2.5F);
