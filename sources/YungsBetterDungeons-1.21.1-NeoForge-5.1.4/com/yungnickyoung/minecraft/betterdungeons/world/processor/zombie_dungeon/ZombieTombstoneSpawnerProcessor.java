...
public class ZombieTombstoneSpawnerProcessor extends StructureProcessor {
   ...
   public StructureBlockInfo processBlock(
      ...
      if (blockInfoGlobal.state().getBlock() == Blocks.BLACK_STAINED_GLASS) {
         MobSpawnerData spawner = MobSpawnerData.builder().setEntityType(EntityType.SKELETON).build();
         spawner.nextSpawnData.getEntityToSpawn().put("HandItems", (Tag)Util.make(new ListTag(), handItemsTag -> {
            Tag ironSwordNbt = (Tag)ItemStack.CODEC.encodeStart(NbtOps.INSTANCE, new ItemStack(Items.IRON_SWORD)).getOrThrow();
