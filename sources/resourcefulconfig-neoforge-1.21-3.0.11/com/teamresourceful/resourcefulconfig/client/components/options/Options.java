...
public final class Options {
   public static void populateOptions(OptionsListWidget widget, Map<String, ResourcefulConfigEntry> entries, List<ResourcefulConfigButton> buttons) {
      ...
      buttons.forEach(button -> {
         switch (button.position()) {
            case BEFORE:
               buttonsBefore.put(button.target(), button);
               break;
            case AFTER:
               buttonsAfter.put(button.target(), button);
         }
      });
      buttonsBefore.get("").forEach(button -> addButton(widget, button));

      for (Entry<String, ResourcefulConfigEntry> value : entries.entrySet()) {
         EntryData options = value.getValue().options();
         if (!options.hasOption(Option.HIDDEN)) {
   ...
   private static void populateValueEntry(OptionsListWidget list, ResourcefulConfigValueEntry entry) {
      ...
      switch (entry.type()) {
         ...
         case SHORT:
         case LONG:
            OptionRange rangex = WholeOptionRange.of(entry);
            if (rangex.hasRange() && data.hasOption(Option.SLIDER)) {
               widgets.add(new RangeOptionWidget(rangex));
            } else {
               widgets.add(
                  switch (entry.type()) {
                     case BYTE -> new NumberOptionWidget(entry::getByte, entry::setByte, parseNumber(data, Byte::parseByte), NumberOptionWidget.INTEGER_FILTER);
                     case SHORT -> new NumberOptionWidget(
                        entry::getShort, entry::setShort, parseNumber(data, Short::parseShort), NumberOptionWidget.INTEGER_FILTER
                     );
                     case LONG -> new NumberOptionWidget(entry::getLong, entry::setLong, parseNumber(data, Long::parseLong), NumberOptionWidget.INTEGER_FILTER);
         ...
         case STRING:
            if (data.hasOption(Option.MULTILINE)) {
               ...
               widgets.add(new MultilineStringOptionWidget(() -> String.join("\n", (String[])entry.getArray()), s -> entry.setArray(s.split("\n"))));
            } else {
         ...
         case ENUM:
            ...
            break;
         case INTEGER:
            OptionRange rangexx = WholeOptionRange.of(entry);
            if (rangexx.hasRange() && data.hasOption(Option.SLIDER)) {
               widgets.add(new RangeOptionWidget(rangexx));
               ...
               widgets.add(new ColorOptionWidget(color.presets(), color.alpha(), () -> color.alpha() ? entry.getInt() : entry.getInt() | 0xFF000000, value -> {
                  value = color.alpha() ? value : value & 16777215;
                  entry.setInt(value);
               }));
               ...
               widgets.add(
                  new NumberOptionWidget<Integer>(entry::getInt, entry::setInt, parseNumber(data, Integer::parseInt), NumberOptionWidget.INTEGER_FILTER)
               );
            }
            break;
         case OBJECT:
            throw new IllegalStateException("Unexpected value: " + entry.type());
      }

      SpriteButton reset = SpriteButton.builder(12, 12)
         .padding(2)
