...
public class KeyCodeHelper {
   ...
   public static Component getKeyName(int key) {
      return key < 0 ? (Component)HARD_CODED_KEYCODES.computeIfAbsent(key, $ -> {
         int button = -key - 100;
         return Component.translatable("key.mouse", new Object[]{button + 1});
      }) : (Component)HARD_CODED_KEYCODES.computeIfAbsent(key, $ -> {
         String keyName = GLFW.glfwGetKeyName(key, 0);
         return keyName == null ? Component.literal("???") : Component.literal(keyName.toLowerCase(Locale.ROOT));
      });
   }

   public static boolean isMouseKeyPressed(int button) {
      Minecraft mc = Minecraft.getInstance();
      return GLFW.glfwGetMouseButton(mc.getWindow().getWindow(), button) == 1;
   }
