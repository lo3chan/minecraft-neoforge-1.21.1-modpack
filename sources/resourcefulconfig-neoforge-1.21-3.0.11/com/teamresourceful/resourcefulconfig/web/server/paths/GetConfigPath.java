...
public record GetConfigPath(WebVerifier verifier) implements BasePath {
   ...
   private static JsonObject createWebConfig(ResourcefulConfig config) {
      try {
         ...
         throw new RuntimeException("Failed to create config json for config: " + config.id(), var8);
      }
   }
   ...
   private static JsonArray createEntries(ResourcefulConfig config) {
      ...
      config.entries().forEach((id, entry) -> {
         ...
         if (entry instanceof ResourcefulConfigValueEntry valueEntry) {
            if (!valueEntry.isArray()) {
               switch (entry.type()) {
                  case BYTE:
                     createNumber(json, valueEntry, ResourcefulConfigValueEntry::getByte);
                     break;
                  case SHORT:
                     createNumber(json, valueEntry, ResourcefulConfigValueEntry::getShort);
                     break;
                  case INTEGER:
                     createNumber(json, valueEntry, ResourcefulConfigValueEntry::getInt);
                     break;
                  case LONG:
                     createNumber(json, valueEntry, ResourcefulConfigValueEntry::getLong);
                     break;
                  case FLOAT:
                     createNumber(json, valueEntry, ResourcefulConfigValueEntry::getFloat);
                     break;
                  case DOUBLE:
                     createNumber(json, valueEntry, ResourcefulConfigValueEntry::getDouble);
                     break;
                  case ENUM:
                     ...
                     for (Enum<?> e : getEnumConstants(valueEntry.objectType())) {
                        options.add(e.name());
                     }
   ...
   private static <T extends Number> void createNumber(JsonObject json, ResourcefulConfigValueEntry entry, Function<ResourcefulConfigValueEntry, T> getter) {
      ...
      T def = (T)entry.defaultOrElse((T)WebServerUtils.ZERO);
      T current = (T)getter.apply(entry);
