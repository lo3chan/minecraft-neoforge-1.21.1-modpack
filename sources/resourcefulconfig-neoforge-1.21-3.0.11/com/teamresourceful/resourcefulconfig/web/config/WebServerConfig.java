...
public record WebServerConfig(boolean enabled, int port, Optional<String> configSite, Validator validator) {
   public static final Codec<WebServerConfig> CODEC = RecordCodecBuilder.create(
   ...
   public static final WebServerConfig DEFAULT = new WebServerConfig(
      false, 7903, Optional.empty(), new IfValidator(Set.of(), new PasswordValidator(UUID.randomUUID().toString()), Optional.empty())
   );

   public boolean valid(UserJwtPayload info) {
      return this.enabled() && this.validator.test(info);
   }
