...
public class DedicatedServerConfig implements ResourcefulConfig {
   ...
   private static final LinkedHashMap<String, ResourcefulConfigEntry> ENTRIES = (LinkedHashMap<String, ResourcefulConfigEntry>)Util.make(
      new LinkedHashMap(),
      entries -> {
         ...
         entries.put(
            "max-players", new DedicatedServerEntry("max-players", 20, (server, value) -> ((PlayerListAccessor)server.getPlayerList()).setMaxPlayers(value))
         );
      }
   );
