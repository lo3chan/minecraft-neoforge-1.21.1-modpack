...
public class EntityAccessorClientHandler implements AccessorClientHandler<EntityAccessor> {
   ...
   public List<IServerDataProvider<EntityAccessor>> shouldRequestData(EntityAccessor accessor) {
      List<IServerDataProvider<EntityAccessor>> providers = WailaCommonRegistration.instance().getEntityNBTProviders(accessor.getEntity());
      return providers.isEmpty() ? List.of() : providers.stream().filter(provider -> provider.shouldRequestData(accessor)).toList();
   }
   ...
   public IElement getIcon(EntityAccessor accessor) {
      ...
      for (IComponentProvider<EntityAccessor> provider : WailaClientRegistration.instance().getEntityIconProviders(entity, this::isEnabled)) {
         try {
            ...
            WailaExceptionHandler.handleErr(var7, provider, null);
         }
      }
   ...
   public void gatherComponents(EntityAccessor accessor, Function<IJadeProvider, ITooltip> tooltipProvider) {
      for (IComponentProvider<EntityAccessor> provider : WailaClientRegistration.instance().getEntityProviders(accessor.getEntity(), this::isEnabled)) {
         ...
         try {
            ...
            WailaExceptionHandler.handleErr(var10, provider, tooltip::add);
         } finally {
