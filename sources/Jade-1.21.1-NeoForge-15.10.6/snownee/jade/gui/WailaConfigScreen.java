...
public class WailaConfigScreen extends PreviewOptionsScreen {
   ...
   public WailaConfigScreen(Screen parent) {
      ...
      Builder<KeyMapping, Key> keyMapBuilder = ImmutableMap.builder();
      ...
      ImmutableMap<KeyMapping, Key> keyMap = keyMapBuilder.build();
      this.canceller = () -> {
   ...
   public OptionsList createOptions() {
      ...
      options.choices("display_fluids", general::getDisplayFluids, general::setDisplayFluids, builder -> builder.withTooltip(mode -> {
      ...
      if (!WailaConfig.ConfigGeneral.itemModNameTooltipDisabledByMods.isEmpty()) {
         ...
         WailaConfig.ConfigGeneral.itemModNameTooltipDisabledByMods.stream().<MutableComponent>map(Component::literal).forEach(value::appendDescription);
      ...
      options.slider("reach_distance", general::getExtendedReach, general::setExtendedReach, 0.0F, 20.0F, f -> Mth.floor(f * 2.0F) / 2.0F);
      options.choices("perspective_mode", general::getPerspectiveMode, general::setPerspectiveMode, builder -> builder.withTooltip(mode -> {
      ...
      options.add(
         new OptionButton(
            Component.translatable(OptionsList.Entry.makeKey("overlay_pos")), Button.builder(adjust, w -> this.startAdjustingPosition()).size(100, 20)
         )
      );
      options.choices(
         "overlay_theme", () -> overlay.getTheme().id, IThemeHelper.get().getThemes().stream().filter($ -> !$.hidden).map($ -> $.id).toList(), id -> {
            if (!Objects.equals(id, overlay.getTheme().id)) {
               ...
               if (theme.changeOpacity != 0.0F) {
                  this.opacityEntry.setValue(theme.changeOpacity);
               }
            }
      ...
      options.add(new OptionButton(title, Button.builder(reset, w -> this.minecraft.setScreen(new ConfirmScreen(bl -> {
         if (bl) {
            ...
            try {
               ...
               Jade.LOGGER.error("", var6x);
            }
         }

         this.minecraft.setScreen(this);
         this.options.setScrollAmount(this.options.getMaxScroll());
      }, title, Component.translatable(OptionsList.Entry.makeKey("reset_settings.confirm")), reset, Component.translatable("gui.cancel")))).size(100, 20)));
      return options;
   }
}
