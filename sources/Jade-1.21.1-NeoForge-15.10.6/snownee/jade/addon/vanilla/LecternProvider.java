...
public enum LecternProvider implements IBlockComponentProvider, StreamServerDataProvider<BlockAccessor, ItemStack> {
   ...
   public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
      ItemStack stack = this.decodeFromData(accessor).orElse(ItemStack.EMPTY);
      if (!stack.isEmpty()) {
         IElementHelper helper = IElementHelper.get();
         tooltip.add(helper.smallItem(stack));
         tooltip.append(
            helper.text(IDisplayHelper.get().stripColor(stack.getHoverName()))
               .message(I18n.get("narration.jade.bookName", new Object[]{stack.getHoverName().getString()}))
         );
      }
   }

   public boolean shouldRequestData(BlockAccessor accessor) {
      return (Boolean)accessor.getBlockState().getValue(LecternBlock.HAS_BOOK);
   }
