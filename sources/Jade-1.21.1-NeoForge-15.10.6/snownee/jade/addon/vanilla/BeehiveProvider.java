...
public enum BeehiveProvider implements IBlockComponentProvider, StreamServerDataProvider<BlockAccessor, Byte> {
   ...
   public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
      ...
      int level = (Integer)state.getValue(BeehiveBlock.HONEY_LEVEL);
      IThemeHelper t = IThemeHelper.get();
      MutableComponent value = Component.translatable("jade.fraction", new Object[]{level, 5});
      tooltip.add(Component.translatable("jade.beehive.honey", new Object[]{level == 5 ? t.success(value) : t.info(value)}));
      Byte b = this.decodeFromData(accessor).orElse(null);
      if (b != null) {
         boolean full = b > 0;
         int bees = Math.abs(b);
         tooltip.add(Component.translatable("jade.beehive.bees", new Object[]{full ? t.success(bees) : t.info(bees)}));
      }
   }
