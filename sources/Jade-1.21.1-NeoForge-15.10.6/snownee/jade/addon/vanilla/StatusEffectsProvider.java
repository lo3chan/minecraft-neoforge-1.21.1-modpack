...
public enum StatusEffectsProvider implements IEntityComponentProvider, StreamServerDataProvider<EntityAccessor, List<MobEffectInstance>> {
   ...
   public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
      List<MobEffectInstance> effects = this.decodeFromData(accessor).orElse(List.of());
      if (!effects.isEmpty()) {
         ...
         for (MobEffectInstance effect : effects) {
            ...
            box.add(((MobEffect)effect.getEffect().value()).getCategory() == MobEffectCategory.HARMFUL ? t.danger(s) : t.success(s));
         }
