...
public class HarvestToolProvider implements IBlockComponentProvider, ResourceManagerReloadListener {
   ...
   public final Cache<BlockState, ImmutableList<ItemStack>> resultCache = CacheBuilder.newBuilder().expireAfterAccess(5L, TimeUnit.MINUTES).build();
   ...
   public List<IElement> getText(BlockAccessor accessor, IPluginConfig config) {
      ...
      if (!state.requiresCorrectToolForDrops() && !config.get(JadeIds.MC_EFFECTIVE_TOOL)) {
         ...
         try {
            tools = (List<ItemStack>)this.resultCache.get(state, () -> getTool(state, accessor.getLevel(), accessor.getPosition()));
         } catch (ExecutionException var12) {
            Jade.LOGGER.error("Failed to get harvest tool", var12);
         }
         ...
         if (tools.isEmpty()) {
            ...
            for (ItemStack tool : tools) {
               elements.add(IElementHelper.get().item(tool, 0.75F).translate(new Vec2(-1.0F, offsetY)).size(ITEM_SIZE).message(null));
            }
            ...
            if (!elements.isEmpty()) {
               ...
               if (state.requiresCorrectToolForDrops() || !canHarvest) {
                  ...
                  elements.add(IElementHelper.get().text(text).scale(0.75F).zOffset(800).size(Vec2.ZERO).translate(new Vec2(-3.0F, 6.25F + offsetY)));
               }
            }
   ...
   private void tagsUpdated(RegistryAccess registryAccess, boolean client) {
      if (client) {
         ...
         try {
            this.shearableBlocks = Collections.unmodifiableList(
               LootTableMineableCollector.execute(registryAccess.registryOrThrow(Registries.LOOT_TABLE), Items.SHEARS.getDefaultInstance())
            );
         } catch (Throwable var4) {
            Jade.LOGGER.error("Failed to collect shearable blocks", var4);
         }
      }
   }
   ...
   static {
      ...
      if (CommonProxy.isPhysicallyClient()) {
         registerHandler(
            SimpleToolHandler.create(
               JadeIds.JADE("pickaxe"),
               List.of(Items.WOODEN_PICKAXE, Items.GOLDEN_PICKAXE, Items.STONE_PICKAXE, Items.IRON_PICKAXE, Items.DIAMOND_PICKAXE, Items.NETHERITE_PICKAXE)
            )
         );
         registerHandler(
            SimpleToolHandler.create(
               JadeIds.JADE("axe"), List.of(Items.WOODEN_AXE, Items.GOLDEN_AXE, Items.STONE_AXE, Items.IRON_AXE, Items.DIAMOND_AXE, Items.NETHERITE_AXE)
            )
         );
         registerHandler(
            SimpleToolHandler.create(
               JadeIds.JADE("shovel"),
               List.of(Items.WOODEN_SHOVEL, Items.GOLDEN_SHOVEL, Items.STONE_SHOVEL, Items.IRON_SHOVEL, Items.DIAMOND_SHOVEL, Items.NETHERITE_SHOVEL)
            )
         );
         registerHandler(
            SimpleToolHandler.create(
               JadeIds.JADE("hoe"), List.of(Items.WOODEN_HOE, Items.GOLDEN_HOE, Items.STONE_HOE, Items.IRON_HOE, Items.DIAMOND_HOE, Items.NETHERITE_HOE)
            )
         );
         registerHandler(
            SimpleToolHandler.create(JadeIds.JADE("sword"), List.of(Items.WOODEN_SWORD)).addExtraBlock(Blocks.BAMBOO).addExtraBlock(Blocks.BAMBOO_SAPLING)
         );
