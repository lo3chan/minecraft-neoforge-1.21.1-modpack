...
public enum BlockFaceProvider implements IBlockComponentProvider {
   ...
   public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
      tooltip.replace(JadeIds.CORE_OBJECT_NAME, (UnaryOperator<List<List<IElement>>>)(lists -> {
         List<IElement> lastList = (List<IElement>)lists.getLast();
         lastList.add(IElementHelper.get().text(Component.translatable("jade.blockFace", new Object[]{directionName(accessor.getSide())})));
         return lists;
      }));
   }
