...
public interface BakedModelFactory {
   ...
   default GeoQuad[] buildQuads(UVUnion uvUnion, BakedModelFactory.VertexSet vertices, Cube cube, float textureWidth, float textureHeight, boolean mirror) {
      return new GeoQuad[]{
         this.buildQuad(vertices, cube, uvUnion, textureWidth, textureHeight, mirror, Direction.WEST),
         this.buildQuad(vertices, cube, uvUnion, textureWidth, textureHeight, mirror, Direction.EAST),
         this.buildQuad(vertices, cube, uvUnion, textureWidth, textureHeight, mirror, Direction.NORTH),
         this.buildQuad(vertices, cube, uvUnion, textureWidth, textureHeight, mirror, Direction.SOUTH),
         this.buildQuad(vertices, cube, uvUnion, textureWidth, textureHeight, mirror, Direction.UP),
         this.buildQuad(vertices, cube, uvUnion, textureWidth, textureHeight, mirror, Direction.DOWN)
      };
   }

   default GeoQuad buildQuad(
      BakedModelFactory.VertexSet vertices, Cube cube, UVUnion uvUnion, float textureWidth, float textureHeight, boolean mirror, Direction direction
   ) {
      if (!uvUnion.isBoxUV()) {
         ...
         double[][] uvData = switch (direction) {
            case WEST -> new double[][]{{uv[0] + uvSizeVec.z + uvSizeVec.x, uv[1] + uvSizeVec.z}, {uvSizeVec.z, uvSizeVec.y}};
            case EAST -> new double[][]{{uv[0], uv[1] + uvSizeVec.z}, {uvSizeVec.z, uvSizeVec.y}};
            case NORTH -> new double[][]{{uv[0] + uvSizeVec.z, uv[1] + uvSizeVec.z}, {uvSizeVec.x, uvSizeVec.y}};
            case SOUTH -> new double[][]{{uv[0] + uvSizeVec.z + uvSizeVec.x + uvSizeVec.z, uv[1] + uvSizeVec.z}, {uvSizeVec.x, uvSizeVec.y}};
            case UP -> new double[][]{{uv[0] + uvSizeVec.z, uv[1]}, {uvSizeVec.x, uvSizeVec.z}};
            case DOWN -> new double[][]{{uv[0] + uvSizeVec.z + uvSizeVec.x, uv[1] + uvSizeVec.z}, {uvSizeVec.x, -uvSizeVec.z}};
   ...
   public static final class Builtin implements BakedModelFactory {
      ...
      public GeoCube constructCube(Cube cube, ModelProperties properties, GeoBone bone) {
         ...
         GeoQuad[] quads = this.buildQuads(
            cube.uv(),
            new BakedModelFactory.VertexSet(origin, vertexSize, inflate),
            cube,
   ...
   public record VertexSet(
      ...
      public GeoVertex[] verticesForQuad(Direction direction, boolean boxUv, boolean mirror) {
         return switch (direction) {
            case WEST -> mirror ? this.quadEast() : this.quadWest();
            case EAST -> mirror ? this.quadWest() : this.quadEast();
            case NORTH -> this.quadNorth();
            case SOUTH -> this.quadSouth();
            case UP -> mirror && !boxUv ? this.quadDown() : this.quadUp();
            case DOWN -> mirror && !boxUv ? this.quadUp() : this.quadDown();
            default -> throw new MatchException(null, null);
         };
      }
   }
