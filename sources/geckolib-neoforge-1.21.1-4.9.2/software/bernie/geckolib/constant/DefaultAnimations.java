...
public final class DefaultAnimations {
   ...
   public static <T extends GeoAnimatable> AnimationController<T> basicPredicateController(
      T animatable, RawAnimation optionA, RawAnimation optionB, BiFunction<T, AnimationState<T>, Boolean> predicate
   ) {
   ...
   public static <T extends GeoAnimatable> AnimationController<T> getSpawnController(
      T animatable, Function<AnimationState<T>, Object> objectSupplier, int ticks
   ) {
      return new AnimationController<>(
         animatable, "Spawn", 0, state -> animatable.getTick(objectSupplier.apply(state)) <= ticks ? state.setAndContinue(SPAWN) : PlayState.STOP
      );
   }
   ...
   public static <T extends Mob & GeoAnimatable> AnimationController<T> genericWalkFlyIdleController(T entity) {
      return new AnimationController<>(
         ...
         "Walk/Fly/Idle",
         0,
         state -> entity.onGround() || !entity.isNoAi() && entity.getBlockPosBelowThatAffectsMyMovement().getY() + 1 == entity.getY()
            ? state.setAndContinue(state.isMoving() ? WALK : IDLE)
            : state.setAndContinue(state.isMoving() ? FLY : IDLE_FLYING)
      );
   }

   public static <T extends Entity & GeoAnimatable> AnimationController<T> genericWalkRunIdleController(T entity) {
      return new AnimationController<>(
         entity, "Walk/Run/Idle", 0, state -> state.isMoving() ? state.setAndContinue(entity.isSprinting() ? RUN : WALK) : state.setAndContinue(IDLE)
      );
   }
}
