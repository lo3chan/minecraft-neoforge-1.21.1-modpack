...
public class PlayerTransformer {
   ...
   public void updateModel(AbstractClientPlayer entity, PlayerModel model, float swing, CallbackInfo info) {
      ...
      if (this.mc.level != null && !this.renderingFirstPersonArm) {
         ...
         if (entity instanceof PlayerData data) {
            ...
            float timePassed = passedTicks * 50.0F;
            ...
            if ((isCameraEntity || NEABaseMod.config.applyRotationLockToEveryone)
               && !data.isDisableBodyRotation()
               && (!isCameraEntity || !NEABaseMod.config.limitRotationLockToFP || this.mc.options.getCameraType() == CameraType.FIRST_PERSON)) {
               if ((NEABaseMod.config.rotationLock == RotationLock.SMOOTH || NEABaseMod.config.rotationLock == RotationLock.NONE && data.isRotateBodyToHead())
   ...
   private void interpolate(ModelPart model, float[] last, int offset, float timePassed, boolean differentFrame, float speed, float delta) {
      if (timePassed > 50.0F) {
         ...
         float amount = Math.min(speed, 1.0F);
