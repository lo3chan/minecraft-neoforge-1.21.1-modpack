...
public class ColorEntry extends TextFieldListEntry<Integer> {
   ...
   public ColorEntry(
      ...
      if (colorValue.hasError()) {
         ...
         this.resetButton.onPress = button -> this.textFieldWidget.setValue(this.getHexColorString(defaultValue.get()));
      }
   }
   ...
   public void render(GuiGraphics graphics, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
      ...
      if (Minecraft.getInstance().font.isBidirectional()) {
         this.colorDisplayWidget.setX(x + this.resetButton.getWidth() + this.textFieldWidget.getWidth());
      } else {
         this.colorDisplayWidget.setX(this.textFieldWidget.getX() - 23);
      }
   ...
   public Optional<Component> getError() {
      ...
      return colorValue.hasError()
         ? Optional.of(Component.translatable("text.cloth-config.error.color." + colorValue.getError().name().toLowerCase(Locale.ROOT)))
         : super.getError();
   }
