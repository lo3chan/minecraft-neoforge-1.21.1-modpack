...
public abstract class AbstractConfigEntry<T>
   ...
   protected FormattedCharSequence[] wrapLinesToScreen(Component[] lines) {
      return this.wrapLines(lines, this.screen.width);
   }

   protected FormattedCharSequence[] wrapLines(Component[] lines, int width) {
      Font font = Minecraft.getInstance().font;
      return Arrays.stream(lines).map(line -> font.split(line, width)).flatMap(Collection::stream).toArray(FormattedCharSequence[]::new);
   }
