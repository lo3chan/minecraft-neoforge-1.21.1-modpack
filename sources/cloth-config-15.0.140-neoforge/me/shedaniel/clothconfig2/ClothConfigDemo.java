   ...
   public static ConfigBuilder getConfigBuilderWithDemo() {
      ...
      testing.addEntry(
         entryBuilder.startModifierKeyCodeField(
               Component.literal("Cool Modifier Key"), ModifierKeyCode.of(Type.KEYSYM.getOrCreate(79), Modifier.of(false, true, false))
            )
            .setDefaultValue(ModifierKeyCode.of(Type.KEYSYM.getOrCreate(79), Modifier.of(false, true, false)))
            .build()
      );
      testing.addEntry(
         entryBuilder.startDoubleList(Component.literal("A list of Doubles"), Arrays.asList(1.0, 2.0, 3.0))
            .setDefaultValue(Arrays.asList(1.0, 2.0, 3.0))
            .build()
      );
      testing.addEntry(
         entryBuilder.startLongList(Component.literal("A list of Longs"), Arrays.asList(1L, 2L, 3L))
            .setDefaultValue(Arrays.asList(1L, 2L, 3L))
            .setInsertButtonEnabled(false)
            .build()
      );
      testing.addEntry(
         entryBuilder.startStrList(Component.literal("A list of Strings"), Arrays.asList("abc", "xyz"))
      ...
      colors.add(
         (AbstractConfigListEntry)entryBuilder.startColorField(Component.literal("An alpha color field"), -16711681)
            .setDefaultValue(-16711681)
            .setAlphaMode(true)
            .build()
      );
      colors.add(
         (AbstractConfigListEntry)entryBuilder.startColorField(Component.literal("An alpha color field"), -1)
            ...
            (elem, nestedListListEntry) -> {
               if (elem == null) {
                  ...
                  return new MultiElementListEntry<>(
                     Component.literal("Pair"),
                     newDefaultElemValue,
                     Lists.newArrayList(
                        new AbstractConfigListEntry[]{
                           entryBuilder.startIntField(Component.literal("Left"), newDefaultElemValue.getLeft()).setDefaultValue(10).build(),
                  ...
                  return new MultiElementListEntry<>(
                     ...
                     Lists.newArrayList(
                        new AbstractConfigListEntry[]{
                           entryBuilder.startIntField(Component.literal("Left"), (Integer)elem.getLeft()).setDefaultValue(10).build(),
                           entryBuilder.startIntField(Component.literal("Right"), (Integer)elem.getRight()).setDefaultValue(10).build()
                        }
                     ),
                     true
                  );
      ...
      BooleanListEntry dependency = entryBuilder.startBooleanToggle(Component.literal("A cool toggle"), false)
         .setTooltip(Component.literal("Toggle me..."))
         .build();
      depends.add((AbstractConfigListEntry)dependency);
      Collection<BooleanListEntry> toggles = new LinkedList<>();
      toggles.add(
         entryBuilder.startBooleanToggle(Component.literal("I only work when cool is toggled..."), true).setRequirement(Requirement.isTrue(dependency)).build()
      );
      toggles.add(
         entryBuilder.startBooleanToggle(Component.literal("I only appear when cool is toggled..."), true)
            .setDisplayRequirement(Requirement.isTrue(dependency))
            .build()
      );
      depends.addAll(toggles);
      depends.add(
         (AbstractConfigListEntry)entryBuilder.startBooleanToggle(Component.literal("I only work when cool matches both of these toggles ^^"), true)
      ...
      depends.add(
         (AbstractConfigListEntry)entryBuilder.startLongList(Component.literal("Even lists!"), Arrays.asList(1L, 2L, 3L))
            .setDefaultValue(Arrays.asList(1L, 2L, 3L))
            .setRequirement(Requirement.isTrue(dependency))
            .build()
      );
      EnumListEntry<DependencyDemoEnum> enumDependency = entryBuilder.startEnumSelector(
            Component.literal("Select a good or bad option"), DependencyDemoEnum.class, DependencyDemoEnum.OKAY
         )
         .build();
      ...
      depends.add(
         (AbstractConfigListEntry)entryBuilder.startBooleanToggle(Component.literal("I only work when a good option is chosen..."), true)
      ...
      depends.add(
         (AbstractConfigListEntry)entryBuilder.startBooleanToggle(Component.literal("I need a good option AND a cool toggle!"), true)
            .setTooltip(Component.literal("Select good or better and also toggle cool"))
            .setRequirement(
      ...
      depends.add(
         (AbstractConfigListEntry)entryBuilder.startBooleanToggle(Component.literal("I only work when numbers are extreme!"), true)
      ...
      testing.addEntry(
         entryBuilder.startTextDescription(
               Component.translatable(
                  ...
                  new Object[]{
                     Component.literal("ClothConfig")
                        .withStyle(
                           s -> s.withBold(true)
                              .withHoverEvent(
                                 new HoverEvent(Action.SHOW_ITEM, new ItemStackInfo((ItemStack)Util.make(new ItemStack(Items.PINK_WOOL), stack -> {
                                    stack.set(DataComponents.CUSTOM_NAME, Component.literal("(・∀・)"));
                                    stack.enchant(
                                       VanillaRegistries.createLookup().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.EFFICIENCY), 10
                                    );
                                 })))
                              )
                        ),
                     Component.translatable("text.cloth-config.testing.2")
                        .withStyle(
                           s -> s.withColor(ChatFormatting.BLUE)
                              .withHoverEvent(new HoverEvent(Action.SHOW_TEXT, Component.literal("https://shedaniel.gitbook.io/cloth-config/")))
                              .withClickEvent(
                                 new ClickEvent(net.minecraft.network.chat.ClickEvent.Action.OPEN_URL, "https://shedaniel.gitbook.io/cloth-config/")
                              )
                        ),
                     Component.translatable("text.cloth-config.testing.3")
                        .withStyle(
                           s -> s.withColor(ChatFormatting.GREEN)
                              .withClickEvent(
                                 new ClickEvent(
                                    net.minecraft.network.chat.ClickEvent.Action.OPEN_FILE,
                                    Utils.getConfigFolder().getParent().resolve("options.txt").toString()
                                 )
                              )
                        )
                  }
