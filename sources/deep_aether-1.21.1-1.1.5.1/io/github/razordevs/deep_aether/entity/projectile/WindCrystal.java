...
public class WindCrystal extends AbstractCrystal {
   ...
   private float getDamage() {
      return this.isFriendly() ? 6.0F : (float)(10.0 + (10.0 * (Double)DeepAetherConfig.COMMON.eots_damage_multiplier.get() - 10.0));
   }
   ...
   protected void onHitBlock(BlockHitResult result) {
      ...
      if (!this.isFriendly()) {
         if (result.getDirection() == Direction.UP) {
            ...
            new WindCrystal(this.level(), this, 0.3 + offset, 0.0, 0.3 - offset);
            new WindCrystal(this.level(), this, -0.3 + offset, 0.0, 0.3 + offset);
            new WindCrystal(this.level(), this, 0.3 - offset, 0.0, -0.3 - offset);
            new WindCrystal(this.level(), this, -0.3 - offset, 0.0, -0.3 + offset);
         }

         this.level().playSound(this, result.getBlockPos(), (SoundEvent)AetherSoundEvents.ENTITY_ICE_CRYSTAL_EXPLODE.get(), SoundSource.HOSTILE, 1.0F, 1.0F);
         if (this.isBreakable(this.level().getBlockState(result.getBlockPos())) && EventHooks.canEntityGrief(this.level(), this)) {
   ...
   protected void onHit(HitResult pResult) {
      super.onHit(pResult);
      if (this.level().isClientSide) {
         this.level().addParticle(ParticleTypes.SNOWFLAKE, this.getX(), this.getY() + (this.random.nextFloat() - 0.5), this.getZ(), 0.0, 0.0, 0.0);
         this.level().addParticle(ParticleTypes.SNOWFLAKE, this.getX(), this.getY() + (this.random.nextFloat() - 0.5), this.getZ(), 0.0, 0.0, 0.0);
      }
   }

   private boolean isBreakable(BlockState blockState) {
      return !blockState.is(Blocks.VALKYRIE_QUEEN_UNBREAKABLE)
   ...
   public boolean isFriendly() {
      return (Boolean)this.entityData.get(IS_FRIENDLY);
   }
}
