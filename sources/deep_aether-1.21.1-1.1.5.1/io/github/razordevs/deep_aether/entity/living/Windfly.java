...
import com.aetherteam.aether.entity.passive.Aerwhale.BlankLookControl;
import com.aetherteam.aether.entity.passive.Aerwhale.SetTravelCourseGoal;
...
public class Windfly extends AetherAnimal implements FlyingAnimal {
   ...
   public Windfly(EntityType<Windfly> type, Level world) {
      ...
      this.moveControl = new Windfly.WindflyMoveControl(this);
      this.setPathfindingMalus(PathType.WATER, 0.0F);
   }

   protected void registerGoals() {
      this.goalSelector.addGoal(0, new SetTravelCourseGoal(this));
   }

   public static Builder createAttributes() {
      return FlyingMob.createMobAttributes()
         .add(Attributes.MAX_HEALTH, 6.0)
         .add(Attributes.FLYING_SPEED, 0.15000000596046448)
   ...
   public void tick() {
      ...
      if (this.random.nextInt(90) == 0) {
         ItemEntity shard = new ItemEntity(this.level(), 0.0, 0.0, 0.0, new ItemStack((ItemLike)AetherItems.AMBROSIUM_SHARD.get()));
         shard.moveTo(this.position());
         this.level().addFreshEntity(shard);
      }

      if (this.random.nextInt(7) == 0) {
         this.level().addParticle(ParticleTypes.END_ROD, this.getX() + this.random.nextFloat(), this.getY() + 0.3, this.getZ() - 0.5, 0.0, 0.0, 0.0);
      }
   ...
   public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
      return (AgeableMob)((EntityType)DAEntities.WINDFLY.get()).create(serverLevel);
   }
