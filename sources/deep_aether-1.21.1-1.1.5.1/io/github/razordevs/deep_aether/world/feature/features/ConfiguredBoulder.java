...
public class ConfiguredBoulder extends Feature<ConfiguredBoulder.Config> {
   public ConfiguredBoulder(Codec<ConfiguredBoulder.Config> codec) {
      super(codec);
   }

   public boolean place(FeaturePlaceContext<ConfiguredBoulder.Config> context) {
      BlockPos origin = context.origin();
      WorldGenLevel level = context.level();
      RandomSource rand = context.random();

      ConfiguredBoulder.Config config;
      for (config = (ConfiguredBoulder.Config)context.config(); origin.getY() > level.getMinBuildHeight() + 3; origin = origin.below()) {
   ...
   public record Config(BlockStateProvider state) implements FeatureConfiguration {
      public static final Codec<ConfiguredBoulder.Config> CODEC = BlockStateProvider.CODEC
         .fieldOf("state")
         .xmap(ConfiguredBoulder.Config::new, config -> config.state)
         .codec();
   }
}
