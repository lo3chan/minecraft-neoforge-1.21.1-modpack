...
public class PoisonLakeFeature extends Feature<AetherLakeConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<AetherLakeConfiguration> context) {
      ...
      AetherLakeConfiguration aetherLakeConfiguration = (AetherLakeConfiguration)context.config();
                  ...
                  if (booleans[(l1 * 16 + i2) * 8 + i3]) {
                     ...
                     if (this.canReplaceBlock(level.getBlockState(offsetPos))) {
                        ...
                        if (flag1) {
                           ...
                           this.createBarrier(offsetPos, level);
                        }
                     }
                  }
               }
         ...
         if (fluidBlockState.getFluidState().is(FluidTags.WATER)) {
            for (int k2 = 0; k2 < 16; k2++) {
               for (int k3 = 0; k3 < 16; k3++) {
                  ...
                  if (((Biome)level.getBiome(offsetPos).value()).shouldFreeze(level, offsetPos, false) && this.canReplaceBlock(level.getBlockState(offsetPos))) {
   ...
   private boolean mayBarrierReplace(BlockState state) {
      return !state.getFluidState().is((Fluid)DAFluids.POISON_FLUID.get()) && !state.getFluidState().is((Fluid)DAFluids.POISON_FLOWING.get())
         ? this.canReplaceBlock(state) && !state.isSolid()
