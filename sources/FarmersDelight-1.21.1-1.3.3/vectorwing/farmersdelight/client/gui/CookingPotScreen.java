...
public class CookingPotScreen extends AbstractContainerScreen<CookingPotMenu> implements RecipeUpdateListener {
   private static final WidgetSprites RECIPE_BUTTON = new WidgetSprites(
      ResourceLocation.withDefaultNamespace("recipe_book/button"), ResourceLocation.withDefaultNamespace("recipe_book/button")
   );
   private static final ResourceLocation BACKGROUND_TEXTURE = ResourceLocation.fromNamespaceAndPath("farmersdelight", "textures/gui/cooking_pot.png");
   ...
   private void renderHeatIndicatorTooltip(GuiGraphics gui, int mouseX, int mouseY) {
      if (this.isHovering(HEAT_ICON.x, HEAT_ICON.y, HEAT_ICON.width, HEAT_ICON.height, mouseX, mouseY)) {
         String key = "cooking_pot." + (((CookingPotMenu)this.menu).isHeated() ? "heated" : "not_heated");
   ...
   protected void renderMealDisplayTooltip(GuiGraphics gui, int mouseX, int mouseY) {
      if (this.minecraft != null
         ...
         && ((CookingPotMenu)this.menu).getCarried().isEmpty()
         ...
         if (this.hoveredSlot.index == 6) {
            ...
            ItemStack containerStack = ((CookingPotMenu)this.menu).blockEntity.getContainer();
   ...
   protected boolean hasClickedOutside(double mouseX, double mouseY, int x, int y, int buttonIdx) {
      boolean flag = mouseX < x || mouseY < y || mouseX >= x + this.imageWidth || mouseY >= y + this.imageHeight;
