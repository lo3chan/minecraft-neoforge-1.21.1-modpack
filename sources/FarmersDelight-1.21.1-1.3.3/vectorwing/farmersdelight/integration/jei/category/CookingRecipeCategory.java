...
public class CookingRecipeCategory implements IRecipeCategory<RecipeHolder<CookingPotRecipe>> {
   ...
   private final Component title = TextUtils.JEI("cooking");
   private final IDrawable background;
   private final IDrawable icon;

   public CookingRecipeCategory(IGuiHelper helper) {
      ResourceLocation widgetBackgroundImage = ResourceLocation.fromNamespaceAndPath("farmersdelight", "textures/gui/jei/cooking_pot.png");
      ResourceLocation interfaceImage = ResourceLocation.fromNamespaceAndPath("farmersdelight", "textures/gui/cooking_pot.png");
   ...
   public void getTooltip(ITooltipBuilder tooltip, RecipeHolder<CookingPotRecipe> recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
      if (ClientRenderUtils.isCursorInsideBounds(61, 2, 22, 28, mouseX, mouseY)) {
         int cookTime = ((CookingPotRecipe)recipe.value()).getCookTime();
         if (cookTime > 0) {
            int cookTimeSeconds = cookTime / 20;
            tooltip.add(Component.translatable("gui.jei.category.smelting.time.seconds", new Object[]{cookTimeSeconds}));
         }

         float experience = ((CookingPotRecipe)recipe.value()).getExperience();
         if (experience > 0.0F) {
