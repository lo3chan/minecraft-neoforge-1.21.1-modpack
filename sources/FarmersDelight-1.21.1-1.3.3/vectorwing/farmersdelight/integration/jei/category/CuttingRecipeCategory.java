...
public class CuttingRecipeCategory implements IRecipeCategory<RecipeHolder<CuttingBoardRecipe>> {
   ...
   private final Component title = TextUtils.JEI("cutting");
   private final IDrawable background;
   private final IDrawable icon;

   public CuttingRecipeCategory(IGuiHelper helper) {
      ResourceLocation backgroundImage = ResourceLocation.fromNamespaceAndPath("farmersdelight", "textures/gui/jei/cutting_board.png");
      ...
      this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack((ItemLike)ModItems.CUTTING_BOARD.get()));
   }
   ...
   public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<CuttingBoardRecipe> holder, IFocusGroup focusGroup) {
      ...
      builder.addSlot(RecipeIngredientRole.INPUT, 16, 27).addIngredients((Ingredient)recipe.getIngredients().get(0));
      ...
      for (int i = 0; i < size; i++) {
         ...
         ((IRecipeSlotBuilder)builder.addSlot(RecipeIngredientRole.OUTPUT, 76 + xOffset, 10 + yOffset)
            ...
            .addTooltipCallback((slotView, tooltip) -> {
               ...
               if (chance != 1.0F) {
                  tooltip.add(1, TextUtils.JEI("chance", chance < 0.01 ? "<1" : (int)(chance * 100.0F)).withStyle(ChatFormatting.GOLD));
               }
            });
      }
   }
