...
public class VillagerEvents {
      ...
      if (Configuration.ENABLE_FARMERS_BUY_FD_CROPS.get()) {
         ...
         if (professionKey != null) {
            if (professionKey.getPath().equals("farmer")) {
               ((List)trades.get(1)).add(emeraldForItemsTrade((ItemLike)ModItems.ONION.get(), 26, 16, 2));
               ((List)trades.get(1)).add(emeraldForItemsTrade((ItemLike)ModItems.TOMATO.get(), 26, 16, 2));
               ((List)trades.get(2)).add(emeraldForItemsTrade((ItemLike)ModItems.CABBAGE.get(), 16, 16, 5));
               ((List)trades.get(2)).add(emeraldForItemsTrade((ItemLike)ModItems.RICE.get(), 20, 16, 5));
            }
         }
      }
   }
   ...
   public static void onWandererTrades(WandererTradesEvent event) {
      if (Configuration.ENABLE_WANDERING_TRADER_SELLS_FD_ITEMS.get()) {
         ...
         trades.add(itemForEmeraldTrade((ItemLike)ModItems.CABBAGE_SEEDS.get(), 1, 12));
         trades.add(itemForEmeraldTrade((ItemLike)ModItems.TOMATO_SEEDS.get(), 1, 12));
         trades.add(itemForEmeraldTrade((ItemLike)ModItems.RICE.get(), 1, 12));
         trades.add(itemForEmeraldTrade((ItemLike)ModItems.ONION.get(), 1, 12));
      }
   }
