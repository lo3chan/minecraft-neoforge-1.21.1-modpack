...
public class WildCropGeneration {
   ...
   private static ResourceKey<ConfiguredFeature<?, ?>> registerConfiguredFeatureKey(String name) {
      return ResourceKey.create(Registries.CONFIGURED_FEATURE, ResourceLocation.fromNamespaceAndPath("farmersdelight", name));
   }

   private static ResourceKey<PlacedFeature> registerPlacedFeatureKey(String name) {
      return ResourceKey.create(Registries.PLACED_FEATURE, ResourceLocation.fromNamespaceAndPath("farmersdelight", name));
   }

   public static void bootstrapConfiguredFeatures(BootstrapContext<ConfiguredFeature<?, ?>> context) {
      context.register(
         FEATURE_PATCH_SANDY_SHRUB,
         new ConfiguredFeature(Feature.RANDOM_PATCH, new RandomPatchConfiguration(32, 2, 3, plantPlacedFeature(ModBlocks.SANDY_SHRUB.get(), BlockTags.SAND)))
      );
      context.register(FEATURE_PATCH_WILD_CABBAGES, wildCropConfiguredFeature(ModBlocks.WILD_CABBAGES.get(), ModBlocks.SANDY_SHRUB.get(), BlockTags.SAND));
      context.register(FEATURE_PATCH_WILD_BEETROOTS, wildCropConfiguredFeature(ModBlocks.WILD_BEETROOTS.get(), ModBlocks.SANDY_SHRUB.get(), BlockTags.SAND));
      context.register(
         FEATURE_PATCH_WILD_CARROTS, wildCropConfiguredFeature(ModBlocks.WILD_CARROTS.get(), Blocks.SHORT_GRASS, Blocks.COARSE_DIRT, BlockTags.DIRT)
      );
      context.register(FEATURE_PATCH_WILD_ONIONS, wildCropConfiguredFeature(ModBlocks.WILD_ONIONS.get(), Blocks.ALLIUM, BlockTags.DIRT));
      context.register(FEATURE_PATCH_WILD_POTATOES, wildCropConfiguredFeature(ModBlocks.WILD_POTATOES.get(), Blocks.FERN, BlockTags.DIRT));
      context.register(FEATURE_PATCH_WILD_TOMATOES, wildCropConfiguredFeature(ModBlocks.WILD_TOMATOES.get(), Blocks.DEAD_BUSH, ModTags.Blocks.TERRAIN));
      context.register(
         FEATURE_PATCH_WILD_RICE,
         new ConfiguredFeature(
            ModBiomeFeatures.WILD_RICE.get(), new RandomPatchConfiguration(96, 7, 3, plantPlacedFeature(ModBlocks.WILD_RICE.get(), BlockTags.DIRT))
         )
      );
      context.register(FEATURE_PATCH_BROWN_MUSHROOM_COLONIES, mushroomColonyConfiguredFeature(ModBlocks.BROWN_MUSHROOM_COLONY.get(), Blocks.BROWN_MUSHROOM));
      context.register(FEATURE_PATCH_RED_MUSHROOM_COLONIES, mushroomColonyConfiguredFeature(ModBlocks.RED_MUSHROOM_COLONY.get(), Blocks.RED_MUSHROOM));
   }
   ...
   private static ConfiguredFeature<?, ?> wildCropConfiguredFeature(Block primaryBlock, Block secondaryBlock, TagKey<Block> blocksToTarget) {
      return defaultWildCropConfiguredFeature(plantPlacedFeature(primaryBlock, blocksToTarget), plantPlacedFeature(secondaryBlock, blocksToTarget), null);
   }
   ...
   private static ConfiguredFeature<?, ?> wildCropConfiguredFeature(Block primaryBlock, Block secondaryBlock, Block floorBlock, TagKey<Block> blocksToTarget) {
      return defaultWildCropConfiguredFeature(
         plantPlacedFeature(primaryBlock, blocksToTarget), plantPlacedFeature(secondaryBlock, blocksToTarget), floorPlacedFeature(floorBlock, blocksToTarget)
      );
   }
               ...
               Holder.direct(
                  new ConfiguredFeature(
                     ...
                     new SimpleBlockConfiguration(
                        new RandomizedIntStateProvider(BlockStateProvider.simple(colonyBlock), MushroomColonyBlock.COLONY_AGE, UniformInt.of(0, 3))
                     )
                  )
               ),
   ...
   private static ConfiguredFeature<?, ?> defaultWildCropConfiguredFeature(
      Holder<PlacedFeature> primaryFeature, Holder<PlacedFeature> secondaryFeature, @Nullable Holder<PlacedFeature> floorFeature
   ) {
      return new ConfiguredFeature(ModBiomeFeatures.WILD_CROP.get(), new WildCropConfiguration(64, 6, 3, primaryFeature, secondaryFeature, floorFeature));
   }

   private static Holder<PlacedFeature> plantPlacedFeature(Block block, Block blocksToPlaceOn) {
      return Holder.direct(
         new PlacedFeature(
            Holder.direct(new ConfiguredFeature(Feature.SIMPLE_BLOCK, new SimpleBlockConfiguration(SimpleStateProvider.simple(block)))),
      ...
      return List.of(
         BlockPredicateFilter.forPredicate(
            BlockPredicate.allOf(
               BlockPredicate.matchesBlocks(new Block[]{Blocks.AIR}), BlockPredicate.matchesBlocks(new Vec3i(0, -1, 0), new Block[]{blockToPlaceOn})
            )
         )
      );
   }
   ...
   private static List<PlacementModifier> placeOnTopOfModifier(TagKey<Block> blocksToPlaceOn) {
      return List.of(
         BlockPredicateFilter.forPredicate(
            BlockPredicate.allOf(BlockPredicate.matchesBlocks(new Block[]{Blocks.AIR}), BlockPredicate.matchesTag(new Vec3i(0, -1, 0), blocksToPlaceOn))
         )
      );
   }
   ...
   private static List<PlacementModifier> replaceBlockModifier(TagKey<Block> blocksToReplace) {
      return List.of(
         BlockPredicateFilter.forPredicate(BlockPredicate.allOf(BlockPredicate.replaceable(new Vec3i(0, 1, 0)), BlockPredicate.matchesTag(blocksToReplace)))
      );
   }

   private static PlacedFeature createPlacedFeature(
      HolderGetter<ConfiguredFeature<?, ?>> featureGetter, ResourceKey<ConfiguredFeature<?, ?>> feature, int rarity
   ) {
      return new PlacedFeature(
         featureGetter.getOrThrow(feature),
         List.of(
