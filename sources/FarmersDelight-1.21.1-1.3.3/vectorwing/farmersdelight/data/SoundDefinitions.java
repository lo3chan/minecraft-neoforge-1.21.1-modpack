...
public class SoundDefinitions extends SoundDefinitionsProvider {
   protected SoundDefinitions(PackOutput output, ExistingFileHelper helper) {
      super(output, "farmersdelight", helper);
   }

   public void registerSounds() {
      this.generateNewSoundWithSubtitle(ModSounds.BLOCK_COOKING_POT_BOIL, "block/cooking_pot/boil_water", 2);
      this.generateNewSoundCustomSubtitle(
         ModSounds.BLOCK_COOKING_POT_BOIL_SOUP, "block/cooking_pot/boil_soup", 3, "subtitles.farmersdelight.block.cooking_pot.boil"
      );
      ...
      this.generateExistingSoundWithSubtitle(ModSounds.BLOCK_FOOD_TAKE_PORTION, (SoundEvent)SoundEvents.ARMOR_EQUIP_GENERIC.value());
   ...
   public void generateNewSoundCustomSubtitle(Supplier<SoundEvent> event, String baseSoundDirectory, int numberOfSounds, @Nullable String subtitle) {
      ...
      for (int i = 1; i <= numberOfSounds; i++) {
         definition.with(
            Sound.sound(ResourceLocation.fromNamespaceAndPath("farmersdelight", baseSoundDirectory + (numberOfSounds > 1 ? i : "")), SoundType.SOUND)
         );
      }
   ...
   public void generateExistingSound(Supplier<SoundEvent> event, SoundEvent referencedSound, boolean subtitle) {
      ...
      this.add(event, definition.with(Sound.sound(referencedSound.getLocation(), SoundType.EVENT)));
   }
}
