...
public class ItemModels extends ItemModelProvider {
   ...
   public static final ResourceLocation MUG = ResourceLocation.fromNamespaceAndPath("farmersdelight", "item/mug");
   ...
   protected void registerModels() {
      Set<Item> items = BuiltInRegistries.ITEM
         .stream()
         .filter(i -> "farmersdelight".equals(BuiltInRegistries.ITEM.getKey(i).getNamespace()))
      ...
      this.itemGeneratedModel(ModItems.DEBUG_PUMPKIN_PIE.get(), this.resourceItem("debug_pumpkin_pie"));
      ...
      this.blockBasedModel(ModItems.ORGANIC_COMPOST.get(), "_stage0");
      items.remove(ModItems.ORGANIC_COMPOST.get());
      this.blockBasedModel(ModItems.ROPE_FENCE.get(), "_inventory");
      ...
      takeAll(items, mugItems.toArray(new Item[0])).forEach(item -> this.itemMugModel(item, this.resourceItem(this.itemName(item))));
      ...
      takeAll(items, spriteBlockItems.toArray(new Item[0]))
         .forEach(
            item -> ((ItemModelBuilder)this.withExistingParent(this.itemName(item), "item/generated"))
               .texture("layer0", this.resourceItem(this.itemName(item)))
         );
      ...
      takeAll(items, flatBlockItems.toArray(new Item[0])).forEach(item -> this.itemGeneratedModel(item, this.resourceBlock(this.itemName(item))));
      ...
      takeAll(items, handheldItems.toArray(new Item[0])).forEach(item -> this.itemHandheldModel(item, this.resourceItem(this.itemName(item))));
   ...
   public void blockBasedModel(Item item, ResourceLocation block) {
      this.withExistingParent(this.itemName(item), block);
   }
   ...
   public ResourceLocation resourceItem(String path) {
      return ResourceLocation.fromNamespaceAndPath("farmersdelight", "item/" + path);
   }
