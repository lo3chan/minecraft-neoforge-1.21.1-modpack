...
public class JeiConfig implements IModConfig {
   ...
   public ActionResult canPlayerEdit(Player player) {
      ...
      return !context.isClient() || !context.isMainMenu() && !context.isLocalPlayer() ? ActionResult.fail() : ActionResult.success();
   }
   ...
   public boolean isChanged() {
      return this.categories
         .stream()
         .anyMatch(category -> category.getConfigValues().stream().anyMatch(value -> !Objects.equals(value.getValue(), value.getDefaultValue())));
   }

   @Override
   public Optional<Runnable> restoreDefaultsTask() {
      return Optional.of(() -> this.categories.forEach(category -> category.getConfigValues().forEach(JeiConfig::restoreDefaultValue)));
   }
