...
public class FrameworkServerHandler {
         ...
         if (config == null) {
            ...
            if (processSyncData(message, true)) {
               ...
               ServerPlayHelper.sendMessageToOperators(
                  Component.translatable("configured.chat.config_updated", new Object[]{player.getName(), config.getFileName()})
   ...
   public static void handleRequestConfig(ServerPlayer player, MessageFramework.Request message, Consumer<Component> disconnect) {
      ...
      if (!context.isDedicatedServer()) {
         ...
         disconnect.accept(Component.translatable("configured.multiplayer.disconnect.bad_config_packet"));
      } else if (ServerPlayHelper.canEditServerConfigs(player)) {
         ...
         if (config == null) {
            ...
            Constants.LOG
               .error("{} tried requesting a Framework config that is not allowed to be requested: '{}'", player.getGameProfile().getName(), message.id());
            disconnect.accept(Component.translatable("configured.multiplayer.disconnect.bad_config_packet"));
         } else if (!config.isLoaded()) {
            Constants.LOG
               .error(
                  "{} tried requesting the Framework config '{}', however it is not loaded. Something went terribly wrong...",
