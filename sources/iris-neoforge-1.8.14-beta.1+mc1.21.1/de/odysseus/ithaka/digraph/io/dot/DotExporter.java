...
public class DotExporter {
   ...
   private <V> void writeEdge(
      Writer writer,
      int level,
      V source,
      V target,
      int edgeWeight,
      DotProvider<V, ?> provider,
      DotExporter.Cluster<V, ?> sourceCluster,
      DotExporter.Cluster<V, ?> targetCluster
   ) throws IOException {
      this.indent(writer, level);
   ...
   private <V, G extends Digraph<V>> Map<V, DotExporter.Cluster<V, G>> createClusters(
      G digraph, DotProvider<V, G> provider, DigraphProvider<? super V, G> subgraphs
   ) {
      Map<V, DotExporter.Cluster<V, G>> clusters = new HashMap<>();
      if (subgraphs != null) {
         for (V vertex : digraph.vertices()) {
            G subgraph = subgraphs.get(vertex);
            if (subgraph != null && subgraph.getVertexCount() > 0) {
               clusters.put(vertex, new DotExporter.Cluster<>("cluster_" + provider.getNodeId(vertex), subgraph));
            }
         }
      }
   ...
   public <V, G extends Digraph<V>> void export(DotProvider<V, G> provider, G digraph, DigraphProvider<V, G> subgraphs, Writer writer) throws IOException {
      ...
      writer.write(this.lineSpeparator);
      Map<V, DotExporter.Cluster<V, G>> clusters = this.createClusters(digraph, provider, subgraphs);
      if (!clusters.isEmpty()) {
   ...
   private <V, G extends Digraph<V>> void writeNodesAndEdges(
      Writer writer, int level, DotProvider<V, G> provider, G digraph, Map<V, DotExporter.Cluster<V, G>> clusters, DigraphProvider<V, G> subgraphs
   ) throws IOException {
      for (V vertex : digraph.vertices()) {
   ...
   private <V, G extends Digraph<V>> void writeCluster(
      Writer writer, int level, DotProvider<V, G> provider, V subgraphVertex, DotExporter.Cluster<V, G> cluster, DigraphProvider<V, G> subgraphs
   ) throws IOException {
      this.indent(writer, level);
      ...
      Map<V, DotExporter.Cluster<V, G>> subclusters = this.createClusters(cluster.subgraph, provider, subgraphs);
