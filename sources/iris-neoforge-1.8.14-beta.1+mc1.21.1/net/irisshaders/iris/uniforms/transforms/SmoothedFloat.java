...
public class SmoothedFloat implements FloatSupplier {
   ...
   private void update() {
      if (!this.hasInitialValue) {
         ...
         float smoothingFactor = 1.0F - exponentialDecayFactor(newValue > this.accumulator ? this.decayConstantUp : this.decayConstantDown, lastFrameTime);
         this.accumulator = lerp(this.accumulator, newValue, smoothingFactor);
      }
   }
