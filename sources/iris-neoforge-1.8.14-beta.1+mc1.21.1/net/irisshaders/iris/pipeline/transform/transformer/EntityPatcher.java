...
public class EntityPatcher {
   ...
   public static void patchOverlayColor(ASTParser t, TranslationUnit tree, Root root, VanillaParameters parameters) {
      ...
      if (parameters.type.glShaderType == ShaderType.VERTEX) {
         ...
         tree.parseAndInjectNodes(
            t,
            ASTInjectionPoint.BEFORE_DECLARATIONS,
            new String[]{"patch out vec4 entityColorTCS;", "in vec4 entityColor[];", "out vec4 iris_vertexColorTCS[];", "in vec4 iris_vertexColor[];"}
         );
         tree.prependMainFunctionBody(
            t, new String[]{"entityColorTCS = entityColor[gl_InvocationID];", "iris_vertexColorTCS[gl_InvocationID] = iris_vertexColor[gl_InvocationID];"}
         );
         ...
         tree.parseAndInjectNodes(
            t,
            ASTInjectionPoint.BEFORE_DECLARATIONS,
            new String[]{"out vec4 entityColorTES;", "patch in vec4 entityColorTCS;", "out vec4 iris_vertexColorTES;", "in vec4 iris_vertexColorTCS[];"}
         );
         ...
         tree.parseAndInjectNodes(
            t,
            ASTInjectionPoint.BEFORE_DECLARATIONS,
            new String[]{"out vec4 entityColorGS;", "in vec4 entityColor[];", "out vec4 iris_vertexColorGS;", "in vec4 iris_vertexColor[];"}
         );
   ...
   public static void patchEntityId(ASTParser t, TranslationUnit tree, Root root, VanillaParameters parameters) {
      ...
      if (parameters.type.glShaderType == ShaderType.VERTEX) {
         ...
         tree.parseAndInjectNodes(
            t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat out ivec3 iris_entityInfoTCS[];", "flat in ivec3 iris_entityInfo[];"}
         );
         ...
         tree.parseAndInjectNodes(
            t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat out ivec3 iris_entityInfoTES;", "flat in ivec3 iris_entityInfoTCS[];"}
         );
         ...
         tree.parseAndInjectNodes(
            t,
            ASTInjectionPoint.BEFORE_DECLARATIONS,
            new String[]{"flat out ivec3 iris_entityInfoGS;", "flat in ivec3 iris_entityInfo" + (parameters.hasTesselation ? "TES" : "") + "[];"}
         );
