...
public class IrisRenderingPipeline implements WorldRenderingPipeline, ShaderRenderingPipeline {
   ...
   public IrisRenderingPipeline(ProgramSet programSet) {
      ...
      this.shaderMap = new ShaderMap(key -> {
         try {
            if (key.isShadow()) {
               return this.shadowRenderTargets != null ? this.createShadowShader(key.getName(), this.resolver.resolve(key.getProgram()), key) : null;
            } else {
               return this.createShader(key.getName(), this.resolver.resolve(key.getProgram()), key);
            }
         } catch (FakeChainedJsonException var3x) {
            this.destroyShaders();
            throw var3x.getTrueException();
      ...
      if (programSet.getPackDirectives().supportsColorCorrection()) {
         ...
         this.colorSpaceConverter = new ColorSpaceFragmentConverter(main.width, main.height, IrisVideoSettings.colorSpace);
      }
   ...
   private ComputeProgram[] createShadowComputes(ComputeSource[] compute, ProgramSet programSet) {
      ...
      for (int i = 0; i < programs.length; i++) {
         ...
         if (source != null && !source.getSource().isEmpty()) {
            ...
            } catch (ShaderCompileException var10) {
               throw var10;
            } catch (RuntimeException var11) {
               throw new RuntimeException("Shader compilation failed for compute " + source.getName() + "!", var11);
            }
      ...
      for (int i = 0; i < programs.length; i++) {
         ...
         if (source != null && !source.getSource().isEmpty()) {
            ...
            try {
               ...
               throw new RuntimeException("Shader compilation failed for setup compute " + source.getName() + "!", var12);
            }
   ...
   private ShaderInstance createShader(
      String name,
      ...
      Supplier<ImmutableSet<Integer>> flipped = () -> this.isBeforeTranslucent ? this.flippedAfterPrepare : this.flippedAfterTranslucent;
      ExtendedShader extendedShader = ShaderCreator.create(
         this,
         name,
         source,
         programId,
   ...
   private ShaderInstance createFallbackShader(String name, ShaderKey key) throws IOException {
      ...
      FallbackShader shader = ShaderCreator.createFallback(
         name,
         beforeTranslucent,
         afterTranslucent,
         key.getAlphaTest(),
   ...
   private ShaderInstance createFallbackShadowShader(String name, ShaderKey key) throws IOException {
      GlFramebuffer framebuffer = this.shadowRenderTargets.createShadowFramebuffer(ImmutableSet.of(), new int[]{0});
      FallbackShader shader = ShaderCreator.createFallback(
         name,
         framebuffer,
         framebuffer,
   ...
   private ShaderInstance createShadowShader(
      String name,
      ProgramSource source,
      ProgramId programId,
      AlphaTest fallbackAlpha,
      ...
      GlFramebuffer framebuffer = this.shadowRenderTargets
         .createShadowFramebuffer(ImmutableSet.of(), source.getDirectives().hasUnknownDrawBuffers() ? new int[]{0, 1} : source.getDirectives().getDrawBuffers());
      boolean isLines = programId == ProgramId.Line && this.resolver.has(ProgramId.Line);
   ...
   public void beginLevelRendering() {
      ...
      boolean changed = this.renderTargets
         .resizeIfNeeded(
            ((Blaze3dRenderTargetExt)main).iris$getDepthBufferVersion(), depthTextureId, main.width, main.height, depthBufferFormat, this.packDirectives
         );
      if (changed) {
      ...
      Vector3d fogColor3 = CapturedRenderingState.INSTANCE.getFogColor();
      Vector4f fogColor = new Vector4f((float)fogColor3.x, (float)fogColor3.y, (float)fogColor3.z, 1.0F);
      UnmodifiableIterator skyType = passes.iterator();

      while (skyType.hasNext()) {
         ClearPass clearPass = (ClearPass)skyType.next();
      ...
      SkyType skyTypex = Minecraft.getInstance().level.effects().skyType();
      if (this.shouldRenderSkyDisc && (skyTypex == SkyType.NORMAL || Minecraft.getInstance().level.dimensionType().hasSkyLight())) {
         RenderSystem.depthMask(false);
         RenderSystem.setShaderColor(fogColor.x, fogColor.y, fogColor.z, fogColor.w);
         this.horizonRenderer
            .renderHorizon(
               CapturedRenderingState.INSTANCE.getGbufferModelView(), CapturedRenderingState.INSTANCE.getGbufferProjection(), GameRenderer.getPositionShader()
            );
