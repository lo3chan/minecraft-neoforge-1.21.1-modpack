...
public class ProfileElementWidget extends BaseOptionElementWidget<OptionMenuProfileElement> {
   ...
   public void init(ShaderPackScreen screen, NavigationController navigation) {
      ...
      this.profileLabel = (Component)profileName.<MutableComponent>map(name -> GuiUtil.translateOrDefault(Component.literal(name), "profile." + name))
         .orElse(PROFILE_CUSTOM);
   }

   @Override
   public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float tickDelta, boolean hovered) {
      this.updateRenderParams(this.bounds.width() - (Minecraft.getInstance().font.width(PROFILE_LABEL) + 16));
