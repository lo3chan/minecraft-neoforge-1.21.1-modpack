...
public class GlTexture extends GlResource implements TextureAccess {
   ...
   public GlTexture(
      ...
      IrisRenderSystem.texParameteri(texture, target.getGlType(), 10242, filteringData.shouldClamp() ? '脯' : 10497);
      if (sizeY > 0) {
         IrisRenderSystem.texParameteri(texture, target.getGlType(), 10243, filteringData.shouldClamp() ? '脯' : 10497);
      }

      if (sizeZ > 0) {
         IrisRenderSystem.texParameteri(texture, target.getGlType(), 32882, filteringData.shouldClamp() ? '脯' : 10497);
      }
