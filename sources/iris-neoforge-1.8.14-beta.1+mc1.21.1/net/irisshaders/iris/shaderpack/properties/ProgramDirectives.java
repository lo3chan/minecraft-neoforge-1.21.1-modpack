...
public class ProgramDirectives {
   ...
   private ProgramDirectives(
      int[] drawBuffers,
      ViewportData viewportScale,
      @Nullable AlphaTest alphaTestOverride,
      Optional<BlendModeOverride> blendModeOverride,
   ...
   public ProgramDirectives(
      ProgramSource source, ShaderProperties properties, Set<Integer> supportedRenderTargets, @Nullable BlendModeOverride defaultBlendOverride
   ) {
      ...
      this.drawBuffers = optionalCommentDirective.<int[]>map(commentDirective -> {
      ...
      if (properties != null) {
         this.viewportScale = (ViewportData)properties.getViewportScaleOverrides().getOrDefault(source.getName(), ViewportData.defaultValue());
         this.alphaTestOverride = (AlphaTest)properties.getAlphaTestOverrides().get(source.getName());
         BlendModeOverride blendModeOverride = (BlendModeOverride)properties.getBlendModeOverrides().get(source.getName());
         List<BufferBlendInformation> bufferBlendInformations = (List<BufferBlendInformation>)properties.getBufferBlendOverrides().get(source.getName());
      ...
      supportedRenderTargets.forEach(index -> {
         ...
         if (index < LEGACY_RENDER_TARGETS.size()) {
            directiveHolder.acceptConstBooleanDirective((String)LEGACY_RENDER_TARGETS.get(index) + "MipmapEnabled", mipmapHandler);
         }
      });
   ...
   private static Optional<CommentDirective> getAppliedDirective(
      Optional<CommentDirective> optionalDrawbuffersDirective, Optional<CommentDirective> optionalRendertargetsDirective
   ) {
      if (optionalDrawbuffersDirective.isPresent() && optionalRendertargetsDirective.isPresent()) {
         return optionalDrawbuffersDirective.get().getLocation() > optionalRendertargetsDirective.get().getLocation()
            ? optionalDrawbuffersDirective
            : optionalRendertargetsDirective;
      } else {
         return optionalDrawbuffersDirective.isPresent() ? optionalDrawbuffersDirective : optionalRendertargetsDirective;
      }
   }

   public ProgramDirectives withOverriddenDrawBuffers(int[] drawBuffersOverride) {
      return new ProgramDirectives(
         drawBuffersOverride,
         this.viewportScale,
         this.alphaTestOverride,
         this.blendModeOverride,
         this.bufferBlendInformations,
         this.mipmappedBuffers,
         this.explicitFlips
      );
   }
