...
public class ShadowRenderer {
      ...
      if ((this.packCullingState != ShadowCullState.DEFAULT || !this.packHasVoxelization) && this.packCullingState != ShadowCullState.DISTANCE) {
         ...
         if (distance >= Minecraft.getInstance().options.getEffectiveRenderDistance() * 16 && !isReversed) {
            ...
            distanceInfo = distanceInfo
               + (Minecraft.getInstance().isLocalServer() ? "(capped by normal render distance)" : "(capped by normal/server render distance)");
            boxCuller = null;
         } else {
         ...
         String cullingInfo = (isReversed ? "Safe Zone" : "Advanced") + " Frustum Culling enabled";
         ...
         return isReversed
            ? holder.setInfo(
               new SafeZoneCullingFrustum(projView, PROJECTION, shadowLightVectorFromOrigin, boxCuller, new BoxCuller(this.halfPlaneLength * renderMultiplier)),
               distanceInfo,
               cullingInfo
            )
         ...
         double distancex = this.halfPlaneLength * renderMultiplier;
         String reason;
         ...
         if (!(distancex <= 0.0) && !(distancex > Minecraft.getInstance().options.getEffectiveRenderDistance() * 16)) {
            String distanceInfo = distancex + " blocks (set by shader pack)";
            String cullingInfo = "distance only " + reason;
            BoxCuller boxCuller = new BoxCuller(distancex);
   ...
   public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
      if (IrisVideoSettings.getOverriddenShadowDistance(IrisVideoSettings.shadowDistance) != 0) {
         ...
         PoseStack modelView = createShadowModelView(this.sunPathRotation, this.intervalSize, this.nearPlane, this.farPlane);
         ...
         if (this.fov != null) {
            ...
            shadowProjection = ShadowMatrices.createOrthoMatrix(
               ...
               Mth.equal(this.nearPlane, -1.0F) ? -DHCompat.getRenderDistance() * 16 : this.nearPlane,
               Mth.equal(this.farPlane, -1.0F) ? DHCompat.getRenderDistance() * 16 : this.farPlane
            );
         }
         ...
         BufferSource bufferSource = this.buffers.bufferSource();
      ...
      for (BlockEntity entity : visibleBlockEntities) {
         if (!lightsOnly || entity.getBlockState().getLightEmission() != 0) {
            ...
            if (!hasEntityFrustum || !culler.isCulled(pos.getX() - 1, pos.getY() - 1, pos.getZ() - 1, pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1)) {
               ...
               Minecraft.getInstance().getBlockEntityRenderDispatcher().render(entity, tickDelta, modelView, this.buffers.bufferSource());
               modelView.popPose();
               shadowBlockEntities++;
            }
         }
   ...
   private int renderEntities(
      ...
      for (Entity entityx : renderedEntities) {
         float realTickDelta = Minecraft.getInstance().level.tickRateManager().isEntityFrozen(entityx)
            ? tickDelta
            : CapturedRenderingState.INSTANCE.getRealTickDelta();
         levelRenderer.invokeRenderEntity(entityx, cameraX, cameraY, cameraZ, realTickDelta, modelView, bufferSource);
      }
      ...
      if (dispatcher.shouldRender(player, frustum, cameraX, cameraY, cameraZ) && !player.isSpectator()) {
         ...
         if (!player.getPassengers().isEmpty()) {
            for (int i = 0; i < player.getPassengers().size(); i++) {
               float realTickDelta = Minecraft.getInstance().level.tickRateManager().isEntityFrozen((Entity)player.getPassengers().get(i))
                  ? tickDelta
                  : CapturedRenderingState.INSTANCE.getRealTickDelta();
               levelRenderer.invokeRenderEntity((Entity)player.getPassengers().get(i), cameraX, cameraY, cameraZ, realTickDelta, modelView, bufferSource);
               shadowEntities++;
            }
         }

         if (player.getVehicle() != null) {
            float realTickDelta = Minecraft.getInstance().level.tickRateManager().isEntityFrozen(player.getVehicle())
               ? tickDelta
               : CapturedRenderingState.INSTANCE.getRealTickDelta();
         ...
         float realTickDelta = Minecraft.getInstance().level.tickRateManager().isEntityFrozen(player)
            ? tickDelta
            : CapturedRenderingState.INSTANCE.getRealTickDelta();
   ...
   private String getEntitiesDebugString() {
      return !this.shouldRenderEntities && !this.shouldRenderPlayer
         ? "disabled by pack"
         : this.renderedShadowEntities + "/" + Minecraft.getInstance().level.getEntityCount();
   }
