...
public class SnowSettingsScreen extends LegacyOptionsSubScreen {
   ...
   private final CycleOption<Boolean> PHYSICS_SNOW_TRACKS = CycleOption.createOnOff(
      "physicsmod.menu.snow.snowtracks", gameOptions -> ConfigClient.snowTracks, (gameOptions, option, value) -> {
   ...
   public SnowSettingsScreen(Screen parent, Options options) {
      super(parent, options, Component.translatable("physicsmod.menu.snow.title.pro"));
            ...
            button -> PopupWidget.create(
               ...
               response -> {
                  if (response == PopupWidget.PopupResponse.YES) {
                     ...
                     this.minecraft.setScreen(new SnowSettingsScreen(this.lastScreen, this.options));
                     Minecraft.getInstance().levelRenderer.allChanged();
                  } else {
                     this.list.children().clear();
                     this.minecraft.setScreen(new SnowSettingsScreen(this.lastScreen, this.options));
   ...
   public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
      ...
      for (FormattedCharSequence sequence : this.info) {
         guiGraphics.drawString(this.font, sequence, (this.width - 300) / 2, 187 + lineY, 16777045);
         lineY += 10;
      }
