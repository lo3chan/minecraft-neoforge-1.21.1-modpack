   ...
   public boolean initAsyncData(PhysicsWorld world, VerletSimulation simulation) {
      if (ConfigClient.windPhysics) {
         ...
         if (data.points.size() > 0 && offset != null) {
            ...
            double xForce = noise.noise(x, z, 0.5 + y + time / 100.0);
            double zForce = noise.noise(x, z, 1641.5 + y + time / 100.0);
            double yForce = noise.noise(x, z, 89641.5 + y + time / 100.0);
            double strength = noise.noise(x, z, -11.5 + y + time / 7.0) * 16.0;
            ...
            double addStrength = domain.getWindStrength(ix, iy, iz);
