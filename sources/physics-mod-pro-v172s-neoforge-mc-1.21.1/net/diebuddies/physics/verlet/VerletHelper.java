...
public class VerletHelper {
   ...
   public boolean movePointOutOfBoxFaster(Vector3d point, float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
      if (point.x > minX && point.x < maxX && point.y > minY && point.y < maxY && point.z > minZ && point.z < maxZ) {
         double[] dist = new double[]{point.x - minX, maxX - point.x, point.y - minY, maxY - point.y, point.z - minZ, maxZ - point.z};
   ...
   public boolean movePointOutOfBox(Vector3d point, float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
      if (point.x > minX && point.x < maxX && point.y > minY && point.y < maxY && point.z > minZ && point.z < maxZ) {
         double dx = Math.min(point.x - minX, maxX - point.x);
         double dy = Math.min(point.y - minY, maxY - point.y);
         double dz = Math.min(point.z - minZ, maxZ - point.z);
         if (dx <= dy && dx <= dz) {
            point.x = point.x < (minX + maxX) * 0.5 ? minX : maxX;
         } else if (dy <= dx && dy <= dz) {
            point.y = point.y < (minY + maxY) * 0.5 ? minY : maxY;
         } else {
            point.z = point.z < (minZ + maxZ) * 0.5 ? minZ : maxZ;
         }
      ...
      if (point.x > minX && point.x < maxX && point.y > minY && point.y < maxY && point.z > minZ && point.z < maxZ) {
         ...
         if (lengthSquared != 0.0) {
            ...
            if (!Double.isNaN(dist) && dist < length && dist > 0.0) {
               point.set(rayOrigin.add(rayDir.mul(dist - 0.009999999776482582)));
               return true;
            }
         }
