...
public class BoxRigidBody extends IRigidBody {
   ...
   public static BoxRigidBody create(PhysicsEntity entity, float width, float height, float depth, float offsetx, float offsety, float offsetz, boolean dynamic) {
      ...
      try {
         ...
         PxBoxGeometry boxGeometry = PxBoxGeometry.createAt(
            mem,
            MemoryStack::nmalloc,
            width * 0.5F * (float)Math.abs(scale.x) * entity.enlargeHitbox.x * entity.scale,
            height * 0.5F * (float)Math.abs(scale.y) * entity.enlargeHitbox.y * entity.scale,
            depth * 0.5F * (float)Math.abs(scale.z) * entity.enlargeHitbox.z * entity.scale
         );
   ...
   public static BoxRigidBody create(PhysicsEntity entity, boolean dynamic) {
      ...
      try {
         ...
         Vector3f min = new Vector3f(3.4028235E38F, 3.4028235E38F, 3.4028235E38F);
         ...
         for (int i = 0; i < models.size(); i++) {
            ...
            if (!model.onlyVisual) {
               ...
               for (int j = 0; j < positions.size(); j++) {
                  ...
                  tmp.set(position).mul(scaleX, scaleY, scaleZ);
                  min.min(tmp);
                  max.max(tmp);
                  numPoints++;
               }
      ...
      try {
         ...
         capsuleShape.setLocalPose(
            PxTransform.createAt(
               ...
               PxQuat.createAt(mem, MemoryStack::nmalloc, 0.0F, 0.0F, s, (float)Math.cos(a))
            )
         );
