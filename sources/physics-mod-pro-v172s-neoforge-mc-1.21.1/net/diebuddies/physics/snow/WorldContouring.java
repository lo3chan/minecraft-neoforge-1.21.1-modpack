...
public class WorldContouring extends IWorld<ChunkContouring> implements Runnable {
   ...
   public WorldContouring(SnowWorld snowWorld, int minChunkY, int maxChunkY) {
      ...
      this.playerPosition = new Vector3d(pos.x * IChunk.CHUNK_MULTIPLE, pos.y * IChunk.CHUNK_MULTIPLE, pos.z * IChunk.CHUNK_MULTIPLE);
   ...
   private void createModulationLayer() {
      ...
      double dividerLarge = (double)repeatableAfter / tilesLarge;
      PerlinNoise perlinLarge = new PerlinNoise(new Random(0L), tilesLarge);
      int tilesSmall = IChunk.CHUNK_SIZE / 2;
      double dividerSmall = (double)repeatableAfter / tilesSmall;
      ...
      for (int x = 0; x < IChunk.CHUNK_SIZE; x++) {
         for (int y = 0; y < IChunk.CHUNK_SIZE; y++) {
            for (int z = 0; z < IChunk.CHUNK_SIZE; z++) {
               double noiseLarge = perlinLarge.noise(x / dividerLarge, y / dividerLarge, z / dividerLarge) * 0.5 + 0.5;
               double noiseSmall = perlinSmall.noise(x / dividerSmall, y / dividerSmall, z / dividerSmall);
   ...
   private void updateChangedChunks(int chunkX, int chunkZ) {
      ...
      float radius = new Vector3f(IChunk.CHUNK_SIZE / 2.0F + 3.0F).length() * IChunk.CHUNK_MULTIPLE_INV;
      ObjectRBTreeSet<ChunkRenderUpdate> updates = new ObjectRBTreeSet(new Comparator<ChunkRenderUpdate>() {
         public int compare(ChunkRenderUpdate o1, ChunkRenderUpdate o2) {
            return Double.compare(o1.distance, o2.distance);
         }
      });
      List<ChunkRenderUpdate> updateAnyways = new ObjectArrayList();
      double longestDistance = 1.7976931348623157E308;
      Vector3d position = new Vector3d(this.playerPosition);
      ...
      while (it.hasNext()) {
         ...
         if (chunk.getWorld() == null || !chunk.needsUpdate()) {
            ...
            if (distance < longestDistance || size < placeSize) {
               if (size > placeSize) {
                  updates.remove(updates.last());
               }

               updates.add(new ChunkRenderUpdate(distance, chunk));
               longestDistance = ((ChunkRenderUpdate)updates.last()).distance;
            }
         }
      }

      ObjectBidirectionalIterator var15 = updates.iterator();

      while (var15.hasNext()) {
         ChunkRenderUpdate cru = (ChunkRenderUpdate)var15.next();
         this.checkCRU(cru);
      }
      ...
      while (chunkIt.hasNext()) {
         ...
         if (chunkRenderUpdate.chunk.getWorld() == null) {
            chunkIt.remove();
         } else if (!chunkRenderUpdate.chunk.hasPriority()) {
      ...
      Iterator<ChunkRenderUpdate> var18 = this.chunkRenderUpdates.iterator();

      while (var18.hasNext()) {
         ChunkRenderUpdate chunkRenderUpdate = (ChunkRenderUpdate)var18.next();
         if (chunkRenderUpdate.chunk.getWorld() == null) {
            var18.remove();
         } else if (chunkRenderUpdate.chunk.hasPriority()) {
            this.tasks.add(0, chunkRenderUpdate.event);
   ...
   private double calculateChunkDistance(ChunkContouring chunk, Vector3d position, float radius) {
      double distance = position.distanceSquared(chunk.chunkPosMiddle);
      if (this.frustum
         .testSphere(
            (float)(chunk.xVoxel + IChunk.CHUNK_SIZE_HALF - position.x) * IChunk.CHUNK_MULTIPLE_INV,
            (float)(chunk.yVoxel + IChunk.CHUNK_SIZE_HALF - position.y) * IChunk.CHUNK_MULTIPLE_INV,
            (float)(chunk.zVoxel + IChunk.CHUNK_SIZE_HALF - position.z) * IChunk.CHUNK_MULTIPLE_INV,
            radius
         )) {
         distance *= 0.1;
      }
         ...
         for (int j = y - adjRadius; j < y + adjRadius; j++) {
            for (int k = z - adjRadius; k < z + adjRadius; k++) {
               ...
               if (c != null) {
                  ...
                  byte calcData = (byte)(strength * distance);
                  ...
                  byte oldData = c.getDataByteFast(voxelX, voxelY, voxelZ);
                  if (oldData > calcData) {
                     if (sdf.hasPriority()) {
                        c.setPriority(true);
                     }
   ...
   public void uploadInformation() {
      ...
      this.playerPosition.set(pos.x * IChunk.CHUNK_MULTIPLE, pos.y * IChunk.CHUNK_MULTIPLE, pos.z * IChunk.CHUNK_MULTIPLE);
   }
   ...
   public void removeChunkMesh(Vector3i chunkPosition, boolean seam) {
      this.updateMeshesEvent
         .addEvent(() -> this.snowWorld.removeChunkEntity(chunkPosition.x, chunkPosition.y + (seam ? this.heightChunks : 0), chunkPosition.z));
   }

   public void addChunkMesh(Vector3i chunkPosition, RawMesh mesh, int lod, boolean seam) {
      Vector3i newPosition = new Vector3i(chunkPosition);
