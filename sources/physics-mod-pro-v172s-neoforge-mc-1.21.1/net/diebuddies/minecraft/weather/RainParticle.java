...
public class RainParticle extends WeatherParticle {
   ...
   public RainParticle(ClientLevel clientLevel, double x, double y, double z, double vx, double vy, double vz) {
      ...
      this.quadSize = 0.23F;
      this.setColor(255, 255, 255, (int)((175 + (int)(Math.random() * 40.0)) * ConfigClient.particleRainOpacity));
      ...
      this.dampingZ = 1.0;
      this.gravity = 0.0F;
      this.rotationTo(movementRotation, (float)vx, (float)vy, (float)vz).rotateX((float)(Math.random() * 10.0));
      this.transformUnit(movementRotation, -this.quadSize, -this.quadSize, this.v0 = new Vector3f());
      this.transformUnit(movementRotation, -this.quadSize, this.quadSize, this.v1 = new Vector3f());
      this.transformUnit(movementRotation, this.quadSize, this.quadSize, this.v2 = new Vector3f());
      this.transformUnit(movementRotation, this.quadSize, -this.quadSize, this.v3 = new Vector3f());
   }
   ...
   public void transformUnit(Quaternionf src, float x, float y, Vector3f dest) {
      ...
      dest.set(
         org.joml.Math.fma(org.joml.Math.fma(-2.0F, yy + zz, 1.0F), x, 2.0F * (xy - zw) * y),
         org.joml.Math.fma(2.0F * (xy + zw), x, org.joml.Math.fma(-2.0F, xx + zz, 1.0F) * y),
         org.joml.Math.fma(2.0F * (xz - yw), x, 2.0F * (yz + xw) * y)
      );
   }
   ...
   public void render(VertexConsumer vertexConsumer, Camera camera, float renderPercent) {
      ...
      double px = org.joml.Math.lerp(this.xo, this.x, renderPercent);
      double py = org.joml.Math.lerp(this.yo, this.y, renderPercent);
      double pz = org.joml.Math.lerp(this.zo, this.z, renderPercent);
      ...
      if (StarterClient.sodium) {
         ...
         vertexConsumer.addVertex(this.v0.x + currentX, this.v0.y + currentY, this.v0.z + currentZ)
            .setUv(s1, t1)
            .setColor(this.r, this.g, this.b, this.a)
            .setLight(light);
         vertexConsumer.addVertex(this.v1.x + currentX, this.v1.y + currentY, this.v1.z + currentZ)
            .setUv(s1, t0)
            .setColor(this.r, this.g, this.b, this.a)
            .setLight(light);
         vertexConsumer.addVertex(this.v2.x + currentX, this.v2.y + currentY, this.v2.z + currentZ)
            .setUv(s0, t0)
            .setColor(this.r, this.g, this.b, this.a)
            .setLight(light);
         vertexConsumer.addVertex(this.v3.x + currentX, this.v3.y + currentY, this.v3.z + currentZ)
            .setUv(s0, t1)
            .setColor(this.r, this.g, this.b, this.a)
            .setLight(light);
      }
