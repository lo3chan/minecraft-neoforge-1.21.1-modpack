...
public class SprintEntangledProperty extends AbstractEntangledProperty {
   ...
   public void onInventoryTick(Entity user, ItemStack stack, Level level, int inventorySlot, boolean isCurrentItem) {
      if (user instanceof Player player && user.isSprinting() != this.getToggle(stack) && player.getInventory().getItem(inventorySlot) == stack) {
   ...
   public void onProjectileTick(ItemStack stack, Projectile projectile) {
      if (projectile instanceof InfusedItemProjectile infusedItemProjectile && infusedItemProjectile.getItem() == stack) {
         ...
         if (user != null && user.isSprinting() != this.getToggle(stack)) {
            this.setToggle(stack, user.isSprinting());
