...
public class BRBSmithingTrimRecipe extends SmithingTrimRecipe implements BRBSmithingRecipe {
   ...
   public static ArrayList<BRBSmithingTrimRecipe> from(SmithingTrimRecipe recipe) {
      ...
      for (ItemStack base : recipeAccessor.getUnderlyingBase().getItems()) {
         results.add(
            new BRBSmithingTrimRecipe(recipeAccessor.getUnderlyingTemplate(), recipeAccessor.getUnderlyingBase(), base, recipeAccessor.getUnderlyingAddition())
         );
      }
   ...
   public ItemStack getResult(ResourceKey<TrimMaterial> trimMaterialResourceKey, RegistryAccess registryAccess, BRBBookCategories.Category category) {
      Optional<Reference<TrimMaterial>> material = registryAccess.registryOrThrow(Registries.TRIM_MATERIAL).getHolder(trimMaterialResourceKey);
      Optional<Reference<TrimPattern>> trim = TrimPatterns.getFromTemplate(registryAccess, this.getTemplate().getItems()[0]);
      ItemStack itemStack = this.itemStackBase.copy();
      if (material.isPresent() && trim.isPresent()) {
         ArmorTrim armorTri = new ArmorTrim((Holder)material.get(), (Holder)trim.get());
