...
import com.github.alexthe666.alexsmobs.client.render.AMStraddleboardTint;
...
public class ClientProxy extends CommonProxy {
   ...
   public static void onItemColors(Item event) {
      ...
      if (AMItemRegistry.STRADDLEBOARD.get() != null) {
         event.register((stack, colorIn) -> AMStraddleboardTint.tintOf(stack, colorIn >= 1), new ItemLike[]{(ItemLike)AMItemRegistry.STRADDLEBOARD.get()});
      } else {
   ...
   public static void onBlockColors(Block event) {
      ...
      event.register(
         ...
         new net.minecraft.world.level.block.Block[]{AMBlockRegistry.RAINBOW_GLASS.get()}
      );
   }
   ...
   public static void onRegisterGuiLayers(RegisterGuiLayersEvent event) {
      event.registerAbove(
         ...
         AMCompat.rl("alexsmobs", "farseer_static"),
         (guiGraphics, deltaTracker) -> ClientEvents.renderStaticOverlay(guiGraphics, deltaTracker.getGameTimeDeltaPartialTick(false))
      );
   }
   ...
   public void clientInit() {
      ...
      try {
         ItemProperties.register(
            AMItemRegistry.BLOOD_SPRAYER.get(),
            AMCompat.rl("empty"),
            (stack, p_239428_1_, p_239428_2_, j) -> ItemBloodSprayer.isUsable(stack)
                  && (!(p_239428_2_ instanceof Player) || !AMCompat.isOnCooldown(((Player)p_239428_2_).getCooldowns(), AMItemRegistry.BLOOD_SPRAYER.get()))
               ? 0.0F
               : 1.0F
         );
         ItemProperties.register(
            AMItemRegistry.HEMOLYMPH_BLASTER.get(),
            AMCompat.rl("empty"),
            (stack, p_239428_1_, p_239428_2_, j) -> ItemHemolymphBlaster.isUsable(stack)
                  && (!(p_239428_2_ instanceof Player) || !AMCompat.isOnCooldown(((Player)p_239428_2_).getCooldowns(), AMItemRegistry.HEMOLYMPH_BLASTER.get()))
               ? 0.0F
               : 1.0F
         );
         ItemProperties.register(
         ...
         ItemProperties.register(
            AMItemRegistry.SHIELD_OF_THE_DEEP.get(),
            AMCompat.rl("blocking"),
            (stack, p_239421_1_, p_239421_2_, j) -> p_239421_2_ != null && p_239421_2_.isUsingItem() && p_239421_2_.getUseItem() == stack ? 1.0F : 0.0F
         );
         ItemProperties.register(
            AMItemRegistry.SOMBRERO.get(), AMCompat.rl("silly"), (stack, p_239421_1_, p_239421_2_, j) -> AlexsMobs.isAprilFools() ? 1.0F : 0.0F
         );
         ItemProperties.register(
            AMItemRegistry.TENDON_WHIP.get(), AMCompat.rl("active"), (stack, p_239421_1_, holder, j) -> ItemTendonWhip.isActive(stack, holder) ? 1.0F : 0.0F
         );
         ItemProperties.register(AMItemRegistry.PUPFISH_LOCATOR.get(), AMCompat.rl("in_chunk"), (stack, world, entity, j) -> {
            ...
            return entity != null && entity.getX() >= x && entity.getX() <= x + 16 && entity.getZ() >= z && entity.getZ() <= z + 16 ? 1.0F : 0.0F;
         });
         ItemProperties.register(
            AMItemRegistry.SKELEWAG_SWORD.get(),
            AMCompat.rl("blocking"),
            (stack, p_239421_1_, p_239421_2_, j) -> p_239421_2_ != null && p_239421_2_.isUsingItem() && p_239421_2_.getUseItem() == stack ? 1.0F : 0.0F
         );
