...
public class ParticleDna extends SimpleAnimatedParticle {
   private ParticleDna(ClientLevel world, double x, double y, double z, double motionX, double motionY, double motionZ, SpriteSet sprites) {
      ...
      this.quadSize = this.quadSize * (1.5F + this.random.nextFloat() * 0.6F);
      ...
      this.alpha = 0.0F;
      this.roll = (float)(1.5707963267948966 * this.random.nextFloat());
   }
   ...
   public void tick() {
      ...
      this.roll = this.roll + (float)Math.random() * (0.9424779F * this.alpha);
   }
