...
public abstract class LayerVoidWormGlow<T extends LivingEntity> extends RenderLayer<T, EntityModel<T>> {
      ...
      if (this.isGlowing(worm) || special) {
         if (special) {
            ...
            AMRenderTypes.renderMerged(
               bufferIn,
               AMRenderTypes.VOID_WORM_PORTAL_OVERLAY,
               RenderType.entityCutoutNoCull(texture),
               consumer -> AMRenderCompat.renderToBuffer(this.layerModel, matrixStackIn, consumer, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F)
            );
         } else {
            ...
            AMRenderCompat.renderToBuffer(
               this.getParentModel(),
               matrixStackIn,
               bufferIn.getBuffer(RenderType.eyes(texture)),
               240,
               LivingEntityRenderer.getOverlayCoords(worm, 1.0F),
               1.0F,
               1.0F,
               1.0F,
               f
   ...
   private boolean isSpecialRenderer(ResourceLocation resourceLocation) {
      ...
      } else if (this.resourceManager.getResource(resourceLocation).isPresent()) {
         Resource resource = (Resource)this.resourceManager.getResource(resourceLocation).get();
