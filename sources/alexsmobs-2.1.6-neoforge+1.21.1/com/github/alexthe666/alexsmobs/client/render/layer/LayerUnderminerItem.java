...
public class LayerUnderminerItem extends RenderLayer<EntityUnderminer, EntityModel<EntityUnderminer>> {
   ...
   public void render(
      ...
      if (!entitylivingbaseIn.isFullyHidden()) {
         ...
         this.translateToHand(entitylivingbaseIn.getMainArm(), matrixStackIn);
         matrixStackIn.translate(f, f1, -0.15F);
         matrixStackIn.mulPose(Axis.XP.rotationDegrees(-90.0F));
         matrixStackIn.mulPose(Axis.YP.rotationDegrees(180.0F));
         ItemInHandRenderer renderer = Minecraft.getInstance().getEntityRenderDispatcher().getItemInHandRenderer();
         int light = itemstack.is(AMItemRegistry.GHOSTLY_PICKAXE.get()) ? 15728880 : packedLightIn;
         AMRenderCompat.renderItemInHand(
            renderer, entitylivingbaseIn, itemstack, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, false, matrixStackIn, bufferIn, light
         );
   ...
   protected void translateToHand(HumanoidArm arm, PoseStack matrixStack) {
      if (this.getParentModel() instanceof ModelUnderminerDwarf) {
         ((ModelUnderminerDwarf)this.getParentModel()).translateToHand(arm, matrixStack);
      } else if (this.getParentModel() instanceof ModelUnderminerHumanoid) {
         ((ModelUnderminerHumanoid)this.getParentModel()).translateToHand(arm, matrixStack);
      } else if (this.getParentModel() instanceof ArmedModel) {
         ((ArmedModel)this.getParentModel()).translateToHand(arm, matrixStack);
      }
   }
}
