...
public class LayerTusklinGear extends RenderLayer<EntityTusklin, ModelTusklin> {
   ...
   public void render(
      ...
      if (entitylivingbaseIn.isSaddled()) {
         ...
         ((ModelTusklin)this.getParentModel())
            .renderToBuffer(
      ...
      if (!entitylivingbaseIn.getShoeStack().isEmpty()) {
         VertexConsumer ivertexbuilder = AMRenderCompat.armorFoilBuffer(
            bufferIn, RenderType.armorCutoutNoCull(TEXTURE_SHOES), entitylivingbaseIn.getShoeStack().hasFoil()
         );
         ((ModelTusklin)this.getParentModel())
            .renderToBuffer(
               matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F
