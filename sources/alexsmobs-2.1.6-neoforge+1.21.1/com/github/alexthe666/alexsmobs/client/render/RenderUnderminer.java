...
public class RenderUnderminer extends MobRenderer<EntityUnderminer, EntityModel<EntityUnderminer>> {
   ...
   public static final List<ResourceLocation> BREAKING_LOCATIONS = IntStream.range(0, 10)
      .mapToObj(destroyStage -> AMCompat.rl("alexsmobs:textures/block/ghostly_pickaxe/destroy_stage_" + destroyStage + ".png"))
      ...
      if (!AMPlatform.postCancelled(new Pre(entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn))) {
         ...
         if (rendertype != null && !entityIn.isFullyHidden()) {
            ...
            this.renderUnderminerModel(
               matrixStackIn, bufferIn, rendertype, partialTicks, packedLightIn, i, flag1 ? 0.15F : Mth.clamp(alpha, 0.0F, 1.0F), entityIn
            );
         } else {
         ...
         if (miningPos != null) {
            ...
            matrixStackIn.translate(miningPos.getX() - d0, miningPos.getY() - d1, miningPos.getZ() - d2);
            int progress = Math.round((DESTROY_TYPES.size() - 1) * Mth.clamp(entityIn.getMiningProgress(), 0.0F, 1.0F));
            this.renderBreaking(matrixStackIn, bufferIn, entityIn, miningPos, progress);
            matrixStackIn.popPose();
         }
      }
   }
   ...
   private void renderBreaking(PoseStack matrixStackIn, MultiBufferSource bufferIn, EntityUnderminer entityIn, BlockPos miningPos, int progress) {
      this.renderBreakingLegacy(matrixStackIn, bufferIn, entityIn, miningPos, progress);
   }
   ...
   private void renderBreakingLegacy(PoseStack matrixStackIn, MultiBufferSource bufferIn, EntityUnderminer entityIn, BlockPos miningPos, int progress) {
      ...
      ModelData modelData = entityIn.level().getModelDataManager().getAt(miningPos);
      Minecraft.getInstance()
         .getBlockRenderer()
         .renderBreakingTexture(
            entityIn.level().getBlockState(miningPos),
            miningPos,
            entityIn.level(),
            matrixStackIn,
            vertexconsumer1,
   ...
   private void renderUnderminerModel(
      ...
      AMRenderCompat.renderToBuffer(
         this.model,
         matrixStackIn,
         source.getBuffer(defRenderType),
         packedLightIn,
