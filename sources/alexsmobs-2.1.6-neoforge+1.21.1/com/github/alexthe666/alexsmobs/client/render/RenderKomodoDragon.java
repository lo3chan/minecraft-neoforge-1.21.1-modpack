...
public class RenderKomodoDragon extends MobRenderer<EntityKomodoDragon, ModelKomodoDragon> {
   ...
   public RenderKomodoDragon(Context renderManagerIn) {
      ...
      this.addLayer(new RenderKomodoDragon.LayerSaddle(this));
   }
   ...
   static class LayerSaddle extends RenderLayer<EntityKomodoDragon, ModelKomodoDragon> {
      ...
      public void render(
         ...
         if (entitylivingbaseIn.isMaid()) {
            VertexConsumer maid = bufferIn.getBuffer(AMRenderTypes.entityCutoutNoCull(RenderKomodoDragon.TEXTURE_MAID));
            ((ModelKomodoDragon)this.getParentModel()).copyPropertiesTo(MAID_MODEL);
            ...
            MAID_MODEL.renderToBuffer(
               matrixStackIn, maid, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F
            );
         }

         if (entitylivingbaseIn.isSaddled()) {
            VertexConsumer saddle = bufferIn.getBuffer(AMRenderTypes.entityCutoutNoCull(RenderKomodoDragon.TEXTURE_SADDLE));
            ((ModelKomodoDragon)this.getParentModel()).copyPropertiesTo(SADDLE_MODEL);
            ...
            SADDLE_MODEL.renderToBuffer(
               matrixStackIn, saddle, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F
            );
         }
      }
   }
