...
public class RenderTiger extends MobRenderer<EntityTiger, ModelTiger> {
   ...
   public void render(EntityTiger entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
      if (!AMPlatform.postCancelled(new Pre(entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn))) {
         ...
         ((ModelTiger)this.model).attackTime = this.getAttackAnim(entityIn, partialTicks);
         boolean shouldSit = entityIn.isPassenger() && entityIn.getVehicle() != null && AMCompat.shouldRiderSit(entityIn.getVehicle());
         ((ModelTiger)this.model).riding = shouldSit;
         ((ModelTiger)this.model).young = entityIn.isBaby();
         ...
         ((ModelTiger)this.model).prepareMobModel(entityIn, f5, f8, partialTicks);
         ((ModelTiger)this.model).setupAnim(entityIn, f5, f8, f7, f2, f6);
         ...
         if (rendertype != null) {
            ...
            this.shadowRadius = 0.6F * (1.0F - stealthLevel * 0.1F);
            VertexConsumer ivertexbuilder = bufferIn.getBuffer(rendertype);
            int i = getOverlayCoords(entityIn, this.getWhiteOverlayProgress(entityIn, partialTicks));
            ((ModelTiger)this.model)
               .renderToBuffer(
                  matrixStackIn, ivertexbuilder, packedLightIn, i, 1.0F, 1.0F, 1.0F, flag1 ? 0.15F : Mth.clamp(1.0F - stealthLevel * 0.1F, 0.0F, 1.0F)
               );
         }
   ...
   private <E extends Entity> void renderLeash(EntityTiger tiger, float p_115463_, PoseStack p_115464_, MultiBufferSource p_115465_, E p_115466_) {
      ...
      double d0 = Mth.lerp(p_115463_, tiger.yBodyRot, tiger.yBodyRotO) * 0.017453292F + 1.5707963267948966;
      Vec3 vec31 = tiger.getLeashOffset(p_115463_);
      double d1 = Math.cos(d0) * vec31.z + Math.sin(d0) * vec31.x;
      double d2 = Math.sin(d0) * vec31.z - Math.cos(d0) * vec31.x;
      double d3 = Mth.lerp(p_115463_, tiger.xo, tiger.getX()) + d1;
      double d4 = Mth.lerp(p_115463_, tiger.yo, tiger.getY()) + vec31.y;
      double d5 = Mth.lerp(p_115463_, tiger.zo, tiger.getZ()) + d2;
      ...
      float f4 = (float)(Mth.fastInvSqrt(f * f + f2 * f2) * 0.02500000037252903 / 2.0);
      float f5 = f2 * f4;
      float f6 = f * f4;
