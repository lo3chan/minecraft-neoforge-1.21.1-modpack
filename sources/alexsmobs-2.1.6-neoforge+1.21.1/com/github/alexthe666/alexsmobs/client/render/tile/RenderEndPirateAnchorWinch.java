...
public class RenderEndPirateAnchorWinch<T extends TileEntityEndPirateAnchorWinch> implements BlockEntityRenderer<T> {
   ...
   public void render(T tileEntityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
      ...
      boolean east = (Boolean)tileEntityIn.getBlockState().getValue(BlockEndPirateAnchorWinch.EASTORWEST);
      ...
      for (float i = 0.0F; i < tileEntityIn.getChainLengthForRender(); i += 0.5F) {
         ...
         CHAIN_MODEL.renderToBuffer(
            matrixStackIn, bufferIn.getBuffer(RenderType.entityCutout(TEXTURE_CHAIN)), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F
         );
         CHAIN_MODEL.renderToBuffer(
            matrixStackIn, bufferIn.getBuffer(RenderType.eyes(TEXTURE_CHAIN)), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F
         );
      ...
      WINCH_MODEL.renderToBuffer(
         matrixStackIn, bufferIn.getBuffer(RenderType.entityCutout(TEXTURE)), combinedLightIn, combinedOverlayIn, 1.0F, 1.0F, 1.0F, 1.0F
      );
      ...
      if (tileEntityIn.hasAnchor()) {
         ...
         RenderEndPirateAnchor.ANCHOR_MODEL
            .renderToBuffer(
               matrixStackIn,
               bufferIn.getBuffer(RenderType.entityCutout(RenderEndPirateAnchor.TEXTURE_ANCHOR)),
               combinedLightIn,
               combinedOverlayIn,
               1.0F,
               1.0F,
         ...
         RenderEndPirateAnchor.ANCHOR_MODEL
            .renderToBuffer(
               matrixStackIn,
               bufferIn.getBuffer(RenderType.eyes(RenderEndPirateAnchor.TEXTURE_ANCHOR_GLOW)),
               combinedLightIn,
               combinedOverlayIn,
               1.0F,
               1.0F,
