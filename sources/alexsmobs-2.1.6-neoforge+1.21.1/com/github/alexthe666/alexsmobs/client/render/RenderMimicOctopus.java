...
public class RenderMimicOctopus extends MobRenderer<EntityMimicOctopus, ModelMimicOctopus> {
   ...
   public RenderMimicOctopus(Context renderManagerIn) {
      ...
      this.addLayer(new RenderMimicOctopus.OverlayLayer(this));
   }
   ...
   public void render(EntityMimicOctopus entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
      ...
      if (livingentity != null) {
         ...
         float f19 = Mth.cos(f7 + 3.1415927F) * 0.2F;
         float f20 = Mth.sin(f7 + 3.1415927F) * 0.2F;
   ...
   static class OverlayLayer extends RenderLayer<EntityMimicOctopus, ModelMimicOctopus> {
      ...
      public void render(
         ...
         if (entitylivingbaseIn.getPrevMimicState() != null) {
            ...
            VertexConsumer prev = buffer.getBuffer(AMRenderTypes.entityTranslucent(this.getFor(entitylivingbaseIn.getPrevMimicState())));
            if (entitylivingbaseIn.getPrevMimicState() == entitylivingbaseIn.getMimicState()) {
               alphaPrev *= a;
            }

            ((ModelMimicOctopus)this.getParentModel())
               .renderToBuffer(matrixStackIn, prev, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), r, g, b, alphaPrev);
         }

         float alphaCurrent = transProgress * 0.2F;
         VertexConsumer current = buffer.getBuffer(AMRenderTypes.entityTranslucent(this.getFor(entitylivingbaseIn.getMimicState())));
         ((ModelMimicOctopus)this.getParentModel())
            .renderToBuffer(matrixStackIn, current, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), r, g, b, a * alphaCurrent);
         VertexConsumer eyes = buffer.getBuffer(AMRenderTypes.entityTranslucent(RenderMimicOctopus.TEXTURE_EYES));
         ((ModelMimicOctopus)this.getParentModel())
            .renderToBuffer(matrixStackIn, eyes, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 1.0F, 1.0F, 1.0F, 1.0F);
      }
