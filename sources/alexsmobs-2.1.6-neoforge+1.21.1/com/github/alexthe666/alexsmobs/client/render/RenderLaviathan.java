...
public class RenderLaviathan extends MobRenderer<EntityLaviathan, ModelLaviathan> {
   ...
   public RenderLaviathan(Context renderManagerIn) {
      ...
      this.addLayer(new RenderLaviathan.LayerOverlays(this));
   }
   ...
   private <E extends Entity> void renderRein(EntityLaviathan mob, float partialTick, PoseStack p_115464_, MultiBufferSource p_115465_, E rider, boolean left) {
      ...
      if (head != null) {
         ...
         Vec3 vec3 = rider instanceof LivingEntity
            ? this.getReinPosition((LivingEntity)rider, partialTick, left, shake)
            : rider.getRopeHoldPosition(partialTick);
         double d0 = Mth.lerp(partialTick, mob.yBodyRot, mob.yBodyRotO) * 0.017453292F + 1.5707963267948966;
         Vec3 vec31 = new Vec3(
            (left ? -0.05F - yawAdd : 0.05F + yawAdd) + shake, 0.45F - headYaw * 0.2F + this.getHeadBobForReins(mob, partialTick), 0.10000000149011612
         );
   ...
   private Vec3 getReinPosition(LivingEntity entity, float p_36374_, boolean left, float shake) {
      ...
      float f = Mth.lerp(p_36374_ * 0.5F, entity.getXRot(), entity.xRotO) * 0.017453292F;
      float f1 = Mth.lerp(p_36374_, entity.yBodyRotO, entity.yBodyRot) * 0.017453292F;
   ...
   static class LayerOverlays extends RenderLayer<EntityLaviathan, ModelLaviathan> {
      ...
      public void render(
         ...
         if (!laviathan.isObsidian()) {
            ...
            ((ModelLaviathan)this.getParentModel())
               .renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
         }
