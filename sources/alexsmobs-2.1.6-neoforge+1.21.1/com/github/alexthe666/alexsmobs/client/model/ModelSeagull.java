...
public class ModelSeagull extends AdvancedEntityModel<EntitySeagull> {
   ...
   public void setupAnim(EntitySeagull entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      this.head.rotateAngleY = (float)(this.head.rotateAngleY + Math.toRadians(entity.getFlightLookYaw()) * flyProgress * 0.20000000298023224);
   ...
   public void renderToBuffer(
      PoseStack matrixStackIn, VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha
   ) {
      if (this.young) {
         float f = 1.45F;
