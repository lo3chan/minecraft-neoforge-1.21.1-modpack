...
public class ModelGiantSquid extends AdvancedEntityModel<EntityGiantSquid> {
   ...
   public void setupAnim(EntityGiantSquid entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      ...
      float f = (pitch - 90.0F) * 0.017453292F;
      ...
      if (look != null) {
         ...
         double eyeXz = Mth.sqrt((float)Math.abs(d1)) * -2.0F * (float)Math.signum(d1);
         this.left_pupil.setScale(eyeScale, eyeScale, eyeScale);
         this.left_pupil.rotationPointZ = this.left_pupil.rotationPointZ - (float)Mth.clamp(-eyeXz, -maxEyeDist / eyeScale, maxEyeDist / eyeScale);
         this.left_pupil.rotationPointY = this.left_pupil.rotationPointY + (float)Mth.clamp(-d0, -maxEyeDist / eyeScale, maxEyeDist / eyeScale);
         this.right_pupil.setScale(eyeScale, eyeScale, eyeScale);
         this.right_pupil.rotationPointZ = this.right_pupil.rotationPointZ + (float)Mth.clamp(eyeXz, -maxEyeDist / eyeScale, maxEyeDist / eyeScale);
         this.right_pupil.rotationPointY = this.right_pupil.rotationPointY + (float)Mth.clamp(-d0, -maxEyeDist / eyeScale, maxEyeDist / eyeScale);
      }
   }
   ...
   private float getArmRot(EntityGiantSquid entity, int offset, float partialTick, boolean pitch) {
      ...
      return Mth.clamp(rotWrap, -50.0F, 50.0F) * 0.4F * 0.017453292F;
   }
