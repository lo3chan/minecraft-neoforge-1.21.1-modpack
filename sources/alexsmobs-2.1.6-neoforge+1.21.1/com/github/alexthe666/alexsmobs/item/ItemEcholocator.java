   ...
   private List<BlockPos> getNearbyPortals(BlockPos blockpos, ServerLevel world, int range) {
      if (this.type == ItemEcholocator.EchoType.ENDER) {
         ...
         Stream<BlockPos> stream = pointofinterestmanager.findAll(
            poiTypeHolder -> poiTypeHolder.is(AMPointOfInterestRegistry.END_PORTAL_FRAME_KEY), Predicates.alwaysTrue(), blockpos, range, Occupancy.ANY
         );
      ...
      if (!worldIn.isClientSide() && worldIn instanceof ServerLevel) {
         ...
         if (this.type == ItemEcholocator.EchoType.ENDER) {
            ...
            for (BlockPos portalPosx : portals) {
               if (pos == null || pos.distSqr(playerPos) > portalPosx.distSqr(playerPos)) {
                  pos = portalPosx;
               }
            }
         } else {
            ...
            if (AMCompat.contains(nbt, "CavePos") && AMCompat.getBoolean(nbt, "ValidCavePos")) {
               ...
               if (this.isCaveAir(worldIn, pos) || 1000000.0 < pos.distSqr(playerPos)) {
                  nbt.putBoolean("ValidCavePos", false);
                  AMCompat.setTag(stack, nbt);
               }
            } else {
               for (BlockPos portalPosxx : portals) {
                  if (pos == null || pos.distSqr(playerPos) < portalPosxx.distSqr(playerPos)) {
                     pos = portalPosxx;
                  }
               }
         ...
         if (pos != null) {
            ...
            worldIn.playSound(
               (Player)null, whaleEcho.getX(), whaleEcho.getY(), whaleEcho.getZ(), AMSoundRegistry.CACHALOT_WHALE_CLICK.get(), SoundSource.PLAYERS, 1.0F, 1.0F
            );
            AMCompat.hurtAndBreak(stack, 1, livingEntityIn, livingEntityIn.getUsedItemHand());
         }
      }

      AMCompat.addCooldown(livingEntityIn.getCooldowns(), this, 5);
