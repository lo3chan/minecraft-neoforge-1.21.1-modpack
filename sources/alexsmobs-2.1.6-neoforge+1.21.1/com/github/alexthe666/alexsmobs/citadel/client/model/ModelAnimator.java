      ...
      if (this.correctAnimation) {
         ...
         if (animationTick >= this.prevTempTick && animationTick < this.tempTick) {
            if (stationary) {
               ...
               float frameTime = Minecraft.getInstance().isPaused() ? 0.0F : Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true);
               float tick = (animationTick - this.prevTempTick + frameTime) / (this.tempTick - this.prevTempTick);
               float inc = Mth.sin((float)(tick * 3.141592653589793 / 2.0));
               float dec = 1.0F - inc;
