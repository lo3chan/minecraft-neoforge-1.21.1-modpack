...
public interface ICustomCollisions {
   double MAX_SWEEP_PER_TICK = 16.0;
   ...
   static Vec3 getAllowedMovementForEntity(Entity entity, Vec3 vecInRaw) {
      ...
      if (entity.maxUpStep() > 0.0F && flag3 && (flag || flag2)) {
         ...
         Vec3 vec32 = collideBoundingBox2(entity, new Vec3(0.0, entity.maxUpStep(), 0.0), aabb.expandTowards(vecIN.x, 0.0, vecIN.z), entity.level(), list);
   ...
   private static Vec3 clampSweep(Vec3 vec) {
      return Math.abs(vec.x) <= 16.0 && Math.abs(vec.y) <= 16.0 && Math.abs(vec.z) <= 16.0 && !Double.isNaN(vec.x + vec.y + vec.z)
         ? vec
         : new Vec3(clampAxis(vec.x), clampAxis(vec.y), clampAxis(vec.z));
   }

   private static double clampAxis(double d) {
      return Double.isNaN(d) ? 0.0 : Math.max(-16.0, Math.min(16.0, d));
   }
