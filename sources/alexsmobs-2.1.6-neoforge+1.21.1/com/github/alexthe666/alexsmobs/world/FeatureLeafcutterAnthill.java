...
public class FeatureLeafcutterAnthill extends Feature<NoneFeatureConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
      if (context.level().getRandom().nextFloat() > 0.0175F) {
         ...
         int y = context.level().getHeight(Types.WORLD_SURFACE_WG, pos.getX() + x, pos.getZ() + z);
         ...
         if (!context.level().getFluidState(heightPos.below()).isEmpty()) {
            ...
            for (int i = 0; i < outOfGround; i++) {
               ...
               for (BlockPos lvt_13_1_ : BlockPos.betweenClosed(heightPos.offset(-lvt_8_1_, 0, -lvt_10_1_), heightPos.offset(lvt_8_1_, 3, lvt_10_1_))) {
                  if (lvt_13_1_.distSqr(heightPos) <= radius * radius) {
            ...
            if (context.level().getBlockState(heightPos.above(outOfGround + 1)).getBlock() != AMBlockRegistry.LEAFCUTTER_ANTHILL.get()
               && context.level().getBlockState(heightPos.above(outOfGround - 1)).getBlock() != AMBlockRegistry.LEAFCUTTER_ANTHILL.get()) {
               context.level().setBlock(heightPos.above(outOfGround), AMBlockRegistry.LEAFCUTTER_ANTHILL.get().defaultBlockState(), 4);
               if (context.level().getBlockEntity(heightPos.above(outOfGround)) instanceof TileEntityLeafcutterAnthill beehivetileentity) {
            ...
            for (BlockPos lvt_13_1_x : BlockPos.betweenClosed(
               heightPos.offset(-lvt_8_1_, -lvt_9_1_, -lvt_10_1_), heightPos.offset(lvt_8_1_, lvt_9_1_, lvt_10_1_)
            )) {
               if (lvt_13_1_x.distSqr(heightPos) < radius * radius) {
                  BlockState block = AMBlockRegistry.LEAFCUTTER_ANT_CHAMBER.get().defaultBlockState();
                  context.level().setBlock(lvt_13_1_x, block, 4);
               }
            }
