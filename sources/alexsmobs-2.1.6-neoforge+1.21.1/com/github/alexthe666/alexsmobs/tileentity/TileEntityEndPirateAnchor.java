...
public class TileEntityEndPirateAnchor extends BlockEntity {
   ...
   public AABB getRenderBoundingBox() {
      return AMPlatform.encapsulating(this.worldPosition.offset(-1, 0, -1), this.worldPosition.offset(1, 3, 1));
   }
   ...
   public boolean hasAllAnchorBlocks() {
      for (BlockPos pos : getValidBBPositions((Boolean)this.getBlockState().getValue(BlockEndPirateAnchor.EASTORWEST))) {
         if (!(this.getLevel().getBlockState(this.getBlockPos().offset(pos)).getBlock() instanceof BlockEndPirateAnchor)) {
            return false;
         }
      }
