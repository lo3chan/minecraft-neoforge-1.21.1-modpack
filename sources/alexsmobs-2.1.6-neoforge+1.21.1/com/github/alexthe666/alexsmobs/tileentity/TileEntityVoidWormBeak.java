...
public class TileEntityVoidWormBeak extends BlockEntity {
   ...
   public void tick() {
      ...
      if (this.getBlockState().getBlock() instanceof BlockVoidWormBeak) {
         powered = (Boolean)this.getBlockState().getValue(BlockVoidWormBeak.POWERED);
      }
      ...
      if (this.chompProgress >= 5.0F && !this.level.isClientSide() && this.ticksExisted % 5 == 0) {
         ...
         for (LivingEntity entity : this.level
            .getEntitiesOfClass(LivingEntity.class, new AABB((double)i - d0, (double)j - d0, (double)k - d0, (double)i + d0, (double)j + d0, (double)k + d0))) {
