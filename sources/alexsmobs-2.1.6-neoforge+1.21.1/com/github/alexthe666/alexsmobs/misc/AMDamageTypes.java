...
public class AMDamageTypes {
   ...
   public static DamageSource causeFarseerDamage(LivingEntity attacker) {
      return new AMDamageTypes.DamageSourceRandomMessages(
         ((Registry)attacker.level().registryAccess().registry(Registries.DAMAGE_TYPE).get()).getHolderOrThrow(FARSEER), attacker
      );
   }

   public static DamageSource causeFreddyBearDamage(LivingEntity attacker) {
      return new DamageSource(((Registry)attacker.level().registryAccess().registry(Registries.DAMAGE_TYPE).get()).getHolderOrThrow(FREDDY), attacker);
   }
   ...
   private static class DamageSourceRandomMessages extends DamageSource {
      ...
      public Component getLocalizedDeathMessage(LivingEntity attacked) {
         ...
         return livingentity != null
            ? Component.translatable(s1, new Object[]{attacked.getDisplayName(), livingentity.getDisplayName()})
            : Component.translatable(s, new Object[]{attacked.getDisplayName()});
      }
   }
}
