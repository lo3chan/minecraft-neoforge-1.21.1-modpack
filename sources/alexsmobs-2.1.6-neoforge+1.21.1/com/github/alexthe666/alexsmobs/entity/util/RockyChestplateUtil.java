...
public class RockyChestplateUtil {
   ...
   public static void rollFor(LivingEntity roller, int ticks) {
      ...
      if (ticks == 30) {
         lassoedTag.putInt("RockyRollTimestampAlexsMobs", roller.tickCount);
      }
   ...
   public static int getRollingTicksLeft(LivingEntity entity) {
      ...
      return lassoedTag.contains("RockyRollTicksAlexsMobs") ? AMCompat.getInt(lassoedTag, "RockyRollTicksAlexsMobs") : 0;
   }

   public static int getRollingTimestamp(LivingEntity entity) {
      CompoundTag lassoedTag = CitadelEntityData.getOrCreateCitadelTag(entity);
      return lassoedTag.contains("RockyRollTimestampAlexsMobs") ? AMCompat.getInt(lassoedTag, "RockyRollTimestampAlexsMobs") : 0;
   }
   ...
   public static void tickRockyRolling(LivingEntity roller) {
      if (roller.isInWaterOrBubble()) {
         roller.setDeltaMovement(roller.getDeltaMovement().add(0.0, -0.014999999664723873, 0.0));
      }
      ...
      if (rollCounter == 0) {
         if (roller.isSprinting()
            && !roller.isShiftKeyDown()
            && (!(roller instanceof Player) || !((Player)roller).getAbilities().flying)
            && canRollAgain(roller)
            && !roller.isPassenger()) {
            update = true;
            rollFor(roller, 30);
         ...
         float f = roller.getYRot() * 0.017453292F;
         float f1 = roller.isInWaterOrBubble() ? 0.05F : 0.15F;
         Vec3 rollDelta = new Vec3(vec3.x + -Mth.sin(f) * f1, 0.0, vec3.z + Mth.cos(f) * f1);
         double rollY = !roller.isInWaterOrBubble() && !roller.isShiftKeyDown() ? (rollCounter >= 30 ? 0.27 : vec3.y) : -0.10000000149011612;
