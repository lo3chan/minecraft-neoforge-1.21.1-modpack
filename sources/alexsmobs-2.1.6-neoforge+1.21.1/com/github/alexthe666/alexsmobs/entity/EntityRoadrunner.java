...
public class EntityRoadrunner extends Animal {
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(4, new TemptGoal(this, 1.1, AMCompat.ingredientOf(AMTagRegistry.ROADRUNNER_BREEDABLES), false));
      this.goalSelector.addGoal(5, new AnimalAIWanderRanged(this, 50, 1.0, 25, 7));
      ...
      this.targetSelector.addGoal(2, new HurtByTargetGoal(this, new Class[]{EntityRattlesnake.class, Player.class}).setAlertOthers(new Class[0]));
   }
   ...
   protected SoundEvent getAmbientSound() {
      return !this.isMeep() && this.random.nextInt(2000) != 0 ? AMSoundRegistry.ROADRUNNER_IDLE.get() : AMSoundRegistry.ROADRUNNER_MEEP.get();
   }
   ...
   public void aiStep() {
      ...
      if (!this.level().isClientSide() && this.isAlive() && !this.isBaby() && --this.timeUntilNextFeather <= 0) {
         AMCompat.spawnAtLocation(this, (ItemLike)AMItemRegistry.ROADRUNNER_FEATHER.get());
   ...
   public void tick() {
      ...
      if (this.isMeep()) {
         ...
         this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0.44999998807907104);
         ...
         for (int i = 0; i < 2; i++) {
            this.level()
               .addParticle(
                  ...
                  this.getX() - vector3d.x * f2 + f,
                  this.getY() + this.random.nextFloat() * 0.2F,
                  this.getZ() - vector3d.z * f2 + f1,
                  0.0,
                  0.0,
                  0.0
               );
