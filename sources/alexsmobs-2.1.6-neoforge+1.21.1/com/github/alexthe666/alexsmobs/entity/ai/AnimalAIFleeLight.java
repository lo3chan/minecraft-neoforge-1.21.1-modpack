...
public class AnimalAIFleeLight extends Goal {
   ...
   public boolean canUse() {
      if (this.creature.getTarget() != null || this.creature.getRandom().nextInt(this.executeChance) != 0) {
         return false;
      } else {
         return this.world.getMaxLocalRawBrightness(this.creature.blockPosition()) < this.lightLevel ? false : this.isPossibleShelter();
      }
   }
   ...
   protected Vec3 findPossibleShelter() {
      ...
      for (int lvt_3_1_ = 0; lvt_3_1_ < 10; lvt_3_1_++) {
         ...
         if (this.creature.level().getMaxLocalRawBrightness(lvt_4_1_) < this.lightLevel) {
