...
public class SmartClimbPathNavigator extends GroundPathNavigation {
   ...
   public void tick() {
      ...
      if (!this.isDone()) {
         ...
         Vec3 xzOff = new Vec3(this.pathToPosition.getX() + 0.5F - this.mob.getX(), 0.0, this.pathToPosition.getZ() + 0.5F - this.mob.getZ());
         double dist = xzOff.length();
         if (!(dist < this.mob.getBbWidth()) && !(this.mob.getY() > this.pathToPosition.getY())) {
            this.mob.getMoveControl().setWantedPosition(this.pathToPosition.getX(), this.mob.getY(), this.pathToPosition.getZ(), this.speedModifier);
         } else {
   ...
   protected void doStuckDetection(Vec3 vec) {
      if (this.tick - this.lastStuckCheck > 40) {
         if (vec.distanceToSqr(new Vec3(this.lastStuckCheckPos.x, vec.y, this.lastStuckCheckPos.z)) < 2.25) {
            this.stop();
         }
