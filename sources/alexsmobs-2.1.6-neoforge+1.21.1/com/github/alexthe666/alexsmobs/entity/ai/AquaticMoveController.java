   ...
   public void tick() {
      ...
      if (this.entity instanceof ISemiAquatic && ((ISemiAquatic)this.entity).shouldStopMoving()) {
         ...
         if (this.operation == Operation.MOVE_TO && !this.entity.getNavigation().isDone()) {
            ...
            float f1 = (float)(this.speedModifier * this.entity.getAttributeValue(Attributes.MOVEMENT_SPEED) * this.speedMulti);
            this.entity.setSpeed(f1 * 0.4F);
            this.entity.setDeltaMovement(this.entity.getDeltaMovement().add(0.0, this.entity.getSpeed() * d1 * 0.6, 0.0));
         } else {
            this.entity.setSpeed(0.0F);
         }
      }
