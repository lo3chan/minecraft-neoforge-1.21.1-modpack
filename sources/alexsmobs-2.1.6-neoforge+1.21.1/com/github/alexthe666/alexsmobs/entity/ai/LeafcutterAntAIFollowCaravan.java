...
public class LeafcutterAntAIFollowCaravan extends Goal {
   ...
   public boolean canUse() {
      long worldTime = this.ant.level().getGameTime() % 10L;
      if (this.ant.getNoActionTime() >= 100 && worldTime != 0L) {
         return false;
      } else if (this.ant.getRandom().nextInt(this.executionChance) != 0 && worldTime != 0L) {
         return false;
      } else if (!this.ant.shouldLeadCaravan() && !this.ant.isBaby() && !this.ant.isQueen() && !this.ant.inCaravan() && !this.ant.hasLeaf()) {
         double dist = 15.0;
         List<EntityLeafcutterAnt> list = this.ant
            .level()
            .getEntitiesOfClass(EntityLeafcutterAnt.class, this.ant.getBoundingBox().inflate(dist, dist / 2.0, dist));
   ...
   public void tick() {
      if (this.ant.inCaravan() && !this.ant.shouldLeadCaravan()) {
         ...
         if (llamaentity != null) {
            ...
            Vec3 vector3d = new Vec3(llamaentity.getX() - this.ant.getX(), llamaentity.getY() - this.ant.getY(), llamaentity.getZ() - this.ant.getZ())
               .normalize()
