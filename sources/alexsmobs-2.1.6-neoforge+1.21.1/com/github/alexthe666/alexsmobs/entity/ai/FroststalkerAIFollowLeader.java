   ...
   public boolean canUse() {
      if (this.mob.hasFollowers()) {
         ...
         List<Player> playerList = this.mob
            .level()
            .getEntitiesOfClass(Player.class, this.mob.getBoundingBox().inflate(range, range, range), EntityFroststalker.VALID_LEADER_PLAYERS);
         Player closestPlayer = null;

         for (Player player : playerList) {
            if (closestPlayer == null || player.distanceTo(this.mob) < closestPlayer.distanceTo(this.mob)) {
         ...
         if (closestPlayer == null) {
            List<EntityFroststalker> list = this.mob
               .level()
               .getEntitiesOfClass(EntityFroststalker.class, this.mob.getBoundingBox().inflate(range, range, range), froststalkerPredicate);
