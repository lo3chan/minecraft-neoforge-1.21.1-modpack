...
public class EntityOrca extends TamableAnimal implements IAnimatedEntity {
   ...
   protected EntityOrca(EntityType type, Level worldIn) {
      ...
      this.moveControl = new EntityOrca.MoveHelperController(this);
   ...
   public static Builder bakeAttributes() {
      return Monster.createMonsterAttributes()
         ...
         .add(Attributes.MOVEMENT_SPEED, 1.350000023841858);
   }
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(2, new EntityOrca.SwimWithPlayerGoal(this, 4.0));
      ...
      this.goalSelector.addGoal(6, new OrcaAIMelee(this, 1.2000000476837158, true));
               ...
               for (int b = (int)Math.round(this.getBoundingBox().minY) - 1; b <= (int)Math.round(this.getBoundingBox().maxY) + 1 && b <= 127; b++) {
                  for (int c = (int)Math.round(this.getBoundingBox().minZ); c <= (int)Math.round(this.getBoundingBox().maxZ); c++) {
                     ...
                     if (!state.isAir()
                        ...
                        && state.is(AMTagRegistry.ORCA_BREAKABLES)
                        && fluidState.isEmpty()
                        && block != Blocks.AIR) {
   ...
   public void tick() {
      ...
      if (this.isNoAi()) {
         ...
         if (this.level().isClientSide() && this.isInWater() && this.getDeltaMovement().lengthSqr() > 0.03) {
            ...
            for (int i = 0; i < 2; i++) {
               this.level()
                  .addParticle(
                     ParticleTypes.DOLPHIN, this.getX() - vector3d.x * f2 + f, this.getY() - vector3d.y, this.getZ() - vector3d.z * f2 + f1, 0.0, 0.0, 0.0
                  );
               this.level()
                  .addParticle(
                     ParticleTypes.DOLPHIN, this.getX() - vector3d.x * f2 - f, this.getY() - vector3d.y, this.getZ() - vector3d.z * f2 - f1, 0.0, 0.0, 0.0
      ...
      if (attackTarget != null && this.distanceTo(attackTarget) < attackTarget.getBbWidth() + this.getBbWidth() + 2.0F) {
         ...
         if (this.getAnimation() == ANIMATION_TAILSWING && this.getAnimationTick() == 6) {
            float damagex = (int)this.getAttributeValue(Attributes.ATTACK_DAMAGE);
            if (attackTarget instanceof Drowned || attackTarget instanceof Guardian) {
               damagex *= 2.0F;
            }

            boolean flag = AMCompat.hurt(attackTarget, this.damageSources().mobAttack(this), damagex);
            if (flag) {
               AMCompat.enchantDamageEffects(this, attackTarget);
               this.playSound(SoundEvents.DOLPHIN_ATTACK, 1.0F, 1.0F);
            }
            ...
            float yRotRad = this.getYRot() * 0.017453292F;
            AMCompat.knockback(attackTarget, 1.0, Mth.sin(yRotRad), -Mth.cos(yRotRad));
            float knockbackResist = (float)Mth.clamp(1.0 - this.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE), 0.0, 1.0);
            attackTarget.setDeltaMovement(attackTarget.getDeltaMovement().add(0.0, knockbackResist * 0.4F, 0.0));
         }
      }
   ...
   public void onJumpHit(LivingEntity entityIn) {
      boolean flag = AMCompat.hurt(entityIn, this.damageSources().mobAttack(this), (int)this.getAttributeValue(Attributes.ATTACK_DAMAGE));
      if (flag) {
         ...
         if (this.operation == Operation.MOVE_TO && !this.dolphin.getNavigation().isDone()) {
            ...
            if (d3 < 2.500000277905201E-7) {
               ...
               if (this.dolphin.isInWater()) {
                  ...
                  float xRotRad = this.dolphin.getXRot() * 0.017453292F;
   ...
   static class SwimWithPlayerGoal extends Goal {
      ...
      public boolean canUse() {
         ...
         return this.targetPlayer == null ? false : this.targetPlayer.isSwimming() && this.dolphin.getTarget() != this.targetPlayer;
      }

      public boolean canContinueToUse() {
         return this.targetPlayer != null
            && this.dolphin.getTarget() != this.targetPlayer
            && this.targetPlayer.isSwimming()
            && this.dolphin.distanceToSqr(this.targetPlayer) < 256.0;
      }
      ...
      public void tick() {
         this.dolphin.getLookControl().setLookAt(this.targetPlayer, this.dolphin.getMaxHeadYRot() + 20, this.dolphin.getMaxHeadXRot());
         if (this.dolphin.distanceToSqr(this.targetPlayer) < 10.0) {
            this.dolphin.getNavigation().stop();
         } else {
