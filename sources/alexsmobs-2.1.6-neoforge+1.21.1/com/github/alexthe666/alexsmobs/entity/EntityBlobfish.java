...
public class EntityBlobfish extends WaterAnimal implements FlyingAnimal, Bucketable {
   ...
   protected void handleAirSupply(int p_209207_1_) {
      if (this.isAlive() && !this.isInWaterOrBubble() && !this.isSlimed()) {
         this.setAirSupply(p_209207_1_ - 1);
         if (this.getAirSupply() == -20) {
            this.setAirSupply(0);
            this.hurt(this.damageSources().drown(), this.random.nextInt(2) == 0 ? 1.0F : 0.0F);
         }
      } else {
   ...
   public boolean requiresCustomPersistence() {
      return super.requiresCustomPersistence() || this.hasCustomName() || this.fromBucket() || this.isSlimed();
   }
   ...
   public EntityDimensions getDefaultDimensions(Pose poseIn) {
      EntityDimensions dimensions = super.getDefaultDimensions(poseIn).scale(this.getBlobfishScale());
   ...
   public float getBlobfishScale() {
      return (Float)this.entityData.get(BLOBFISH_SCALE);
   }
   ...
   public boolean isSlimed() {
      return (Boolean)this.entityData.get(SLIMED);
   }
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(3, new AnimalAISwimBottom(this, 1.0, 7));
   }
   ...
   public SpawnGroupData finalizeSpawn(ServerLevelAccessor worldIn, DifficultyInstance difficultyIn, MobSpawnType reason, @Nullable SpawnGroupData spawnDataIn) {
      this.setBlobfishScale(0.75F + this.random.nextFloat() * 0.5F);
   ...
   public void tick() {
      ...
      float f2 = -((float)this.getDeltaMovement().y * 2.2F * 57.295776F);
      this.setXRot(f2);
      if (!this.isInWater()) {
         if (this.onGround()) {
            if (!this.wasOnGround) {
   ...
   public static boolean canBlobfishSpawn(
      ...
      return reason == MobSpawnType.SPAWNER
         || pos.getY() <= AMConfig.blobfishSpawnHeight
            && iServerWorld.getFluidState(pos).is(FluidTags.WATER)
            && iServerWorld.getFluidState(pos.above()).is(FluidTags.WATER);
   }
