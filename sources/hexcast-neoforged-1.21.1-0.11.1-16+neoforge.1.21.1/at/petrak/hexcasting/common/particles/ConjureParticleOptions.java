...
public record ConjureParticleOptions(int color) implements ParticleOptions {
   ...
   public static class Type extends ParticleType<ConjureParticleOptions> {
      public static final MapCodec<ConjureParticleOptions> CODEC = RecordCodecBuilder.mapCodec(
         instance -> instance.group(Codec.INT.fieldOf("color").forGetter(o -> o.color)).apply(instance, ConjureParticleOptions::new)
      );
      public static final StreamCodec<RegistryFriendlyByteBuf, ConjureParticleOptions> STREAM_CODEC = ByteBufCodecs.INT
         .map(ConjureParticleOptions::new, ConjureParticleOptions::color);
