...
public class EntityWallScroll extends HangingEntity {
   private static final EntityDataAccessor<Boolean> SHOWS_STROKE_ORDER = SynchedEntityData.defineId(EntityWallScroll.class, EntityDataSerializers.BOOLEAN);
   ...
   public void recalculateDisplay() {
      ...
      if (patternTag != null) {
         ...
         if (this.level().isClientSide) {
            Pair<Float, List<Vec2>> pair = RenderLib.getCenteredPattern(
               this.pattern, 42.666668F * this.blockSize, 42.666668F * this.blockSize, 5.3333335F * this.blockSize
            );
            List<Vec2> dots = (List<Vec2>)pair.getSecond();
            float readOffset = this.getShowsStrokeOrder() ? 0.2F : 0.0F;
            float lastProp = this.getShowsStrokeOrder() ? 0.8F : 1.0F;
            List<Vec2> zappyPoints = RenderLib.makeZappy(
               dots, RenderLib.findDupIndices(this.pattern.positions()), 10, 0.4F, 0.0F, 0.0F, readOffset, lastProp, this.getId()
            );
   ...
   protected AABB calculateBoundingBox(BlockPos pos, Direction direction) {
      ...
      center = center.relative(sideways, this.offsetForSize(this.getWidth())).relative(Direction.UP, this.offsetForSize(this.getHeight()));
      Axis axis = direction.getAxis();
      double xSize = axis == Axis.X ? 0.0625 : width;
      double zSize = axis == Axis.Z ? 0.0625 : width;
   ...
   public InteractionResult interactAt(Player pPlayer, Vec3 pVec, InteractionHand pHand) {
      ...
      if (handStack.is(HexItems.AMETHYST_DUST) && !this.getShowsStrokeOrder()) {
         ...
         pPlayer.level().playSound(pPlayer, this, HexSounds.SCROLL_DUST, SoundSource.PLAYERS, 1.0F, 1.0F);
   ...
   public Packet<ClientGamePacketListener> getAddEntityPacket(ServerEntity serverEntity) {
      return IXplatAbstractions.INSTANCE
         .toVanillaClientboundPacket(
            new MsgNewWallScrollS2C(
               new ClientboundAddEntityPacket(this, serverEntity), this.pos, this.direction, this.scroll, this.getShowsStrokeOrder(), this.blockSize
            )
         );
   }
