package at.petrak.hexcasting.common.casting.actions.math.trig

import at.petrak.hexcasting.api.casting.OperatorUtils
import at.petrak.hexcasting.api.casting.castables.ConstMediaAction
import at.petrak.hexcasting.api.casting.eval.CastingEnvironment
import at.petrak.hexcasting.api.casting.eval.OperationResult
import at.petrak.hexcasting.api.casting.eval.vm.CastingImage
import at.petrak.hexcasting.api.casting.eval.vm.SpellContinuation
import at.petrak.hexcasting.api.casting.iota.DoubleIota
import at.petrak.hexcasting.api.casting.iota.Iota
import at.petrak.hexcasting.api.casting.mishaps.MishapDivideByZero
import kotlin.jvm.internal.SourceDebugExtension

@SourceDebugExtension(["SMAP\nOpTan.kt\nKotlin\n*S Kotlin\n*F\n+ 1 OpTan.kt\nat/petrak/hexcasting/common/casting/actions/math/trig/OpTan\n+ 2 ActionUtils.kt\nat/petrak/hexcasting/api/casting/OperatorUtils\n*L\n1#1,24:1\n300#2:25\n*S KotlinDebug\n*F\n+ 1 OpTan.kt\nat/petrak/hexcasting/common/casting/actions/math/trig/OpTan\n*L\n21#1:25\n*E\n"])
public object OpTan : ConstMediaAction {
   public open val argc: Int
      public open get() {
         return 1;
      }


   public override fun execute(args: List<Iota>, env: CastingEnvironment): List<Iota> {
      val angle: Double = OperatorUtils.getDouble(args, 0, this.getArgc());
      if (Math.cos(angle) == 0.0) {
         val var10000: MishapDivideByZero.Companion = MishapDivideByZero.Companion;
         val var10001: Any = args.get(0);
         throw var10000.tan(var10001 as DoubleIota);
      } else {
         return CollectionsKt.listOf(new DoubleIota(Math.tan(angle)));
      }
   }

   override fun getMediaCost(): Long {
      return ConstMediaAction.DefaultImpls.getMediaCost(this);
   }

   override fun executeWithOpCount(args: MutableList<Iota>, env: CastingEnvironment): ConstMediaAction.CostMediaActionResult {
      return ConstMediaAction.DefaultImpls.executeWithOpCount(this, args, env);
   }

   override fun operate(env: CastingEnvironment, image: CastingImage, continuation: SpellContinuation): OperationResult {
      return ConstMediaAction.DefaultImpls.operate(this, env, image, continuation);
   }
}
