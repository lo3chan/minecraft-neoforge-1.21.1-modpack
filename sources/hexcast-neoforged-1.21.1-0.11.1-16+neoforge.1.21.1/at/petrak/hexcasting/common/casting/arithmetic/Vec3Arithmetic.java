...
public enum Vec3Arithmetic implements Arithmetic {
   ...
   public static final List<HexPattern> OPS = List.of(PACK, UNPACK, ADD, SUB, MUL, DIV, ABS, POW, FLOOR, CEIL, MOD);
   ...
   public static OperatorUnary make1(Function<Vec3, Vec3> op) {
      return new OperatorUnary(ACCEPTS, i -> new Vec3Iota(op.apply(Operator.downcast(i, HexIotaTypes.VEC3).getVec3())));
   }

   public static OperatorUnary make1Double(Function<Vec3, Double> op) {
      return new OperatorUnary(ACCEPTS, i -> new DoubleIota(op.apply(Operator.downcast(i, HexIotaTypes.VEC3).getVec3())));
   }
