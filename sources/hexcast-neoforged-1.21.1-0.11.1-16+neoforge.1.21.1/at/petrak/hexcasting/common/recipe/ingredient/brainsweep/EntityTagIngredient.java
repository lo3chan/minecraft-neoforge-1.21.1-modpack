...
public class EntityTagIngredient extends BrainsweepeeIngredient {
   ...
   public Component getName() {
      ...
      return moddersDidAGoodJob ? Component.translatable(key) : Component.literal("#" + this.entityTypeTag.location());
   }
   ...
   public Entity exampleEntity(Level level) {
      Iterator<Holder<EntityType<?>>> someEntityTys = BuiltInRegistries.ENTITY_TYPE.getTagOrEmpty(this.entityTypeTag).iterator();
      if (someEntityTys.hasNext()) {
         Holder<EntityType<?>> someTy = someEntityTys.next();
         return ((EntityType)someTy.value()).create(level);
      } else {
         return null;
      }
   }
   ...
   public JsonObject serialize() {
      ...
      obj.addProperty("type", BrainsweepeeIngredient.Type.ENTITY_TAG.getSerializedName());
   ...
   public static EntityTagIngredient deserialize(JsonObject obj) {
      ResourceLocation tagLoc = ResourceLocation.tryParse(GsonHelper.getAsString(obj, "tag"));
      if (tagLoc == null) {
         throw new IllegalArgumentException("unknown tag " + obj);
      } else {
         TagKey<EntityType<?>> type = TagKey.create(Registries.ENTITY_TYPE, tagLoc);
   ...
   public static EntityTagIngredient read(FriendlyByteBuf buf) {
      ...
      TagKey<EntityType<?>> type = TagKey.create(Registries.ENTITY_TYPE, typeLoc);
      return new EntityTagIngredient(type);
   }
   ...
   public String getSomeKindOfReasonableIDForEmi() {
      ResourceLocation resloc = this.entityTypeTag.location();
