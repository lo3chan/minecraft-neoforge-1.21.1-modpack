...
public class ForgeHexScrollLootMod extends LootModifier {
   public static final Supplier<MapCodec<ForgeHexScrollLootMod>> CODEC = Suppliers.memoize(
      () -> RecordCodecBuilder.mapCodec(
         inst -> codecStart(inst).and(Codec.INT.fieldOf("countRange").forGetter(it -> it.countRange)).apply(inst, ForgeHexScrollLootMod::new)
      )
   );
   ...
   protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
      ...
      for (int i = 0; i < count; i++) {
         ItemStack newStack = new ItemStack(HexItems.SCROLL_LARGE);
   ...
   public MapCodec<ForgeHexScrollLootMod> codec() {
      return (MapCodec<ForgeHexScrollLootMod>)ForgeHexLootMods.INJECT_SCROLLS.get();
   }
}
