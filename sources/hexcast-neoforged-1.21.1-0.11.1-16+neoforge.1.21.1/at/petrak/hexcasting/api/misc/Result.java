...
public abstract sealed class Result<T, E> permits Result.Ok, Result.Err {
   ...
   public T unwrap() {
      if (this instanceof Result.Ok<T, E> ok) {
         return ok.ok;
      } else {
   ...
   public E unwrapErr() {
      if (this instanceof Result.Err<T, E> err) {
         return err.err;
      } else {
   ...
   public <T2, E2> Result<T2, E2> match(Function<T, T2> okBranch, Function<E, E2> errBranch) {
      if (this instanceof Result.Ok<T, E> ok) {
         return (Result<T2, E2>)(new Result.Ok<>(okBranch.apply(ok.ok)));
      } else if (this instanceof Result.Err<T, E> err) {
         return (Result<T2, E2>)(new Result.Err<>(errBranch.apply(err.err)));
      } else {
