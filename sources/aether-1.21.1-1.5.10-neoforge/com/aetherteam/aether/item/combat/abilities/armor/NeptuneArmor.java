...
public interface NeptuneArmor {
   static void boostWaterSwimming(LivingEntity entity) {
      ...
      if ((!EquipmentUtil.hasFullNeptuneSet(entity) || !entity.isInWaterOrBubble()) && entity instanceof Player playerx) {
         ((AetherPlayerAttachment)playerx.getData(AetherDataAttachments.AETHER_PLAYER)).setNeptuneSubmergeLength(0.0);
      }
   }

   private static float boostWithDepthStrider(LivingEntity entity) {
      float defaultBoost = 0.4F;
      float depthStriderModifier = Math.min((float)EnchantmentHelper.getEnchantmentLevel(entity.level().holderOrThrow(Enchantments.INFINITY), entity), 3.0F);
