...
public class BiomeParameterRecipeCategory<R extends AbstractBiomeParameterRecipe> extends AetherBlockStateRecipeCategory<R> implements BiomeTooltip {
   ...
   protected void populateTooltip(BlockStateRecipeDisplay<R> display, Tooltip tooltip) {
      if (((AbstractBiomeParameterRecipe)display.getRecipe()).getBiome().isPresent()) {
         this.populateBiomeInformation(
            ((AbstractBiomeParameterRecipe)display.getRecipe()).getBiome().get().left(),
            ((AbstractBiomeParameterRecipe)display.getRecipe()).getBiome().get().right(),
            tooltip
         );
      }
   }
   ...
   public static BiomeParameterRecipeCategory<PlacementConversionRecipe> placementConversion() {
      return new BiomeParameterRecipeCategory<>(
         "placement_conversion", AetherREIServerPlugin.PLACEMENT_CONVERSION, 84, 28, EntryStacks.of((ItemLike)AetherItems.AETHER_PORTAL_FRAME.get())
      );
   }

   public static BiomeParameterRecipeCategory<SwetBallRecipe> swetBall() {
      return new BiomeParameterRecipeCategory<>(
         "swet_ball_conversion", AetherREIServerPlugin.SWET_BALL_CONVERSION, 84, 28, EntryStacks.of((ItemLike)AetherItems.SWET_BALL.get())
      );
   }
}
