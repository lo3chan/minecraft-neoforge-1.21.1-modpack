      ...
      if (Minecraft.getInstance().level != null && (biomeKey.isPresent() || biomeTag.isPresent())) {
         ...
         if (biomeKey.isPresent()) {
            ...
            Minecraft.getInstance()
               ...
               .getTagOrEmpty(biomeTag.get())
               .forEach(
                  biomeHolder -> biomeHolder.unwrapKey()
                     .ifPresent(key -> tooltip.add(Component.literal(key.location().toString()).withStyle(ChatFormatting.DARK_GRAY)))
               );
         }
      }
   }
