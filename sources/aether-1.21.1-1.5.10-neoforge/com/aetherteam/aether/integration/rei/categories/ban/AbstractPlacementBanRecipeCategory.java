...
public abstract class AbstractPlacementBanRecipeCategory<T, S extends Predicate<T>, F extends RecipeInput, R extends AbstractPlacementBanRecipe<T, S, F>>
   extends AbstractRecipeCategory<PlacementBanRecipeDisplay<R>>
   ...
   public List<Widget> setupDisplay(PlacementBanRecipeDisplay<R> display, Rectangle bounds) {
      ...
      if (bypassBlock.isPresent() && !bypassBlock.get().isEmpty()) {
         Point bypassPoint = this.startingOffset(bounds);
         bypassPoint.translate(99, 1);
         Slot bypassSlot = Widgets.createSlot(bypassPoint)
            .entries(
               REIClientUtils.setupRendering(
                  (Collection)display.getInputEntries().get(1), bypassBlock.get().getPairs(), tooltip -> this.populateTooltip(display, tooltip)
               )
            );
         widgets.add(bypassSlot);
         MutableComponent text = Component.translatable("gui.aether.jei.bypass");
         Point labelPoint = new Point(bounds.getCenterX(), bounds.getCenterY() - 9 / 2);
         widgets.add(Widgets.createLabel(labelPoint, text).color(-8355712, -4473925));
      }
