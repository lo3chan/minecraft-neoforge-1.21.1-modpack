...
public class ItemBanRecipeCategory extends AbstractPlacementBanRecipeCategory<ItemStack, Ingredient, SingleRecipeInput, ItemBanRecipe> {
   public static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath("aether", "item_placement_ban");
   public static final RecipeType<ItemBanRecipe> RECIPE_TYPE = RecipeType.create("aether", "item_placement_ban", ItemBanRecipe.class);
   ...
   public void setRecipe(IRecipeLayoutBuilder builder, ItemBanRecipe recipe, IFocusGroup focusGroup) {
      ...
      if (!recipe.getBypassBlock().isEmpty() && !recipe.getBypassBlock().get().isEmpty()) {
         ((IRecipeSlotBuilder)builder.addSlot(RecipeIngredientRole.INPUT, 1, 1).addIngredients(ingredient))
            .addTooltipCallback((recipeSlotView, tooltip) -> this.populateAdditionalInformation(recipe, tooltip));
      } else {
         ((IRecipeSlotBuilder)builder.addSlot(RecipeIngredientRole.INPUT, 50, 1).addIngredients(ingredient))
            .addTooltipCallback((recipeSlotView, tooltip) -> this.populateAdditionalInformation(recipe, tooltip));
      }
