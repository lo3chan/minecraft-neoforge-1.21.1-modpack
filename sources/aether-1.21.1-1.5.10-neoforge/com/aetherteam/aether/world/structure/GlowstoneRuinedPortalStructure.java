...
public class GlowstoneRuinedPortalStructure extends Structure {
   ...
   public static final MapCodec<GlowstoneRuinedPortalStructure> CODEC = RecordCodecBuilder.mapCodec(
      codec -> codec.group(
            settingsCodec(codec),
            ExtraCodecs.nonEmptyList(GlowstoneRuinedPortalStructure.Setup.CODEC.codec().listOf()).fieldOf("setups").forGetter(structure -> structure.setups)
         )
         .apply(codec, GlowstoneRuinedPortalStructure::new)
   );

   public GlowstoneRuinedPortalStructure(StructureSettings settings, List<GlowstoneRuinedPortalStructure.Setup> setups) {
      super(settings);
      this.setups = setups;
   }
   ...
   protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
      ...
      GlowstoneRuinedPortalStructure.Setup portalSetup = null;
      if (this.setups.size() > 1) {
         float f = 0.0F;

         for (GlowstoneRuinedPortalStructure.Setup setup1 : this.setups) {
            f += setup1.weight();
         }

         float f1 = worldGenRandom.nextFloat();

         for (GlowstoneRuinedPortalStructure.Setup setup2 : this.setups) {
            f1 -= setup2.weight() / f;
            if (f1 < 0.0F) {
               portalSetup = setup2;
               break;
         ...
         portalSetup = (GlowstoneRuinedPortalStructure.Setup)this.setups.getFirst();
      }

      if (portalSetup == null) {
         throw new IllegalStateException();
      } else {
         GlowstoneRuinedPortalStructure.Setup setup3 = portalSetup;
         ...
         if (worldGenRandom.nextFloat() < 0.05F) {
            location = ResourceLocation.fromNamespaceAndPath(
               "aether", STRUCTURE_LOCATION_GIANT_PORTALS[worldGenRandom.nextInt(STRUCTURE_LOCATION_GIANT_PORTALS.length)]
            );
         } else {
            location = ResourceLocation.fromNamespaceAndPath("aether", STRUCTURE_LOCATION_PORTALS[worldGenRandom.nextInt(STRUCTURE_LOCATION_PORTALS.length)]);
         }
         ...
         int i = chunkGenerator.getBaseHeight(
               centerPos.getX(), centerPos.getZ(), GlowstoneRuinedPortalPiece.getHeightMapType(setup3.placement()), levelHeightAccessor, randomState
            )
            - 1;
         ...
         return validCorners(chunkGenerator, levelHeightAccessor, chunkPos, randomState, spawnPos.getY())
            ? Optional.of(
               new GenerationStub(
                  spawnPos,
                  builder -> builder.addPiece(
                     new GlowstoneRuinedPortalPiece(
                        context.structureTemplateManager(), spawnPos, setup3.placement(), pieceProperties, location, rotation, mirror, blockPos
                     )
                  )
               )
            )
   ...
   private static int findSuitableY(
      ...
      List<NoiseColumn> noiseColumns = positions.stream()
         .map(pos -> chunkGenerator.getBaseColumn(pos.getX(), pos.getZ(), heightAccessor, randomState))
         .toList();
      Types heightmap$types = verticalPlacement == GlowstoneRuinedPortalPiece.VerticalPlacement.ON_OCEAN_FLOOR ? Types.OCEAN_FLOOR : Types.WORLD_SURFACE;
   ...
   public record Setup(
      GlowstoneRuinedPortalPiece.VerticalPlacement placement,
      float airPocketProbability,
      float mossiness,
      boolean overgrown,
      boolean vines,
      boolean replaceWithHolystone,
      float weight
   ) {
      public static final MapCodec<GlowstoneRuinedPortalStructure.Setup> CODEC = RecordCodecBuilder.mapCodec(
         codec -> codec.group(
               ...
               ExtraCodecs.POSITIVE_FLOAT.fieldOf("weight").forGetter(GlowstoneRuinedPortalStructure.Setup::weight)
            )
            .apply(codec, GlowstoneRuinedPortalStructure.Setup::new)
      );
   }
}
