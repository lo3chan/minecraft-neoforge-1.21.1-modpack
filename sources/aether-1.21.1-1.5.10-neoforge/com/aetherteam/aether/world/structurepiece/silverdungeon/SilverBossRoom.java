...
public class SilverBossRoom extends SilverDungeonPiece {
   public SilverBossRoom(StructureTemplateManager manager, String name, BlockPos pos, Rotation rotation, Holder<StructureProcessorList> processors) {
      super(
         (StructurePieceType)AetherStructurePieceTypes.SILVER_BOSS_ROOM.get(),
         manager,
         name,
         makeSettings(manager, SilverDungeonPiece.makeLocation(name)).setRotation(rotation),
         pos,
         processors
      );
   ...
   public SilverBossRoom(StructurePieceSerializationContext context, CompoundTag tag) {
      super(
         ...
         resourceLocation -> makeSettings(context.structureTemplateManager(), ResourceLocation.parse(tag.getString("Template")))
      );
   }

   private static StructurePlaceSettings makeSettings(StructureTemplateManager templateManager, ResourceLocation name) {
      return new StructurePlaceSettings().addProcessor(makeBoxProcessor(templateManager, name)).setFinalizeEntities(true);
   }
   ...
   protected void handleDataMarker(String name, BlockPos pos, ServerLevelAccessor level, RandomSource random, BoundingBox box) {
      if (name.equals("Treasure Chest")) {
         ...
         TreasureChestBlockEntity.setDungeonType(level, chest, ResourceLocation.fromNamespaceAndPath("aether", "silver"));
