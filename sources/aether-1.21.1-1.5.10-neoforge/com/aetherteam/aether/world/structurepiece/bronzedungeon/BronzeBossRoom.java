...
public class BronzeBossRoom extends BronzeDungeonPiece {
   public BronzeBossRoom(StructureTemplateManager manager, String name, BlockPos pos, Rotation rotation, Holder<StructureProcessorList> processors) {
      this(
         (StructurePieceType)AetherStructurePieceTypes.BRONZE_BOSS_ROOM.get(),
         manager,
         name,
         AetherTemplateStructurePiece.makeSettingsWithPivot(makeSettings(), manager, BronzeDungeonPiece.makeLocation(name), rotation),
         pos,
         processors
      );
   }
   ...
   protected void handleDataMarker(String name, BlockPos pos, ServerLevelAccessor level, RandomSource random, BoundingBox box) {
      if (name.equals("Treasure Chest")) {
         ...
         TreasureChestBlockEntity.setDungeonType(level, chest, ResourceLocation.fromNamespaceAndPath("aether", "bronze"));
