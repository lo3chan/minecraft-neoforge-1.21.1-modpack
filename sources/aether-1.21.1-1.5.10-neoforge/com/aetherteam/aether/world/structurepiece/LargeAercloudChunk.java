...
public class LargeAercloudChunk extends StructurePiece {
   ...
   public LargeAercloudChunk(Set<BlockPos> positions, BlockStateProvider blocks, BoundingBox bounds, Direction direction) {
      super((StructurePieceType)AetherStructurePieceTypes.LARGE_AERCLOUD.get(), 0, bounds);
   ...
   public LargeAercloudChunk(StructurePieceSerializationContext context, CompoundTag tag) {
      super((StructurePieceType)AetherStructurePieceTypes.LARGE_AERCLOUD.get(), tag);

      for (Tag value : tag.getList("Positions", 11)) {
         int[] position = ((IntArrayTag)value).getAsIntArray();
         this.positions.add(new BlockPos(position[0], position[1], position[2]));
      }

      this.blocks = (BlockStateProvider)BlockStateProvider.CODEC.parse(new Dynamic(NbtOps.INSTANCE, tag.get("Blocks"))).getPartialOrThrow();
   }
   ...
   public void postProcess(
      WorldGenLevel level, StructureManager manager, ChunkGenerator generator, RandomSource random, BoundingBox bounds, ChunkPos chunkPos, BlockPos blockPos
   ) {
