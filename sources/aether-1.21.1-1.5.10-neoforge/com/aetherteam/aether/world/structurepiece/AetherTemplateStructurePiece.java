...
public abstract class AetherTemplateStructurePiece extends TemplateStructurePiece {
   ...
   public AetherTemplateStructurePiece(
   ...
   protected static Holder<StructureProcessorList> readProcessors(CompoundTag tag, RegistryAccess access) {
      ...
      return (Holder<StructureProcessorList>)StructureProcessorType.LIST_CODEC
   ...
   protected static void writeProcessors(CompoundTag tag, RegistryAccess access, Holder<StructureProcessorList> processors) {
      DynamicOps<Tag> ops = RegistryOps.create(NbtOps.INSTANCE, access);
      StructureProcessorType.LIST_CODEC.encodeStart(ops, processors).resultOrPartial(Aether.LOGGER::error).ifPresent(entry -> tag.put("Processors", entry));
   }
   ...
   protected static StructurePlaceSettings addProcessors(StructurePlaceSettings settings, Holder<StructureProcessorList> processors) {
      ((StructureProcessorList)processors.value()).list().forEach(settings::addProcessor);
      return settings;
   }

   public static StructurePlaceSettings makeSettingsWithPivot(
      StructurePlaceSettings settings, StructureTemplateManager templateManager, ResourceLocation name, Rotation rotation
   ) {
