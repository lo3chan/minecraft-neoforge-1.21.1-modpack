...
public abstract class GoldDungeonPiece extends AetherTemplateStructurePiece {
   public static final RuleProcessor LOCKED_HELLFIRE_STONE = new RuleProcessor(
      ImmutableList.of(
         new ProcessorRule(
            new RandomBlockMatchTest((Block)AetherBlocks.LOCKED_HELLFIRE_STONE.get(), 0.1F),
            AlwaysTrueTest.INSTANCE,
            ((Block)AetherBlocks.LOCKED_LIGHT_HELLFIRE_STONE.get()).defaultBlockState()
         )
      )
   );
   public static final RuleProcessor MOSSY_HOLYSTONE = new RuleProcessor(
      ImmutableList.of(
         new ProcessorRule(
   ...
   protected static ResourceLocation makeLocation(String name) {
      return ResourceLocation.fromNamespaceAndPath("aether", "gold_dungeon/" + name);
   }
}
