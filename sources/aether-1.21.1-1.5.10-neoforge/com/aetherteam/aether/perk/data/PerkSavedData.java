...
public class PerkSavedData extends SavedData {
   ...
   public CompoundTag save(CompoundTag tag, Provider lookupProvider) {
      ...
      for (Entry<UUID, MoaData> moaDataEntry : this.storedSkinData.entrySet()) {
      ...
      for (Entry<UUID, Halo> haloEntry : this.storedHaloData.entrySet()) {
      ...
      for (Entry<UUID, DeveloperGlow> developerGlowEntry : this.storedDeveloperGlowData.entrySet()) {
   ...
   public static PerkSavedData load(CompoundTag tag, Provider lookupProvider) {
      PerkSavedData data = create();
      ...
      for (String key : tag.getAllKeys()) {
         ...
         if (key.equals("StoredHaloData")) {
            ...
            for (String storedHaloDataKey : storedHaloDataTag.getAllKeys()) {
               ...
               UUID playerUUIDx = UUID.fromString(storedHaloDataKey);
               String hexColor = null;
               if (haloEntryTag.contains("HexColor")) {
                  hexColor = haloEntryTag.getString("HexColor");
               }

               data.storedHaloData.put(playerUUIDx, new Halo(hexColor));
            }
         }
         ...
         if (key.equals("StoredDeveloperGlowData")) {
            ...
            for (String storedDeveloperGlowDataKey : storedDeveloperGlowDataTag.getAllKeys()) {
               ...
               UUID playerUUIDx = UUID.fromString(storedDeveloperGlowDataKey);
               String hexColor = null;
               if (developerGlowEntryTag.contains("HexColor")) {
                  hexColor = developerGlowEntryTag.getString("HexColor");
               }

               data.storedDeveloperGlowData.put(playerUUIDx, new DeveloperGlow(hexColor));
            }
         }
      }
   ...
   public static PerkSavedData compute(DimensionDataStorage dataStorage) {
      return (PerkSavedData)dataStorage.computeIfAbsent(new Factory(PerkSavedData::create, PerkSavedData::load, null), "perks");
   }
