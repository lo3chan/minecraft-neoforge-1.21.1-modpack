...
public record AetherSelectorLoot(Provider registries) implements LootTableSubProvider {
   public void generate(BiConsumer<ResourceKey<LootTable>, Builder> builder) {
      builder.accept(AetherLoot.WHIRLWIND_JUNK, LootTable.lootTable().withPool(whirlwindLoot()));
      builder.accept(
         AetherLoot.EVIL_WHIRLWIND_JUNK,
         LootTable.lootTable()
            .withPool(
               whirlwindLoot()
                  .add(
                     LootItem.lootTableItem(Items.AIR)
                        .apply(WhirlwindSpawnEntity.builder(EntityTypePredicate.of(EntityType.CREEPER), ConstantInt.of(1)))
                        .setWeight(60)
                  )
            )
      );
   ...
   private static net.minecraft.world.level.storage.loot.LootPool.Builder whirlwindLoot() {
