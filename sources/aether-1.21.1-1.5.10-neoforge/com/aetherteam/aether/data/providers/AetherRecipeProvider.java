...
public abstract class AetherRecipeProvider extends NitrogenRecipeProvider {
   public AetherRecipeProvider(PackOutput output, CompletableFuture<Provider> lookupProvider, String id) {
   ...
   protected ShapedRecipeBuilder makePendant(Supplier<? extends Item> pendant, Item material) {
      return this.makePendant(pendant, material, Ingredient.of(Items.STRINGS));
   }

   protected ShapedRecipeBuilder makePendantWithTag(Supplier<? extends Item> pendant, TagKey<Item> material, String unlockName) {
      return this.makePendantWithTag(pendant, material, Ingredient.of(Items.STRINGS), unlockName);
   }

   protected ShapedRecipeBuilder makeCape(Supplier<? extends Item> cape, Item material) {
      return ShapedRecipeBuilder.shaped(RecipeCategory.COMBAT, (ItemLike)cape.get())
         .define('#', material)
         .pattern("##")
         .pattern("##")
         .pattern("##")
   ...
   protected AltarRepairBuilder repairingRecipe(RecipeCategory category, ItemLike item, int duration) {
      return AltarRepairBuilder.repair(Ingredient.of(new ItemStack[]{new ItemStack(item, 1)}), category, duration).unlockedBy(getHasName(item), has(item));
   }
   ...
   protected AetherCookingRecipeBuilder enchantingRecipe(RecipeCategory category, ItemLike result, ItemLike ingredient, float experience, int duration) {
      return AetherCookingRecipeBuilder.generic(
            Ingredient.of(new ItemStack[]{new ItemStack(ingredient, 1)}),
            category,
            result,
            experience,
            duration,
            (AetherCookingSerializer<?>)AetherRecipeSerializers.ENCHANTING.get(),
            EnchantingRecipe::new
         )
         .unlockedBy(getHasName(ingredient), has(ingredient));
   }

   protected AetherCookingRecipeBuilder enchantingRecipe(
      RecipeCategory category, ItemLike result, TagKey<Item> ingredient, float experience, int duration, String unlockName
   ) {
      return AetherCookingRecipeBuilder.generic(
   ...
   protected AetherCookingRecipeBuilder freezingRecipe(RecipeCategory category, ItemLike result, ItemLike ingredient, float experience, int duration) {
      return AetherCookingRecipeBuilder.generic(
            ...
            experience,
            duration,
            (AetherCookingSerializer<?>)AetherRecipeSerializers.FREEZING.get(),
            FreezingRecipe::new
         )
         .unlockedBy(getHasName(ingredient), has(ingredient));
   }

   protected AetherCookingRecipeBuilder freezingRecipeWithTag(
      RecipeCategory category, ItemLike result, TagKey<Item> ingredient, float experience, int duration, String unlockName
   ) {
      return AetherCookingRecipeBuilder.generic(
            Ingredient.of(ingredient),
   ...
   protected AetherCookingRecipeBuilder freezingRecipeWithUnlockTag(
      RecipeCategory category, ItemLike result, ItemLike ingredient, TagKey<Item> unlock, float experience, int duration, String unlockName
   ) {
      return AetherCookingRecipeBuilder.generic(
            Ingredient.of(new ItemLike[]{ingredient}),
            category,
   ...
   protected IncubationBuilder moaIncubationRecipe(EntityType<?> entity, ResourceKey<MoaType> moaType, ItemLike ingredient) {
      ...
      return IncubationBuilder.incubation(Ingredient.of(new ItemLike[]{ingredient}), entity, tag, 1000).unlockedBy(getHasName(ingredient), has(ingredient));
   }

   protected BlockStateRecipeBuilder ambrosiumEnchanting(Block result, Block ingredient) {
      return BlockStateRecipeBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), result, AmbrosiumRecipe::new);
   }

   protected BlockStateRecipeBuilder swetBallConversion(Block result, Block ingredient) {
      return BlockStateRecipeBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), result, SwetBallRecipe::new);
   }

   protected BiomeParameterRecipeBuilder swetBallConversionTag(Block result, Block ingredient, TagKey<Biome> tagKey) {
      return BiomeParameterRecipeBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), result, tagKey, SwetBallRecipe::new);
   }

   protected BlockStateRecipeBuilder icestoneFreezable(Block result, Block ingredient) {
      return BlockStateRecipeBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), result, IcestoneFreezableRecipe::new);
   }

   protected BlockStateRecipeBuilder accessoryFreezable(Block result, Block ingredient) {
      return BlockStateRecipeBuilder.recipe(
         BlockStateIngredient.of(new BlockPropertyPair[]{this.pair(ingredient, Optional.of(this.map(BlockStateProperties.LEVEL, 0)))}),
         result,
         AccessoryFreezableRecipe::new
      );
   }

   protected BiomeParameterRecipeBuilder convertPlacement(Block result, Block ingredient, TagKey<Biome> biome) {
      return BiomeParameterRecipeBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), result, biome, PlacementConversionRecipe::new);
   }

   protected BiomeParameterRecipeBuilder convertPlacementWithProperties(
      Block result,
      Reference2ObjectArrayMap<Property<?>, Comparable<?>> resultProperties,
      Block ingredient,
      Reference2ObjectArrayMap<Property<?>, Comparable<?>> ingredientProperties,
      TagKey<Biome> biome
   ) {
      return BiomeParameterRecipeBuilder.recipe(
         BlockStateIngredient.of(new BlockPropertyPair[]{this.pair(ingredient, Optional.of(ingredientProperties))}),
         result,
         resultProperties,
         biome,
         PlacementConversionRecipe::new
      );
   }

   protected PlacementBanBuilder banItemPlacement(ItemLike ingredient, TagKey<Biome> biome) {
      return ItemBanBuilder.recipe(Ingredient.of(new ItemLike[]{ingredient}), Optional.empty(), Either.right(biome));
   }

   protected PlacementBanBuilder banItemPlacementWithBypass(ItemLike ingredient, TagKey<Block> bypass, TagKey<Biome> biome) {
      return ItemBanBuilder.recipe(Ingredient.of(new ItemLike[]{ingredient}), Optional.of(BlockStateIngredient.of(bypass)), Either.right(biome));
   }

   protected PlacementBanBuilder banBlockPlacement(Block ingredient, TagKey<Biome> biome) {
      return BlockBanBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), Optional.empty(), Either.right(biome));
   }

   protected PlacementBanBuilder banBlockPlacementWithBypass(Block ingredient, TagKey<Block> bypass, TagKey<Biome> biome) {
      return BlockBanBuilder.recipe(BlockStateIngredient.of(new Block[]{ingredient}), Optional.of(BlockStateIngredient.of(bypass)), Either.right(biome));
   }
