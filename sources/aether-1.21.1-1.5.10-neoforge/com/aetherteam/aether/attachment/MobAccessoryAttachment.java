...
public class MobAccessoryAttachment {
   ...
   public static final Codec<MobAccessoryAttachment> CODEC = RecordCodecBuilder.create(
   ...
   public MobAccessoryAttachment() {
      this.accessoryDropChances = new HashMap<>(
         Map.ofEntries(
            Map.entry("hand", 0.085F), Map.entry("necklace", 0.085F), Map.entry("aether:gloves_slot", 0.085F), Map.entry("aether:pendant_slot", 0.085F)
         )
      );
   }
   ...
   public float getEquipmentDropChance(SlotTypeReference identifier) {
      return this.accessoryDropChances.containsKey(identifier.slotName()) ? this.getAccessoryDropChances().get(identifier.slotName()) : 0.0F;
   }
