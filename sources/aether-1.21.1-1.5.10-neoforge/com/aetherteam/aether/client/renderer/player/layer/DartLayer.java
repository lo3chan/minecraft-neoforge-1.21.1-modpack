...
public class DartLayer<T extends LivingEntity, M extends PlayerModel<T>> extends StuckInBodyLayer<T, M> {
   ...
   public DartLayer(
      ...
      Function<Entity, AbstractDart> dart,
      Function<AetherPlayerAttachment, Integer> dartCount,
      float offset
   ) {
      super(renderer);
   ...
   public void render(
      ...
      if (i > 0) {
         for (int j = 0; j < i; j++) {
            ...
            Cube cube = modelPart.getRandomCube(randomSource);
   ...
   protected int numStuck(T entity) {
      return entity instanceof Player player ? this.dartCount.apply((AetherPlayerAttachment)player.getData(AetherDataAttachments.AETHER_PLAYER)) : 0;
   }
}
