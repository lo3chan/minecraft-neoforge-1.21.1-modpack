...
public class TntPresentRenderer extends EntityRenderer<TntPresent> {
   ...
   public TntPresentRenderer(Context context) {
      super(context);
   ...
   public void render(TntPresent present, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
      ...
      if (present.getFuse() - partialTicks + 1.0F < 10.0F) {
         float f = 1.0F - (present.getFuse() - partialTicks + 1.0F) / 10.0F;
      ...
      TntMinecartRenderer.renderWhiteSolidBlock(
         this.blockRenderer, ((Block)AetherBlocks.PRESENT.get()).defaultBlockState(), poseStack, buffer, packedLight, present.getFuse() / 5 % 2 == 0
      );
      poseStack.popPose();
