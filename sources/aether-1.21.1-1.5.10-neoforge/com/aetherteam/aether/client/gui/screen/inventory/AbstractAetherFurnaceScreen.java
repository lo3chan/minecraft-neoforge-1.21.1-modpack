...
public abstract class AbstractAetherFurnaceScreen<T extends AbstractAetherFurnaceMenu>
   extends AbstractRecipeBookScreen<SingleRecipeInput, AbstractCookingRecipe, T, AbstractFurnaceRecipeBookComponent> {
   private final ResourceLocation texture;
   private final ResourceLocation litProgressSprite;
   private final ResourceLocation burnProgressSprite;

   public AbstractAetherFurnaceScreen(
      T menu,
      AbstractFurnaceRecipeBookComponent recipeBook,
   ...
   protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int x, int y) {
      ...
      if (((AbstractAetherFurnaceMenu)this.getMenu()).isLit()) {
         int litProgress = ((AbstractAetherFurnaceMenu)this.getMenu()).getLitProgress() + 1;
         guiGraphics.blitSprite(this.litProgressSprite, 14, 14, 0, 14 - litProgress, left + 57, top + 36 + 13 - litProgress, 14, litProgress);
      }

      int burnProgress = ((AbstractAetherFurnaceMenu)this.getMenu()).getBurnProgress();
