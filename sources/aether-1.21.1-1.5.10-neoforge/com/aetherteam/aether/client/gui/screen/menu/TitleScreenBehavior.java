...
public interface TitleScreenBehavior {
   default void handleImageButtons(TitleScreen titleScreen, int xOffset) {
      for (GuiEventListener renderable : titleScreen.children()) {
         if (renderable instanceof Button button) {
            ...
            if (isImageButton(buttonText) && ((AbstractWidgetAccessor)button).aether$getAlpha() > 0.01) {
               button.visible = true;
            }
         }
      }
         ...
         if (child instanceof AbstractWidget widget) {
            ...
            if (message.getString().contains("<essential_")) {
               ...
               if (languageButton != null) {
                  widget.visible = ((AbstractWidgetAccessor)languageButton).aether$getAlpha() > 0.01;
               }
   ...
   static boolean isImageButton(Component buttonText) {
      return buttonText.equals(Component.translatable("options.accessibility")) || buttonText.equals(Component.translatable("options.language"));
   }
