...
public interface RecipeBookBehavior<T extends RecipeBookMenu<?, ?>, V extends AbstractContainerScreen<T> & RecipeUpdateListener> {
   ...
   default boolean hasClickedOutside(V screen, double mouseX, double mouseY, int guiLeft, int guiTop, int mouseButton) {
      boolean flag = mouseX < guiLeft || mouseY < guiTop || mouseX >= guiLeft + screen.getXSize() || mouseY >= guiTop + screen.getYSize();
      return screen.getRecipeBookComponent()
            .hasClickedOutside(mouseX, mouseY, screen.getGuiLeft(), screen.getGuiTop(), screen.getXSize(), screen.getYSize(), mouseButton)
         && flag;
   }
