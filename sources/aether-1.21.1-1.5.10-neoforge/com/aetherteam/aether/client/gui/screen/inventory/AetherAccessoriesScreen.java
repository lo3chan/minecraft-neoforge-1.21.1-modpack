...
public class AetherAccessoriesScreen
   ...
   public static final WidgetSprites ACCESSORIES_BUTTON = new WidgetSprites(
      ResourceLocation.fromNamespaceAndPath("aether", "inventory/accessories_button"),
      ResourceLocation.fromNamespaceAndPath("aether", "inventory/accessories_button_highlighted")
   );
   public static final WidgetSprites SKINS_BUTTON = new WidgetSprites(
      ResourceLocation.fromNamespaceAndPath("aether", "skins/skins_button"), ResourceLocation.fromNamespaceAndPath("aether", "skins/skins_button_highlighted")
   );
   public static final WidgetSprites CUSTOMIZATION_BUTTON = new WidgetSprites(
      ResourceLocation.fromNamespaceAndPath("aether", "customization/customization_button"),
      ResourceLocation.fromNamespaceAndPath("aether", "customization/customization_button_highlighted")
   );
   private static final ResourceLocation ACCESSORIES_INVENTORY = ResourceLocation.fromNamespaceAndPath("aether", "textures/gui/inventory/accessories.png");
   private static final ResourceLocation ACCESSORIES_INVENTORY_CREATIVE = ResourceLocation.fromNamespaceAndPath(
      "aether", "textures/gui/inventory/accessories_creative.png"
   );
   ...
   private void updateRenderButtons() {
      ...
      for (Slot slot : ((AetherAccessoriesMenu)this.menu).slots) {
         if (slot instanceof AccessoriesBasedSlot accessoriesSlot) {
            ...
            this.cosmeticButtons.put(accessoriesSlot, (ToggleButton)this.addWidget(slotButton));
         }
      }
   }
   ...
   public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
      if (this.getRecipeBookComponent().isVisible() && this.widthTooNarrow) {
         ...
         for (GuiEventListener widget : this.children()) {
            if (widget instanceof ToggleButton renderButton && renderButton.isHovered()) {
         ...
         if (this.getMinecraft().player != null) {
            if (this.getMinecraft().player.isCreative() && this.destroyItemSlot == null) {
               ...
               ((AetherAccessoriesMenu)this.getMenu()).slots.add(this.destroyItemSlot);
            } else if (!this.getMinecraft().player.isCreative() && this.destroyItemSlot != null) {
               ((AetherAccessoriesMenu)this.getMenu()).slots.remove(this.destroyItemSlot);
   ...
   protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
      if (this.getMinecraft().player != null) {
         ...
         guiGraphics.blit(
            this.getMinecraft().player.isCreative() ? ACCESSORIES_INVENTORY_CREATIVE : ACCESSORIES_INVENTORY,
            i,
            j,
            0,
            0,
            this.getXSize() + this.creativeXOffset(),
            this.getYSize()
         );
   ...
   protected boolean isHovering(int rectX, int rectY, int rectWidth, int rectHeight, double pointX, double pointY) {
      return this.isRenderButtonHovered
         ? false
         : (!this.widthTooNarrow || !this.getRecipeBookComponent().isVisible()) && super.isHovering(rectX, rectY, rectWidth, rectHeight, pointX, pointY);
   }
   ...
   protected void slotClicked(@Nullable Slot slot, int slotId, int mouseButton, ClickType type) {
      ...
      if (this.getMinecraft().player != null && this.getMinecraft().gameMode != null) {
         ...
         if ((slot != null || type == ClickType.QUICK_CRAFT) && (slot == null || slot.mayPickup(this.getMinecraft().player))) {
            if (slot == this.destroyItemSlot && this.destroyItemSlot != null && flag) {
               ...
               PacketDistributor.sendToServer(new ClearItemPacket(this.getMinecraft().player.getId()), new CustomPacketPayload[0]);
            }
         }
