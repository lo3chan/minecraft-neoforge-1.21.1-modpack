...
public class BiomeParameterRecipeSerializer<T extends AbstractBiomeParameterRecipe> extends BlockStateRecipeSerializer<T> {
   ...
   public BiomeParameterRecipeSerializer(
      BiomeParameterRecipeSerializer.Factory<T> factory, com.aetherteam.nitrogen.recipe.recipes.AbstractBlockStateRecipe.Factory<T> superFactory
   ) {
      super(superFactory);
   ...
   public T fromNetwork(RegistryFriendlyByteBuf buffer) {
      Optional<Either<ResourceKey<Biome>, TagKey<Biome>>> biome = buffer.readOptional(
         buf -> (Either)BlockStateRecipeUtil.STREAM_CODEC.decode((RegistryFriendlyByteBuf)buf)
      );
   ...
   public interface Factory<T extends AbstractBiomeParameterRecipe> {
      T create(Optional<Either<ResourceKey<Biome>, TagKey<Biome>>> var1, BlockStateIngredient var2, BlockPropertyPair var3, Optional<ResourceLocation> var4);
   }
}
