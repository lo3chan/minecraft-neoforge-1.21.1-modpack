...
public class CollideGoal extends Goal {
   ...
   public boolean canUse() {
      return this.slider.isAwake() && !this.slider.isDeadOrDying()
         ? this.slider.attackCooldown() <= 0 || this.slider.getDeltaMovement().length() > 0.08
         : false;
   }
   ...
   public void tick() {
      ...
      for (Entity entity : this.slider.level().getEntities(this.slider, collisionBounds)) {
         if (entity instanceof LivingEntity livingEntity
            && entity.hurt(AetherDamageTypes.entityDamageSource(this.slider.level(), AetherDamageTypes.CRUSH, this.slider), 6.0F)) {
