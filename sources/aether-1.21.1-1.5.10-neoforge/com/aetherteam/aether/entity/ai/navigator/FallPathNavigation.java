...
public class FallPathNavigation extends GroundPathNavigation {
   ...
   protected void followThePath() {
      ...
      double d0 = Math.abs(this.mob.getX() - (vec3i.getX() + (this.mob.getBbWidth() + 1.0F) / 2.0));
      double d1 = Math.abs(this.mob.getY() - vec3i.getY());
      double d2 = Math.abs(this.mob.getZ() - (vec3i.getZ() + (this.mob.getBbWidth() + 1.0F) / 2.0));
      float fallDistance = this.mob.getMaxFallDistance();
      boolean flag = d0 <= this.maxDistanceToWaypoint && d2 <= this.maxDistanceToWaypoint && d1 < fallDistance;
