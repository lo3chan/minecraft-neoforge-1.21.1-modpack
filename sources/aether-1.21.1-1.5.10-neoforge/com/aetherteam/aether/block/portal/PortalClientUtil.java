...
public class PortalClientUtil {
   public static void handleAetherPortal(Player player, AetherPlayerAttachment attachment) {
      if (player instanceof LocalPlayer localPlayer && !(Minecraft.getInstance().screen instanceof ReceivingLevelScreen)) {
         attachment.oPortalIntensity = attachment.portalIntensity;
         float f = 0.0F;
         if (localPlayer.portalProcess != null
            && localPlayer.portalProcess.isInsidePortalThisTick()
            && localPlayer.portalProcess.isSamePortal((Portal)AetherBlocks.AETHER_PORTAL.get())) {
   ...
   public static void playTriggerSound() {
      Minecraft.getInstance()
         .getSoundManager()
         .play(
            PortalTriggerSoundInstance.forLocalAmbience(
               Minecraft.getInstance().player,
               (SoundEvent)AetherSoundEvents.BLOCK_AETHER_PORTAL_TRIGGER.get(),
               Minecraft.getInstance().level.getRandom().nextFloat() * 0.4F + 0.8F,
               0.25F
            )
         );
   }
      ...
      Minecraft.getInstance()
         ...
         .play(
            SimpleSoundInstance.forLocalAmbience(
               (SoundEvent)AetherSoundEvents.BLOCK_AETHER_PORTAL_TRAVEL.get(), Minecraft.getInstance().level.getRandom().nextFloat() * 0.4F + 0.8F, 0.25F
            )
         );
   }
}
