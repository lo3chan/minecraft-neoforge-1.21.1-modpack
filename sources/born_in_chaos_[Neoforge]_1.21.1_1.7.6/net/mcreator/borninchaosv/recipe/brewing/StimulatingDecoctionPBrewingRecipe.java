...
@EventBusSubscriber
public class StimulatingDecoctionPBrewingRecipe implements IBrewingRecipe {
   @SubscribeEvent
   public static void init(RegisterBrewingRecipesEvent event) {
      event.getBuilder().addRecipe(new StimulatingDecoctionPBrewingRecipe());
   }
   ...
   public boolean isIngredient(ItemStack ingredient) {
      return Ingredient.of(new ItemStack[]{new ItemStack((ItemLike)BornInChaosV1ModItems.STIMULATING_DECOCTION.get())}).test(ingredient);
   }

   public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
      return this.isInput(input) && this.isIngredient(ingredient)
         ? PotionContents.createItemStack(input.getItem(), BornInChaosV1ModPotions.STIMULATING_POTION)
         : ItemStack.EMPTY;
   }
}
