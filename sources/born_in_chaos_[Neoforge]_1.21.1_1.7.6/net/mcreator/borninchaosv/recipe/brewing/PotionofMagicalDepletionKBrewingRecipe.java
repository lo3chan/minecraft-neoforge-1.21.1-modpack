...
@EventBusSubscriber
public class PotionofMagicalDepletionKBrewingRecipe implements IBrewingRecipe {
   @SubscribeEvent
   public static void init(RegisterBrewingRecipesEvent event) {
      event.getBuilder().addRecipe(new PotionofMagicalDepletionKBrewingRecipe());
   }
   ...
   public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
      return this.isInput(input) && this.isIngredient(ingredient)
         ? PotionContents.createItemStack(input.getItem(), BornInChaosV1ModPotions.POTION_OF_MAGICAL_DEPLETION)
         : ItemStack.EMPTY;
   }
}
