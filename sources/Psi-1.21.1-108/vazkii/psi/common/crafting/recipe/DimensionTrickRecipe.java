   ...
   public static class Serializer implements RecipeSerializer<DimensionTrickRecipe> {
      ...
      private static final MapCodec<DimensionTrickRecipe> CODEC = RecordCodecBuilder.mapCodec(
         instance -> instance.group(
               ((TrickRecipe.Serializer)ModCraftingRecipes.TRICK_RECIPE_SERIALIZER.get()).codec().forGetter(d -> d),
               ResourceLocation.CODEC.fieldOf("dimensionKey").forGetter(d -> d.dimensionKey.location())
            )
            .apply(
               instance,
               (recipe, dimensionId) -> new DimensionTrickRecipe(
                  recipe.getPiece(), recipe.getInput(), recipe.getResultItem(RegistryAccess.EMPTY), recipe.getAssembly(), dimensionId
               )
            )
      );

      private static DimensionTrickRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
         TrickRecipe recipe = (TrickRecipe)((TrickRecipe.Serializer)ModCraftingRecipes.TRICK_RECIPE_SERIALIZER.get()).streamCodec().decode(buf);
         ResourceLocation dimensionId = (ResourceLocation)ResourceLocation.STREAM_CODEC.decode(buf);
      ...
      private static void toNetwork(RegistryFriendlyByteBuf buf, DimensionTrickRecipe recipe) {
         ((TrickRecipe.Serializer)ModCraftingRecipes.TRICK_RECIPE_SERIALIZER.get()).streamCodec().encode(buf, recipe);
