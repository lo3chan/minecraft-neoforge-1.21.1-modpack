...
public class ContainerCADAssembler extends AbstractContainerMenu {
   ...
   public ContainerCADAssembler(int windowId, Inventory playerInventory, TileCADAssembler assembler) {
      super((MenuType)ModBlocks.containerCADAssembler.get(), windowId);
      ...
      this.addSlot(new SlotItemHandler(assemblerInv, EnumCADComponent.ASSEMBLY.ordinal() + 1, 120, 91));
      this.addSlot(new SlotItemHandler(assemblerInv, EnumCADComponent.CORE.ordinal() + 1, 100, 91));
      this.addSlot(new SlotItemHandler(assemblerInv, EnumCADComponent.SOCKET.ordinal() + 1, 140, 91));
      this.addSlot(new SlotItemHandler(assemblerInv, EnumCADComponent.BATTERY.ordinal() + 1, 110, 111));
      this.addSlot(new SlotItemHandler(assemblerInv, EnumCADComponent.DYE.ordinal() + 1, 130, 111));
      ...
      for (int armorSlot = 0; armorSlot < 4; armorSlot++) {
         ...
         this.addSlot(new Slot(playerInventory, playerSize - 2 - armorSlot, xs - 27, ys + 18 * armorSlot) {
            ...
            public Pair<ResourceLocation, ResourceLocation> getNoItemIcon() {
               return Pair.of(InventoryMenu.BLOCK_ATLAS, (ResourceLocation)InventoryMenu.TEXTURE_EMPTY_SLOTS.get(slot));
            }
         });
      }
   ...
   public boolean stillValid(@NotNull Player playerIn) {
      return !playerIn.isRemoved() && this.assembler.getBlockPos().distToCenterSqr(playerIn.position()) <= 64.0;
   }

   @NotNull
   public ItemStack quickMoveStack(@NotNull Player playerIn, int from) {
      ItemStack mergeStack = ItemStack.EMPTY;
      if (from <= this.slots.size() - 1 && from >= 0) {
