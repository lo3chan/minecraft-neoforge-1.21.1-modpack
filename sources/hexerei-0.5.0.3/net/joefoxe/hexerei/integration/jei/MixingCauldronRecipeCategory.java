...
public class MixingCauldronRecipeCategory implements IRecipeCategory<MixingCauldronRecipe> {
   ...
   public void getTooltip(ITooltipBuilder tooltip, MixingCauldronRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
      if (recipe.getHeatCondition() != FluidMixingRecipe.HeatCondition.NONE && this.isHovering(mouseX, mouseY, 79.0, 59.0, 24.0, 18.0)) {
         ...
         if (Screen.hasShiftDown()) {
            ...
            tooltip.add(
               Component.translatable(
                     "tooltip.hexerei.heat_source_shown",
      ...
      if (recipe.getMoonCondition() != MoonPhases.MoonCondition.NONE && this.isHovering(mouseX, mouseY, 79.0, 35.0, 24.0, 18.0)) {
         ...
         if (Screen.hasShiftDown()) {
            ...
            tooltip.add(Component.translatable("tooltip.hexerei.recipe_moon_1").withStyle(Style.EMPTY.withColor(TextColor.fromRgb(10066329))));
         } else {
            tooltip.add(
               Component.translatable(
                     "[%s]", new Object[]{Component.translatable("tooltip.hexerei.shift").withStyle(Style.EMPTY.withColor(TextColor.fromRgb(11184640)))}
            ...
            tooltip.add(
               Component.translatable("tooltip.hexerei.recipe_moon", new Object[]{Component.translatable(recipe.getMoonCondition().getNameTranslated())})
                  .withStyle(Style.EMPTY.withColor(TextColor.fromRgb(10066329)))
            );
         }
      }
   ...
   public void setRecipe(IRecipeLayoutBuilder builder, MixingCauldronRecipe recipe, IFocusGroup focuses) {
      ...
      if (!output.isEmpty()
         ...
         builder.addSlot(RecipeIngredientRole.OUTPUT, 152, 71)
            ...
            .addFluidStack(output.getFluid(), 2000L, output.getComponentsPatch())
            .addRichTooltipCallback((recipeSlotView, tooltip) -> {
               List<Component> tooltips = new ArrayList<>();
               ((PotionContents)output.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY)).addPotionTooltip(tooltips::add, 1.0F, 20.0F);
      ...
      if (!input.isEmpty()) {
         builder.addSlot(RecipeIngredientRole.INPUT, 20, 57)
            ...
            .addRichTooltipCallback((recipeSlotView, tooltip) -> {
               List<Component> tooltips = new ArrayList<>();
               ((PotionContents)input.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY)).addPotionTooltip(tooltips::add, 1.0F, 20.0F);
               tooltip.addAll(tooltips);
            });
      }
   ...
   public void draw(MixingCauldronRecipe recipe, IRecipeSlotsView view, GuiGraphics guiGraphics, double mouseX, double mouseY) {
      ...
      if (recipe.getHeatCondition() != FluidMixingRecipe.HeatCondition.HEATED && recipe.getHeatCondition() != FluidMixingRecipe.HeatCondition.SUPERHEATED) {
         ...
         float newHeatSource = (float)(Minecraft.getInstance().clientTickCount % 200L) / 200.0F;
         float craftPercent = (float)(Minecraft.getInstance().clientTickCount % 100L) / 100.0F;
         boolean showOutput = Minecraft.getInstance().clientTickCount % 200L > 100L;
         ...
         float percentNeeded = recipe.getFluidLevelsConsumed() / 2000.0F;
         ...
         if (!showOutput) {
            ...
            } else if (output.getFluid().is(Fluids.GASEOUS)) {
            ...
            if (!item2.isEmpty()) {
               ...
               if (fillPercentage > 0.0F) {
                  guiGraphics.pose().translate(0.0, Math.sin(3.141592653589793 * ClientEvents.getClientTicks() / 60.0 + 20.0) / 10.0 * 0.2, 0.0);
                  guiGraphics.pose()
                     .mulPose(
                        Axis.YP
                           .rotationDegrees(
                              (float)(44.0 + 2.0 * Math.sin((ClientEvents.getClientTicks() + 20.0F) / 40.0F))
                                 - (1.0F - craftPercent) * (1.0F - craftPercent) * 720.0F
                           )
                     );
               ...
               guiGraphics.pose().scale(0.4F, 0.4F, 0.4F);
               this.renderItem(item2, minecraft.level, guiGraphics.pose(), buffer, 15728880);
               guiGraphics.pose().popPose();
            }
         }
         ...
         float newHeatSourcex = (float)(Minecraft.getInstance().clientTickCount % 200L) / 200.0F;
         float craftPercentx = (float)(Minecraft.getInstance().clientTickCount % 100L) / 100.0F;
         boolean showOutputx = Minecraft.getInstance().clientTickCount % 200L > 100L;
         ...
         Component outputNamex = recipe.getOutput().getHoverName();
         ...
         BlockRenderDispatcher renderererx = Minecraft.getInstance().getBlockRenderer();
         renderererx.getBlockModel(blockStatex);
         BakedModel bakedModelx = renderererx.getBlockModel(blockStatex);
         RenderSystem.setShaderTexture(0, InventoryMenu.BLOCK_ATLAS);
         RenderSystem.enableBlend();
         RenderSystem.blendFunc(SourceFactor.SRC_ALPHA, DestFactor.ONE_MINUS_SRC_ALPHA);
         RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
         ...
         MixingCauldronRenderer.renderFluidBlockGUI(
            guiGraphics.pose(), bufferx, new FluidStack(net.minecraft.world.level.material.Fluids.LAVA, 2000), 1.0F, OverlayTexture.NO_OVERLAY
         );
         guiGraphics.pose().popPose();
         float fillPercentagex = 1.0F;
         if (!showOutputx) {
            if (input.getFluid().is(Fluids.GASEOUS)) {
               MixingCauldronRenderer.renderFluidGUI(guiGraphics.pose(), bufferx, input, fillPercentagex, 1.0F, OverlayTexture.NO_OVERLAY);
            } else {
               MixingCauldronRenderer.renderFluidGUI(guiGraphics.pose(), bufferx, input, 1.0F, fillPercentagex, OverlayTexture.NO_OVERLAY);
            }
            ...
            float percentNeededx = recipe.getFluidLevelsConsumed() / 2000.0F;
            float percentChangedx = percentNeededx * craftPercentx;
            fillPercentagex = 1.0F - percentChangedx;
            if (output.isEmpty()) {
               if (input.getFluid().is(Fluids.GASEOUS)) {
                  MixingCauldronRenderer.renderFluidGUI(guiGraphics.pose(), bufferx, input, fillPercentagex, 1.0F, OverlayTexture.NO_OVERLAY);
               } else {
               ...
               MixingCauldronRenderer.renderFluidGUI(guiGraphics.pose(), bufferx, output, fillPercentagex, 1.0F, OverlayTexture.NO_OVERLAY);
            } else {
               MixingCauldronRenderer.renderFluidGUI(guiGraphics.pose(), bufferx, output, 1.0F, fillPercentagex, OverlayTexture.NO_OVERLAY);
            }
                  ...
                  guiGraphics.pose()
                     .mulPose(
                        Axis.YP
                           .rotationDegrees(
                              (float)(44.0 + 2.0 * Math.sin((ClientEvents.getClientTicks() + 20.0F) / 40.0F))
                                 - (1.0F - craftPercentx) * (1.0F - craftPercentx) * 720.0F
                           )
                     );
                  guiGraphics.pose().mulPose(Axis.XP.rotationDegrees((float)(82.5 + 5.0 * Math.cos((ClientEvents.getClientTicks() + 22.0F) / 40.0F))));
                  guiGraphics.pose().mulPose(Axis.ZP.rotationDegrees((float)(-2.5 + 5.0 * Math.cos((ClientEvents.getClientTicks() + 24.0F) / 40.0F))));
               } else {
                  guiGraphics.pose().mulPose(Axis.YP.rotationDegrees(45.0F - (1.0F - craftPercentx) * (1.0F - craftPercentx) * 720.0F));
                  guiGraphics.pose().mulPose(Axis.XP.rotationDegrees(85.0F));
                  guiGraphics.pose().mulPose(Axis.ZP.rotationDegrees(-2.5F));
               }

               guiGraphics.pose().scale(0.4F, 0.4F, 0.4F);
               this.renderItem(item2, minecraftx.level, guiGraphics.pose(), bufferx, 15728880);
               guiGraphics.pose().popPose();
            }
         }
