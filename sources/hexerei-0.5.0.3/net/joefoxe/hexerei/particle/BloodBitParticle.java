...
public class BloodBitParticle extends TextureSheetParticle {
   ...
   private static final ParticleRenderType renderType = new ParticleRenderType() {
      ...
      public BufferBuilder begin(Tesselator tesselator, TextureManager textureManager) {
         RenderSystem.setShaderTexture(0, BloodBitParticle.TEXTURE_BLANK);
      ...
      for (int i = 0; i < CUBE.length / 4; i++) {
         for (int j = 0; j < 4; j++) {
            ...
            builder.addVertex((float)vec.x, (float)vec.y, (float)vec.z)
               .setUv(0.0F, 0.0F)
               .setColor(Mth.clamp(this.rCol * 0.8F, 0.0F, 1.0F), Mth.clamp(this.gCol * 0.8F, 0.0F, 1.0F), Mth.clamp(this.bCol * 0.8F, 0.0F, 1.0F), this.alpha)
