...
public class PotionCandleEffect extends AbstractCandleEffect {
      ...
      if (this.effect != null) {
         if (candleData.lit) {
            if (candleData.cooldown >= 160) {
               ...
               if (!level.isClientSide()) {
                  applyEffects(
                     ...
                     5.0F,
                     this.effect.isInstantenous() ? 1 : (int)(duration * candleData.getEffectDurationMultiplier()),
                     Math.max(0, (int)candleData.getEffectAmplifierMultiplier() - 1),
                     this.effect,
                     candleData.effectParticle,
                     candleData
                  );
               }
            ...
            if (candleData.effectParticle != null && level.isClientSide() && candleData.effectParticle != null && candleData.effectParticle.size() > 0) {
               ...
               try {
                  this.particle = ParticleArgument.readParticle(new StringReader(resourceLocation), level.registryAccess());
   ...
   private static void applyEffects(
      Level pLevel, BlockPos pPos, float size, int duration, int amplifier, @Nullable MobEffect pPrimary, List<String> particle, CandleData candleData
   ) {
      if (pPrimary != null) {
         AABB aabb = new AABB(pPos).inflate(size).expandTowards(0.0, size * 4.0F < 4.0F ? 4.0 : size * 4.0F, 0.0);
         ...
         Optional<Reference<MobEffect>> effectHolder = BuiltInRegistries.MOB_EFFECT.getHolder(id);
   ...
   public static void spawnParticles(Level pLevel, List<String> particle, LivingEntity living) {
      ...
      for (int i = 0; i < 5; i++) {
         ...
         if (particle != null) {
            try {
               pLevel.addParticle(
                  ParticleArgument.readParticle(new StringReader(particle.get(random.nextInt(particle.size()))), pLevel.registryAccess()),
                  living.getX(),
                  living.getY() + heightOffset,
                  living.getZ(),
                  offset.x / 32.0,
