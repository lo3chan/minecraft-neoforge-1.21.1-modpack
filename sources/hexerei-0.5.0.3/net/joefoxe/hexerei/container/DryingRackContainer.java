...
public class DryingRackContainer extends AbstractContainerMenu {
   ...
   public DryingRackContainer(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
      super((MenuType)ModContainers.DRYING_RACK_CONTAINER.get(), windowId);
      this.tileEntity = world.getBlockEntity(pos);
      this.playerEntity = player;
      this.playerInventory = new InvWrapper(playerInventory);
      this.layoutPlayerInventorySlots(8, 86);
      if (this.tileEntity instanceof DryingRackTile dryingRack) {
         IItemHandler handler = new InvWrapper(dryingRack);
         this.addSlot(new SlotItemHandler(handler, 0, 80, 5));
         this.addSlot(new SlotItemHandler(handler, 1, 102, 14));
         this.addSlot(new SlotItemHandler(handler, 7, 58, 14));
      }
   }
   ...
   public boolean stillValid(Player playerIn) {
      return stillValid(
         ContainerLevelAccess.create(this.tileEntity.getLevel(), this.tileEntity.getBlockPos()), playerIn, (Block)ModBlocks.HERB_DRYING_RACK.get()
      );
   }
