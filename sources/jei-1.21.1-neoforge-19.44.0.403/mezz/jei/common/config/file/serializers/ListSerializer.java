...
public class ListSerializer<T> implements IJeiConfigListValueSerializer<T> {
   ...
   public DeserializeResult<List<T>> deserialize(String string) {
      ...
      List<T> results = Arrays.stream(split).map(String::trim).filter(s -> !s.isEmpty()).map(this.valueSerializer::deserialize).<T>mapMulti((r, c) -> {
         r.getResult().ifPresent((Consumer<? super T>)c);
