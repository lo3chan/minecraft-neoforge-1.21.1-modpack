...
public class PacketRecipeTransfer extends PlayToServerPacket<PacketRecipeTransfer> {
   ...
   public static final Type<PacketRecipeTransfer> TYPE = new Type(ResourceLocation.fromNamespaceAndPath("jei", "recipe_transfer"));
   ...
   static List<Slot> getSlots(AbstractContainerMenu container, List<Integer> slotIds) {
      if (slotIds.size() > container.slots.size()) {
         LOGGER.error(
            "Recipe transfer packet has too many slot ids {} for container {} with {} slots", slotIds.size(), container.getClass(), container.slots.size()
         );
         return null;
      } else {
         ...
         for (int slotId : slotIds) {
            if (slotId < 0 || slotId >= container.slots.size()) {
               LOGGER.error("Recipe transfer packet has invalid slot id {} for container {}", slotId, container.getClass());
               return null;
            }
