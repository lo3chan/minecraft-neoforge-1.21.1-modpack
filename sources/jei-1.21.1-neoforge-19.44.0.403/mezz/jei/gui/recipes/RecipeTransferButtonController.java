...
public class RecipeTransferButtonController implements IIconButtonController {
   ...
   public void updateState(IButtonState state) {
      ...
      updateStateForTransferError(state, this.recipeTransferError);
   }
   ...
   public boolean onPress(IJeiUserInput input) {
      if (!input.isSimulate()) {
         ...
         if (parentContainer != null
            && player != null
            && RecipeTransferUtil.transferRecipe(recipeTransferManager, parentContainer, this.recipeLayout, player, maxTransfer)) {
   ...
   public void drawExtras(GuiGraphics guiGraphics, Rect2i buttonArea, int mouseX, int mouseY, float partialTicks) {
      ...
      if (recipeTransferError != null) {
         ...
         if (buttonArea.contains(mouseX, mouseY)) {
            ...
            runWithRestoredPose(
               poseStack, () -> recipeTransferError.showError(guiGraphics, mouseX, mouseY, recipeSlotsView, recipeRect.getX(), recipeRect.getY())
            );
         }
      }
   }
   ...
   static int getMissingCountHint(@Nullable IRecipeTransferError recipeTransferError) {
      return recipeTransferError == null ? 0 : recipeTransferError.getMissingCountHint();
   }
}
