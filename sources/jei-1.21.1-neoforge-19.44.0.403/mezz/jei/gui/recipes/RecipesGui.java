...
public class RecipesGui extends Screen implements IRecipesGui, IRecipeFocusSource {
   ...
   public RecipesGui(
      ...
      this.inputHandler = new UserInputRouter(
         "RecipesGui",
         this.layouts.createInputHandler(),
         new RecipesGui.UserInputHandler(this),
         this.optionButtons.createInputHandler(),
         this.recipeGuiTabs.createInputHandler(),
         this.nextRecipeCategory.createInputHandler(),
         this.previousRecipeCategory.createInputHandler(),
         this.nextPage.createInputHandler(),
         this.previousPage.createInputHandler()
      );
   }
   ...
   public void init() {
      ...
      RecipeGuiSizing.Size recipeGuiSize = RecipeGuiSizing.calculateInitialSize(
         this.height, clientConfig.centerSearchBarEnabled().getValue(), clientConfig.maxRecipeGuiHeight().getValue()
      );
      ...
      this.idealArea = new ImmutableRect2i(guiLeft, guiTop, 198, ySize);
   ...
   public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
      if (this.minecraft != null) {
         ...
         guiGraphics.fill(
            RenderType.gui(),
            this.previousRecipeCategory.getX() + this.previousRecipeCategory.getWidth(),
            this.previousRecipeCategory.getY(),
            this.nextRecipeCategory.getX(),
            this.nextRecipeCategory.getY() + this.nextRecipeCategory.getHeight(),
            805306368
         );
         guiGraphics.fill(
            RenderType.gui(),
            this.previousPage.getX() + this.previousPage.getWidth(),
            this.previousPage.getY(),
            this.nextPage.getX(),
            this.nextPage.getY() + this.nextPage.getHeight(),
            805306368
         );
         RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
         ...
         if (DebugConfig.isDebugGuisEnabled()) {
            guiGraphics.fill(
               ...
               this.idealArea.getX(),
               this.idealArea.getY(),
               this.idealArea.getX() + this.idealArea.getWidth(),
               this.idealArea.getY() + this.idealArea.getHeight(),
               1140915968
            );
            guiGraphics.fill(
               RenderType.gui(),
   ...
   public boolean isMouseOver(double mouseX, double mouseY) {
      return this.minecraft != null && this.minecraft.screen == this
         ? this.area.contains(mouseX, mouseY) || this.optionButtons.getArea().contains(mouseX, mouseY)
         : false;
   }
   ...
   public Stream<IClickableIngredientInternal<?>> getIngredientUnderMouse(double mouseX, double mouseY) {
      return this.isOpen()
         ? Stream.concat(this.recipeCatalysts.getIngredientUnderMouse(mouseX, mouseY), this.layouts.getIngredientUnderMouse(mouseX, mouseY))
         : Stream.empty();
   }
   ...
   public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
      return this.inputHandler.handleMouseScrolled(mouseX, mouseY, scrollX, scrollY) ? true : super.mouseScrolled(mouseX, mouseY, scrollX, scrollY);
   }
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
      ...
      return handled ? true : super.mouseClicked(mouseX, mouseY, mouseButton);
   }
   ...
   public <T> Optional<T> getIngredientUnderMouse(IIngredientType<T> ingredientType) {
      ...
      return this.getIngredientUnderMouse(x, y)
   ...
   public Optional<IRecipeLayoutWithButtons<?>> getRecipeLayoutUnderMouse(double mouseX, double mouseY) {
      return !this.isOpen() ? Optional.empty() : this.layouts.getRecipeLayoutUnderMouse(mouseX, mouseY);
   }
   ...
   private void updateLayout() {
      if (this.init) {
         ImmutableRect2i titleArea = MathUtil.union(this.previousRecipeCategory.getArea(), this.nextRecipeCategory.getArea())
            .cropLeft(this.previousRecipeCategory.getWidth() + 14)
            .cropRight(this.nextRecipeCategory.getWidth() + 14);
         ...
         List<IRecipeLayoutWithButtons<?>> recipeLayoutsWithButtons = this.logic
            .getVisibleRecipeLayoutsWithButtons(availableHeight, 4, containerMenu, this.bookmarks, this);
   ...
   private ImmutableRect2i getRecipeLayoutsArea() {
      return new ImmutableRect2i(
         this.area.getX() + 6, this.area.getY() + this.headerHeight + 2, this.area.getWidth() - 12, this.area.getHeight() - (this.headerHeight + 6 + 2)
      );
   }
   ...
   public AbstractContainerMenu getParentContainerMenu() {
      ...
      return screen instanceof AbstractContainerScreen<?> containerScreen ? containerScreen.getMenu() : null;
   }
   ...
   public IGuiProperties getProperties() {
      if (this.width > 0 && this.height > 0) {
         ...
         return guiXSize > 0 && guiYSize > 0
            ? new GuiProperties(
               (Class<? extends Screen>)this.getClass(), recipeArea.getX() - extraWidth, recipeArea.getY(), guiXSize, guiYSize, this.width, this.height
            )
            : null;
      } else {
         return null;
   ...
   private static class UserInputHandler implements IUserInputHandler {
      ...
      public Optional<IUserInputHandler> handleUserInput(Screen screen, UserInput input, IInternalKeyMappings keyBindings) {
         ...
         if (!this.recipesGui.isMouseOver(mouseX, mouseY)
            || !this.recipesGui.recipeCategoryTitle.isMouseOver(mouseX, mouseY)
            || !input.is(keyBindings.getLeftClick())
            || !input.isSimulate() && !this.recipesGui.logic.showAllRecipes()) {
