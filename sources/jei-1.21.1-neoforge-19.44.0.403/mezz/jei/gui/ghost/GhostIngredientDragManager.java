...
public class GhostIngredientDragManager {
   ...
   public GhostIngredientDragManager(
      IRecipeFocusSource source, IScreenHelper screenHelper, IIngredientManager ingredientManager, IClientToggleState toggleState
   ) {
      this.source = source;
   ...
   private void drawGhostIngredientHighlights(GuiGraphics guiGraphics, int mouseX, int mouseY) {
      if (this.ghostIngredientDrag != null) {
         ...
         ITypedIngredient<?> hovered = this.source
   ...
   private List<Rect2i> getHoveredTargetAreas(@Nullable ITypedIngredient<?> hovered) {
      if (hovered == null) {
         ...
         if (currentScreen == null) {
            ...
            for (IGhostIngredientHandler<Screen> handler : this.screenHelper.getGhostIngredientHandlers(currentScreen)) {
               if (handler.shouldHighlightTargets()) {
                  for (IGhostIngredientHandler.Target<?> target : handler.getTargetsTyped(currentScreen, hovered, false)) {
   ...
   private <T extends Screen, V> boolean handleClickGhostIngredient(T currentScreen, IDraggableIngredientInternal<V> clicked, UserInput input) {
      ...
      if (handlerDataList.isEmpty()) {
         ...
         this.ghostIngredientDrag = new GhostIngredientDrag<>(
            handlerDataList, ingredientRenderer, ingredient, input.getMouseX(), input.getMouseY(), clickedArea
         );
         return true;
      }
   }
   ...
   private class DragHandler implements IDragHandler {
      ...
      public Optional<IDragHandler> handleDragStart(Screen screen, UserInput input) {
         ...
         return player == null
            ? Optional.empty()
            : GhostIngredientDragManager.this.source
               ...
               .flatMap(
                  clicked -> {
                     ...
                     return mouseItem.isEmpty()
                           && GhostIngredientDragManager.this.handleClickGhostIngredient(screen, (IDraggableIngredientInternal<?>)clicked, input)
                        ? Optional.of(this)
                        : Optional.empty();
                  }
               );
      ...
      public boolean handleDragComplete(Screen screen, UserInput input) {
         if (GhostIngredientDragManager.this.ghostIngredientDrag == null) {
            ...
            if (!success && GhostIngredientDrag.canStart(GhostIngredientDragManager.this.ghostIngredientDrag, mouseX, mouseY)) {
               GhostIngredientReturning.create(GhostIngredientDragManager.this.ghostIngredientDrag, mouseX, mouseY)
                  .ifPresent(GhostIngredientDragManager.this.ghostIngredientsReturning::add);
            }
