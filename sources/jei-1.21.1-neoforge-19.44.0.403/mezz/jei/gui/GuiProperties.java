...
public record GuiProperties(Class<? extends Screen> screenClass, int guiLeft, int guiTop, int guiXSize, int guiYSize, int screenWidth, int screenHeight)
   ...
   public static GuiProperties create(AbstractContainerScreen<?> containerScreen) {
      if (containerScreen.width > 0 && containerScreen.height > 0) {
         ...
         return width > 0 && height > 0
            ? new GuiProperties((Class<? extends Screen>)containerScreen.getClass(), x, y, width, height, containerScreen.width, containerScreen.height)
            : null;
      } else {
         return null;
      }
