...
public class RecipeBookmark<R, I> implements IBookmark {
   ...
   public static <T> RecipeBookmark<T, ?> create(IRecipeLayoutDrawable<T> recipeLayoutDrawable, IIngredientManager ingredientManager) {
      ...
      if (recipeUid == null) {
         ...
         if (output != null) {
            ...
            output = findFirst(recipeSlotsView, RecipeIngredientRole.INPUT);
            if (output != null) {
               output = ingredientManager.normalizeTypedIngredient(output);
   ...
   public RecipeBookmark(
      IRecipeCategory<R> recipeCategory, R recipe, ResourceLocation recipeUid, ITypedIngredient<I> displayIngredient, boolean displayIsOutput
   ) {
   ...
   public boolean equals(Object obj) {
      return !(obj instanceof RecipeBookmark<?, ?> recipeBookmark)
         ? false
         : recipeBookmark.recipeUid.equals(this.recipeUid) && this.recipeCategory.getRecipeType().equals(recipeBookmark.recipeCategory.getRecipeType());
   }

   @Override
   public String toString() {
      return "RecipeBookmark{recipeCategory="
         + this.recipeCategory.getRecipeType()
         + ", recipe="
         + this.recipe
         + ", recipeUid="
         + this.recipeUid
   ...
   public <T> boolean isRecipe(RecipeType<T> otherType, T otherRecipe) {
      ...
      if (recipeType.equals(otherType)) {
         ...
         if (recipeClass.isInstance(otherRecipe)) {
            R castRecipe = (R)recipeClass.cast(otherRecipe);
            ResourceLocation otherUid = this.recipeCategory.getRegistryName(castRecipe);
            return this.recipeUid.equals(otherUid);
         }
      }
