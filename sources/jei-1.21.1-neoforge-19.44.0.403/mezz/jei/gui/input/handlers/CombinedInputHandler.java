...
public class CombinedInputHandler implements IUserInputHandler {
   ...
   public Optional<IUserInputHandler> handleUserInput(Screen screen, UserInput input, IInternalKeyMappings keyBindings) {
      return switch (input.getInputType()) {
         case IMMEDIATE, SIMULATE -> this.handleClickInternal(screen, input, keyBindings);
   ...
   static Optional<IUserInputHandler> handleClickInternal(
      List<IUserInputHandler> inputHandlers, Function<IUserInputHandler, Optional<IUserInputHandler>> handleInput
   ) {
   ...
   public String toString() {
      String inputHandlersString = this.inputHandlers.stream().<CharSequence>map(IUserInputHandler::toString).collect(Collectors.joining(", ", "[", "]"));
