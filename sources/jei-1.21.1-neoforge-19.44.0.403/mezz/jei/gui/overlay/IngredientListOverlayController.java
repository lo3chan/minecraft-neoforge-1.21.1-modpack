...
class IngredientListOverlayController {
   ...
   private final IngredientListOverlayController.Config config;
   ...
   private static IngredientListOverlayController.Config getConfig(final IClientConfig clientConfig) {
      return new IngredientListOverlayController.Config() {
         @Override
   ...
   IngredientListOverlayController(
      IGuiPropertiesCache guiPropertiesCache,
      IngredientListOverlayController.Config config,
   ...
   private void onGuiPropertiesChanged() {
      ...
      if (guiProperties == null) {
         ...
         try {
            ...
            LOGGER.error("Failed to update JEI bounds for screen with properties : {}", guiProperties, var4);
   ...
   private void updateBounds(IGuiProperties guiProperties, Set<ImmutableRect2i> guiExclusionAreas) {
      IngredientListOverlayLayout.Layout layout = IngredientListOverlayLayout.calculate(
         guiProperties,
         this.config.isCenterSearchBarEnabled(),
         this.config.isLookupHistoryEnabled(),
         this.lookupHistory.isDisplayedOnThisSide(),
         this.lookupHistory.getDisplayHeight()
      );
      ...
      IngredientListOverlayLayout.SearchAndConfigAreas searchAndConfigAreas = layout.getSearchAndConfigAreas(
         this.contentsView.hasRoom(), this.contentsView.getBackgroundArea()
      );
      this.searchField.setValue(this.filterTextSource.getFilterText());
