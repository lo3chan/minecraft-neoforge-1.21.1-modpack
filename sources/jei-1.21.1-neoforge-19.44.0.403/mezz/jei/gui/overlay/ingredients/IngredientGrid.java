...
public class IngredientGrid implements IIngredientGrid {
   ...
   private void updateSlots(int smoothScrollRowPixelOffset) {
      ...
      for (IngredientGridLayout.SlotLayout slotLayout : IngredientGridLayout.calculateSlots(
         this.area, this.guiExclusionAreas, this.mouseExclusionPoint, smoothScrollRowPixelOffset
      )) {
   ...
   private Optional<IngredientListSlot> getHighlightedSlot(Minecraft minecraft, int mouseX, int mouseY) {
      return this.isMouseOver(mouseX, mouseY) && !this.deleteItemHandler.shouldDeleteItemOnClick(minecraft, mouseX, mouseY)
         ? this.ingredientListRenderer.getSlots().filter(s -> s.getArea().contains(mouseX, mouseY)).filter(s -> s.getOptionalElement().isPresent()).findFirst()
         : Optional.empty();
   }

   private static void drawHighlight(GuiGraphics guiGraphics, ImmutableRect2i area) {
      guiGraphics.fillGradient(
         RenderType.guiOverlay(), area.getX(), area.getY(), area.getX() + area.getWidth(), area.getY() + area.getHeight(), -2130706433, -2130706433, 0
      );
   }
   ...
   public void drawTooltips(Minecraft minecraft, GuiGraphics guiGraphics, int mouseX, int mouseY) {
      if (this.isMouseOver(mouseX, mouseY)) {
         if (this.deleteItemHandler.shouldDeleteItemOnClick(minecraft, mouseX, mouseY)) {
            ...
            this.ingredientListRenderer
               .getSlots()
               .filter(s -> s.isMouseOver(mouseX, mouseY))
               .map(IngredientListSlot::getOptionalElement)
               .flatMap(Optional::stream)
               .findFirst()
               .ifPresent(element -> this.drawTooltip(guiGraphics, mouseX, mouseY, element));
         }
      }
   }
      ...
      if (clientConfig.showCreativeTabNamesEnabled().getValue()) {
         ...
         if (!itemStack.isEmpty()) {
            for (CreativeModeTab itemGroup : CreativeModeTabs.allTabs()) {
               if (itemGroup.shouldDisplay() && itemGroup.getType() == Type.CATEGORY && itemGroup.contains(itemStack)) {
   ...
   public Stream<IClickableIngredientInternal<?>> getIngredientUnderMouse(double mouseX, double mouseY) {
      return this.ingredientListRenderer
         .getSlots()
         .filter(s -> s.isMouseOver(mouseX, mouseY))
