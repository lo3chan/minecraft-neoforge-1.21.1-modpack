...
public final class IngredientGridTooltipHelper {
   ...
   public <T> void getIngredientTooltip(
      JeiTooltip tooltip, ITypedIngredient<T> typedIngredient, IIngredientRenderer<T> ingredientRenderer, IIngredientHelper<T> ingredientHelper
   ) {
      ...
      if (this.ingredientFilterConfig.searchIngredientAliases().getValue()) {
   ...
   private <T> void addIngredientAliasesToTooltip(JeiTooltip tooltip, ITypedIngredient<T> typedIngredient, IIngredientManager ingredientManager) {
      ...
      if (!aliases.isEmpty()) {
         ...
         tooltip.add(Component.translatable("jei.tooltip.item.search.aliases").withStyle(ChatFormatting.GRAY));

         for (String alias : aliases) {
            tooltip.add(Component.literal("• " + alias).withStyle(ChatFormatting.GRAY));
         }
      }
   }
   ...
   private <T> void addColorSearchInfoToTooltip(JeiTooltip tooltip, ITypedIngredient<T> typedIngredient, IIngredientHelper<T> ingredientHelper) {
      ...
      if (!colorNamesString.isEmpty()) {
         Component colorTranslation = Component.translatable("jei.tooltip.item.colors", new Object[]{colorNamesString}).withStyle(ChatFormatting.GRAY);
   ...
   private static void addEditModeInfoToTooltip(JeiTooltip tooltip, IInternalKeyMappings keyBindings) {
      ...
      tooltip.add(Component.translatable("gui.jei.editMode.description").withStyle(ChatFormatting.DARK_GREEN));
