...
public class IngredientGridWithNavigation implements IIngredientListOverlayContents {
   ...
   public IngredientGridWithNavigation(
      ...
      this.inputHandler = new CombinedInputHandler(
         debugName, this.scrollbar, this.controller, this.ingredientGrid.getInputHandler(), this.navigation.createInputHandler()
      );
   ...
   public void updateBounds(ImmutableRect2i availableArea, Set<ImmutableRect2i> guiExclusionAreas, @Nullable ImmutablePoint2i mouseExclusionPoint) {
      ...
      IngredientGridWithNavigationLayout layout = this.calculateLayout(
         availableArea, guiExclusionAreas, mouseExclusionPoint, this.ingredientSource.getElements().size()
      );
      this.applyLayout(layout, guiExclusionAreas, mouseExclusionPoint);
   }

   private IngredientGridWithNavigationLayout calculateLayout(
      ImmutableRect2i availableArea, Set<ImmutableRect2i> guiExclusionAreas, @Nullable ImmutablePoint2i mouseExclusionPoint, int ingredientCount
   ) {
      return this.gridConfig.navigationMode().getValue().usesScrollbar()
         ? IngredientGridScrollbarLayout.calculate(this.gridConfig, availableArea, guiExclusionAreas, mouseExclusionPoint, ingredientCount)
         : IngredientGridButtonNavigationLayout.calculate(this.gridConfig, availableArea, guiExclusionAreas, mouseExclusionPoint, ingredientCount);
   }
   ...
   public Stream<IClickableIngredientInternal<?>> getIngredientUnderMouse(double mouseX, double mouseY) {
      this.updateLayoutIfDirty();
      return !this.active ? Stream.empty() : this.ingredientGrid.getIngredientUnderMouse(mouseX, mouseY).map(this.controller::createPageAnchorIngredient);
   }

   @Override
   public Stream<IDraggableIngredientInternal<?>> getDraggableIngredientUnderMouse(double mouseX, double mouseY) {
      this.updateLayoutIfDirty();
      return !this.active ? Stream.empty() : this.ingredientGrid.getDraggableIngredientUnderMouse(mouseX, mouseY);
   }

   @Override
   public <T> Stream<T> getVisibleIngredients(IIngredientType<T> ingredientType) {
      this.updateLayoutIfDirty();
      return !this.active ? Stream.empty() : this.ingredientGrid.getVisibleIngredients(ingredientType);
   }
   ...
   public int size() {
      this.updateLayoutIfDirty();
      return !this.active ? 0 : this.ingredientGrid.size();
   }

   public Stream<IngredientListSlot> getSlots() {
      this.updateLayoutIfDirty();
      return !this.active ? Stream.empty() : this.ingredientGrid.getSlots();
   }
}
