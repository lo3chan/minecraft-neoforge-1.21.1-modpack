...
public class MaximalRectangle {
   ...
   public static Stream<ImmutableRect2i> getLargestRectangles(ImmutableRect2i area, Collection<ImmutableRect2i> exclusionAreas, int samplingScale) {
      exclusionAreas = exclusionAreas.stream().filter(area::intersects).collect(Collectors.toUnmodifiableSet());
   ...
   private ImmutableRect2i getRect(int row, int column, int width, int height) {
      return width != 0 && height != 0
         ? new ImmutableRect2i(
            this.area.getX() + column * this.samplingScale,
            this.area.getY() + row * this.samplingScale,
            width * this.samplingScale,
            height * this.samplingScale
         )
