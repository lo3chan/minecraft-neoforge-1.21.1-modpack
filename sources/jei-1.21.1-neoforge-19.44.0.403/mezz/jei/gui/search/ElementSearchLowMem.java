...
public class ElementSearchLowMem implements IElementSearch {
   ...
   public Set<IListElement<?>> getSearchResults(ElementPrefixParser.TokenInfo tokenInfo) {
      ...
      if (token.isEmpty()) {
         ...
         return this.elementInfoList
            .stream()
            .filter(elementInfo -> matches(token, prefixInfo, (IListElementInfo<?>)elementInfo))
   ...
   @Nullable
   private static <T> IListElement<T> checkForMatch(
      IListElement<?> element, IIngredientType<T> ingredientType, Object uid, Function<ITypedIngredient<T>, Object> uidFunction
   ) {
      ...
      if (cast == null) {
         ...
         return uid.equals(elementUid) ? cast : null;
      }
   }
   ...
   private static <T> IListElement<T> optionalCast(IListElement<?> element, IIngredientType<T> ingredientType) {
      ...
      return (IListElement<T>)(typedIngredient.getType() == ingredientType ? element : null);
   }
