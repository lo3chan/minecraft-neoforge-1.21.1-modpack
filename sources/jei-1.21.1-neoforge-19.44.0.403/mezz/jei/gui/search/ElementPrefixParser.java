...
public class ElementPrefixParser {
   ...
   public ElementPrefixParser(
      IIngredientManager ingredientManager, IIngredientFilterConfig config, IColorHelper colorHelper, ISearchStorageBuilderFactory searchStorageBuilderFactory
   ) {
      ...
      this.addPrefix(
         new PrefixInfo<>("mod_names", '@', config.modNameSearchMode()::getValue, info -> info.getModNames(config), limitedStringStorageBuilderFactory)
      );
      this.addPrefix(
         new PrefixInfo<>("tags", '#', config.tagSearchMode()::getValue, e -> e.getTagStrings(ingredientManager), limitedStringStorageBuilderFactory)
      );
      this.addPrefix(
         new PrefixInfo<>(
            "tooltips", '$', config.tooltipSearchMode()::getValue, e -> e.getTooltipStrings(config, ingredientManager), searchStorageBuilderFactory
         )
      );
      this.addPrefix(
         new PrefixInfo<>(
      ...
      this.addPrefix(
         new PrefixInfo<>(
            "colors", '^', config.colorSearchMode()::getValue, e -> e.getColorNames(ingredientManager, colorHelper), limitedStringStorageBuilderFactory
         )
      );
      this.addPrefix(
         new PrefixInfo<>(
   ...
   public Collection<PrefixInfo<IListElementInfo<?>, IListElement<?>>> allPrefixInfos() {
      Collection<PrefixInfo<IListElementInfo<?>, IListElement<?>>> values = new ArrayList<>(this.map.values());
   ...
   public Optional<ElementPrefixParser.TokenInfo> parseToken(String token) {
      if (token.isEmpty()) {
         ...
         PrefixInfo<IListElementInfo<?>, IListElement<?>> prefixInfo = (PrefixInfo<IListElementInfo<?>, IListElement<?>>)this.map.get(firstChar);
         if (prefixInfo == null || prefixInfo.getMode() == SearchMode.DISABLED) {
            return Optional.of(new ElementPrefixParser.TokenInfo(token, this.noPrefix));
         } else {
