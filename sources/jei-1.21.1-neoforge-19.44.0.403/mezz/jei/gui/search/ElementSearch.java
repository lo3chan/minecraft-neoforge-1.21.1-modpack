...
public class ElementSearch implements IElementSearch {
   ...
   public ElementSearch(ElementPrefixParser elementPrefixParser, Collection<IListElementInfo<?>> infos, IIngredientManager ingredientManager) {
      ...
      for (PrefixInfo<IListElementInfo<?>, IListElement<?>> prefixInfo : elementPrefixParser.allPrefixInfos()) {
         ...
         PrefixedSearchable<IListElementInfo<?>, IListElement<?>> prefixedSearchable = new PrefixedSearchable<>(searchStorage, prefixInfo);
   ...
   public Set<IListElement<?>> getSearchResults(ElementPrefixParser.TokenInfo tokenInfo) {
      ...
      if (token.isEmpty()) {
         ...
         if (prefixInfo == this.noPrefix) {
            ...
            ISearchable<IListElement<?>> searchable = (ISearchable<IListElement<?>>)this.prefixedSearchables.get(prefixInfo);
   ...
   public <T> IListElement<T> findElement(ITypedIngredient<T> ingredient, IIngredientHelper<T> ingredientHelper) {
      ...
      return (IListElement<T>)(listElement != null && listElement.getTypedIngredient().getType().equals(ingredient.getType()) ? listElement : null);
   }
