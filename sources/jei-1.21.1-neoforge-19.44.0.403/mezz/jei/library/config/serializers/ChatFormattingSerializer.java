...
public class ChatFormattingSerializer implements IJeiConfigListValueSerializer<ChatFormatting> {
   ...
   public DeserializeResult<List<ChatFormatting>> deserialize(String string) {
      ...
      if (string.isEmpty()) {
         ...
         List<ChatFormatting> result = Arrays.stream(strings).<ChatFormatting>mapMulti((s, c) -> {
            IJeiConfigValueSerializer.IDeserializeResult<ChatFormatting> deserializeResult = VALUE_SERIALIZER.deserialize(s);
            deserializeResult.getResult().ifPresent((Consumer<? super ChatFormatting>)c);
   ...
   public String getValidValuesDescription() {
      ...
      return "A chat formatting string.\nUse these formatting colors:\n%s\nWith these formatting options:\n%s"
         .formatted(this.serialize(validColors), this.serialize(validFormats));
   }
   ...
   private static class ChatFormattingValueSerializer implements IJeiConfigValueSerializer<ChatFormatting> {
      ...
      public IJeiConfigValueSerializer.IDeserializeResult<ChatFormatting> deserialize(String string) {
         ...
         if (chatFormatting == null) {
            ...
            return ChatFormattingSerializer.INVALID_VALUES.contains(chatFormatting)
               ? new DeserializeResult<>(null, "Chat Formatting '%s' is not valid".formatted(string))
               : new DeserializeResult<>(chatFormatting);
         }
      }
      ...
      @Override
      public String getValidValuesDescription() {
         String validValuesString = ChatFormattingSerializer.VALID_VALUES.stream().map(this::serialize).collect(Collectors.joining(", "));
