...
public final class ShieldDecorationRecipeMaker {
   ...
   private static RecipeHolder<CraftingRecipe> createRecipe(BannerItem banner) {
      NonNullList<Ingredient> inputs = NonNullList.of(
         Ingredient.EMPTY, new Ingredient[]{Ingredient.of(new ItemLike[]{Items.SHIELD}), Ingredient.of(new ItemLike[]{banner})}
      );
      ItemStack output = createOutput(banner);
      ResourceLocation id = ResourceLocation.fromNamespaceAndPath("minecraft", "jei.shield.decoration." + banner.getDescriptionId());
