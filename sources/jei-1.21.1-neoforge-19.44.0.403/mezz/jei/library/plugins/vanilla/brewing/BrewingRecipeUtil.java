...
public class BrewingRecipeUtil {
   ...
   private final Map<Object, Integer> brewingStepCache = new HashMap<>();
   ...
   private int getBrewingSteps(Object potionOutputUid, Set<Object> previousSteps) {
      ...
      if (cachedBrewingSteps != null) {
         ...
         Collection<Object> prevPotionUids = this.potionMap.get(potionOutputUid);
         int minPrevSteps = prevPotionUids.stream().mapToInt(prevPotion -> this.getBrewingSteps(prevPotion, previousSteps)).min().orElse(2147483647);
         int brewingSteps = minPrevSteps == 2147483647 ? 2147483647 : minPrevSteps + 1;
