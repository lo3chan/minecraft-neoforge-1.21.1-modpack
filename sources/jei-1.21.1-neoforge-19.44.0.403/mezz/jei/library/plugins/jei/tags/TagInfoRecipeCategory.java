...
public class TagInfoRecipeCategory<R extends ITagInfoRecipe, T extends RecipeType<R>> extends AbstractRecipeCategory<R> {
   ...
   public TagInfoRecipeCategory(IGuiHelper guiHelper, T recipeType, ResourceLocation registryLocation) {
      super(recipeType, createTitle(registryLocation), guiHelper.createDrawableItemLike(Items.NAME_TAG), 142, 110);
   }
   ...
   private static Component createTitle(ResourceLocation registryLocation) {
      ...
      return language.has(registryNameTranslationKey)
         ? Component.translatable(registryNameTranslationKey)
         : Component.translatable("gui.jei.category.tagInformation", new Object[]{StringUtils.capitalize(registryLocation.getPath())});
   }

   public void setRecipe(IRecipeLayoutBuilder builder, R recipe, IFocusGroup focuses) {
      builder.addInputSlot().addTypedIngredients(recipe.getTypedIngredients()).setStandardSlotBackground();
   ...
   public void createRecipeExtras(IRecipeExtrasBuilder builder, R recipe, IFocusGroup focuses) {
      ...
      List<FormattedText> text = List.of(tagName, Component.literal(tag.location().toString()).withStyle(ChatFormatting.GRAY));
      builder.addText(text, this.getWidth() - 22, 20)
         .setPosition(22, 0)
         .setColor(-11513776)
         .setLineSpacing(0)
