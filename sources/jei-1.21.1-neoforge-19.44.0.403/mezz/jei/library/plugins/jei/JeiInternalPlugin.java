...
public class JeiInternalPlugin implements IModPlugin {
   ...
   public ResourceLocation getPluginUid() {
      return ResourceLocation.fromNamespaceAndPath("jei", "internal");
   }
   ...
   public void registerCategories(IRecipeCategoryRegistration registration) {
      ...
      if (clientConfig.showTagRecipesEnabled().getValue()) {
         RegistryUtil.getRegistryAccess().registries().forEach(entry -> {
            ...
            createAndRegisterTagCategory(registration, this.tagInfoRecipeMakers, ingredientManager, registry);
         });
      }
   }
   ...
   private static <B> void createAndRegisterTagCategory(
      IRecipeCategoryRegistration registration, List<TagInfoRecipeMaker<?, ?>> tagInfoRecipeMakers, IIngredientManager ingredientManager, Registry<B> registry
   ) {
      registry.getAny()
         .ifPresent(
            holder -> {
               ...
               if (type != null) {
                  ...
                  IIngredientTypeWithSubtypes<B, Object> typeWithSubtypes = ingredientManager.<B, Object>getIngredientTypeWithSubtypesFromBase(ingredient)
                     .orElse(null);
                  if (typeWithSubtypes == null || !createAndRegisterTagCategory(registration, tagInfoRecipeMakers, registry, ingredient, typeWithSubtypes)) {
                     if (!(ingredient instanceof ItemLike) || !createAndRegisterItemLikeTagCategory(registration, tagInfoRecipeMakers, registry)) {
                        ;
                     }
                  }
               }
   ...
   private static <B extends ItemLike> boolean createAndRegisterItemLikeTagCategory(
      IRecipeCategoryRegistration registration, List<TagInfoRecipeMaker<?, ?>> tagInfoRecipeMakers, Registry<B> registry
   ) {
      IJeiHelpers jeiHelpers = registration.getJeiHelpers();
      IGuiHelper guiHelper = jeiHelpers.getGuiHelper();
      ...
      RecipeType<ITagInfoRecipe> recipeType = createTagInfoRecipeType(registryLocation);
      registration.addRecipeCategories(new TagInfoRecipeCategory<>(guiHelper, recipeType, registryLocation));
      tagInfoRecipeMakers.add(new TagInfoRecipeMaker<>(VanillaTypes.ITEM_STACK, recipeType, i -> i.asItem().getDefaultInstance(), registryKey));
      return true;
   }
}
