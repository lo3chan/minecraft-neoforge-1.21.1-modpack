...
public class FluidTankRenderer<T> implements IIngredientRenderer<T> {
   ...
   public FluidTankRenderer(IPlatformFluidHelperInternal<T> fluidHelper) {
      this(
         ...
         FluidTankRenderer.TooltipMode.ITEM_LIST,
         16,
         16,
   ...
   public FluidTankRenderer(
      ...
      this(
         ...
         type,
         capacity,
         showCapacity ? FluidTankRenderer.TooltipMode.SHOW_AMOUNT_AND_CAPACITY : FluidTankRenderer.TooltipMode.SHOW_AMOUNT,
         width,
         height,
         tilingDirection
      );
   ...
   private FluidTankRenderer(
      ...
      IIngredientTypeWithSubtypes<Fluid, T> type,
      long capacity,
      FluidTankRenderer.TooltipMode tooltipMode,
      int width,
      int height,
      TilingDirection tilingDirection
   ) {
   ...
   public void render(GuiGraphics guiGraphics, T ingredient, int posX, int posY) {
      ...
      if (!fluid.isSame(Fluids.EMPTY)) {
         this.fluidHelper.getStillFluidSprite(ingredient).ifPresent(fluidStillSprite -> {
            ...
            if (amount > 0L) {
               long longScaledAmount = amount * this.height / this.capacity;
               int scaledAmount = Math.clamp(longScaledAmount, 1, this.height);
               drawTiledSprite(guiGraphics, this.width, this.height, fluidColor, scaledAmount, fluidStillSprite, this.tilingDirection, posX, posY);
            }
         });
      }
   }
   ...
   private static void drawTiledSprite(
      ...
      Tile tileScaling = new Tile(spriteWidth, spriteHeight);
      ...
      try {
         ScalableDrawable.blitTiledSpriteWithColor(
            guiGraphics, sprite, tileScaling, posX - xShift, posY - yShift, tiledWidth + xShift, scaledAmount + yShift, color
         );
      } finally {
   ...
   private static int getShift(int desired, int sprite) {
      ...
      return remainder == 0 ? 0 : sprite - remainder;
   }
      ...
      if (fluidType.isSame(Fluids.EMPTY)) {
         ...
         if (this.tooltipMode == FluidTankRenderer.TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
            MutableComponent amountString = Component.translatable(
               "jei.tooltip.liquid.amount.with.capacity", new Object[]{nf.format(milliBuckets), nf.format(this.capacity)}
            );
            tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
         } else if (this.tooltipMode == FluidTankRenderer.TooltipMode.SHOW_AMOUNT) {
            MutableComponent amountString = Component.translatable("jei.tooltip.liquid.amount", new Object[]{nf.format(milliBuckets)});
            tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
         }
