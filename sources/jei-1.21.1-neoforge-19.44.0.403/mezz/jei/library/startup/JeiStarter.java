...
public final class JeiStarter {
   ...
   public JeiStarter(StartData data) {
      ...
      this.vanillaPlugin = PluginHelper.getPluginWithClass(VanillaPlugin.class, this.plugins)
   ...
   public void start() {
      if (this.running) {
         ...
         if (level == null) {
            ...
            PluginCaller.callOnPlugins("Configuring JEI", this.plugins, p -> p.configureJei(new PluginAwareJeiFeatures(Internal.getJeiFeatures(), p)));
            ...
            EditModeConfig.FileSerializer editModeSerializer = new EditModeConfig.FileSerializer(
               configDir.resolve("blacklist.json"), registryAccess, codecHelper
            );
            ...
            JeiHelpers jeiHelpers = PluginLoader.createJeiHelpers(
               modAliases, this.modIdFormatConfig, colorHelper, editModeConfig, focusFactory, codecHelper, ingredientManager, subtypeManager
            );
            ...
            RecipeManager recipeManager = PluginLoader.createRecipeManager(
               this.plugins, this.vanillaPlugin, this.recipeCategorySortingConfig, jeiHelpers, ingredientManager
            );
            IRecipeTransferManager recipeTransferManager = PluginLoader.createRecipeTransferManager(
               this.vanillaPlugin, this.plugins, jeiHelpers, this.data.serverConnection()
            );
            ...
            RuntimeRegistration runtimeRegistration = new RuntimeRegistration(
               recipeManager, jeiHelpers, editModeConfig, ingredientManager, recipeTransferManager, screenHelper, searchStorageBuilderFactory
            );
   ...
   private void verifyClientRecipes(Minecraft minecraft) {
      ...
      if (Internal.hasClientSyncedRecipes() && clientRecipes.isEmpty()) {
         ...
         if (!serverConnection.isJeiOnServer() && serverConnection.isSameModLoader()) {
            ...
            writeChatMessage(minecraft, Component.translatable(key, new Object[]{serverBrand}).withStyle(ChatFormatting.RED));
            LOGGER.warn(Translator.translateToLocalFormatted(key, serverBrand));
         } else if (ClientConnectionHelper.hasServerBrand("vanilla")) {
