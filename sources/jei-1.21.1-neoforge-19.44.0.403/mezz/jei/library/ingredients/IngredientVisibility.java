...
public class IngredientVisibility implements IIngredientVisibility {
   ...
   private final List<IIngredientVisibility.IListener> listeners = new ArrayList<>();

   public IngredientVisibility(
      IngredientBlacklistInternal blacklist, IClientToggleState toggleState, EditModeConfig editModeConfig, IIngredientManager ingredientManager
   ) {
   ...
   public <V> boolean isIngredientVisible(IIngredientType<V> ingredientType, V ingredient) {
      ...
      return typedIngredient == null ? false : this.isIngredientVisible(typedIngredient, ingredientHelper);
   }
   ...
   public <V> boolean isIngredientVisible(ITypedIngredient<V> typedIngredient, IIngredientHelper<V> ingredientHelper) {
      if (this.blacklist.isIngredientBlacklistedByApi(typedIngredient, ingredientHelper)) {
         ...
         return ingredientHelper.isHiddenFromRecipeViewersByTags(typedIngredient)
            ? false
            : this.toggleState.isEditModeEnabled() || !this.editModeConfig.isIngredientHiddenUsingConfigFile(typedIngredient);
      }
   }
