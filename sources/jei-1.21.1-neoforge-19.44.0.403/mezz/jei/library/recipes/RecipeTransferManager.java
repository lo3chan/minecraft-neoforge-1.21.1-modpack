...
public class RecipeTransferManager implements IRecipeTransferManager {
   ...
   private <C extends AbstractContainerMenu, R> Optional<IRecipeTransferHandler<C, R>> getHandler(
      Class<? extends C> containerClass, @Nullable MenuType<C> menuType, RecipeType<?> recipeType
   ) {
      ...
      if (handler == null) {
         ...
         return !handlerMenuType.isEmpty() && !handlerMenuType.get().equals(menuType) ? Optional.empty() : Optional.of((IRecipeTransferHandler<C, R>)handler);
      }
   }
}
