...
public class DungeonCategory extends BlankJEIRecipeCategory<DungeonEntry> {
   ...
   public void setRecipe(@NotNull IRecipeLayoutBuilder builder, @NotNull DungeonEntry recipe, @NotNull IFocusGroup focuses) {
      int x = 6;
      int y = 44;
      int slots = Math.min(recipe.amountOfItems((IFocus<ItemStack>)focuses.getFocuses(VanillaTypes.ITEM_STACK).findFirst().orElse(null)), ITEMS_PER_PAGE);

      for (int i = 0; i < slots; i++) {
         builder.addSlot(RecipeIngredientRole.OUTPUT, x, y)
            .addTooltipCallback(new DungeonTooltip(recipe))
            .addItemStacks(recipe.getItems((IFocus<ItemStack>)focuses.getFocuses(VanillaTypes.ITEM_STACK).findFirst().orElse(null), i, slots));
         x += SPACING_X;
