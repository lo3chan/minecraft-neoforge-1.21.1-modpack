...
public class LootTableHelper {
   ...
   public static List<LootDrop> toDrops(LootTable table) {
      ...
      LootTableFetcher lootTableFetcher = getLootTableFetcher();
      getPools(table)
         .forEach(
            pool -> {
               float totalWeight = getLootEntries(pool)
                  .stream()
               ...
               getLootEntries(pool)
                  ...
                  .map(entry -> new LootDrop((Item)entry.item.value(), entry.weight / totalWeight, entry.conditions, entry.functions))
               ...
               getLootEntries(pool)
                  ...
                  .map(entry -> toDrops(lootTableFetcher.getLootTable(ResourceKey.create(Registries.LOOT_TABLE, entry.name))))
                  .forEach(drops::addAll);
            }
         );
   ...
   public static Map<ResourceKey<LootTable>, Supplier<LivingEntity>> getAllMobLootTables() {
      ...
      for (Entry<DyeColor, ResourceKey<LootTable>> entry : sheepColors.entrySet()) {
