...
public class MapKeys {
   ...
   public static String getKey(BlockState state, ServerLevel serverLevel, BlockPos pos) {
      ...
      if (!state.isRandomlyTicking()) {
         try {
            return (String)keyCache.get(state, () -> getKeyUncached(block, state, serverLevel, pos));
         } catch (ExecutionException var5) {
            LogHelper.error("Cache error", var5);
         }
      }
   ...
   private static String getKeyUncached(Block block, BlockState state, ServerLevel serverLevel, BlockPos pos) {
      ...
      return pickBlock != null && pickBlock.getItem() != null ? getKey(pickBlock) : block.getDescriptionId();
   }
