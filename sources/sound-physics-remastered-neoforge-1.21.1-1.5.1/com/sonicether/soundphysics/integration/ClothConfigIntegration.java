...
public class ClothConfigIntegration {
   public static Screen createConfigScreen(Screen parent) {
      ConfigBuilder builder = ConfigBuilder.create().setParentScreen(parent).setTitle(Component.translatable("cloth_config.sound_physics_remastered.settings"));
      ...
      for (Entry<BlockDefinition, Float> entry : SoundPhysicsMod.REFLECTIVITY_CONFIG.getBlockDefinitions().entrySet()) {
         FloatListEntry e = ((FloatFieldBuilder)entryBuilder.startFloatField(entry.getKey().getName(), entry.getValue())
               .setMin(0.01F)
               .setMax(10.0F)
               .setDefaultValue(defaultReflectivityMap.getOrDefault(entry.getKey(), SoundPhysicsMod.CONFIG.defaultBlockReflectivity.get())))
            .setSaveConsumer(value -> SoundPhysicsMod.REFLECTIVITY_CONFIG.setBlockDefinitionValue(entry.getKey(), value))
            .build();
      ...
      for (Entry<BlockDefinition, Float> entry : SoundPhysicsMod.OCCLUSION_CONFIG.getBlockDefinitions().entrySet()) {
         FloatListEntry e = ((FloatFieldBuilder)entryBuilder.startFloatField(entry.getKey().getName(), entry.getValue())
               .setMin(0.0F)
               .setMax(10.0F)
               .setDefaultValue(defaultOcclusionMap.getOrDefault(entry.getKey(), SoundPhysicsMod.CONFIG.defaultBlockOcclusionFactor.get())))
            .setSaveConsumer(value -> SoundPhysicsMod.OCCLUSION_CONFIG.setBlockDefinitionValue(entry.getKey(), value))
            .build();
   ...
   private static <T> AbstractConfigListEntry<T> fromConfigEntry(ConfigEntryBuilder entryBuilder, Component name, Component description, ConfigEntry<T> entry) {
      if (entry instanceof DoubleConfigEntry e) {
         return ((DoubleFieldBuilder)((DoubleFieldBuilder)entryBuilder.startDoubleField(name, e.get())
                  .setTooltip(new Component[]{description})
                  .setMin(e.getMin()))
               .setMax(e.getMax()))
         ...
         return ((FloatFieldBuilder)((FloatFieldBuilder)entryBuilder.startFloatField(name, e.get()).setTooltip(new Component[]{description}).setMin(e.getMin()))
               .setMax(e.getMax()))
            .setDefaultValue(e::getDefault)
            .setSaveConsumer(d -> e.set(d))
            .build();
      } else if (entry instanceof IntegerConfigEntry e) {
         return ((IntFieldBuilder)((IntFieldBuilder)entryBuilder.startIntField(name, e.get()).setTooltip(new Component[]{description}).setMin(e.getMin()))
               .setMax(e.getMax()))
            .setDefaultValue(e::getDefault)
            .setSaveConsumer(i -> e.set(i))
            .build();
      } else if (entry instanceof BooleanConfigEntry e) {
         return entryBuilder.startBooleanToggle(name, e.get())
            .setTooltip(new Component[]{description})
            .setDefaultValue(e::getDefault)
            .setSaveConsumer(b -> e.set(b))
            .build();
      } else {
         return entry instanceof StringConfigEntry e
            ? entryBuilder.startStrField(name, e.get())
               .setTooltip(new Component[]{description})
               .setDefaultValue(e::getDefault)
