...
public class ETFArmorHandler {
   ...
   public void renderBaseEmissive(PoseStack matrices, MultiBufferSource vertexConsumers, Model model, float red, float green, float blue) {
      if (this.texture != null && ETF.config().getConfig().canDoEmissiveTextures()) {
         ResourceLocation emissive = this.texture.getEmissiveIdentifierOfCurrentState();
         if (emissive != null) {
            ...
            model.renderToBuffer(matrices, textureVert, 15728882, OverlayTexture.NO_OVERLAY);
            ETFRenderContext.startSpecialRenderOverlayPhase();
         }
      }
   }
   ...
   public void renderTrimEmissive(PoseStack matrices, MultiBufferSource vertexConsumers, Model model) {
      if (this.trimTexture != null && ETF.config().getConfig().canDoEmissiveTextures()) {
         ResourceLocation emissive = this.trimTexture.getEmissiveIdentifierOfCurrentState();
         if (emissive != null) {
            VertexConsumer textureVert = vertexConsumers.getBuffer(RenderType.armorCutoutNoCull(emissive));
            ETFRenderContext.startSpecialRenderOverlayPhase();
            if (ETF.IRIS_DETECTED) {
