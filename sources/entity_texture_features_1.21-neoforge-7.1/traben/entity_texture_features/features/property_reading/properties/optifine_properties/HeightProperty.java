...
public class HeightProperty extends SimpleIntegerArrayProperty {
   protected HeightProperty(Properties properties, int propertyNum) throws RandomProperty.RandomPropertyException {
      super(getGenericIntegerSplitWithRanges(properties, propertyNum, new String[]{"heights", "height"}));
   }
   ...
   public static HeightProperty getPropertyOrNull(Properties properties, int propertyNum) {
      try {
         if (!properties.containsKey("heights." + propertyNum)
            && !properties.containsKey("height." + propertyNum)
      ...
      } catch (RandomProperty.RandomPropertyException var4) {
         return null;
      }
   }
