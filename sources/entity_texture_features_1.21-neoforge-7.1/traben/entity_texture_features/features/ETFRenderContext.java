...
public class ETFRenderContext {
   ...
   public static boolean canRenderInBrightMode() {
      ...
      if (setForBrightMode) {
         return currentEntity != null ? currentEntity.canRenderBright() : true;
      } else {
         return false;
      }
   }
   ...
   public static boolean shouldEmissiveUseCullingLayer() {
      return currentEntity != null ? currentEntity.isBlockEntity() : true;
   }
   ...
   public static RenderType modifyRenderLayerIfRequired(RenderType value) {
      if (isCurrentlyRenderingEntity() && isAllowedToRenderLayerTextureModify()) {
         ETFConfig.RenderLayerOverride layer = ETF.config().getConfig().getRenderLayerOverride();
         if (layer != null && !value.isOutline() && value instanceof ETFRenderLayerWithTexture multiphase) {
