...
public record JanksonJsonOps(boolean compressed) implements DynamicOps<JsonElement> {
         ...
         if (value instanceof String) {
            ...
            try {
               ...
               if ((byte)l == l) {
                  ...
                  return (U)((int)l == l ? outOps.createInt((int)l) : outOps.createLong(l));
               }
            } catch (ArithmeticException var9) {
               double d = bigDecimal.doubleValue();
               return (U)((float)d == d ? outOps.createFloat((float)d) : outOps.createDouble(d));
            }
         }
      }
   }
   ...
   public DataResult<Number> getNumberValue(JsonElement input) {
      ...
      return input instanceof JsonPrimitive primitive && primitive.getValue() instanceof Boolean
         ? DataResult.success(primitive.getValue() ? 1 : 0)
   ...
   public DataResult<String> getStringValue(JsonElement input) {
      return !(
            input instanceof JsonPrimitive primitive && (primitive.getValue() instanceof String || primitive.getValue() instanceof Number && this.compressed)
         )
         ? DataResult.error(() -> "Not a string: " + input)
         : DataResult.success(primitive.asString());
   }
   ...
   public DataResult<JsonElement> mergeToMap(JsonElement map, JsonElement key, JsonElement value) {
      ...
      } else if (key instanceof JsonPrimitive primitive && (primitive.getValue() instanceof String || this.compressed)) {
      ...
      return !(input instanceof JsonObject)
         ...
         : DataResult.success(
            ((JsonObject)input)
               .entrySet()
               .stream()
               .map(entry -> Pair.of(new JsonPrimitive(entry.getKey()), entry.getValue() instanceof JsonNull ? null : entry.getValue()))
         );
   }

   public DataResult<Consumer<BiConsumer<JsonElement, JsonElement>>> getMapEntries(JsonElement input) {
      return !(input instanceof JsonObject) ? DataResult.error(() -> "Not a JSON object: " + input) : DataResult.success((Consumer<BiConsumer>)c -> {
   ...
   public DataResult<MapLike<JsonElement>> getMap(JsonElement input) {
      return input instanceof JsonObject object ? DataResult.success(new CommentsTrackerMapLike<JsonElement>() {
         @Nullable
   ...
   public DataResult<Stream<JsonElement>> getStream(JsonElement input) {
      return input instanceof JsonArray
         ? DataResult.success(((JsonArray)input).stream().map(e -> (JsonElement)(e instanceof JsonNull ? null : e)))
         : DataResult.error(() -> "Not a json array: " + input);
   }

   public DataResult<Consumer<Consumer<JsonElement>>> getList(JsonElement input) {
      return input instanceof JsonArray ? DataResult.success((Consumer<Consumer>)c -> {
   ...
   public ListBuilder<JsonElement> listBuilder() {
      return new JanksonJsonOps.ArrayBuilder();
   }
   ...
   public RecordBuilder<JsonElement> mapBuilder() {
      return new JanksonJsonOps.JsonRecordBuilder();
   }
   ...
   private class JsonRecordBuilder extends AbstractStringBuilder<JsonElement, JsonObject> implements CommentsTracker {
         ...
         if (prefix != null && !(prefix instanceof JsonNull)) {
            if (prefix instanceof JsonObject prefixObject) {
               ...
               for (Entry<String, JsonElement> entryx : builder.entrySet()) {
                  ...
                  result.put(key, entryx.getValue());
                  if (this.comments.containsKey(key)) {
                     result.setComment(key, this.comments.get(key));
                  }
               }
