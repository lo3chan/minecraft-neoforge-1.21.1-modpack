...
public class TimeOfDayCondition implements Condition {
   public static final Codec<TimeOfDayCondition> CODEC = RecordCodecBuilder.create(
      builder -> builder.group(
            LongPair.createLongPairCodec("min_time", "max_time")
               .listOf()
               .fieldOf("times_of_day")
               .forGetter(timeOfDayCondition -> timeOfDayCondition.timesOfDay),
   ...
   public TimeOfDayCondition(List<LongPair> timesOfDay, long dayLength) {
      ...
      if (timesOfDay.isEmpty()) {
         ...
         for (LongPair longPair : this.timesOfDay) {
            if (longPair.getVal1() > this.dayLength
               || longPair.getVal1() < 0L
               || longPair.getVal2() > this.dayLength
               || longPair.getVal2() < 0L
               || longPair.getVal1() > longPair.getVal2()) {
               throw new IllegalArgumentException(
                  "minTime & maxTime must be between 0 & " + dayLength + " and minTime cannot be greater than maxTime! Pair: \"" + longPair + "\" failed."
               );
            }
         }
      }
