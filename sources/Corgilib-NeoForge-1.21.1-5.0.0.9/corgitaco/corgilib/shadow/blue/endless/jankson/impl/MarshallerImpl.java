...
import corgitaco.corgilib.shadow.blue.endless.jankson.Comment;
...
import corgitaco.corgilib.shadow.blue.endless.jankson.annotation.SerializedName;
import corgitaco.corgilib.shadow.blue.endless.jankson.annotation.Serializer;
...
import corgitaco.corgilib.shadow.blue.endless.jankson.impl.serializer.DeserializerFunctionPool;
import corgitaco.corgilib.shadow.blue.endless.jankson.magic.TypeMagic;
...
public class MarshallerImpl implements Marshaller {
   ...
   public <T> void registerSerializer(Class<T> clazz, Function<T, JsonElement> serializer) {
      this.serializers.put(clazz, (it, marshaller) -> serializer.apply((T)it));
   }
   ...
   public MarshallerImpl() {
      ...
      this.registerSerializer(Character.class, it -> new JsonPrimitive("" + it));
      this.registerSerializer(String.class, JsonPrimitive::new);
      this.registerSerializer(Byte.class, it -> new JsonPrimitive((long)it.byteValue()));
      this.registerSerializer(Short.class, it -> new JsonPrimitive((long)it.shortValue()));
      this.registerSerializer(Integer.class, it -> new JsonPrimitive((long)it.intValue()));
      this.registerSerializer(Long.class, JsonPrimitive::new);
      this.registerSerializer(Float.class, it -> new JsonPrimitive((double)it.floatValue()));
      this.registerSerializer(Double.class, JsonPrimitive::new);
      this.registerSerializer(Boolean.class, JsonPrimitive::new);
      this.registerSerializer(void.class, it -> JsonNull.INSTANCE);
      this.registerSerializer(char.class, it -> new JsonPrimitive("" + it));
      this.registerSerializer(byte.class, it -> new JsonPrimitive((long)it.byteValue()));
      this.registerSerializer(short.class, it -> new JsonPrimitive((long)it.shortValue()));
      this.registerSerializer(int.class, it -> new JsonPrimitive((long)it.intValue()));
      this.registerSerializer(long.class, JsonPrimitive::new);
      this.registerSerializer(float.class, it -> new JsonPrimitive((double)it.floatValue()));
      this.registerSerializer(double.class, JsonPrimitive::new);
      this.registerSerializer(boolean.class, JsonPrimitive::new);
            ...
            if (clazz.equals(String.class)) {
               if (elem instanceof JsonObject) {
                  ...
                  throw new DeserializationException(
                     "Encountered unexpected JsonElement type while deserializing to string: " + elem.getClass().getCanonicalName()
                  );
               } else {
                  return null;
               }
               ...
               if (func != null) {
                  ...
                  throw new DeserializationException(
                     "Don't know how to unpack value '" + elem.toString() + "' into target type '" + clazz.getCanonicalName() + "'"
                  );
               } else {
                  return null;
               }
               ...
               if (clazz.isPrimitive()) {
                  ...
                  if (this.typeAdapters.containsKey(clazz)) {
                     ...
                     T result = (T)this.typeFactories.get(clazz).get();
         ...
         if (serializer != null) {
            ...
            for (Entry<Class<?>, BiFunction<Object, Marshaller, JsonElement>> entry : this.serializers.entrySet()) {
               if (entry.getKey().isAssignableFrom(obj.getClass())) {
                  JsonElement resultx = entry.getValue().apply(obj, this);
                  ...
                  if (JsonElement.class.isAssignableFrom(clazz)) {
                     ...
                     if (params.length == 0) {
                        try {
                           ...
                           JsonElement resultxx = (JsonElement)m.invoke(obj);
                           if (!access) {
                              m.setAccessible(false);
                           }

                           if (resultxx instanceof JsonObject) {
                              ((JsonObject)resultxx).setMarshaller(this);
                           }

                           if (resultxx instanceof JsonArray) {
                              ((JsonArray)resultxx).setMarshaller(this);
                           }
                     ...
                     if (params.length == 1 && Marshaller.class.isAssignableFrom(params[0].getType())) {
                        try {
                           boolean accessx = m.isAccessible();
                           if (!accessx) {
                              m.setAccessible(true);
                           }

                           JsonElement resultxxx = (JsonElement)m.invoke(obj, this);
                           if (!accessx) {
                              m.setAccessible(false);
                           }

                           if (resultxxx instanceof JsonObject) {
                              ((JsonObject)resultxxx).setMarshaller(this);
                           }

                           if (resultxxx instanceof JsonArray) {
                              ((JsonArray)resultxxx).setMarshaller(this);
                           }
            ...
            if (obj instanceof Enum) {
               ...
               JsonObject resultxxxx = new JsonObject();
               ...
               for (Field f : obj.getClass().getFields()) {
                  if (!Modifier.isStatic(f.getModifiers()) && !Modifier.isTransient(f.getModifiers())) {
                     ...
                     try {
                        ...
                        if (comment == null) {
                           resultxxxx.put(name, this.serialize(child));
                        } else {
                           resultxxxx.put(name, this.serialize(child), comment.value());
                        }
                     } catch (IllegalAccessException | IllegalArgumentException var15) {
                     }
                  }
               }
               ...
               for (Field fx : obj.getClass().getDeclaredFields()) {
                  if (!Modifier.isPublic(fx.getModifiers()) && !Modifier.isStatic(fx.getModifiers()) && !Modifier.isTransient(fx.getModifiers())) {
                     ...
                     try {
                        ...
                        SerializedName nameAnnotationx = fx.getAnnotation(SerializedName.class);
                        if (nameAnnotationx != null) {
                           namex = nameAnnotationx.value();
                        }

                        Comment comment = fx.getAnnotation(Comment.class);
                        if (comment == null) {
                           resultxxxx.put(namex, this.serialize(childx));
                        } else {
                           resultxxxx.put(namex, this.serialize(childx), comment.value());
                        }
               ...
               for (Entry<?, ?> entryx : ((Map)obj).entrySet()) {
                  ...
                  resultxxxx.put(k, this.serialize(v));
               }
