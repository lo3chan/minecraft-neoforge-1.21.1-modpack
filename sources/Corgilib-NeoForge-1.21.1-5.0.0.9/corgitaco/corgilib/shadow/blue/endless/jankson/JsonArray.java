...
import corgitaco.corgilib.shadow.blue.endless.jankson.impl.serializer.CommentSerializer;
...
public class JsonArray extends JsonElement implements List<JsonElement>, Iterable<JsonElement> {
   ...
   public void toJson(Writer writer, JsonGrammar grammar, int depth) throws IOException {
      ...
      for (int i = 0; i < this.entries.size(); i++) {
         JsonArray.Entry entry = this.entries.get(i);
         if (grammar.printWhitespace) {
      ...
      if (other != null && other instanceof JsonArray) {
         ...
         if (a.size() != b.size()) {
            ...
            for (int i = 0; i < a.size(); i++) {
               JsonArray.Entry ae = a.get(i);
               JsonArray.Entry be = b.get(i);
   ...
   public boolean remove(Object o) {
      for (int i = 0; i < this.entries.size(); i++) {
         JsonArray.Entry cur = this.entries.get(i);
   ...
   public int indexOf(Object obj) {
      if (obj == null) {
         ...
         for (int i = 0; i < this.entries.size(); i++) {
            JsonElement val = this.entries.get(i).value;
   ...
   public JsonElement set(int index, JsonElement element) {
      JsonArray.Entry cur = new JsonArray.Entry(element);
      JsonArray.Entry old = this.entries.get(index);
      if (old != null) {
