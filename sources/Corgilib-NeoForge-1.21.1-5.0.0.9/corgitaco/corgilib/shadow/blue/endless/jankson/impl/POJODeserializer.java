...
import corgitaco.corgilib.shadow.blue.endless.jankson.annotation.Deserializer;
...
import corgitaco.corgilib.shadow.blue.endless.jankson.impl.serializer.InternalDeserializerFunction;
      ...
      if (source.containsKey(fieldName)) {
         ...
         if (elem != null && elem != JsonNull.INSTANCE) {
            try {
               ...
               if (failFast) {
                  throw new DeserializationException(
                     "There was a problem unpacking field " + f.getName() + " of class " + parent.getClass().getCanonicalName(), var9
                  );
               }
            }
         } else {
            ...
            try {
               ...
               if (failFast) {
                  throw new DeserializationException(
                     "Couldn't set field \"" + f.getName() + "\" of class \"" + parent.getClass().getCanonicalName() + "\"", var10
                  );
               }
            }
         }
   ...
   private static <A, B> InternalDeserializerFunction<B> makeDeserializer(@Nonnull Method m, @Nonnull Class<A> sourceClass, @Nonnull Class<B> targetClass) {
      if (!m.getReturnType().equals(targetClass)) {
         ...
         if (params.length == 1) {
            ...
            return params[1].getType().equals(Marshaller.class) ? (o, marshaller) -> {
               try {
