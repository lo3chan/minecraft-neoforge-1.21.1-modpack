...
public class KeyframeAnimationPlayer implements IActualAnimation<KeyframeAnimationPlayer> {
   ...
   public KeyframeAnimationPlayer(@NotNull KeyframeAnimation animation, int t, boolean mutable) {
      if (animation == null) {
         ...
         for (Entry<String, KeyframeAnimation.StateCollection> part : animation.getBodyParts().entrySet()) {
            this.bodyParts.put(part.getKey(), new KeyframeAnimationPlayer.BodyPart(mutable ? part.getValue().copy() : part.getValue()));
         }
   ...
   public Vec3f get3DTransform(@NotNull String modelName, @NotNull TransformType type, float tickDelta, @NotNull Vec3f value0) {
      KeyframeAnimationPlayer.BodyPart part = this.bodyParts.get(modelName);
      if (part == null) {
         return value0;
      } else {
         switch (type) {
   ...
   public KeyframeAnimationPlayer.BodyPart getPart(String string) {
      KeyframeAnimationPlayer.BodyPart part = this.bodyParts.get(string);
   ...
   public class Axis {
      ...
      private KeyframeAnimation.KeyFrame findBefore(int pos, float currentState) {
         if (pos == -1) {
            return KeyframeAnimationPlayer.this.currentTick < KeyframeAnimationPlayer.this.data.beginTick
               ? new KeyframeAnimation.KeyFrame(0, currentState)
               : (
                  KeyframeAnimationPlayer.this.currentTick < KeyframeAnimationPlayer.this.data.endTick
                     ? new KeyframeAnimation.KeyFrame(KeyframeAnimationPlayer.this.data.beginTick, this.keyframes.defaultValue)
                     : new KeyframeAnimation.KeyFrame(KeyframeAnimationPlayer.this.data.endTick, this.keyframes.defaultValue)
               );
         } else {
            KeyframeAnimation.KeyFrame frame = this.keyframes.getKeyFrames().get(pos);
            return !KeyframeAnimationPlayer.this.isInfinite()
                  && KeyframeAnimationPlayer.this.currentTick >= KeyframeAnimationPlayer.this.getData().endTick
                  && pos == this.keyframes.length() - 1
                  && frame.tick < KeyframeAnimationPlayer.this.getData().endTick
               ? new KeyframeAnimation.KeyFrame(KeyframeAnimationPlayer.this.getData().endTick, frame.value, frame.ease, frame.easingArg)
               : frame;
         }
      }
      ...
      private KeyframeAnimation.KeyFrame findAfter(int pos, float currentState) {
         if (this.keyframes.length() > pos + 1) {
            ...
            return KeyframeAnimationPlayer.this.currentTick >= KeyframeAnimationPlayer.this.data.endTick
               ? new KeyframeAnimation.KeyFrame(KeyframeAnimationPlayer.this.data.stopTick, currentState)
               : (
                  KeyframeAnimationPlayer.this.currentTick >= KeyframeAnimationPlayer.this.getData().beginTick
                     ? new KeyframeAnimation.KeyFrame(KeyframeAnimationPlayer.this.getData().endTick, this.keyframes.defaultValue)
                     : new KeyframeAnimation.KeyFrame(KeyframeAnimationPlayer.this.getData().beginTick, this.keyframes.defaultValue)
               );
         }
      }
      ...
      protected final float getValueFromKeyframes(KeyframeAnimation.KeyFrame before, KeyframeAnimation.KeyFrame after) {
         ...
         if (tickBefore == tickAfter) {
            ...
            float f = (KeyframeAnimationPlayer.this.currentTick + KeyframeAnimationPlayer.this.tickDelta - tickBefore) / (tickAfter - tickBefore);
   ...
   public class BodyPart {
      ...
      public final KeyframeAnimationPlayer.Axis scaleX;
      public final KeyframeAnimationPlayer.Axis scaleY;
      public final KeyframeAnimationPlayer.Axis scaleZ;
      ...
      public BodyPart(@Nullable KeyframeAnimation.StateCollection part) {
         ...
         if (part != null) {
            ...
            this.scaleX = KeyframeAnimationPlayer.this.new Axis(part.scaleX);
            this.scaleY = KeyframeAnimationPlayer.this.new Axis(part.scaleY);
            this.scaleZ = KeyframeAnimationPlayer.this.new Axis(part.scaleZ);
         } else {
            this.x = null;
            this.y = null;
            this.z = null;
      ...
      public Pair<Float, Float> getBend(Pair<Float, Float> value0) {
         return this.bend == null
            ? value0
            : new Pair<>(this.bendAxis.getValueAtCurrentTick(value0.getLeft()), this.bend.getValueAtCurrentTick(value0.getRight()));
      }
      ...
      public Vec3f getBodyRotation(Vec3f value0) {
         return this.part == null
            ? value0
            : new Vec3f(
               this.pitch.getValueAtCurrentTick(value0.getX()), this.yaw.getValueAtCurrentTick(value0.getY()), this.roll.getValueAtCurrentTick(value0.getZ())
            );
      }
   }
