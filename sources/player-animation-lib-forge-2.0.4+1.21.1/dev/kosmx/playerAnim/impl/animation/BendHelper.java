...
public class BendHelper implements IBendHelper {
   ...
   public void bend(ModelPart modelPart, float axis, float rotation) {
      if (Math.abs(rotation) >= 1.0E-4F) {
         ModelPartAccessor.optionalGetCuboid(modelPart, 0)
            .ifPresent(mutableCuboid -> ((BendableCuboid)mutableCuboid.getAndActivateMutator("bend")).applyBend(axis, rotation));
      } else {
   ...
   public void initBend(ModelPart modelPart, Direction direction) {
      ModelPartAccessor.optionalGetCuboid(modelPart, 0)
         .ifPresent(mutableModelPart -> mutableModelPart.registerMutator("bend", data -> new Builder().setDirection(direction).build(data)));
   }
   ...
   public void initCapeBend(ModelPart modelPart) {
      ModelPartAccessor.optionalGetCuboid(modelPart, 0).ifPresent(mutableModelPart -> mutableModelPart.registerMutator("bend", data -> {
         data.pivot = 6;
         return new Builder().setDirection(Direction.UP).build(data);
      }));
   }
}
