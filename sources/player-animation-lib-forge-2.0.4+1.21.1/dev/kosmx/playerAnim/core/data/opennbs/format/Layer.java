...
public class Layer {
   ...
   public int findAtTick(int tick) {
      int i = -1;
      if (this.notes.size() > this.lastUsedTickPos + 1 && this.notes.get(this.lastUsedTickPos + 1).tick <= tick) {
   ...
   public Layer.Note addNote(int tick) {
      if (this.getLock()) {
         ...
         if (i > 0 && this.notes.get(i).tick == tick) {
            return null;
         } else {
   ...
   public class Note {
      ...
      public float getPitch() {
         return (float)Math.pow(2.0, (this.key + this.pitch / 100 - 45) / 12.0);
      }
