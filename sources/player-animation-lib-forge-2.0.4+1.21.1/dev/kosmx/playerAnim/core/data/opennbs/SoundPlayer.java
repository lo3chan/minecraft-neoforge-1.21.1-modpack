...
public class SoundPlayer {
   ...
   public SoundPlayer(NBS song, Consumer<Layer.Note> soundPlayer, int tickToBegin) {
      this.song = song;
      this.songPerMCTick = song.header.Song_tempo / 2000.0F;
      this.playSound = soundPlayer;
      this.mcTick = tickToBegin;
   }

   public void tick() {
      int newSongTick = (int)(this.mcTick++ * this.songPerMCTick);
      if (newSongTick > this.song.header.Loop_start_tick && this.song.header.Loop_on_off()) {
         ...
         newSongTick = (newSongTick - this.song.header.Loop_start_tick) % (this.song.getLength() - this.song.header.Loop_start_tick)
            + this.song.header.Loop_start_tick;
