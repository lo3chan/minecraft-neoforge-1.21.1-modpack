...
public class AnimationJson implements JsonDeserializer<List<KeyframeAnimation>>, JsonSerializer<KeyframeAnimation> {
   ...
   public List<KeyframeAnimation> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
      ...
      if (!node.has("emote")) {
         return GeckoLibSerializer.deserialize(node);
      } else {
         int version = 1;
         if (node.has("version")) {
      ...
      for (JsonElement n : node) {
         ...
         for (Entry<String, JsonElement> entry : obj.entrySet()) {
            if (!entry.getKey().equals("tick") && !entry.getKey().equals("comment") && !entry.getKey().equals("easing") && !entry.getKey().equals("turn")) {
               this.addBodyPartIfExists(emote, entry.getKey(), entry.getValue(), degrees, tick, easing, easingArg, turn, version);
            }
         }
      }
   }

   private void addBodyPartIfExists(
      KeyframeAnimation.AnimationBuilder emote, String name, JsonElement node, boolean degrees, int tick, String easing, int turn, int version
   ) {
      this.addBodyPartIfExists(emote, name, node, degrees, tick, easing, null, turn, version);
   }

   private void addBodyPartIfExists(
      KeyframeAnimation.AnimationBuilder emote, String name, JsonElement node, boolean degrees, int tick, String easing, Float easingArg, int turn, int version
   ) {
   ...
   private void addPartIfExists(KeyframeAnimation.StateCollection.State part, String name, JsonObject node, boolean degrees, int tick, String easing, int turn) {
      this.addPartIfExists(part, name, node, degrees, tick, easing, null, turn);
   }

   private void addPartIfExists(
      KeyframeAnimation.StateCollection.State part, String name, JsonObject node, boolean degrees, int tick, String easing, Float easingArg, int turn
   ) {
      if (node.has(name)) {
   ...
   static {
      ...
      builder.registerTypeAdapter(getListedTypeToken(), animationJson);
