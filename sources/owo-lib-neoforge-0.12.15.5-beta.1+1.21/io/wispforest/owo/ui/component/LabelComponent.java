...
public class LabelComponent extends BaseComponent {
   ...
   protected final Observable<Integer> lineHeight = Observable.of(9);
      ...
      context.drawManaged(() -> {
         for (int i = 0; i < this.wrappedText.size(); i++) {
            ...
            switch (this.horizontalTextAlignment) {
               case CENTER:
                  renderX = lambdaX + (this.width - this.textRenderer.width(renderText)) / 2;
                  break;
               case RIGHT:
                  renderX = lambdaX + (this.width - this.textRenderer.width(renderText));
            }

            int renderY = lambdaY + i * (this.lineHeight() + this.lineSpacing());
            renderY += this.lineHeight() - 9;
   ...
   public boolean shouldDrawTooltip(double mouseX, double mouseY) {
      Style hoveredStyle = this.styleAt((int)(mouseX - this.x), (int)(mouseY - this.y));
   ...
   protected Style styleAt(int mouseX, int mouseY) {
      return this.textRenderer
         .getSplitter()
         .componentStyleAtWidth(this.wrappedText.get(Math.min(mouseY / (this.lineHeight() + this.lineSpacing()), this.wrappedText.size() - 1)), mouseX);
   }
