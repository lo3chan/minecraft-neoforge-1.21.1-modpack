...
public class VanillaWidgetComponent extends BaseComponent {
   ...
   protected int determineVerticalContentSize(Sizing sizing) {
      if (!(this.widget instanceof Button) && !(this.widget instanceof Checkbox) && !(this.widget instanceof SliderComponent)) {
         if (this.widget instanceof EditBox textField) {
            return ((TextFieldWidgetAccessor)textField).owo$drawsBackground() ? 20 : 9;
         } else if (this.widget instanceof TextAreaComponent textArea && textArea.maxLines() > 0) {
            return Mth.clamp(textArea.getInnerHeight() / 9 + 1, 2, textArea.maxLines()) * 9 + (textArea.displayCharCount() ? 21 : 9);
         } else {
   ...
   private void applyToWidget() {
      ClickableWidgetAccessor accessor = (ClickableWidgetAccessor)this.widget;
   ...
   public void draw(OwoUIDrawContext context, int mouseX, int mouseY, float partialTicks, float delta) {
      this.widget.render(context, mouseX, mouseY, 0.0F);
   }
   ...
   public boolean onMouseDown(double mouseX, double mouseY, int button) {
      return this.widget.mouseClicked(this.x + mouseX, this.y + mouseY, button) | super.onMouseDown(mouseX, mouseY, button);
   }

   @Override
   public boolean onMouseUp(double mouseX, double mouseY, int button) {
      return this.widget.mouseReleased(this.x + mouseX, this.y + mouseY, button) | super.onMouseUp(mouseX, mouseY, button);
   }

   @Override
   public boolean onMouseScroll(double mouseX, double mouseY, double amount) {
      return this.widget.mouseScrolled(this.x + mouseX, this.y + mouseY, 0.0, amount) | super.onMouseScroll(mouseX, mouseY, amount);
   }

   @Override
   public boolean onMouseDrag(double mouseX, double mouseY, double deltaX, double deltaY, int button) {
      return this.widget.mouseDragged(this.x + mouseX, this.y + mouseY, button, deltaX, deltaY) | super.onMouseDrag(mouseX, mouseY, deltaX, deltaY, button);
   }
