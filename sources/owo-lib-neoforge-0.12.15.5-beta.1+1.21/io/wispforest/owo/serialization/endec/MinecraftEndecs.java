...
public final class MinecraftEndecs {
   ...
   public static final Endec<BlockPos> BLOCK_POS = Endec.ifAttr(
         SerializationAttributes.HUMAN_READABLE, vectorEndec("BlockPos", Endec.INT, BlockPos::new, Vec3i::getX, Vec3i::getY, Vec3i::getZ)
      )
      .orElse(Endec.LONG.xmap(BlockPos::of, BlockPos::asLong));
   public static final Endec<ChunkPos> CHUNK_POS = Endec.ifAttr(SerializationAttributes.HUMAN_READABLE, Endec.INT.listOf().validate(ints -> {
         if (ints.size() != 2) {
            throw new IllegalStateException("ChunkPos array must have two elements");
         }
      }).xmap(ints -> new ChunkPos((Integer)ints.get(0), (Integer)ints.get(1)), chunkPos -> List.of(chunkPos.x, chunkPos.z)))
   ...
   public static final Endec<BlockHitResult> BLOCK_HIT_RESULT = StructEndecBuilder.of(
      VEC3D.fieldOf("pos", HitResult::getLocation),
      ...
      Endec.BOOLEAN.fieldOf("missed", $ -> $.getType() == Type.MISS),
      (pos, side, blockPos, insideBlock, missed) -> !missed ? new BlockHitResult(pos, side, blockPos, insideBlock) : BlockHitResult.miss(pos, side, blockPos)
   );
   ...
   public static <T> Endec<TagKey<T>> prefixedTagKey(ResourceKey<? extends Registry<T>> registry) {
      return Endec.STRING.xmap(s -> TagKey.create(registry, ResourceLocation.parse(s.substring(1))), tag -> "#" + tag.location());
   }

   private static <C, V> Endec<V> vectorEndec(
      String name, Endec<C> componentEndec, Function3<C, C, C, V> constructor, Function<V, C> xGetter, Function<V, C> yGetter, Function<V, C> zGetter
   ) {
      return componentEndec.listOf()
         ...
         .xmap(
            ...
            vector -> List.of(xGetter.apply((V)vector), yGetter.apply((V)vector), zGetter.apply((V)vector))
         );
   }
}
