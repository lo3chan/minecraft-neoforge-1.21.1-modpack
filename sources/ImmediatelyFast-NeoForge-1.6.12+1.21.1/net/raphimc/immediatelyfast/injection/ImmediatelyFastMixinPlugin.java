   ...
   public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
      if (!mixinClassName.startsWith(this.mixinPackage)) {
         ...
         if (!ImmediatelyFast.config.font_atlas_resizing && packageName.startsWith("font_atlas_resizing")) {
            ...
            return false;
         } else if (packageName.startsWith("core.compat.iris") && PlatformService.INSTANCE.getModVersion("iris").isEmpty()) {
            return false;
         } else if (packageName.startsWith("hud_batching.compat.appleskin") && PlatformService.INSTANCE.getModVersion("appleskin").isEmpty()) {
            return false;
         } else if (packageName.startsWith("hud_batching.compat.journeymap") && PlatformService.INSTANCE.getModVersion("journeymap").isEmpty()) {
            return false;
         } else {
            return packageName.startsWith("hud_batching.compat.xaerominimap") && PlatformService.INSTANCE.getModVersion("xaerominimap").isEmpty()
               ? false
               : !packageName.startsWith("hud_batching.compat.ftbchunks") || !PlatformService.INSTANCE.getModVersion("ftbchunks").isEmpty();
         }
      }
   }
