...
public class CuriosSelectorOptions {
   public static void register() {
      EntitySelectorOptions.register(
         "curios", CuriosSelectorOptions::curioArgument, entitySelectorParser -> true, Component.translatable("argument.entity.options.curios.description")
      );
   }
   ...
   private static boolean matches(Entity entity, Set<String> slots, int min, int max, CompoundTag inputStack, boolean invert, boolean exclusive) {
   ...
   private static boolean hasOnlySlot(Map<String, ICurioStacksHandler> curios, Set<String> slots, int max, boolean invert) {
      ...
      if (invert) {
         ...
         for (Entry<String, ICurioStacksHandler> entryx : curios.entrySet()) {
            if (matches(slots, max, entryx.getKey(), entryx.getValue())) {
               foundSlot = true;
            } else if (foundSlot) {
               return false;
            }
      ...
      if (invert) {
         ...
         for (Entry<String, ICurioStacksHandler> entryx : curios.entrySet()) {
            if (slots.isEmpty() || slots.contains(entryx.getKey())) {
               ICurioStacksHandler stacks = entryx.getValue();
               int limit = max == -1 ? stacks.getSlots() : Math.min(stacks.getSlots(), max);
