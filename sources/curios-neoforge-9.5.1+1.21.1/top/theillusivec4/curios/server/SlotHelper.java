...
public class SlotHelper implements ISlotHelper {
         ...
         .forEach(
            type -> curios.put(
               ...
               new CurioStacksHandler(
                  null, type.getIdentifier(), type.getSize(), type.isVisible(), type.hasCosmetic(), type.canToggleRendering(), type.getDropRule()
               )
            )
         );
      return curios;
   ...
   public SortedMap<ISlotType, ICurioStacksHandler> createSlots(LivingEntity livingEntity) {
      ...
      CuriosApi.getCuriosHelper()
         ...
         .ifPresent(
            handler -> this.getSlotTypes()
               .forEach(
                  type -> curios.put(
                     type,
                     new CurioStacksHandler(
                        handler, type.getIdentifier(), type.getSize(), type.isVisible(), type.hasCosmetic(), type.canToggleRendering(), type.getDropRule()
                     )
                  )
               )
         );
   ...
   public int getSlotsForType(@Nonnull LivingEntity livingEntity, String identifier) {
      return CuriosApi.getCuriosHelper()
         .getCuriosHandler(livingEntity)
         .map(handler -> handler.getStacksHandler(identifier).map(ICurioStacksHandler::getSlots).orElse(0))
         .orElse(0);
   }
   ...
   public void unlockSlotType(String id, LivingEntity livingEntity) {
      CuriosApi.getCuriosHelper().getCuriosHandler(livingEntity).ifPresent(handler -> {
         if (handler.getStacksHandler(id).map(stacksHandler -> stacksHandler.getSlots() == 0).orElse(false)) {
