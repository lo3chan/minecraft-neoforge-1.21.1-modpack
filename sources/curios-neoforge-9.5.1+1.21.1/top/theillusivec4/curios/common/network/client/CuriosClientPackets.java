...
public class CuriosClientPackets {
   public static void handle(SPacketSetIcons data) {
      ...
      if (world != null) {
         ...
         for (Entry<String, ResourceLocation> entry : data.map.entrySet()) {
   ...
   public static void handle(SPacketBreak data) {
      ClientLevel world = Minecraft.getInstance().level;
      if (world != null && world.getEntity(data.entityId()) instanceof LivingEntity livingEntity) {
               ...
               stacks -> {
                  ...
                  possibleCurio.ifPresent(
                     curio -> curio.curioBreak(
                        new SlotContext(
                           data.curioId(), livingEntity, data.slotId(), false, renderStates.size() > data.slotId() && (Boolean)renderStates.get(data.slotId())
                        )
                     )
                  );
      ...
      if (world != null) {
         ...
         if (entity instanceof LivingEntity livingEntity) {
            CuriosApi.getCuriosInventory(livingEntity).ifPresent(handler -> {
               ...
               for (Entry<String, CompoundTag> entry : data.updates.entrySet()) {
               ...
               if (!data.updates.isEmpty()) {
                  NeoForge.EVENT_BUS.post(new SlotModifiersUpdatedEvent(livingEntity, data.updates.keySet()));
               }
      ...
      if (world != null) {
         ...
         if (entity instanceof LivingEntity) {
            CuriosApi.getCuriosInventory((LivingEntity)entity).ifPresent(handler -> {
               ...
               for (Entry<String, CompoundTag> entry : data.map.entrySet()) {
               ...
               if (entity instanceof LocalPlayer localPlayer && localPlayer.containerMenu instanceof ICuriosMenu curiosContainer) {
         ...
         CuriosApi.getCuriosInventory(livingEntity)
            ...
            .ifPresent(
               stacksHandler -> {
                  ...
                  boolean cosmetic = SPacketSyncStack.HandlerType.fromValue(data.handlerType()) == SPacketSyncStack.HandlerType.COSMETIC;
                  if (!compoundNBT.isEmpty()) {
                     NonNullList<Boolean> renderStates = stacksHandler.getRenders();
                     CuriosApi.getCurio(stack)
                        .ifPresent(
                           curio -> curio.readSyncData(
                              new SlotContext(data.curioId(), livingEntity, slot, cosmetic, renderStates.size() > slot && (Boolean)renderStates.get(slot)),
                              compoundNBT
                           )
                        );
                  }
