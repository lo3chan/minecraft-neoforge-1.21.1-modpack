   ...
   private static void processImc(Stream<IMCMessage> messages, boolean create) {
      ...
      messageList.forEach(msgx -> {
         ...
         if (messageObject instanceof SlotTypeMessage) {
            messageMap.computeIfAbsent(msgx.senderModId(), k -> new ArrayList<>()).add((SlotTypeMessage)messageObject);
            ...
            if (iter.hasNext()) {
               ...
               if (firstChild instanceof SlotTypeMessage) {
                  messageMap.computeIfAbsent(msgx.senderModId(), k -> new ArrayList<>()).add((SlotTypeMessage)firstChild);
                  iter.forEachRemaining(child -> messageMap.computeIfAbsent(msgx.senderModId(), k -> new ArrayList<>()).add((SlotTypeMessage)child));
               }
            }
         }
      });
