...
public class ClientEventHandler {
   @SubscribeEvent
   public void onClientTick(Post evt) {
   ...
   public void onAttributeTooltip(AddAttributeTooltipsEvent evt) {
      ...
      GatherSkippedAttributeTooltipsEvent skipped = (GatherSkippedAttributeTooltipsEvent)NeoForge.EVENT_BUS
         .post(new GatherSkippedAttributeTooltipsEvent(stack, context));
      if (!skipped.isSkippingAll()) {
         List<Component> attributesTooltip = new ArrayList<>();
         Player player = context.player();

         for (String identifier : getItemStackSlots(stack, player)) {
            ...
            if (!attributes.isEmpty()) {
               ...
               if (player != null) {
                  AttributeUtil.applyTextFor(stack, attributesTooltip::add, attributes, AttributeTooltipContext.of(player, context, context.flag()));
               }
            }
         }

         evt.addTooltipLines(
            CuriosApi.getCurio(stack).map(curio -> curio.getAttributesTooltip(attributesTooltip, context)).orElse(attributesTooltip).toArray(new Component[0])
         );
      }
   }
      ...
      if (!stack.isEmpty()) {
         ...
         if (!slots.isEmpty()) {
            ...
            for (int j = 0; j < slots.size(); j++) {
               ...
               MutableComponent type = Component.translatableWithFallback(key, Character.toUpperCase(id.charAt(0)) + id.substring(1).toLowerCase());
               if (j < slots.size() - 1) {
                  type = type.append(", ");
               }
            ...
            TooltipContext context = evt.getContext();
            List<Component> toAdd = List.of(slotsTooltip);
            evt.getToolTip().addAll(1, CuriosApi.getCurio(stack).map(curio -> curio.getSlotsTooltip(toAdd, context)).orElse(toAdd));
         }
      }
   }

   private static List<String> getItemStackSlots(ItemStack stack, Player player) {
      Set<String> slots = Set.copyOf(
         (player != null ? CuriosApi.getItemStackSlots(stack, player) : CuriosApi.getItemStackSlots(stack, FMLLoader.getDist() == Dist.CLIENT)).keySet()
      );
