...
public class TomeScreen extends Screen {
   private static final ResourceLocation BOOK_TEXTURE = ResourceLocation.fromNamespaceAndPath("akashictome", "textures/models/book.png");
   ...
   public void render(GuiGraphics pGuiGraphics, int mouseX, int mouseY, float partialTicks) {
      ...
      if (this.tome.has(Registries.TOOL_CONTENT)) {
         ...
         ToolContentComponent contents = (ToolContentComponent)this.tome.get(Registries.TOOL_CONTENT);
         ...
         pGuiGraphics.fill(startX - padding, startY - padding, endX + padding, endY + padding, 570425344);
         pGuiGraphics.fill(startX - padding - extra, startY - padding - extra, endX + padding + extra, endY + padding + extra, 570425344);
         ...
         if (!tooltipStack.isEmpty()) {
            ...
            if (tooltipStack.has(Registries.OG_DISPLAY_NAME)) {
               ogDisplayName = (Component)tooltipStack.get(Registries.OG_DISPLAY_NAME);
            } else {
            ...
            if (tooltipStack.has(Registries.DEFINED_MOD)) {
               tempDefinedMod = (String)tooltipStack.get(Registries.DEFINED_MOD);
            }
         ...
         if (!(Boolean)ConfigHandler.hideBookRender.get()) {
            float f = 1.0F;
            float f1 = 0.0F;
            Lighting.setupForEntityInInventory();
            poseStack.pushPose();
            poseStack.translate((startX + endX) / 2.0, startY - 45, 100.0);
            float f2 = 100.0F;
