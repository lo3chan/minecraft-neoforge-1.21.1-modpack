...
public abstract class SimpleOptionFactory<A extends Annotation, T> implements OptionFactory<A, T> {
   ...
   protected OptionDescription.Builder description(T value, A annotation, ConfigField<T> field, OptionAccess storage) {
      ...
      if (Language.getInstance().has(key)) {
         ...
         for (int i = 1; Language.getInstance().has(key + i); i++) {
      ...
      if (imageOverrideOpt.isPresent()) {
         ...
         if (!imageOverride.factory().equals(EmptyCustomImageFactory.class)) {
            ...
            try {
               imageFactory = AutoGenUtils.constructNoArgsClass(
                  (Class<CustomImage.CustomImageFactory<T>>)imageOverride.factory(),
               ...
               throw new YACLAutoGenException(
                  "'%s': The factory class on @OverrideImage is of incorrect type. Expected %s, got %s."
                     .formatted(field.access().name(), field.access().type().getTypeName(), imageOverride.factory().getTypeParameters()[0].getName())
               );
            }

            builder.customImage(imageFactory.createImage(value, field, storage).thenApply(Optional::of));
         } else {
            if (imageOverride.value().isEmpty()) {
            ...
            switch (extension) {
               ...
               default:
                  throw new YACLAutoGenException(
                     "'%s': Invalid image extension '%s' on @OverrideImage. Expected: ('png','jpg','webp','gif')".formatted(field.access().name(), extension)
                  );
            }
         }
      } else {
