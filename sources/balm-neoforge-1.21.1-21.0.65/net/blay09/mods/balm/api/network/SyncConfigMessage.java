...
public class SyncConfigMessage<TData> implements CustomPacketPayload {
   ...
   public static <TData, TMessage extends SyncConfigMessage<TData>> Function<FriendlyByteBuf, TMessage> createDecoder(
      Class<?> clazz, Function<TData, TMessage> messageFactory, Supplier<TData> dataFactory
   ) {
      return buf -> {
   ...
   private static <TData> void readSyncedFields(FriendlyByteBuf buf, TData data, boolean forceSynced) {
      ...
      for (Field field : syncedFields) {
         ...
         try {
            Object value = readValue(buf, data, field, type);
   ...
   public static <TMessage extends SyncConfigMessage<TData>, TData extends BalmConfigData> void register(
      Type<TMessage> type, Class<TMessage> messageClass, Function<TData, TMessage> messageFactory, Class<TData> dataClass, Supplier<TData> dataFactory
   ) {
      Supplier<TData> copyFactory = createDeepCopyFactory(() -> Balm.getConfig().getBackingConfig(dataClass), dataFactory);
      Balm.getNetworking().registerClientboundPacket(type, messageClass, (buf, message) -> {
         TData data = message.getData();
         writeSyncedFields(buf, data, false);
      }, buf -> {
