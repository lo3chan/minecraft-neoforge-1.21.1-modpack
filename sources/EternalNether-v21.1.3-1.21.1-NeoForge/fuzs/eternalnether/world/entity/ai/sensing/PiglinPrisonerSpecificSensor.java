...
public class PiglinPrisonerSpecificSensor extends Sensor<LivingEntity> {
   public Set<MemoryModuleType<?>> requires() {
      return ImmutableSet.of(
         ...
         new MemoryModuleType[]{MemoryModuleType.NEARBY_ADULT_PIGLINS, MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT, MemoryModuleType.NEAREST_REPELLENT}
      );
   }
   ...
   protected void doTick(ServerLevel level, LivingEntity piglinPrisoner) {
      ...
      for (LivingEntity livingentity : nearestvisiblelivingentities.findAll(entity -> true)) {
         if (livingentity instanceof PiglinBrute piglinbrute) {
            ...
            && (livingentity instanceof WitherSkeleton || livingentity instanceof WitherBoss || livingentity instanceof Hoglin hoglin && hoglin.isAdult())) {
            optional = Optional.of((Mob)livingentity);
         } else if (optional4.isEmpty() && PiglinAi.isZombified(livingentity.getType())) {
