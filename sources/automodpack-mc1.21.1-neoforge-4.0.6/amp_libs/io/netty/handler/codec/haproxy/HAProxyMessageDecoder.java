...
public class HAProxyMessageDecoder extends ByteToMessageDecoder {
   ...
   private ByteBuf decodeStruct(ChannelHandlerContext ctx, ByteBuf buffer) throws Exception {
      if (this.headerExtractor == null) {
         this.headerExtractor = new HAProxyMessageDecoder.StructHeaderExtractor(this.v2MaxHeaderSize);
      }
   ...
   private ByteBuf decodeLine(ChannelHandlerContext ctx, ByteBuf buffer) throws Exception {
      if (this.headerExtractor == null) {
         this.headerExtractor = new HAProxyMessageDecoder.LineHeaderExtractor(108);
      }
   ...
   public static ProtocolDetectionResult<HAProxyProtocolVersion> detectProtocol(ByteBuf buffer) {
      if (buffer.readableBytes() < 12) {
         ...
         if (match(HAProxyConstants.BINARY_PREFIX, buffer, idx)) {
            ...
            return match(HAProxyConstants.TEXT_PREFIX, buffer, idx) ? DETECTION_RESULT_V1 : ProtocolDetectionResult.invalid();
         }
      }
   }
   ...
   private final class LineHeaderExtractor extends HAProxyMessageDecoder.HeaderExtractor {
