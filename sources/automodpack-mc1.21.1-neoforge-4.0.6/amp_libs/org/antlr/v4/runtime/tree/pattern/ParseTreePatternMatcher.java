...
import amp_libs.org.antlr.v4.runtime.ANTLRInputStream;
import amp_libs.org.antlr.v4.runtime.BailErrorStrategy;
import amp_libs.org.antlr.v4.runtime.CommonTokenStream;
import amp_libs.org.antlr.v4.runtime.Lexer;
import amp_libs.org.antlr.v4.runtime.ListTokenSource;
import amp_libs.org.antlr.v4.runtime.Parser;
import amp_libs.org.antlr.v4.runtime.ParserInterpreter;
...
import amp_libs.org.antlr.v4.runtime.misc.MultiMap;
...
public class ParseTreePatternMatcher {
   ...
   public ParseTreePattern compile(String pattern, int patternRuleIndex) {
      ...
      ParserInterpreter parserInterp = new ParserInterpreter(
         this.parser.getGrammarFileName(), this.parser.getVocabulary(), Arrays.asList(this.parser.getRuleNames()), this.parser.getATNWithBypassAlts(), tokens
      );
      ParseTree tree = null;
      ...
      try {
         ...
         throw new ParseTreePatternMatcher.CannotInvokeStartRule(var11);
      }
   ...
   public List<Chunk> split(String pattern) {
      ...
      if (starts.size() > stops.size()) {
         ...
         for (int ix = 0; ix < ntags; ix++) {
            String tag = pattern.substring(starts.get(ix) + this.start.length(), stops.get(ix));
            ...
            if (ix + 1 < ntags) {
               String text = pattern.substring(stops.get(ix) + this.stop.length(), starts.get(ix + 1));
               chunks.add(new TextChunk(text));
            }
         }

         if (ntags > 0) {
            int afterLastTag = stops.get(ntags - 1) + this.stop.length();
            if (afterLastTag < n) {
         ...
         for (int ix = 0; ix < chunks.size(); ix++) {
            Chunk c = chunks.get(ix);
            if (c instanceof TextChunk) {
               TextChunk tc = (TextChunk)c;
               String unescaped = tc.getText().replace(this.escape, "");
               if (unescaped.length() < tc.getText().length()) {
                  chunks.set(ix, new TextChunk(unescaped));
               }
            }
         }
