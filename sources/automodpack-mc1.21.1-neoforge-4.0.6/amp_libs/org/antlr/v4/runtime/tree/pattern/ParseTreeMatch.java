...
public class ParseTreeMatch {
   ...
   public ParseTree get(String label) {
      List<ParseTree> parseTrees = this.labels.get(label);
      return parseTrees != null && parseTrees.size() != 0 ? parseTrees.get(parseTrees.size() - 1) : null;
   }

   public List<ParseTree> getAll(String label) {
      List<ParseTree> nodes = this.labels.get(label);
