...
public class BetterEndPodiumFeature extends Feature<NoneFeatureConfiguration> {
   private static final List<StructureProcessor> PROCESSORS = List.of(
      new BlockReplaceProcessor(
         Blocks.GRAY_CONCRETE.defaultBlockState(), new BlockStateRandomizer(Blocks.BEDROCK.defaultBlockState()), false, false, false, false
      ),
      new DragonEggProcessor()
   );
   private static final StructureProcessor ACTIVE_PORTAL_PROCESSOR = new BlockReplaceProcessor(
      Blocks.RED_CONCRETE.defaultBlockState(), new BlockStateRandomizer(Blocks.END_PORTAL.defaultBlockState()), false, false, false, false
   );
   private static final StructureProcessor INACTIVE_PORTAL_PROCESSOR = new BlockReplaceProcessor(
      Blocks.RED_CONCRETE.defaultBlockState(), new BlockStateRandomizer(Blocks.AIR.defaultBlockState()), false, false, false, false
   );
   ...
   public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
      ...
      boolean placed = this.placeTemplate(level, randomSource, pos, Rotation.NONE, template, numberTimesDragonKilled);
      if (this.isInitialSpawn) {
         BlockPos centerPos = pos.above(6);

         for (Direction direction : Plane.HORIZONTAL) {
            BlockPos crystalPos = centerPos.relative(direction, 8);
            EndCrystal crystal = new EndCrystal((Level)level, crystalPos.getX() + 0.5, crystalPos.getY(), crystalPos.getZ() + 0.5);
   ...
   private ResourceLocation chooseTemplate() {
      if (this.isBottomOnly) {
         return ResourceLocation.fromNamespaceAndPath("betterendisland", "tower_bottom_open");
      } else {
         String towerType = this.isInitialSpawn ? "initial" : "broken";
         String towerName = "tower_" + towerType;
         return ResourceLocation.fromNamespaceAndPath("betterendisland", towerName);
      }
   }
