...
public class CraftingTableBlockEntityRenderer<T extends BlockEntity & Container & WorkbenchVisualsProvider> implements BlockEntityRenderer<T> {
   ...
   public CraftingTableBlockEntityRenderer(Context context) {
   ...
   public void render(T blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight, int packedOverlay) {
      ...
      if (!itemStack.isEmpty()) {
         this.renderResultItem(
            itemStack, blockEntity.getLevel(), blockEntity.getAnimationController().ticks + partialTick, poseStack, multiBufferSource, packedLight
         );
      }
   }
   ...
   private void setupFloatingRenderer(
      CraftingTableAnimationController animationController, float partialTick, PoseStack poseStack, ItemStack itemStack, int index
   ) {
      float shift = (float)Math.abs(((animationController.ticks + partialTick) * 50.0 + index * 1000L) % 5000.0 - 2500.0) / 200000.0F;
      ...
      poseStack.translate(index % 3 * 3.0 / 16.0 + 0.3125 - 0.5, 1.09375, index / 3 * 3.0 / 16.0 + 0.3125 - 0.5);
   ...
   private void setupFlatRenderer(CraftingTableAnimationController animationController, float partialTick, PoseStack poseStack, ItemStack itemStack, int index) {
      ...
      poseStack.mulPose(Axis.YP.rotationDegrees(Mth.lerp(partialTick, animationController.currentAngle, animationController.nextAngle)));
      poseStack.translate(index % 3 * 3.0 / 16.0 + 0.3125 - 0.5, gui3d ? 1.0625 : 1.005, index / 3 * 3.0 / 16.0 + 0.3125 - 0.5);
   ...
   private void renderResultItem(
      ItemStack itemStack, @Nullable Level level, float time, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight
   ) {
