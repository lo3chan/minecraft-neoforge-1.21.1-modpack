...
public abstract class AbstractTagProvider<T> extends TagsProvider<T> {
   ...
   public static final Map<BlockSetVariant, TagKey<Block>> VARIANT_BLOCK_TAGS = ImmutableMap.builder()
      ...
      .build();
   public static final Map<BlockSetVariant, TagKey<Block>> VARIANT_STONE_BLOCK_TAGS = ImmutableMap.builder()
      ...
      .buildKeepingLast();
   public static final Map<BlockSetVariant, TagKey<Block>> VARIANT_WOODEN_BLOCK_TAGS = ImmutableMap.builder()
      .putAll(VARIANT_BLOCK_TAGS)
      ...
      .buildKeepingLast();
   public static final Map<BlockSetVariant, TagKey<Item>> VARIANT_ITEM_TAGS = ImmutableMap.builder()
      ...
      .build();
   public static final Map<BlockSetVariant, TagKey<Item>> VARIANT_STONE_ITEM_TAGS = ImmutableMap.builder()
      .putAll(VARIANT_ITEM_TAGS)
      .put(BlockSetVariant.BUTTON, ItemTags.STONE_BUTTONS)
      .buildKeepingLast();
   public static final Map<BlockSetVariant, TagKey<Item>> VARIANT_WOODEN_ITEM_TAGS = ImmutableMap.builder()
      .putAll(VARIANT_ITEM_TAGS)
      ...
      .buildKeepingLast();
   public static final Map<BlockSetVariant, TagKey<EntityType<?>>> VARIANT_ENTITY_TYPE_TAGS = ImmutableMap.builder()
   ...
   public AbstractTagProvider(ResourceKey<? extends Registry<T>> registryKey, String modId, PackOutput packOutput, CompletableFuture<Provider> registries) {
      super(
         packOutput,
         registryKey,
         registries,
         CompletableFuture.completedFuture(
            tagKey -> Objects.equals(tagKey.location().getNamespace(), modId) ? Optional.empty() : Optional.of(STATIC_TAG_BUILDER)
         )
      );
      this.modId = modId;
      this.registry = LookupHelper.<T>getRegistry(registryKey).orElse(null);
      this.keyExtractor = this.registry != null ? t -> RegistryHelper.getResourceKeyOrThrow(this.registry, t) : null;
   }

   public abstract void addTags(Provider var1);

   protected TagBuilder getOrCreateRawBuilder(TagKey<T> tag) {
      return this.builders.computeIfAbsent(tag.location(), id -> new SortingTagBuilder());
   }
   ...
   public final void generateFor(Map<BlockSetVariant, Reference<T>> variants, Map<BlockSetVariant, TagKey<T>> variantTags) {
      variants.forEach((variant, holder) -> {
         TagKey<T> tagKey = variantTags.get(variant);
         if (tagKey != null) {
            this.tag(tagKey).add((Reference<? extends T>)holder);
         }
      });
   }
