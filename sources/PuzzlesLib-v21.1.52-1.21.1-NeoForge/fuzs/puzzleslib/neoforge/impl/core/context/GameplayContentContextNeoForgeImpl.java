...
public final class GameplayContentContextNeoForgeImpl implements GameplayContentContext {
   private final GameplayContentContextNeoForgeImpl.DataMapBuilder<Holder<? extends ItemLike>, Fraction> furnaceFuels;
   private final Map<Holder<Block>, GameplayContentContextNeoForgeImpl.Flammable> flammables = new LinkedHashMap<>();
   private final GameplayContentContextNeoForgeImpl.DataMapBuilder<Holder<? extends ItemLike>, Float> compostables;
   private final GameplayContentContextNeoForgeImpl.BlockConversionBuilder strippables = new GameplayContentContextNeoForgeImpl.BlockConversionBuilder(
      ItemAbilities.AXE_STRIP
   );
   private final GameplayContentContextNeoForgeImpl.BlockConversionBuilder flattenables = new GameplayContentContextNeoForgeImpl.BlockConversionBuilder(
      ItemAbilities.SHOVEL_FLATTEN
   );
   private final GameplayContentContextNeoForgeImpl.BlockConversionBuilder tillables = new GameplayContentContextNeoForgeImpl.BlockConversionBuilder(
      ItemAbilities.HOE_TILL, HoeItem::onlyIfAirAbove
   );
   private final GameplayContentContextNeoForgeImpl.DataMapBuilder<Holder<Block>, Holder<Block>> oxidizables;
   private final GameplayContentContextNeoForgeImpl.DataMapBuilder<Holder<Block>, Holder<Block>> waxables;
   private final IEventBus eventBus;

   public GameplayContentContextNeoForgeImpl(String modId, IEventBus eventBus) {
      this.eventBus = eventBus;
      this.furnaceFuels = new GameplayContentContextNeoForgeImpl.DataMapBuilder<>(
         modId, NeoForgeDataMaps.FURNACE_FUELS, holder -> BuiltInRegistries.ITEM.wrapAsHolder(((ItemLike)holder.value()).asItem()), fraction -> {
            Fraction fuelBaseValue = Fraction.getFraction(200, 1);
            return new FurnaceFuel(fraction.multiplyBy(fuelBaseValue).intValue());
         }
      );
      this.compostables = new GameplayContentContextNeoForgeImpl.DataMapBuilder<>(
         modId, NeoForgeDataMaps.COMPOSTABLES, holder -> BuiltInRegistries.ITEM.wrapAsHolder(((ItemLike)holder.value()).asItem()), Compostable::new
      );
      this.oxidizables = new GameplayContentContextNeoForgeImpl.DataMapBuilder<>(
         modId, NeoForgeDataMaps.OXIDIZABLES, Function.identity(), holder -> new Oxidizable((Block)holder.value())
      );
      this.waxables = new GameplayContentContextNeoForgeImpl.DataMapBuilder<>(
         modId, NeoForgeDataMaps.WAXABLES, Function.identity(), holder -> new Waxable((Block)holder.value())
      );
   }
         ...
         this.eventBus
            .addListener(
               event -> event.enqueueWork(
                  () -> this.flammables
                     .forEach(
                        (holder, flammable) -> ((FireBlock)Blocks.FIRE)
                           .setFlammable((Block)holder.value(), flammable.encouragement(), flammable.flammability())
                     )
               )
            );
      }

      this.flammables.put(flammableBlock, new GameplayContentContextNeoForgeImpl.Flammable(encouragement, flammability));
   }

   @Override
   public void registerCompostable(Holder<? extends ItemLike> compostableItem, float compostingChance) {
      Preconditions.checkArgument(compostingChance >= 0.0F && compostingChance <= 1.0F, "Value " + compostingChance + " outside of range 0.0 -> 1.0");
   ...
   private static class BlockConversionBuilder {
      ...
      private final Supplier<Map<Block, Block>> supplier = Suppliers.memoize(() -> {
         Builder<Block, Block> builder = ImmutableMap.builder();

         for (Entry<Holder<Block>, Holder<Block>> entry : this.values.entrySet()) {
   ...
   private static class DataMapBuilder<K, V> {
      ...
      public <R, T> DataMapBuilder(String modId, DataMapType<R, T> dataMapType, Function<K, Holder<R>> keyConverter, Function<V, T> valueConverter) {
         this.modId = modId;
         this.factory = context -> new DataMapProvider(context.getPackOutput(), context.getRegistries()) {
            protected void gather(Provider registries) {
               net.neoforged.neoforge.common.data.DataMapProvider.Builder<T, R> builder = this.builder(dataMapType);
               DataMapBuilder.this.values
                  .forEach((key, value) -> builder.add((Holder)keyConverter.apply((K)key), valueConverter.apply((V)value), false, new ICondition[0]));
            }

            public String getName() {
               return super.getName() + " for " + ResourceKey.create(dataMapType.registryKey(), dataMapType.id());
            }
         };
      }
