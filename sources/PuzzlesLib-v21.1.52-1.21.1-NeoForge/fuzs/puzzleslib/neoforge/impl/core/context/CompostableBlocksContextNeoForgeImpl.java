...
@Deprecated
public record CompostableBlocksContextNeoForgeImpl(String modId, Map<Holder<? extends ItemLike>, Compostable> compostingChances)
   implements CompostableBlocksContext {
      ...
      if (this.compostingChances.isEmpty()) {
         DataProviderHelper.registerDataProviders(this.modId, context -> new DataMapProvider(context.getPackOutput(), context.getRegistries()) {
            protected void gather() {
               ...
               for (Entry<Holder<? extends ItemLike>, Compostable> entry : CompostableBlocksContextNeoForgeImpl.this.compostingChances().entrySet()) {
                  Reference<Item> holder = ((ItemLike)entry.getKey().value()).asItem().builtInRegistryHolder();
                  builder.add(holder, entry.getValue(), false, new ICondition[0]);
               }
            }
