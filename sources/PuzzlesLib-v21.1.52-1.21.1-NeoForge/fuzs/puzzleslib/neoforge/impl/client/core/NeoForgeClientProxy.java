...
public class NeoForgeClientProxy extends NeoForgeCommonProxy implements ClientProxyImpl {
   ...
   public <M1, M2> CompletableFuture<Void> registerClientReceiver(
      ...
      return context.enqueueWork(() -> {
         ...
         message.getHandler().handle(message.unwrap(), Minecraft.getInstance(), player.connection, player, player.clientLevel);
      });
   }
   ...
   @Override
   public boolean onRenderTooltip(
      ...
      return ClientHooks.onRenderTooltipPre(
            ItemStack.EMPTY, guiGraphics, mouseX, mouseY, guiGraphics.guiWidth(), guiGraphics.guiHeight(), components, font, positioner
         )
         .isCanceled();
   }
   ...
   public void registerConfigurationScreen(String modId, String... otherModIds) {
      NeoForgeModContainerHelper.getModContainer(modId)
         .registerExtensionPoint(
            ...
            (IConfigScreenFactory)(modContainer, screen) -> new NeoForgeConfigurationScreen(modContainer.getModId(), screen, otherModIds)
         );
   }
}
