...
public final class NeoForgeCapabilityController implements CapabilityController {
   ...
   public NeoForgeCapabilityController(String modId) {
      this.registrar = DeferredRegister.create(Keys.ATTACHMENT_TYPES, modId);
      NeoForgeModContainerHelper.getOptionalModEventBus(modId).ifPresent(this.registrar::register);
   }

   @Override
   public <T extends Entity, C extends CapabilityComponent<T>> EntityCapabilityKey.Mutable<T, C> registerEntityCapability(
      String identifier, Class<C> capabilityType, Supplier<C> capabilityFactory, Class<T> entityType
   ) {
   ...
   @Override
   public <T extends BlockEntity, C extends CapabilityComponent<T>> BlockEntityCapabilityKey<T, C> registerBlockEntityCapability(
      String identifier, Class<C> capabilityType, Supplier<C> capabilityFactory, Class<T> blockEntityType
   ) {
   ...
   @Override
   public <C extends CapabilityComponent<LevelChunk>> LevelChunkCapabilityKey<C> registerLevelChunkCapability(
      String identifier, Class<C> capabilityType, Supplier<C> capabilityFactory
   ) {
      return this.registerCapability(LevelChunk.class, identifier, capabilityFactory, NeoForgeLevelChunkCapabilityKey::new);
   }

   @Override
   public <C extends CapabilityComponent<Level>> LevelCapabilityKey<C> registerLevelCapability(
      String identifier, Class<C> capabilityType, Supplier<C> capabilityFactory
   ) {
      return this.registerCapability(Level.class, identifier, capabilityFactory, NeoForgeLevelCapabilityKey::new);
   }

   private <T, C extends CapabilityComponent<T>, K extends CapabilityKey<T, C>> K registerCapability(
      Class<? extends IAttachmentHolder> holderType,
      String identifier,
      Supplier<C> capabilityFactory,
      NeoForgeCapabilityKey.Factory<T, C, K> capabilityKeyFactory
   ) {
   ...
   private <T, C extends CapabilityComponent<T>, K extends CapabilityKey<T, C>> K registerCapability(
      ...
      DeferredHolder<AttachmentType<?>, AttachmentType<C>> holder = this.registrar.register(identifier, () -> AttachmentType.builder(attachmentHolder -> {
      ...
      }).serialize(new IAttachmentSerializer<Tag, C>() {
         public C read(IAttachmentHolder attachmentHolder, Tag tag, Provider registries) {
            C capabilityComponent = (C)((CapabilityComponent)capabilityFactory.get());
            Objects.requireNonNull(capabilityComponent, "capability component is null");
            capabilityComponent.initialize((CapabilityKey<IAttachmentHolder, CapabilityComponent<IAttachmentHolder>>)capabilityKey[0], attachmentHolder);
            capabilityComponent.read((CompoundTag)tag, registries);
            return capabilityComponent;
         }

         public Tag write(C object, Provider registries) {
