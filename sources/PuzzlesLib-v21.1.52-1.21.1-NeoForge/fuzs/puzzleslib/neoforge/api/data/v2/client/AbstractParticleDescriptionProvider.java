...
public abstract class AbstractParticleDescriptionProvider extends JsonCodecProvider<List<ResourceLocation>> {
   ...
   private final ResourceType textureResourceType = new ResourceType(PackType.CLIENT_RESOURCES, ".png", "textures/particle");
   ...
   public AbstractParticleDescriptionProvider(String modId, PackOutput packOutput, CompletableFuture<Provider> lookupProvider, ExistingFileHelper fileHelper) {
      super(packOutput, Target.RESOURCE_PACK, "particles", PackType.CLIENT_RESOURCES, CODEC, lookupProvider, modId, fileHelper);
   }
   ...
   protected void add(ResourceLocation id, ResourceLocation resourceLocation, int indexStart, int indexEnd) {
      if (indexEnd == -1) {
         ...
         List<ResourceLocation> textures = IntStream.rangeClosed(Math.min(indexStart, indexEnd), Math.max(indexStart, indexEnd))
            .mapToObj(index -> ResourceLocationHelper.fromNamespaceAndPath(resourceLocation.getNamespace(), resourceLocation.getPath() + "_" + index))
   ...
   public void unconditional(ResourceLocation id, List<ResourceLocation> value) {
      ...
      if (!missing.isEmpty()) {
         throw new IllegalArgumentException(
            "Couldn't define particle description %s as it is missing following texture(s): %s".formatted(id, String.join(",", missing))
         );
      } else {
