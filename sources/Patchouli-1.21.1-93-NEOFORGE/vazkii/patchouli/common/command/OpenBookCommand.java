...
public class OpenBookCommand {
   private static final SuggestionProvider<CommandSourceStack> BOOK_ID_SUGGESTER = (ctx, builder) -> SharedSuggestionProvider.suggestResource(
      BookRegistry.INSTANCE.books.keySet(), builder
   );

   public static void register(CommandDispatcher<CommandSourceStack> disp) {
      disp.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("open-patchouli-book").requires(cs -> cs.hasPermission(2)))
            .then(
               Commands.argument("targets", EntityArgument.players())
                  .then(
                     ((RequiredArgumentBuilder)Commands.argument("book", ResourceLocationArgument.id())
                           .suggests(BOOK_ID_SUGGESTER)
                           .executes(ctx -> doIt(EntityArgument.getPlayers(ctx, "targets"), ResourceLocationArgument.getId(ctx, "book"), null, 0)))
                        .then(
                           Commands.argument("entry", ResourceLocationArgument.id())
                              .then(
