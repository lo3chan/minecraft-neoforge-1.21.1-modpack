...
public class Book {
   ...
   private static final ResourceLocation DEFAULT_MODEL = ResourceLocation.fromNamespaceAndPath("patchouli", "book_brown");
   private static final ResourceLocation DEFAULT_BOOK_TEXTURE = ResourceLocation.fromNamespaceAndPath("patchouli", "textures/gui/book_brown.png");
   private static final ResourceLocation DEFAULT_FILLER_TEXTURE = ResourceLocation.fromNamespaceAndPath("patchouli", "textures/gui/page_filler.png");
   private static final ResourceLocation DEFAULT_CRAFTING_TEXTURE = ResourceLocation.fromNamespaceAndPath("patchouli", "textures/gui/crafting.png");
   private static final Map<String, String> DEFAULT_MACROS = (Map<String, String>)Util.make(() -> {
   ...
   public Book(JsonObject root, XplatModContainer owner, ResourceLocation id, boolean external) {
      ...
      if (!this.isExternal && !useResourcePack) {
         String message = "Book %s has use_resource_pack set to false. ".formatted(this.id)
            + "This behaviour was removed in 1.20. The book author should enable this flag and move all book contents clientside to /assets/, leaving the book.json in /data/. See https://vazkiimods.github.io/Patchouli/docs/upgrading/upgrade-guide-120 for details.";
         ...
         if (extensionTargetID != null) {
            String message = "Book %s is declared to extend %s. ".formatted(this.id, extensionTargetID)
               + "This behaviour was removed in 1.20. The author should simply ship the extra content they want to add or override in a resource pack.";
            throw new IllegalArgumentException(message);
         } else {
            ...
            if (this.noBook) {
               this.bookItem = Suppliers.memoize(() -> {
                  try {
                     return ItemStackUtil.loadFromParsed(ItemStackUtil.deserializeStack(customBookItem, VanillaRegistries.createLookup()));
                  } catch (Exception var4x) {
                     PatchouliAPI.LOGGER.warn("Failed to parse item \"{}\" for book {} defined by mod {}, skipping", customBookItem, id, owner.getId(), var4x);
            ...
            for (Entry<String, JsonElement> e : GsonHelper.getAsJsonObject(root, "macros", new JsonObject()).entrySet()) {
               this.macros.put(e.getKey(), GsonHelper.convertToString(e.getValue(), "macro value"));
            }
         }
      }
   }
   ...
   public MutableComponent getSubtitle() {
      ...
      return Component.translatable("patchouli.gui.lexicon.edition_str", new Object[]{editionStr});
   }

   public BookIcon getIcon() {
      return (BookIcon)(this.indexIconRaw != null && !this.indexIconRaw.isEmpty()
         ? BookIcon.from(this.indexIconRaw, RegistryAccess.fromRegistryOfRegistries(BuiltInRegistries.REGISTRY))
         : new BookIcon.StackIcon(this.getBookItem()));
   }
