      ...
      if (mc.player != null && !(mc.screen instanceof GuiBook)) {
         ...
         if (lexSlot > -1) {
            ...
            if (PatchouliConfig.get().useShiftForQuickLookup() ? Screen.hasShiftDown() : Screen.hasControlDown()) {
               ...
               buf.addVertex(cx, cy, 0.0F).setColor(0.0F, 0.5F, 0.0F, a);

               for (float ix = angles; ix > 0.0F; ix--) {
                  double rad = (ix - 90.0F) / 180.0F * 3.141592653589793;
                  buf.addVertex((float)(cx + Math.cos(rad) * r), (float)(cy + Math.sin(rad) * r), 0.0F).setColor(0.0F, 1.0F, 0.0F, 1.0F);
               }

               buf.addVertex(cx, cy, 0.0F).setColor(0.0F, 1.0F, 0.0F, 0.0F);
