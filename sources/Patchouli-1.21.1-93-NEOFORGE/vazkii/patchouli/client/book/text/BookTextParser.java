...
public class BookTextParser {
   ...
   public String expandMacros(@Nullable String text) {
      ...
      if (i == expansionCap) {
         PatchouliAPI.LOGGER
            .warn("Expanded macros for {} iterations without reaching fixpoint, stopping. Make sure you don't have circular macro invocations", expansionCap);
      }
   ...
   static {
      ...
      register(state -> state.gui.getMinecraft().player.getName().getString(), "playername");
      register(state -> {
      ...
      register((parameter, state) -> {
         ...
         if (result == null) {
            state.tooltip = Component.translatable("patchouli.gui.lexicon.keybind_missing", new Object[]{parameter});
            return "N/A";
         } else {
            state.tooltip = Component.translatable("patchouli.gui.lexicon.keybind", new Object[]{Component.translatable(result.getName())});
      ...
      register(
         (parameter, state) -> {
            ...
            if (isExternal) {
               ...
               ResourceLocation href = parameter.contains(":")
