...
public class TextLayouter {
   ...
   public void layout(Font font, List<Span> spans) {
      ...
      do {
         this.y = this.pageY;
         this.words.clear();
         this.smallestOverstep = 2147483647;
   ...
   private boolean adjustScale() {
      this.pageWidth = 1 + Math.min(this.smallestOverstep, (int)(this.basePageWidth * this.getOverflow()));
      return true;
   }

   public float getScale() {
      return (float)this.basePageWidth / this.pageWidth;
   }

   private float getOverflow() {
      return (float)(this.y + this.lineHeight - this.pageY) / (156 - this.pageY);
   }
   ...
   private void breakLine(BreakIterator iterator) {
      ...
      TextLayouter.SpanTail last = this.pending.get(this.pending.size() - 1);
      width -= last.width;
      offset -= last.length;
   ...
   private void breakLine(int textOffset) {
      ...
      for (index = 0; index < this.pending.size(); index++) {
         TextLayouter.SpanTail span = this.pending.get(index);
