...
public class GuiBookEntry extends GuiBook implements IComponentRenderContext {
   ...
   public boolean mouseClickedScaled(double mouseX, double mouseY, int mouseButton) {
      return this.clickPage(this.leftPage, mouseX, mouseY, mouseButton)
   ...
   void drawPage(GuiGraphics graphics, @Nullable BookPage page, int mouseX, int mouseY, float pticks) {
      if (page != null) {
         ...
         graphics.pose().translate(page.left, page.top, 0.0F);
   ...
   private boolean clickPage(@Nullable BookPage page, double mouseX, double mouseY, int mouseButton) {
      return page != null ? page.mouseClicked(mouseX - page.left, mouseY - page.top, mouseButton) : false;
   }
   ...
   private void setupPages() {
      ...
      this.rightPage = rightNum < pages.size() ? pages.get(rightNum) : null;
      if (this.leftPage != null) {
   ...
   public void setHoverTooltip(List<String> tooltip) {
      this.setTooltip(tooltip.stream().<Component>map(Component::literal).collect(Collectors.toList()));
   }
   ...
   public boolean navigateToEntry(ResourceLocation entry, int page, boolean push) {
      BookEntry bookEntry = this.book.getContents().entries.get(entry);
