...
public class GuiButtonBookResize extends GuiButtonBook {
   public GuiButtonBookResize(GuiBook parent, int x, int y, OnPress onPress) {
   ...
   public List<Component> getTooltipLines() {
      ...
      if (size <= 5) {
         ...
         for (int i = 0; i < size - 5; i++) {
            sizeMsg = Component.translatable("patchouli.gui.lexicon.button.resize.verybig.container", new Object[]{sizeMsg});
         }
      }

      return Arrays.asList(this.tooltip.get(0), sizeMsg.withStyle(ChatFormatting.GRAY));
   }
}
