...
public abstract class TemplateComponent implements IVariablesAvailableCallback {
   ...
   public void onVariablesAvailable(UnaryOperator<IVariable> lookup, Provider registries) {
      this.group = lookup.apply(IVariable.wrap(this.group, registries)).asString();
      this.flag = lookup.apply(IVariable.wrap(this.flag, registries)).asString();
      this.advancement = lookup.apply(IVariable.wrap(this.advancement, registries)).asString();
      this.guardPass = this.guard == null || lookup.apply(IVariable.wrap(this.guard, registries)).asBoolean();
   }
}
