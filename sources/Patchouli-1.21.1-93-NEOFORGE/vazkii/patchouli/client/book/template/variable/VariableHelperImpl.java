...
public class VariableHelperImpl implements VariableHelper {
   ...
   public <T> IVariableSerializer<T> serializerForClass(Class<T> clazz) {
      ...
      if (serializer == null && clazz.isArray()) {
         ...
         IVariableSerializer<?> componentSerializer = this.serializerForClass(componentType);
   ...
   @Override
   public <T> IVariable createFromObject(T object, Provider registries) {
      Class<?> clazz = object.getClass();

      for (Entry<Class<?>, IVariableSerializer<?>> e : this.serializers.entrySet()) {
         if (e.getKey().isAssignableFrom(clazz)) {
            return this.create(((IVariableSerializer<T>)e.getValue()).toJson(object, registries), clazz, registries);
         }
      }
   ...
   @Override
   public IVariable createFromJson(JsonElement elem, Provider registries) {
      return this.create(elem, null, registries);
   }

   private IVariable create(JsonElement elem, Class<?> originator, Provider registries) {
