...
public class BookContentsBuilder {
   ...
   private void loadFiles(Provider registries) {
      this.load("categories", BookContentsBuilder::loadCategory, this.categories, registries);
      this.load("entries", (b, l, id, file, r) -> loadEntry(b, l, id, file, this.categories::get, r), this.entries, registries);
      this.load("templates", BookContentsBuilder::loadTemplate, this.templates, registries);
   }
   ...
   private BookContents build(Level level) {
      ...
      return new BookContents(
         this.book, ImmutableMap.copyOf(this.categories), ImmutableMap.copyOf(this.entries), ImmutableMap.copyOf(this.recipeMappings), pamphletCategory
      );
   }

   private <T> void load(String thing, BookContentsBuilder.LoadFunc<T> loader, Map<ResourceLocation, T> builder, Provider registries) {
      ...
      for (ResourceLocation id : foundIds) {
         ...
         T value = loader.load(this.book, contentLoader, id, ResourceLocation.fromNamespaceAndPath(id.getNamespace(), filePath), registries);
         if (value != null) {
   ...
   @Nullable
   private static BookCategory loadCategory(Book book, BookContentLoader loader, ResourceLocation id, ResourceLocation file, Provider registries) {
      BookContentLoader.LoadResult result = loadLocalizedJson(book, loader, file);
      BookCategory category = new BookCategory(result.json().getAsJsonObject(), id, book, registries);
      return category.canAdd() ? category : null;
   }

   @Nullable
   private static BookEntry loadEntry(
      Book book, BookContentLoader loader, ResourceLocation id, ResourceLocation file, Function<ResourceLocation, BookCategory> categories, Provider registries
   ) {
      BookContentLoader.LoadResult result = loadLocalizedJson(book, loader, file);
      BookEntry entry = new BookEntry(result.json().getAsJsonObject(), id, book, result.addedBy(), registries);
      if (entry.canAdd()) {
   ...
   private static Supplier<BookTemplate> loadTemplate(Book book, BookContentLoader loader, ResourceLocation key, ResourceLocation res, Provider registries) {
   ...
   private static BookContentLoader.LoadResult loadLocalizedJson(Book book, BookContentLoader loader, ResourceLocation file) {
      ResourceLocation localizedFile = ResourceLocation.fromNamespaceAndPath(
         file.getNamespace(), file.getPath().replaceAll("en_us", ClientBookRegistry.INSTANCE.currentLang)
      );
   ...
   private interface LoadFunc<T> {
      @Nullable
      T load(Book var1, BookContentLoader var2, ResourceLocation var3, ResourceLocation var4, Provider var5);
   }
}
