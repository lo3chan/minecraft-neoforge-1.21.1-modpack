...
public final class BookCategory extends AbstractReadStateHolder implements Comparable<BookCategory> {
   ...
   public BookCategory(JsonObject root, ResourceLocation id, Book book, Provider registries) {
      this.book = book;
      this.id = id;
   ...
   public void build(BookContentsBuilder builder) {
      if (!this.built) {
         if (!this.isRootCategory()) {
            ...
            BookCategory parentCat = builder.getCategory(ResourceLocation.tryParse(this.parent));
            if (parentCat == null) {
