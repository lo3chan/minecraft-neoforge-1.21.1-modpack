...
public final class PersistentData {
   ...
   public static void save() {
      ...
      try (BufferedWriter w = Files.newBufferedWriter(saveFile, Charsets.UTF_8)) {
   ...
   public static final class BookData {
      ...
      public BookData(JsonObject root) {
         ...
         for (JsonElement e : GsonHelper.getAsJsonArray(root, "viewedEntries", emptyArray)) {
            this.viewedEntries.add(ResourceLocation.tryParse(e.getAsString()));
         }
         ...
         for (JsonElement e : GsonHelper.getAsJsonArray(root, "history", emptyArray)) {
            this.history.add(ResourceLocation.tryParse(e.getAsString()));
         }

         for (JsonElement e : GsonHelper.getAsJsonArray(root, "completedManualQuests", emptyArray)) {
            this.completedManualQuests.add(ResourceLocation.tryParse(e.getAsString()));
         }
      }
   ...
   public static final class DataHolder {
      ...
      public DataHolder(JsonObject root) {
         ...
         for (Entry<String, JsonElement> e : obj.entrySet()) {
            this.bookData.put(ResourceLocation.tryParse(e.getKey()), new PersistentData.BookData(e.getValue().getAsJsonObject()));
         }
      }

      public PersistentData.BookData getBookData(Book book) {
         return this.bookData.computeIfAbsent(book.id, k -> new PersistentData.BookData(new JsonObject()));
      }
