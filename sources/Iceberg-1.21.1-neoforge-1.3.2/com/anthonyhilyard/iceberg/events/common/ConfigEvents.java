...
public final class ConfigEvents {
   public static final Event<ConfigEvents.Register> REGISTER = EventFactory.create(ConfigEvents.Register.class, callbacks -> (clazz, spec, modId) -> {
      for (ConfigEvents.Register callback : callbacks) {
         callback.onRegister(clazz, spec, modId);
      }
   });
   public static final Event<ConfigEvents.Load> LOAD = EventFactory.create(ConfigEvents.Load.class, callbacks -> modId -> {
      for (ConfigEvents.Load callback : callbacks) {
         callback.onLoad(modId);
      }
   });
   public static final Event<ConfigEvents.Reload> RELOAD = EventFactory.create(ConfigEvents.Reload.class, callbacks -> modId -> {
   ...
   public interface Register {
      void onRegister(Class<? extends IcebergConfig<?>> var1, IIcebergConfigSpec var2, String var3);
   }

   @FunctionalInterface
   public interface Reload {
      void onReload(String var1);
   }
}
