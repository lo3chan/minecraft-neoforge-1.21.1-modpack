...
public class ViewerSocket implements ViewerSocketConnection.Listener, AutoCloseable {
      ...
      if (!this.closed) {
         ...
         if (time - this.socketOpenTime > SOCKET_INITIAL_TIMEOUT && time - this.lastPing > SOCKET_ESTABLISHED_TIMEOUT) {
            ...
            try {
               ...
               this.platform.getPlugin().log(Level.WARNING, "Error whilst sending updated sampler data to the socket", var6);
            }
         }
      }
   }
   ...
   public void sendClientTrustedMessage(String clientId) {
      this.socket
         .sendPacket(
            builder -> builder.setServerConnectResponse(
               SparkWebSocketProtos.ServerConnectResponse.newBuilder()
                  .setClientId(clientId)
                  .setState(SparkWebSocketProtos.ServerConnectResponse.State.ACCEPTED)
                  .build()
            )
         );
   }
   ...
   public void sendUpdatedSamplerData(String payloadId) {
      this.socket
         .sendPacket(builder -> builder.setServerUpdateSampler(SparkWebSocketProtos.ServerUpdateSamplerData.newBuilder().setPayloadId(payloadId).build()));
   ...
   public void sendUpdatedStatistics(SparkProtos.PlatformStatistics platform, SparkProtos.SystemStatistics system) {
      this.socket
         .sendPacket(
            builder -> builder.setServerUpdateStatistics(
               SparkWebSocketProtos.ServerUpdateStatistics.newBuilder().setPlatform(platform).setSystem(system).build()
            )
         );
   }
   ...
   private void onClientPing(SparkWebSocketProtos.ClientPing packet, PublicKey publicKey) {
      ...
      this.socket
         .sendPacket(builder -> builder.setServerPong(SparkWebSocketProtos.ServerPong.newBuilder().setOk(!this.closed).setData(packet.getData()).build()));
   }
   ...
   private void onClientConnect(SparkWebSocketProtos.ClientConnect packet, boolean verified, PublicKey publicKey) {
      if (publicKey == null) {
         ...
         SparkWebSocketProtos.ServerConnectResponse.Builder resp = SparkWebSocketProtos.ServerConnectResponse.newBuilder()
            .setClientId(clientId)
            .setSettings(SparkWebSocketProtos.ServerConnectResponse.Settings.newBuilder().setSamplerInterval(60).setStatisticsInterval(10).build());
