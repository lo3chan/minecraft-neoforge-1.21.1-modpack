...
public class ViewerSocketConnection implements BytesocksClient.Listener, AutoCloseable {
   ...
   public void onText(String data) {
      try {
         ...
         this.platform.getPlugin().log(Level.WARNING, "Exception occurred while reading data from the socket", var3);
      }
   }
   ...
   public void sendPacket(Consumer<SparkWebSocketProtos.PacketWrapper.Builder> packetBuilder) {
      ...
      try {
         ...
         this.platform.getPlugin().log(Level.WARNING, "Exception occurred while sending data to the socket", var5);
      }
   }
