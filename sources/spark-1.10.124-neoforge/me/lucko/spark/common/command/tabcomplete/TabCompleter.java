...
public class TabCompleter {
   ...
   public TabCompleter from(int position, CompletionSupplier supplier) {
      Preconditions.checkState(this.from == 2147483647);
      this.suppliers.put(position, supplier);
      this.from = position;
      return this;
   }
   ...
   public List<String> complete(List<String> args) {
      int lastIndex = 0;
      String partial;
      return !args.isEmpty() && !(partial = args.get(lastIndex = args.size() - 1)).trim().isEmpty()
         ? this.getCompletions(lastIndex, partial)
         : this.getCompletions(lastIndex, "");
   }

   private List<String> getCompletions(int position, String partial) {
      return position >= this.from
         ? this.suppliers.get(this.from).supplyCompletions(partial)
         : this.suppliers.getOrDefault(position, CompletionSupplier.EMPTY).supplyCompletions(partial);
   }
}
