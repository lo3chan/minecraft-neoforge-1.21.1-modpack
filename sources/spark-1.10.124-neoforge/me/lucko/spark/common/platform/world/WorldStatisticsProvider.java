   ...
   public SparkProtos.WorldStatistics getWorldStatistics() {
      ...
      if (result == null) {
         ...
         result.getWorlds().forEach((worldName, chunks) -> {
            ...
            List<WorldStatisticsProvider.Region> regions = groupIntoRegions((List<? extends ChunkInfo<?>>)chunks);
            int total = 0;
         ...
         if (gameRules != null) {
            gameRules.getRules()
               .forEach(
                  (ruleName, rule) -> stats.addGameRules(
                     SparkProtos.WorldStatistics.GameRule.newBuilder()
                        .setName(ruleName)
         ...
         if (dataPacks != null) {
            dataPacks.forEach(
               dataPack -> stats.addDataPacks(
                  SparkProtos.WorldStatistics.DataPack.newBuilder()
            ...
            for (WorldStatisticsProvider.Region region : regions) {
               if (region.isAdjacent(chunk)) {
                  ...
                  while (iterator.hasNext()) {
                     WorldStatisticsProvider.Region otherRegion = iterator.next();
