   ...
   private static void delete(JsonElement json, Deque<String> path) {
      if (!path.isEmpty()) {
         if (json.isJsonObject()) {
            ...
            if (expected.equals("*")) {
               keys = jsonObject.entrySet().stream().map(Entry::getKey).collect(Collectors.toList());
            } else if (expected.endsWith("*")) {
               String pattern = expected.substring(0, expected.length() - 1);
               keys = jsonObject.entrySet().stream().map(Entry::getKey).filter(keyx -> keyx.startsWith(pattern)).collect(Collectors.toList());
