...
public interface ThreadDumper {
   ...
   public static final class Regex implements ThreadDumper {
      ...
      @Override
      public ThreadInfo[] dumpThreads(ThreadMXBean threadBean) {
         return this.threadFinder
            .getThreads()
            .filter(thread -> this.isThreadIncluded(thread.getId(), thread.getName()))
            .map(thread -> threadBean.getThreadInfo(thread.getId(), 2147483647))
   ...
   public static final class Specific implements ThreadDumper {
      ...
      @Override
      public ThreadInfo[] dumpThreads(ThreadMXBean threadBean) {
         return threadBean.getThreadInfo(this.ids, 2147483647);
      }
