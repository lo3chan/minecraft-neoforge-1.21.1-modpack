...
public interface ClassSourceLookup {
   ...
   static ClassSourceLookup create(SparkPlatform platform) {
      try {
         ...
         platform.getPlugin().log(Level.WARNING, "Failed to create ClassSourceLookup", var2);
         return NO_OP;
      }
   }

   static ClassSourceLookup.Visitor createVisitor(ClassSourceLookup lookup, Supplier<ClassFinder> classFinderSupplier) {
      return (ClassSourceLookup.Visitor)(lookup == NO_OP
         ? ClassSourceLookup.NoOpVisitor.INSTANCE
         : new ClassSourceLookup.VisitorImpl(lookup, classFinderSupplier.get()));
   }
   ...
   public static final class SourcesMap<T> {
      ...
      public Map<String, String> export() {
         this.map.values().removeIf(Objects::isNull);
         return this.keyToStringFunction.equals(Function.identity())
            ? this.map
            : this.map.entrySet().stream().collect(Collectors.toMap(e -> this.keyToStringFunction.apply(e.getKey()), Entry::getValue));
      }
   ...
   public static class VisitorImpl implements ClassSourceLookup.Visitor {
      ...
      private final ClassSourceLookup.SourcesMap<ClassSourceLookup.MethodCall> methodSources = new ClassSourceLookup.SourcesMap<>(
         ClassSourceLookup.MethodCall::toString
      );
      private final ClassSourceLookup.SourcesMap<ClassSourceLookup.MethodCallByLine> lineSources = new ClassSourceLookup.SourcesMap<>(
         ClassSourceLookup.MethodCallByLine::toString
      );
      ...
      private void visitStackNode(StackTraceNode node) {
         ...
         } else if (node.getLineNumber() != -1) {
            ClassSourceLookup.MethodCallByLine methodCall = new ClassSourceLookup.MethodCallByLine(
               node.getClassName(), node.getMethodName(), node.getLineNumber()
            );
