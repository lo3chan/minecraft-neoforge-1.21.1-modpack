...
class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
   ...
   private List<SmallSortedMap<K, V>.Entry> entryList = Collections.emptyList();
   ...
   public V get(Object o) {
      ...
      return index >= 0 ? this.entryList.get(index).getValue() : this.overflowEntries.get(key);
   }
   ...
   private V removeArrayEntryAt(int index) {
      ...
      if (!this.overflowEntries.isEmpty()) {
         ...
         this.entryList.add(new SmallSortedMap.Entry(iterator.next()));
         iterator.remove();
      }
   ...
   private int binarySearchInArray(K key) {
      ...
      while (left <= right) {
         ...
         int cmpx = key.compareTo(this.entryList.get(mid).getKey());
         if (cmpx < 0) {
            right = mid - 1;
         } else {
            if (cmpx <= 0) {
