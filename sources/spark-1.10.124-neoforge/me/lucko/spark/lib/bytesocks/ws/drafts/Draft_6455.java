...
public class Draft_6455 extends Draft {
      ...
      if (this.log.isTraceEnabled()) {
         this.log
            .trace(
               ...
               framedata.getPayloadData().remaining() > 1000 ? "too big to display" : new String(framedata.getPayloadData().array())
            );
      }
   ...
   private Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException, InvalidDataException {
      if (buffer == null) {
         ...
         if (payloadlength < 0 || payloadlength > 125) {
            Draft_6455.TranslatedPayloadMetaData payloadData = this.translateSingleFramePayloadLength(
         ...
         if (mask) {
            ...
            ((Buffer)buffer).position(buffer.position() + payload.limit());
         }
         ...
         if (this.log.isTraceEnabled()) {
            this.log
               .trace(
                  ...
                  frame.getPayloadData().remaining() > 1000 ? "too big to display" : new String(frame.getPayloadData().array())
               );
         }
   ...
   private Draft_6455.TranslatedPayloadMetaData translateSingleFramePayloadLength(
      ByteBuffer buffer, Opcode optcode, int oldPayloadlength, int maxpacketsize, int oldRealpacketsize
   ...
   private void translateSingleFrameCheckLengthLimit(long length) throws LimitExceededException {
      ...
      } else if (length > this.maxFrameSize) {
         ...
         if (this.incompleteframe != null) {
            try {
               ...
               if (expectedNextByteCount > availableNextByteCount) {
                  ...
                  ((Buffer)buffer).position(buffer.position() + availableNextByteCount);
                  return Collections.emptyList();
               }

               this.incompleteframe.put(buffer.array(), buffer.position(), expectedNextByteCount);
               ((Buffer)buffer).position(buffer.position() + expectedNextByteCount);
               Framedata cur = this.translateSingleFrame((ByteBuffer)((Buffer)this.incompleteframe.duplicate()).position(0));
               frames.add(cur);
         ...
         if (this.currentContinuousFrame.getOpcode() == Opcode.TEXT) {
            ...
            try {
               webSocketImpl.getWebSocketListener()
                  .onWebsocketMessage(webSocketImpl, Charsetfunctions.stringUtf8(this.currentContinuousFrame.getPayloadData()));
   ...
   private ByteBuffer getPayloadFromByteBufferList() throws LimitExceededException {
      ...
      ((Buffer)resultingByteBuffer).flip();
