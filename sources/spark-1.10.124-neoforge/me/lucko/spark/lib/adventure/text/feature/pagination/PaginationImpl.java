...
final class PaginationImpl<T> implements Examinable, Pagination<T> {
   ...
   PaginationImpl(
      ...
      @NotNull final Pagination.Renderer renderer,
      final char lineCharacter,
      @NotNull final Style lineStyle,
      final char previousPageButtonCharacter,
      @NotNull final Style previousPageButtonStyle,
      final char nextPageButtonCharacter,
      @NotNull final Style nextPageButtonStyle,
      @NotNull final Component title,
      @NotNull final Pagination.Renderer.RowRenderer<T> rowRenderer,
      @NotNull final Pagination.PageCommandFunction pageCommand
   ) {
      this.width = width;
   ...
   private Component renderFooterButtons(final int page, final int pages) {
         ...
         buttons.append(
            this.renderer
               .renderPreviousPageButton(
                  this.previousPageButtonCharacter, this.previousPageButtonStyle, ClickEvent.runCommand(this.pageCommand.pageCommand(page - 1))
               )
         );
         if (hasNextPage) {
      ...
      if (hasNextPage) {
         buttons.append(
            this.renderer
               .renderNextPageButton(this.nextPageButtonCharacter, this.nextPageButtonStyle, ClickEvent.runCommand(this.pageCommand.pageCommand(page + 1)))
         );
      }
