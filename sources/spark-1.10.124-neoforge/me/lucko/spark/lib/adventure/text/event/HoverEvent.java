...
public final class HoverEvent<V> implements Examinable, HoverEventSource<V>, StyleBuilderApplicable {
   ...
   public static HoverEvent<HoverEvent.ShowItem> showItem(
      @NotNull final Key item, @Range(from = 0L,to = 2147483647L) final int count, @Nullable final BinaryTagHolder nbt
   ) {
   ...
   @NotNull
   public static HoverEvent<HoverEvent.ShowItem> showItem(
      @NotNull final Keyed item, @Range(from = 0L,to = 2147483647L) final int count, @Nullable final BinaryTagHolder nbt
   ) {
      return showItem(HoverEvent.ShowItem.showItem(item, count, nbt));
   }
   ...
   @NotNull
   public static HoverEvent<HoverEvent.ShowItem> showItem(
      @NotNull final Keyed item, @Range(from = 0L,to = 2147483647L) final int count, @NotNull final Map<Key, ? extends DataComponentValue> dataComponents
   ) {
   ...
   public static final class Action<V> {
      ...
      public static final HoverEvent.Action<HoverEvent.ShowItem> SHOW_ITEM = new HoverEvent.Action<>(
         ...
         true,
         new HoverEvent.Action.Renderer<HoverEvent.ShowItem>() {
            @NotNull
            public <C> HoverEvent.ShowItem render(
               @NotNull final ComponentRenderer<C> renderer, @NotNull final C context, @NotNull final HoverEvent.ShowItem value
            ) {
               return value;
            }
         }
      ...
      public static final HoverEvent.Action<HoverEvent.ShowEntity> SHOW_ENTITY = new HoverEvent.Action<>(
         "show_entity",
         HoverEvent.ShowEntity.class,
         true,
         new HoverEvent.Action.Renderer<HoverEvent.ShowEntity>() {
            @NotNull
            public <C> HoverEvent.ShowEntity render(
               @NotNull final ComponentRenderer<C> renderer, @NotNull final C context, @NotNull final HoverEvent.ShowEntity value
            ) {
      ...
      public static final Index<String, HoverEvent.Action<?>> NAMES = Index.create(
         constant -> constant.name, SHOW_TEXT, SHOW_ITEM, SHOW_ENTITY, SHOW_ACHIEVEMENT
      );
   ...
   public static final class ShowEntity implements Examinable {
      ...
      public HoverEvent.ShowEntity type(@NotNull final Key type) {
         return Objects.requireNonNull(type, "type").equals(this.type) ? this : new HoverEvent.ShowEntity(type, this.id, this.name);
      }
      ...
      public HoverEvent.ShowEntity id(@NotNull final UUID id) {
         return Objects.requireNonNull(id).equals(this.id) ? this : new HoverEvent.ShowEntity(this.type, id, this.name);
      }
   ...
   public static final class ShowItem implements Examinable {
      ...
      private ShowItem(
         @NotNull final Key item,
         @Range(from = 0L,to = 2147483647L) final int count,
         @Nullable final BinaryTagHolder nbt,
         @NotNull final Map<Key, ? extends DataComponentValue> dataComponents
      ) {
         this.item = item;
         this.count = count;
         this.nbt = nbt;
      ...
      public HoverEvent.ShowItem item(@NotNull final Key item) {
         return Objects.requireNonNull(item, "item").equals(this.item) ? this : new HoverEvent.ShowItem(item, this.count, this.nbt, this.dataComponents);
      }
      ...
      public HoverEvent.ShowItem dataComponents(@NotNull final Map<Key, DataComponentValue> holder) {
         return Objects.equals(this.dataComponents, holder)
            ? this
            : new HoverEvent.ShowItem(
               this.item, this.count, null, holder.isEmpty() ? Collections.emptyMap() : Collections.unmodifiableMap(new HashMap<>(holder))
            );
      }
      ...
      public <V extends DataComponentValue> Map<Key, V> dataComponentsAs(@NotNull final Class<V> targetType) {
         if (this.dataComponents.isEmpty()) {
            ...
            for (Entry<Key, DataComponentValue> entry : this.dataComponents.entrySet()) {
      ...
      public boolean equals(@Nullable final Object other) {
         if (this == other) {
            ...
            return this.item.equals(that.item)
               ...
               && Objects.equals(this.dataComponents, that.dataComponents);
         } else {
            return false;
         }
      }
      ...
      public int hashCode() {
         ...
         return 31 * result + Objects.hashCode(this.dataComponents);
      }
