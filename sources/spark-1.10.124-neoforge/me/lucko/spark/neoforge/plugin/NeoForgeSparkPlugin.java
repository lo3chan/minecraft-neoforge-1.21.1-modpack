...
public abstract class NeoForgeSparkPlugin implements SparkPlugin {
   ...
   protected static <T> void registerCommands(CommandDispatcher<T> dispatcher, Command<T> executor, SuggestionProvider<T> suggestor, String... aliases) {
      if (aliases.length != 0) {
         ...
         LiteralArgumentBuilder<T> command = (LiteralArgumentBuilder<T>)((LiteralArgumentBuilder)LiteralArgumentBuilder.literal(mainName).executes(executor))
            .then(RequiredArgumentBuilder.argument("args", StringArgumentType.greedyString()).suggests(suggestor).executes(executor));
   ...
   protected static String[] processArgs(CommandContext<?> context, boolean tabComplete, String... aliases) {
      ...
      return split.length != 0 && Arrays.asList(aliases).contains(split[0]) ? Arrays.copyOfRange(split, 1, split.length) : null;
   }
}
