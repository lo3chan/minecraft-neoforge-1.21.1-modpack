...
public interface ComponentFunctions {
   ...
   default ComponentFunctions kjs$set(Context cx, DataComponentType<?> component, Object value) {
      Optional<?> wrapped = (Optional<?>)DataComponentWrapper.tryWrapComponent(cx, component, value)
         .getOrThrow(
            msg -> new KubeRuntimeException("Failed to wrap data component %s from '%s': %s".formatted(component, value, msg)).source(SourceLine.of(cx))
         );
      return wrapped.isPresent() ? this.kjs$override((DataComponentType<Object>)component, wrapped.get()) : this.kjs$remove(component);
   }
   ...
   default ComponentFunctions kjs$patch(Context cx, DataComponentPatch components) {
      for (Entry<DataComponentType<?>, Optional<?>> entry : components.entrySet()) {
         this.kjs$override((DataComponentType<Object>)entry.getKey(), Cast.to(entry.getValue().orElse(null)));
      }
   ...
   default ComponentFunctions kjs$resetComponents(Context cx) {
      if (this.kjs$getComponentMap() instanceof PatchedDataComponentMap map) {
   ...
   default String kjs$getComponentString(Context cx) {
      return this.kjs$getComponentMap() instanceof PatchedDataComponentMap map
         ? DataComponentWrapper.patchToString(new StringBuilder(), RegistryAccessContainer.of(cx).nbt(), map.asPatch()).toString()
         : "[]";
   }

   default void kjs$setCustomData(CompoundTag tag) {
      if (tag != null && !tag.isEmpty()) {
         this.kjs$override(DataComponents.CUSTOM_DATA, CustomData.of(tag));
      } else {
   ...
   default CompoundTag kjs$getCustomData() {
      CustomData d = this.kjs$get(DataComponents.CUSTOM_DATA);
   ...
   default Component kjs$getCustomName() {
      return this.kjs$get(DataComponents.CUSTOM_NAME);
   }
   ...
   default void kjs$setAdditionalTooltipHidden() {
      this.kjs$setUnit(DataComponents.HIDE_ADDITIONAL_TOOLTIP);
   }

   default void kjs$setTooltipHidden() {
      this.kjs$setUnit(DataComponents.HIDE_TOOLTIP);
   }
   ...
   default void kjs$setEntityData(CompoundTag tag) {
      this.kjs$override(DataComponents.ENTITY_DATA, CustomData.of(tag));
   }
   ...
   default void kjs$setProfile(@Nullable String name, @Nullable UUID uuid) {
      this.kjs$override(
         ...
         new ResolvableProfile(
            Optional.ofNullable(name != null && name.isBlank() ? null : name),
            Optional.ofNullable(uuid != null && uuid.getLeastSignificantBits() == 0L && uuid.getMostSignificantBits() == 0L ? null : uuid),
            new PropertyMap()
         )
      );
   }
