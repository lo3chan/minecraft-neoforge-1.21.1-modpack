...
public abstract class ExplosionKubeEvent implements KubeLevelEvent {
   ...
   public LevelBlock getBlock() {
      return this.level.kjs$getBlock(BlockPos.containing(this.getPosition()));
   }
   ...
   public static class After extends ExplosionKubeEvent {
      ...
      public EntityArrayList getAffectedEntities() {
         return new EntityArrayList(this.affectedEntities);
      }
      ...
      public List<LevelBlock> getAffectedBlocks() {
         List<LevelBlock> list = new ArrayList<>(this.explosion.getToBlow().size());

         for (BlockPos pos : this.explosion.getToBlow()) {
            list.add(this.level.kjs$getBlock(pos));
         }
