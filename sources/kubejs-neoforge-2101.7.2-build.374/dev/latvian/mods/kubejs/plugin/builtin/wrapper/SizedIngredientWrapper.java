...
public interface SizedIngredientWrapper {
   ...
   private static SizedIngredient wrapTrivial(Context cx, Object from) {
      return switch (from) {
         ...
         case ItemStack stack -> Ingredient.of(new ItemStack[]{stack.kjs$withCount(1)}).kjs$withCount(stack.getCount());
         case ItemLike item -> Ingredient.of(new ItemLike[]{item}).kjs$asStack();
   ...
   static DataResult<SizedIngredient> wrapResult(Context cx, Object from) {
      SizedIngredient trivial = wrapTrivial(cx, from);
      if (trivial != null) {
         ...
         try {
            ...
            return DataResult.error(() -> "Error parsing sized ingredient: " + var4);
         }
      } else {
   ...
   static SizedIngredient wrap(Context cx, Object from) {
      ...
      return trivial != null
         ? trivial
         : (SizedIngredient)wrapResult(cx, from)
