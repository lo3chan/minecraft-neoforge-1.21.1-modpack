...
public interface TextWrapper {
   ...
   static MutableComponent wrap(Context cx, @Nullable Object o) {
      Object from = UtilsJS.wrap(o, JSObjectType.ANY);

      return switch (from) {
         case null -> Component.literal("null");
         case MutableComponent component -> component;
         case Component componentx -> componentx.copy();
         case Enum<?> e -> ofString(e.name());
         case StringTag tag -> {
            String s = tag.getAsString();
            yield s.startsWith("{") && s.endsWith("}")
               ? ((Component)ComponentSerialization.CODEC
                     .decode(JsonOps.INSTANCE, (JsonElement)JsonUtils.GSON.fromJson(s, JsonObject.class))
                     .mapOrElse(Pair::getFirst, error -> Component.literal("Error: " + error)))
                  .copy()
               : ofString(s);
         }
         case Tag tagx -> (MutableComponent)NbtUtils.toPrettyComponent(tagx);
         case JsonElement json -> ((Component)ComponentSerialization.CODEC
         ...
         case Map<?, ?> map when map.containsKey("text") || map.containsKey("translate") -> {
            ...
            Object var23 = map.get("extra");
            if (var23 instanceof Iterable) {
               for (Object e : (Iterable)var23) {
         ...
         case Map<?, ?> mapx -> wrap(cx, JsonUtils.of(cx, mapx));
         case Iterable<?> list -> {
   ...
   static Component ofTag(Tag tag) {
      try {
         ...
         return Component.literal("Error: " + var2);
      }
   }
      ...
      if (o == null) {
         ...
         if (json != null) {
            ...
            return switch (var5) {
               ...
               default -> {
                  Action[] var7 = Action.values();
                  int var8 = var7.length;
                  int var9 = 0;

                  while (true) {
                     if (var9 >= var8) {
                        yield new ClickEvent(Action.OPEN_URL, s);
                        break;
                     }
