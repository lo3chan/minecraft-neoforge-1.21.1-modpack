...
public record BlockStateComponent(RecipeComponentType<?> type, boolean preferObjectForm, boolean allowEmpty) implements RecipeComponent<BlockState> {
   ...
   public static final RecipeComponentType<BlockState> BLOCK = RecipeComponentType.unit(
      KubeJS.id("block_state"), type -> new BlockStateComponent(type, true, false)
   );
   public static final RecipeComponentType<BlockState> BLOCK_STRING = RecipeComponentType.unit(
      KubeJS.id("block_state_string"), type -> new BlockStateComponent(type, false, false)
   );
   public static final RecipeComponentType<BlockState> OPTIONAL_BLOCK = RecipeComponentType.unit(
      KubeJS.id("optional_block_state"), type -> new BlockStateComponent(type, true, true)
   );
   public static final RecipeComponentType<BlockState> OPTIONAL_BLOCK_STRING = RecipeComponentType.unit(
      KubeJS.id("optional_block_state_string"), type -> new BlockStateComponent(type, false, true)
   );
   ...
   public BlockState wrap(RecipeScriptContext cx, Object from) {
      return switch (from) {
         ...
         case null, default -> {
            Map<String, Object> map = cx.cx().optionalMapOf(from);
            yield map == null
               ? BlockWrapper.parseBlockState(cx.registries(), String.valueOf(from))
               : (BlockState)BlockState.CODEC.parse(JsonOps.INSTANCE, JsonUtils.GSON.toJsonTree(from)).getPartialOrThrow(message -> {
