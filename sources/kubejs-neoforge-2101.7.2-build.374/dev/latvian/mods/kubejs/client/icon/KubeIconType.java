...
public record KubeIconType<T extends KubeIcon>(ResourceLocation id, MapCodec<T> codec, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec) {
   public static final Lazy<Map<ResourceLocation, KubeIconType<?>>> TYPES = Lazy.map(
      map -> KubeJSPlugins.forEachPlugin(type -> map.put(type.id, type), KubeJSPlugin::registerIconTypes)
   );
   public static final Codec<KubeIconType<?>> CODEC = KubeJSCodecs.KUBEJS_ID.comapFlatMap(id -> {
      KubeIconType<?> type = TYPES.get().get(id);
      return type != null ? DataResult.success(type) : DataResult.error(() -> "Unknown icon type: " + id);
   }, KubeIconType::id);
   public static final StreamCodec<RegistryFriendlyByteBuf, KubeIconType<?>> STREAM_CODEC = KubeJSStreamCodecs.KUBEJS_ID
      .map(s -> TYPES.get().get(s), KubeIconType::id);
