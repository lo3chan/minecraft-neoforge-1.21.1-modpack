...
public class KubeRayTraceResult {
   ...
   public KubeRayTraceResult(Entity from, @Nullable HitResult result, double d) {
      ...
      this.distance = d;
      this.type = result == null ? Type.MISS : result.getType();
      this.hit = result == null ? from.getEyePosition() : result.getLocation();
      if (result instanceof BlockHitResult b && result.getType() == Type.BLOCK) {
         this.block = from.level().kjs$getBlock(b.getBlockPos());
         this.facing = b.getDirection();
         this.entity = null;
      } else if (result instanceof EntityHitResult e && result.getType() == Type.ENTITY) {
         this.block = null;
         this.facing = null;
   ...
   public double getHitX() {
      return this.hit == null ? 0.0 / 0.0 : this.hit.x;
   }

   public double getHitY() {
      return this.hit == null ? 0.0 / 0.0 : this.hit.y;
   }

   public double getHitZ() {
      return this.hit == null ? 0.0 / 0.0 : this.hit.z;
   }
}
