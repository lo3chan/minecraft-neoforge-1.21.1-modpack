...
public record SlotFilter(Ingredient item, int index) {
   ...
   public static final TypeInfo TYPE_INFO = TypeInfo.INT
      .or(IngredientWrapper.TYPE_INFO)
      .or(new JSObjectTypeInfo(List.of(new JSOptionalParam("item", IngredientWrapper.TYPE_INFO, true), new JSOptionalParam("index", TypeInfo.INT, true))));
   public static final Codec<SlotFilter> CODEC = RecordCodecBuilder.create(
   ...
   public static final StreamCodec<RegistryFriendlyByteBuf, SlotFilter> STREAM_CODEC = StreamCodec.composite(
      Ingredient.CONTENTS_STREAM_CODEC, SlotFilter::item, ByteBufCodecs.VAR_INT, SlotFilter::index, SlotFilter::of
   );
   ...
   public static SlotFilter wrap(Context cx, Object o, TypeInfo target) {
      if (o instanceof Number num) {
         ...
         return !(o instanceof String) && !(o instanceof Ingredient) && !(o instanceof NativeRegExp) && !(o instanceof Pattern)
            ? (SlotFilter)((RecordTypeInfo)target).wrap(cx, o, target)
            : of(IngredientWrapper.wrap(cx, o), -1);
      }
   }
