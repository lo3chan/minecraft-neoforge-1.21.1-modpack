...
public interface TextAction {
   ...
   StreamCodec<RegistryFriendlyByteBuf, TextAction> STREAM_CODEC = new StreamCodec<RegistryFriendlyByteBuf, TextAction>() {
      public TextAction decode(RegistryFriendlyByteBuf buf) {
         int id = buf.readByte();
         return (TextAction)TextAction.MAP.get(id).streamCodec().decode(buf);
      }

      public void encode(RegistryFriendlyByteBuf buf, TextAction value) {
         buf.writeByte(value.type().type());
         value.type().streamCodec().encode(buf, (TextAction)Cast.to(value));
      }
   };
