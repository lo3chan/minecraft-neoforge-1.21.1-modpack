...
public interface KubeDataGenerator extends KubeResourceGenerator {
   ...
   default void setCompostable(ItemPredicate items, float chance, boolean canVillagerCompost) {
      this.dataMap(NeoForgeDataMaps.COMPOSTABLES, callback -> add(callback, items, new Compostable(chance, canVillagerCompost)));
   }

   default void removeCompostable(ItemPredicate items) {
      this.dataMap(NeoForgeDataMaps.COMPOSTABLES, callback -> remove(callback, items));
   }

   default void setFurnaceFuel(ItemPredicate items, TickDuration ticks) {
      this.dataMap(NeoForgeDataMaps.FURNACE_FUELS, callback -> add(callback, items, new FurnaceFuel(ticks.intTicks())));
   }

   default void removeFurnaceFuel(ItemPredicate items) {
      this.dataMap(NeoForgeDataMaps.FURNACE_FUELS, callback -> remove(callback, items));
   }
   ...
   private static <T> void add(VirtualDataMapFile<Item, T> dataMap, ItemPredicate filter, T data) {
      ...
      if (tag != null) {
         ...
         for (Item item : filter.kjs$getItemTypes()) {
   ...
   default void setMonsterRoomMobs(EntityType<?> entityType, int weight) {
      this.dataMap(NeoForgeDataMaps.MONSTER_ROOM_MOBS, callback -> callback.accept(entityType.kjs$getIdLocation(), new MonsterRoomMob(Weight.of(weight))));
   }
   ...
   default void setRaidHeroGifts(VillagerProfession profession, ResourceKey<LootTable> lootTable) {
      this.dataMap(
         NeoForgeDataMaps.RAID_HERO_GIFTS, callback -> callback.accept(BuiltInRegistries.VILLAGER_PROFESSION.getKey(profession), new RaidHeroGift(lootTable))
      );
   }

   default void setVibrationFrequency(GameEvent gameEvent, int frequency) {
      this.dataMap(
         NeoForgeDataMaps.VIBRATION_FREQUENCIES, callback -> callback.accept(BuiltInRegistries.GAME_EVENT.getKey(gameEvent), new VibrationFrequency(frequency))
      );
   }
