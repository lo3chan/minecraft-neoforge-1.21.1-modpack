...
public record GeneratedData(ResourceLocation id, Supplier<byte[]> data) implements IoSupplier<InputStream> {
   ...
   public static final GeneratedData PACK_ICON = new GeneratedData(
      KubeJS.id("textures/kubejs_logo.png"),
      () -> {
         try {
            return Files.readAllBytes(
               KubeJS.thisMod.getModInfo().getOwningFile().getFile().findResource(new String[]{"assets", "kubejs", "textures", "kubejs_logo.png"})
            );
   ...
   public static GeneratedData json(ResourceLocation id, Supplier<JsonElement> json) {
      return new GeneratedData(
         id.getPath().endsWith(".json") ? id : ResourceLocation.fromNamespaceAndPath(id.getNamespace(), id.getPath() + ".json"),
         Lazy.of(() -> JsonIO.toString(json.get()).getBytes(StandardCharsets.UTF_8))
      );
   }
   ...
   public boolean equals(Object obj) {
      return obj instanceof GeneratedData g && this.id.equals(g.id);
   }
