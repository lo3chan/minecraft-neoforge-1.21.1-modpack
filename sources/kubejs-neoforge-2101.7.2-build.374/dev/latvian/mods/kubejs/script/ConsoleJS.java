...
public class ConsoleJS {
   ...
   @HideFromJS
   public static final Function<String, String> ERROR_REDUCE = s -> !s.startsWith("java.util.concurrent.ForkJoin") && !s.startsWith("jdk.internal.")
      ? GARBAGE_PATTERN.matcher(s).replaceAll("").replace("dev.latvian.mods.", "…")
      : "";
   ...
   public static ConsoleJS getCurrent(@Nullable Context cx) {
      return cx instanceof KubeJSContext kcx ? kcx.getConsole() : STARTUP;
   }
   ...
   public synchronized void startCapturingErrors() {
      this.setCapturingErrors(true);
   }

   public synchronized void stopCapturingErrors() {
      this.setCapturingErrors(false);
   }
      ...
      if (DevProperties.get().alwaysCaptureErrors) {
         ...
         if (!FMLLoader.isProduction()) {
            if (this.capturingErrors) {
               this.logger.info("Capturing errors for {} scripts enabled", this.scriptType.name);
            } else {
               this.logger.info("Capturing errors for {} scripts disabled", this.scriptType.name);
            }
         }
      }
   }
   ...
   public synchronized void resetFile() {
      ...
      this.scriptType.executor.execute(() -> {
         try {
            ...
            this.logger.error("Failed to clear the log file: {}", String.valueOf(var2));
         }
      });
   }
   ...
   private ConsoleLine line(LogType type, SourceLine sourceLine, Object object, @Nullable Throwable error) {
      ...
      ConsoleLine line = new ConsoleLine(
         this,
         timestamp,
         o != null && !o.getClass().isPrimitive() && !(o instanceof Boolean) && !(o instanceof String) && !(o instanceof Number) && !(o instanceof WrappedJS)
      ...
      } else if (error != null && this.capturingErrors) {
         for (StackTraceElement elx : error.getStackTrace()) {
            if (elx.getFileName() != null && elx.getLineNumber() >= 0 && elx.getClassName().startsWith("dev.latvian.mods.kubejs.")) {
               line.withSourceLine(elx.getFileName(), elx.getLineNumber());
            }
         }
      }
      ...
      if (line.sourceLines.isEmpty()) {
         ContextFactory factory = this.contextFactory == null ? null : this.contextFactory.get();
         if (factory != null) {
   ...
   private ConsoleLine log(LogType type, SourceLine sourceLine, @Nullable Throwable error, Object message) {
      if (this.shouldPrint()) {
         ConsoleLine line = this.line(type, sourceLine, message, error);
         ...
         if (this.writeToFile) {
            ...
            KubeJSWeb.broadcastEvent(this.wsBroadcaster, type.id, "", line);
         }
   ...
   public synchronized void flush(boolean sync) {
      if (!this.writeQueue.isEmpty()) {
         List<String> lines = Arrays.asList(this.writeQueue.toArray(StringUtilsWrapper.EMPTY_STRING_ARRAY));
         this.writeQueue.clear();
         if (sync) {
            try {
               Files.write(this.logFile, lines, StandardOpenOption.APPEND);
            } catch (Exception var4) {
               this.logger.error("Failed to write to the log file: {}", String.valueOf(var4));
            }
         } else {
            this.scriptType.executor.execute(() -> {
               try {
                  ...
                  this.logger.error("Failed to write to the log file: {}", String.valueOf(var3));
               }
            });
         }
      }
   ...
   public ConsoleLine info(Object message) {
      return this.log(LogType.INFO, SourceLine.UNKNOWN, null, message);
   }
   ...
   public ConsoleLine warn(Object message) {
      return this.log(LogType.WARN, SourceLine.UNKNOWN, null, message);
   }

   public ConsoleLine warn(String message, Throwable error, @Nullable Pattern exitPattern) {
      return this.warn(message, SourceLine.UNKNOWN, error, exitPattern);
   }

   public ConsoleLine warn(String message, SourceLine sourceLine, Throwable error, @Nullable Pattern exitPattern) {
      if (this.shouldPrint()) {
         ConsoleLine l = this.log(LogType.WARN, sourceLine, error, messageForPrint(message, error));
   ...
   public ConsoleLine error(Object message) {
      return this.log(LogType.ERROR, SourceLine.UNKNOWN, null, message);
   }

   public ConsoleLine error(String message, Throwable error, @Nullable Pattern exitPattern) {
      return this.error(message, SourceLine.UNKNOWN, error, exitPattern);
   }

   public ConsoleLine error(String message, SourceLine sourceLine, Throwable error, @Nullable Pattern exitPattern) {
      if (this.shouldPrint()) {
         ConsoleLine l = this.log(LogType.ERROR, sourceLine, error, messageForPrint(message, error));
   ...
   public ConsoleLine debug(Object message) {
      return this.shouldPrintDebug() ? this.log(LogType.DEBUG, SourceLine.UNKNOWN, null, message) : null;
   }
   ...
   public Component errorsComponent(String command) {
      return Component.empty()
         ...
         .append(Component.literal(" KubeJS errors found [" + this.errors.size() + "]!").kjs$red())
   ...
   public HTTPResponse getErrorsResponse(KJSHTTPRequest ctx) {
      return HTTPResponse.ok().content(JsonContent.array(json -> {
         for (ConsoleLine error : this.errors) {
   ...
   public HTTPResponse getWarningsResponse(KJSHTTPRequest ctx) {
      return HTTPResponse.ok().content(JsonContent.array(json -> {
         for (ConsoleLine error : this.warnings) {
            json.add(error.toJson());
         }
      }));
   }
