      ...
      for (Half half : halfValues) {
         for (Boolean open : openValues) {
            for (Direction facing : facingValues) {
               bs.variant(
                  "facing=" + facing.getSerializedName() + ",half=" + half.getSerializedName() + ",open=" + open,
                  (Consumer<VariantBlockStateGenerator.Variant>)(v -> {
                     VariantBlockStateGenerator.Model m = v.model(open ? modelOpen : (half == Half.TOP ? modelTop : modelBottom));
                     if (open) {
                        m.y(switch (facing) {
                           case EAST -> 90;
                           case SOUTH -> 180;
