...
public record KubeServerData(Optional<RecipeViewerData> recipeViewerData, List<ItemTooltipData> itemTooltipData) {
   public static final StreamCodec<RegistryFriendlyByteBuf, KubeServerData> STREAM_CODEC = StreamCodec.composite(
      ByteBufCodecs.optional(RecipeViewerData.STREAM_CODEC),
      KubeServerData::recipeViewerData,
      ItemTooltipData.STREAM_CODEC.apply(ByteBufCodecs.list()),
      KubeServerData::itemTooltipData,
      KubeServerData::new
   );

   public static KubeServerData collect() {
      ArrayList<ItemTooltipData> itemTooltipData = new ArrayList<>();
      ItemEvents.MODIFY_TOOLTIPS.post(ScriptType.SERVER, new ModifyItemTooltipsKubeEvent(itemTooltipData::add));
      return new KubeServerData(Optional.ofNullable(RecipeViewerData.collect()), List.copyOf(itemTooltipData));
   }
}
