...
public class KubeJSModEventHandler {
   ...
   public static void creativeTab(BuildCreativeModeTabContentsEvent event) {
      ...
      if (StartupEvents.MODIFY_CREATIVE_TAB.hasListeners(tabId)) {
         StartupEvents.MODIFY_CREATIVE_TAB
            .post(ScriptType.STARTUP, tabId, new CreativeTabKubeEvent(event.getTab(), event.hasPermissions(), new CreativeTabCallbackForge(event)));
      }
   }
   ...
   private static void loadComplete0() {
      ...
      if (!ConsoleJS.STARTUP.errors.isEmpty()) {
         ...
         ConsoleLine[] lines = ConsoleJS.STARTUP.errors.toArray(ConsoleLine.EMPTY_ARRAY);
            ...
            () -> {
               try {
                  HttpResponse<String> response = HttpClient.newBuilder()
                     ...
                     .build()
                     .send(
                        HttpRequest.newBuilder().uri(URI.create("https://v.kubejs.com/update-check?" + KubeJS.QUERY)).GET().build(),
   ...
   public static void registerCapabilities(RegisterCapabilitiesEvent event) {
      for (BlockEntityInfo info : RegistryObjectStorage.BLOCK_ENTITY
         .objects
         .values()
         .stream()
         .filter(BlockEntityBuilder.class::isInstance)
         .map(b -> ((BlockEntityBuilder)b).info)
         .toList()) {
         for (BlockEntityAttachmentInfo attachment : info.attachments.values()) {
            for (BlockCapability<?, ?> capability : attachment.factory().getCapabilities()) {
               event.registerBlockEntity(capability, info.entityType, new KubeJSModEventHandler.KubeEntityCapabilityProvider(capability, attachment));
            }
         }
      }
   }

   private record KubeEntityCapabilityProvider<CAP, SRC>(BlockCapability<CAP, SRC> capability, BlockEntityAttachmentInfo attachment)
      implements ICapabilityProvider<KubeBlockEntity, SRC, CAP> {
      @Nullable
      public CAP getCapability(KubeBlockEntity entity, SRC from) {
         return !this.attachment.directions().isEmpty() && !(from instanceof Direction d && this.attachment.directions().contains(d))
            ? null
            : entity.attachmentArray[this.attachment.index()].attachment().getCapability(this.capability);
      }
   }
}
