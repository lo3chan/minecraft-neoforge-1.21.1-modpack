...
public class EMIAddEntriesKubeEvent implements AddEntriesKubeEvent {
   ...
   public void add(Context cx, Object[] items) {
      for (Object item : items) {
         Object entry = this.type.wrapEntry(cx, item);
         if (this.type == RecipeViewerEntryType.ITEM) {
            this.registry.addEmiStack(EmiStack.of((ItemStack)entry));
         } else if (this.type == RecipeViewerEntryType.FLUID) {
            this.registry.addEmiStack(EMIIntegration.fluid((FluidStack)entry));
         }
      }
   }
}
