      ...
      return !(input instanceof JsonObject)
         ...
         : DataResult.success(
            ((JsonObject)input)
               .entrySet()
               .stream()
               .map(entry -> Pair.of(new JsonPrimitive(entry.getKey()), entry.getValue() instanceof JsonNull ? null : (JsonElement)entry.getValue()))
         );
   }
   ...
   public JsonElement remove(JsonElement input, String key) {
      if (input instanceof JsonObject) {
         ...
         ((JsonObject)input)
            ...
            .forEach(entry -> result.put((String)entry.getKey(), (JsonElement)entry.getValue()));
         return result;
      } else {
         return input;
      }
