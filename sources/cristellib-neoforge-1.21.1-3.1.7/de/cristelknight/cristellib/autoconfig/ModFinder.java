...
public class ModFinder {
   public static void addAutoConfigs(
      String modId, Set<String> structureSets, Map<String, Set<StructureConfig>> structureConfigData, CristelLibRegistry registry
   ) {
      ...
      StructureConfig toggleConfig = StructureConfig.createWithDefaultConfigPath(
         customPath ? ACInfoData.currentData.get(modId).autoConfigPath() : customSubPath + modId, "structure_toggle_config", ConfigType.TOGGLE
      );
      StructureConfig placementConfig = StructureConfig.createWithDefaultConfigPath(
         customPath ? ACInfoData.currentData.get(modId).autoConfigPath() : customSubPath + modId, "structure_placement_config", ConfigType.PLACEMENT
      );
      ...
      if (placementConfig.setsNotEmpty()) {
         configSet.add(placementConfig);
      }

      if (toggleConfig.setsNotEmpty()) {
         configSet.add(toggleConfig);
      }
   ...
   private static ResourceLocation getLocation(Path rootPath) {
      ...
      return namespace.equals("minecraft") ? ResourceLocation.withDefaultNamespace(path) : ResourceLocation.fromNamespaceAndPath(namespace, path);
   }
