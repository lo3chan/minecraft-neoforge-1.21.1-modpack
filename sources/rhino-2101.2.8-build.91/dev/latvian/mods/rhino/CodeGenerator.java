...
class CodeGenerator extends Icode {
   ...
   private void generateFunctionICode(Context cx) {
      ...
      if (theFunction.getDefaultParams() != null) {
         this.itsData.argsHasDefaults = true;
      }
   ...
   private void generateICodeFromTree(Node tree, Context cx) {
      ...
      this.itsData.arity = FunctionNode.calculateFunctionArity(this.scriptOrFn);
   ...
   private void visitExpression(Node node, int contextFlags) {
      ...
      switch (type) {
         ...
         case 38:
         case 71:
            boolean isOptionalChainingCall = node.getIntProp(31, 0) == 1;
            CodeGenerator.CompleteOptionalCallJump completeOptionalCallJump = null;
            if (type == 30) {
               this.visitExpression(child, 0);
            } else {
            ...
            if (type != 71 && callType != 0) {
               this.addIndexOp(isOptionalChainingCall ? -76 : -21, argCount);
   ...
   private CodeGenerator.CompleteOptionalCallJump generateCallFunAndThis(Node left, boolean isOptionalChainingCall) {
   ...
   private CodeGenerator.CompleteOptionalCallJump completeOptionalCallJump() {
      this.addIcode(-1);
      this.stackChange(1);
      ...
      int afterLabel = this.iCodeTop;
      this.addGotoOp(5);
      return new CodeGenerator.CompleteOptionalCallJump(putArgsAndDoCallLabel, afterLabel);
   }
   ...
   private void visitLiteralValue(Node child) {
      ...
      if (childType == 153) {
         ...
         this.addIcode(-57);
      } else if (childType == 154) {
         this.visitExpression(child.getFirstChild(), 0);
         this.addIcode(-58);
      } else if (childType == 165) {
         this.visitExpression(child.getFirstChild(), 0);
         this.addIcode(-30);
      } else {
         this.visitExpression(child, 0);
   ...
   private void addGoto(Node target, int gotoOp) {
      ...
      if (targetPC != -1) {
         ...
         this.fixupTable[top] = (long)label << 32 | gotoPC;
      }
   }
