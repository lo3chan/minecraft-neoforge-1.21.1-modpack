...
class SpecialRef extends Ref {
   ...
   public Object get(Context cx) {
      return switch (this.type) {
         case 0 -> ScriptRuntime.getObjectProp(cx, this.target, this.name);
         case 1 -> this.target.getPrototype(cx);
         case 2 -> this.target.getParentScope();
      ...
      switch (this.type) {
         ...
         case 2:
            ...
            if (this.type == 1) {
               ...
               MemberType typeOfTarget = ScriptRuntime.typeof(cx, this.target);
               if (typeOfTarget == MemberType.FUNCTION) {
                  if (value == null) {
                     this.target.setPrototype(Undefined.SCRIPTABLE_INSTANCE);
                     return value;
                  }
