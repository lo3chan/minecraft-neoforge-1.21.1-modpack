...
final class Arguments extends IdScriptableObject {
   ...
   public Arguments(NativeCall activation, Context cx) {
      ...
      this.defineProperty(
         cx,
         SymbolKey.ITERATOR,
         TopLevel.getBuiltinPrototype(ScriptableObject.getTopLevelScope(parent), TopLevel.Builtins.Array, cx).get(cx, "values", parent),
         2
      );
   }
   ...
   protected int findInstanceIdInfo(String s, Context cx) {
      ...
      if (!cx.isStrictMode() || id != 1 && id != 3) {
         if (id == 0) {
            ...
            int attr = switch (id) {
               case 1 -> this.calleeAttr;
               case 2 -> this.lengthAttr;
               case 3 -> this.callerAttr;
   ...
   void defineAttributesForStrictMode(Context cx) {
      if (cx.isStrictMode()) {
         this.setGetterOrSetter(cx, "caller", 0, new Arguments.ThrowTypeError("caller", cx), true);
         this.setGetterOrSetter(cx, "caller", 0, new Arguments.ThrowTypeError("caller", cx), false);
         this.setGetterOrSetter(cx, "callee", 0, new Arguments.ThrowTypeError("callee", cx), true);
         this.setGetterOrSetter(cx, "callee", 0, new Arguments.ThrowTypeError("callee", cx), false);
   ...
   private static class ThrowTypeError extends BaseFunction {
      ...
      ThrowTypeError(String propertyName, Context cx) {
         ...
         super.setInstanceIdAttributes(3, 7, cx);
      }
