...
public abstract class IdScriptableObject extends ScriptableObject implements IdFunctionCall {
   ...
   protected void setInstanceIdAttributes(int id, int attr, Context cx) {
      throw ScriptRuntime.constructError(
         cx, "InternalError", "Changing attributes not supported for " + this.getClassName() + " " + this.getInstanceIdName(id) + " property"
      );
   }
      ...
      if (key instanceof String name) {
         ...
         if (info != 0) {
            ...
            if (!isAccessorDescriptor(cx, desc)) {
               ...
               Slot slot = this.queryOrFakeSlot(cx, key);
               this.checkPropertyChangeForSlot(cx, name, slot, desc);
               int attr = info >>> 16;
               Object value = getProperty(desc, "value", cx);
               if (value != NOT_FOUND && ((attr & 1) == 0 || (attr & 4) == 0)) {
               ...
               attr = this.applyDescriptorToAttributeBitset(
                  cx, attr, getProperty(desc, "enumerable", cx), getProperty(desc, "writable", cx), getProperty(desc, "configurable", cx)
               );
               this.setAttributes(cx, name, attr);
               return;
            }
         ...
         if (this.prototypeValues != null) {
            ...
            if (id != 0) {
               if (!isAccessorDescriptor(cx, desc)) {
                  ...
                  this.prototypeValues
                     .setAttributes(
                        id,
                        this.applyDescriptorToAttributeBitset(
                           cx, attr, getProperty(desc, "enumerable", cx), getProperty(desc, "writable", cx), getProperty(desc, "configurable", cx)
                        ),
                        cx
                     );
   ...
   protected ScriptableObject getOwnPropertyDescriptor(Context cx, Object id) {
      ...
      if (desc == null) {
         if (id instanceof String) {
            return this.getBuiltInDescriptor((String)id, cx);
         }

         if (ScriptRuntime.isSymbol(id)) {
            if (id instanceof SymbolKey) {
               return this.getBuiltInDescriptor((SymbolKey)id, cx);
            }

            return this.getBuiltInDescriptor(((NativeSymbol)id).getKey(), cx);
         }
      }
   ...
   private ScriptableObject getBuiltInDescriptor(String name, Context cx) {
      ...
      Slot slot = this.getBuiltInSlot(name, cx);
   ...
   private ScriptableObject getBuiltInDescriptor(Symbol key, Context cx) {
      ...
      Slot slot = this.getBuiltInSlot(key, cx);
      return slot == null ? null : buildDataDescriptor(scope, slot.value, slot.getAttributes(), cx);
   }
   ...
   private Slot queryOrFakeSlot(Context cx, Object id) {
      ...
      if (slot == null) {
         if (id instanceof String name) {
            return this.getBuiltInSlot(name, cx);
         }

         if (ScriptRuntime.isSymbol(id)) {
            if (id instanceof SymbolKey symbolKey) {
               return this.getBuiltInSlot(symbolKey, cx);
            }

            return this.getBuiltInSlot(((NativeSymbol)id).getKey(), cx);
         }
      }
   ...
   private Slot getBuiltInSlot(String name, Context cx) {
      ...
      if (info != 0) {
         int id = info & 65535;
         Slot slot = new Slot(name, 0, info >>> 16);
         slot.value = this.getInstanceIdValue(id, cx);
         return slot;
      } else {
         if (this.prototypeValues != null) {
            int id = this.prototypeValues.findId(name);
            if (id != 0) {
               Slot slot = new Slot(name, 0, this.prototypeValues.getAttributes(id, cx));
               slot.value = this.prototypeValues.get(id, cx);
               return slot;
            }
         }
   ...
   private Slot getBuiltInSlot(Symbol key, Context cx) {
      if (this.prototypeValues != null) {
         int id = this.prototypeValues.findId(key);
         if (id != 0) {
            Slot slot = new Slot(key, 0, this.prototypeValues.getAttributes(id, cx));
            slot.value = this.prototypeValues.get(id, cx);
            return slot;
         }
      }
