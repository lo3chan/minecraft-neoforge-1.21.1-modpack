...
public class RegExp {
   ...
   private static Object matchOrReplace(Context cx, Scriptable scope, Scriptable thisObj, Object[] args, RegExp reImpl, GlobData data, NativeRegExp re) {
      ...
      if (data.mode == 3) {
         ...
         result = re.executeRegExp(cx, scope, reImpl, str, indexp, data.mode != 2 && data.mode != 4 ? 1 : 0);
      }
      ...
      switch (actionType) {
         ...
         case 4: {
            ...
            if (useRE) {
               ...
               if (4 == actionType && (re.getFlags() & 1) == 0) {
   ...
   public int find_split(
      Context cx, Scriptable scope, String target, String separator, Scriptable reObj, int[] ip, int[] matchlen, boolean[] matched, String[][] parensp
   ) {
      int i = ip[0];
      int length = target.length();
   ...
   public Object js_split(Context cx, Scriptable scope, String target, Object[] args) {
      ...
      if (args.length >= 1 && args[0] != Undefined.INSTANCE) {
         ...
         int match;
         while (
            (match = find_split(cx, scope, target, separator, reProxy, re, ip, matchlen, matched, parens)) >= 0
               && (!limited || len < limit)
               && match <= target.length()
         ) {
            String substr;
