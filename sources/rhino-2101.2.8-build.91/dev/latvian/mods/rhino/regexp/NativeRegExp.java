...
public class NativeRegExp extends IdScriptableObject implements Function {
   ...
   private static boolean executeREBytecode(REGlobalData gData, String input, int end, Context cx) {
      ...
      boolean instructionCounting = cx.getInstructionObserverThreshold() != 0;
      ...
      while (true) {
         ...
         label292: {
            if (reopIsSimple(op)) {
               nextpc = simpleMatch(gData, input, op, program, pc, end, true, cx);
               result = nextpc >= 0;
               if (result) {
                  pc = nextpc;
               }
               ...
               switch (op) {
                  ...
                  case 48:
                     ...
                     pushProgState(gData, nextpc, max, gData.cp, null, continuationOp, continuationPc);
                     if (greedy) {
                        ...
                        popProgState(gData);
                        int var41 = pc + 4;
                        pc = var41 + getOffset(program, var41);
                        op = program[pc++];
                     }
                     continue;
                  case 29:
                  ...
                  case 51:
                     do {
                        ...
                        if (statexx.min == 0 && (gData.cp == statexx.index || statexx.max == 0)) {
                           result = false;
                           continuationPc = statexx.continuationPc;
                           continuationOp = statexx.continuationOp;
                           pc += 4;
                  ...
                  case 52:
                     ...
                     if (!result) {
                        ...
                        if (new_min == 0) {
                           ...
                           popProgState(gData);
                           int var25 = pc + 4;
                           pc = var25 + getOffset(program, var25);
                           op = program[pc++];
                           continue;
                        }
   ...
   public static Object js_exec(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
      return realThis(thisObj, "exec", cx).execSub(cx, scope, args, 1);
   }
   ...
   private Object js_SymbolMatchAll(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
      if (!ScriptRuntime.isObject(thisObj)) {
         ...
         Constructable defaultConstructor = ScriptRuntime.getExistingCtor(cx, topLevelScope, this.getClassName());
   ...
   Scriptable compile(Context cx, Scriptable scope, Object[] args) {
      if (args.length < 1 || !(args[0] instanceof NativeRegExp thatObj && (args.length == 1 || args[1] == Undefined.INSTANCE))) {
         ...
         if (args.length != 0 && args[0] != Undefined.INSTANCE) {
            if (args[0] instanceof NativeRegExp thatObjx) {
               pattern = new String(thatObjx.re.source);
            } else {
   ...
   private Object execSub(Context cx, Scriptable scopeObj, Object[] args, int matchType) {
      ...
      boolean globalOrSticky = (this.re.flags & 1) != 0 || (this.re.flags & 8) != 0;
      double d = 0.0;
      if (globalOrSticky) {
   ...
   protected Object getInstanceIdValue(int id, Context cx) {
      return switch (id) {
         case 1 -> this.lastIndex;
         case 2 -> new String(this.re.source);
         case 3 -> {
            StringBuilder buf = new StringBuilder();
            this.appendFlags(buf);
            yield buf.toString();
         }
         case 4 -> (this.re.flags & 1) != 0;
         case 5 -> (this.re.flags & 2) != 0;
         case 6 -> (this.re.flags & 4) != 0;
         case 7 -> (this.re.flags & 8) != 0;
   ...
   protected void initPrototypeId(int id, Context cx) {
      if (id == 7) {
         ...
         this.initPrototypeMethod(REGEXP_TAG, id, SymbolKey.MATCH_ALL, "[Symbol.matchAll]", 1, cx);
      } else if (id == 9) {
   ...
   protected int findPrototypeId(Symbol k) {
      if (SymbolKey.MATCH.equals(k)) {
         return 7;
      } else if (SymbolKey.MATCH_ALL.equals(k)) {
         return 8;
      } else {
         return SymbolKey.SEARCH.equals(k) ? 9 : 0;
      }
   }
