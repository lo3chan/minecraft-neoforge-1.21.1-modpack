...
public class NativeGlobal implements IdFunctionCall {
   ...
   public static void init(Context cx, Scriptable scope, boolean sealed) {
      ...
      ScriptableObject.defineProperty(scope, "Infinity", ScriptRuntime.wrapNumber(1.0 / 0.0), 7, cx);
      ScriptableObject.defineProperty(scope, "undefined", Undefined.INSTANCE, 7, cx);
      ScriptableObject.defineProperty(scope, "globalThis", scope, 2, cx);
      ...
      for (TopLevel.NativeErrors error : TopLevel.NativeErrors.values()) {
         if (error != TopLevel.NativeErrors.Error) {
            ...
            if (error == TopLevel.NativeErrors.AggregateError) {
               ctor = new IdFunctionObject(obj, FTAG, 15, name, 2, scope);
            } else {
               ctor = new IdFunctionObject(obj, FTAG, 14, name, 1, scope);
            }
   ...
   private static Object js_escape(Object[] args, Context cx) {
      ...
      if (args.length > 1) {
         ...
         if (Double.isNaN(d) || (mask = (int)d) != d || 0 != (mask & -8)) {
