...
public class ScriptRuntime {
   ...
   public static final Double NaNobj = 0.0 / 0.0;
   ...
   public static boolean isRhinoRuntimeType(Class<?> cl) {
      return cl.isPrimitive()
         ? cl != char.class
         : cl == StringClass || cl == BooleanClass || NumberClass.isAssignableFrom(cl) || ScriptableClass.isAssignableFrom(cl);
   }
   ...
   public static double toNumber(Context cx, Object[] args, int index) {
      return index < args.length ? toNumber(cx, args[index]) : 0.0 / 0.0;
   }
         ...
         if (!isStrWhiteSpaceChar(startChar)) {
            ...
            if (endChar == 'y') {
               ...
               if (start + 7 == end && s.regionMatches(start, "Infinity", 0, 8)) {
                  return startChar == '-' ? -1.0 / 0.0 : 1.0 / 0.0;
               }
   ...
   public static String escapeAndWrapString(String s) {
      char c = (char)(s.indexOf(39) == -1 ? 39 : 34);
      String escaped = escapeString(s, c);
      return c + escaped + c;
   }
   ...
   public static String toString(Context cx, Object val) {
      if (val == null) {
         return "null";
      } else if (val == Undefined.INSTANCE || val == Undefined.SCRIPTABLE_INSTANCE) {
         return "undefined";
      } else if (val instanceof String) {
         return (String)val;
      } else if (val instanceof CharSequence) {
   ...
   static String uneval(Context cx, Scriptable scope, Object value) {
      if (value == null) {
         ...
         return ScriptableObject.hasProperty(obj, "toSource", cx) && ScriptableObject.getProperty(obj, "toSource", cx) instanceof Function f
            ? toString(cx, f.call(cx, scope, obj, EMPTY_OBJECTS))
            : toString(cx, value);
      } else {
   ...
   static Object getIndexObject(String s) {
      ...
      return indexTest >= 0L && indexTest <= 2147483647L ? (int)indexTest : s;
   }

   public static Optional<Double> canonicalNumericIndexString(Context cx, String arg) {
      if ("-0".equals(arg)) {
         return Optional.of(-1.0 / 0.0);
      } else {
         ...
         if (Double.isNaN(num)) {
            ...
            return numStr.equals(arg) ? Optional.of(num) : Optional.empty();
         }
      }
   }
   ...
   static Object getIndexObject(Context cx, double d) {
      int i = (int)d;
      return i == d ? i : toString(cx, d);
   }

   public static ScriptRuntime.StringIdOrIndex toStringIdOrIndex(Context cx, Object id) {
      if (id instanceof Number) {
         double d = ((Number)id).doubleValue();
         if (d < 0.0) {
            return new ScriptRuntime.StringIdOrIndex(toString(cx, id));
         } else {
            int index = (int)d;
            return index == d ? new ScriptRuntime.StringIdOrIndex(index) : new ScriptRuntime.StringIdOrIndex(toString(cx, id));
         }
      } else {
         ...
         long indexTest = indexFromString(s);
         return indexTest >= 0L && indexTest <= 2147483647L ? new ScriptRuntime.StringIdOrIndex((int)indexTest) : new ScriptRuntime.StringIdOrIndex(s);
      }
   }
   ...
   private static Object nameOrFunction(
      Context cx, Scriptable scope, Scriptable parentScope, String name, boolean asFunctionCall, boolean isOptionalChainingCall
   ) {
   ...
   private static IdEnumeration enumInitInOrder(Context cx, IdEnumeration x) {
      if ((x.obj instanceof SymbolScriptable ? ScriptableObject.getProperty(x.obj, SymbolKey.ITERATOR, cx) : null) instanceof Callable f) {
   ...
   private static Callable getNameFunctionAndThisInner(Context cx, Scriptable scope, String name, boolean isOptionalChainingCall) {
      ...
      if (parent == null) {
         ...
         } else if (!isOptionalChainingCall || result != Scriptable.NOT_FOUND && result != null && !Undefined.isUndefined(result)) {
            if (result == Scriptable.NOT_FOUND) {
               throw notFoundError(cx, scope, name);
            } else {
               throw notFunctionError(cx, result, name);
   ...
   private static Callable getElemFunctionAndThisInner(Context cx, Scriptable scope, Object obj, Object elem, boolean isOptionalChainingCall) {
      ...
      } else if (!isOptionalChainingCall || value != Scriptable.NOT_FOUND && value != null && !Undefined.isUndefined(value)) {
   ...
   public static boolean isUnregisteredSymbol(Object obj) {
      return !(obj instanceof NativeSymbol ns) ? obj instanceof SymbolKey : ns.isSymbol() && ns.getKind() != NativeSymbol.SymbolKind.REGISTERED;
   }
      ...
      if (val1 instanceof Number && val2 instanceof Number) {
         ...
         if (val1 instanceof CharSequence || val2 instanceof CharSequence) {
            ...
            return val1 instanceof Number && val2 instanceof Number
               ? wrapNumber(((Number)val1).doubleValue() + ((Number)val2).doubleValue())
               : wrapNumber(toNumber(cx, val1) + toNumber(cx, val2));
         }
      } else {
         throw typeError0(cx, "msg.not.a.number");
      }
   ...
   public static boolean compare(Context cx, Object val1, Object val2, int op) {
      ...
      if (val1 instanceof Number && val2 instanceof Number) {
         ...
         if (val1 instanceof CharSequence && val2 instanceof CharSequence) {
            return switch (op) {
               case 14 -> val1.toString().compareTo(val2.toString()) < 0;
               case 15 -> val1.toString().compareTo(val2.toString()) <= 0;
               case 16 -> val1.toString().compareTo(val2.toString()) > 0;
               case 17 -> val1.toString().compareTo(val2.toString()) >= 0;
   ...
   public static Scriptable createArrowFunctionActivation(
      Context cx, Scriptable scope, NativeFunction funObj, Object[] args, boolean isStrict, boolean argsHasRest
   ) {
   ...
   public static Scriptable newCatchScope(Context cx, Scriptable scope, Throwable t, Scriptable lastCatchScope, String exceptionName) {
      ...
      if (cx.visibleToScripts(t.getClass().getName(), ClassVisibilityContext.EXCEPTION)) {
         catchScopeObject.defineProperty(cx, "__exception__", cx.javaToJS(t, scope), 6);
      }
   ...
   public static void setFunctionProtoAndParent(Context cx, Scriptable scope, BaseFunction fn, boolean es6GeneratorFunction) {
      ...
      fn.setStandardPropertyAttributes(3);
   }
      ...
      for (int end = propertyIds.length; i != end; i++) {
         ...
         if (getterSetter == 0) {
            if (id instanceof String str) {
               ...
               ScriptRuntime.StringIdOrIndex s = toStringIdOrIndex(cx, id);
               if (s.stringId == null) {
            ...
            Object key = indexObj != null ? null : (id instanceof Symbol ? id : toString(cx, id));
   ...
   public static RuntimeException notFunctionError(Context cx, Object obj, Object value, String propertyName) {
      ...
      return value == Scriptable.NOT_FOUND
         ? typeError2(cx, "msg.function.not.found.in", propertyName, objString)
         : typeError3(cx, "msg.isnt.function.in", propertyName, objString, typeof(cx, value).toString());
   }
   ...
   public static Scriptable getTemplateLiteralCallSite(Context cx, Scriptable scope, Object[] strings, int index) {
      ...
      if (callsite instanceof Scriptable) {
         ...
         siteObj.setAttributes(cx, "raw", 2);
         int i = 0;
