...
public class SnowballItemRenderer<T extends Entity>
extends EntityRenderer<T> {
    ...
    public void render(T entity, float animationProgress, float tickDelta, PoseStack poseStack, MultiBufferSource multiBufferSource, int light) {
        if (((Entity)entity).tickCount >= 2 || !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(entity) < 12.25)) {
            if (ConfigClient.snowballModel != 2) {
                RenderHelper.renderMesh(entity, tickDelta, multiBufferSource, this.entityRenderDispatcher, PhysicsMod.SNOWBALL_TEXTURE, PhysicsMod.snowballMesh.get(ConfigClient.snowballModel), poseStack, light, OverlayTexture.NO_OVERLAY, ConfigClient.snowballShade);
            } else {
                poseStack.pushPose();
    ...
    public ResourceLocation getTextureLocation(Entity entity) {
        if (ConfigClient.snowballModel != 2) {
            return PhysicsMod.SNOWBALL_TEXTURE;
        }
