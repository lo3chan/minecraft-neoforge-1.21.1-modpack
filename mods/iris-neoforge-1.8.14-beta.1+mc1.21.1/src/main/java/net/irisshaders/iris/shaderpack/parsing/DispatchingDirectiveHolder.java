...
implements DirectiveHolder {
        ...
        if (type == ConstDirectiveParser.Type.BOOL) {
            ...
            if (consumer != null) {
                if ("true".equals(value)) {
                    ...
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": " + value + " is not a valid boolean value");
                }
                return;
            }
            ...
            if (intConsumer != null) {
                try {
                    ...
                    Iris.logger.error("Failed to process " + String.valueOf(directive), e);
                }
                return;
            }
            ...
            if (consumer != null) {
                ...
                if (!value.startsWith("vec2")) {
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": value was not a valid vec2 constructor");
                }
                ...
                if (parts.length != 2) {
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": expected 2 arguments to a vec2 constructor, got " + parts.length);
                }
                try {
            ...
            if (consumer != null) {
                ...
                if (!value.startsWith("ivec3")) {
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": value was not a valid ivec3 constructor");
                }
                ...
                if (parts.length != 3) {
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": expected 3 arguments to a ivec3 constructor, got " + parts.length);
                }
                try {
            ...
            if (consumer != null) {
                ...
                if (!value.startsWith("vec4")) {
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": value was not a valid vec4 constructor");
                }
                ...
                if (parts.length != 4) {
                    Iris.logger.error("Failed to process " + String.valueOf(directive) + ": expected 4 arguments to a vec4 constructor, got " + parts.length);
                }
                try {
    ...
    private void typeCheckHelper(String expected, Map<String, ?> candidates, ConstDirectiveParser.ConstDirective directive) {
        if (candidates.containsKey(directive.getKey())) {
            Iris.logger.warn("Ignoring " + String.valueOf(directive) + " because it is of the wrong type, a type of " + expected + " is expected.");
        }
    }
