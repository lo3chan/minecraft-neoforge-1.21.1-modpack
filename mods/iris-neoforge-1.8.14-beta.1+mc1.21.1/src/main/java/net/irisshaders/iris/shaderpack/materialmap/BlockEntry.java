...
public record BlockEntry(NamespacedId id, Map<String, String> propertyPredicates) implements Entry
{
    ...
    public static Entry parse(@NotNull String entry) {
        ...
        if (entry.startsWith("%")) {
            NamespacedId id2;
            String[] splitStates = (entry = entry.replace("%", "")).split(":");
            ...
            if (splitStates[1].contains("=")) {
                statesStart = 1;
                id2 = new NamespacedId("minecraft", splitStates[0]);
            } else {
                statesStart = 2;
                id2 = new NamespacedId(splitStates[0], splitStates[1]);
            }
