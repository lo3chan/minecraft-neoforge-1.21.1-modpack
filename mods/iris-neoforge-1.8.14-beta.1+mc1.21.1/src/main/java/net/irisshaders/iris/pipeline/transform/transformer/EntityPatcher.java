...
public class EntityPatcher {
    ...
    public static void patchOverlayColor(ASTParser t, TranslationUnit tree, Root root, VanillaParameters parameters) {
        root.processMatches(t, uniformVec4EntityColor, ASTNode::detachAndDelete);
        if (parameters.type.glShaderType == ShaderType.VERTEX) {
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"uniform sampler2D iris_overlay;", "out vec4 entityColor;", "out vec4 iris_vertexColor;", parameters.inputs.isIE() ? "uniform ivec2 iris_OverlayUV;" : "in ivec2 iris_UV1;"});
            tree.prependMainFunctionBody(t, new String[]{"vec4 overlayColor = texelFetch(iris_overlay, " + (parameters.inputs.isIE() ? "iris_OverlayUV" : "iris_UV1") + ", 0);", "entityColor = vec4(overlayColor.rgb, 1.0 - overlayColor.a);", "iris_vertexColor = iris_Color;", "entityColor.rgb *= float(entityColor.a != 0.0);"});
            ...
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"patch out vec4 entityColorTCS;", "in vec4 entityColor[];", "out vec4 iris_vertexColorTCS[];", "in vec4 iris_vertexColor[];"});
            tree.prependMainFunctionBody(t, new String[]{"entityColorTCS = entityColor[gl_InvocationID];", "iris_vertexColorTCS[gl_InvocationID] = iris_vertexColor[gl_InvocationID];"});
            ...
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"out vec4 entityColorTES;", "patch in vec4 entityColorTCS;", "out vec4 iris_vertexColorTES;", "in vec4 iris_vertexColorTCS[];"});
            tree.prependMainFunctionBody(t, new String[]{"entityColorTES = entityColorTCS;", "iris_vertexColorTES = iris_vertexColorTCS[0];"});
        } else if (parameters.type.glShaderType == ShaderType.GEOMETRY) {
            root.replaceReferenceExpressions(t, "entityColor", "entityColor[0]");
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"out vec4 entityColorGS;", "in vec4 entityColor[];", "out vec4 iris_vertexColorGS;", "in vec4 iris_vertexColor[];"});
            tree.prependMainFunctionBody(t, new String[]{"entityColorGS = entityColor[0];", "iris_vertexColorGS = iris_vertexColor[0];"});
            ...
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"in vec4 entityColor;", "in vec4 iris_vertexColor;"});
    ...
    public static void patchEntityId(ASTParser t, TranslationUnit tree, Root root, VanillaParameters parameters) {
        ...
        if (parameters.type.glShaderType == ShaderType.VERTEX) {
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat out ivec3 iris_entityInfo;", "in ivec3 iris_Entity;"});
            tree.prependMainFunctionBody(t, "iris_entityInfo = iris_Entity;");
        } else if (parameters.type.glShaderType == ShaderType.TESSELATION_CONTROL) {
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat out ivec3 iris_entityInfoTCS[];", "flat in ivec3 iris_entityInfo[];"});
            ...
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat out ivec3 iris_entityInfoTES;", "flat in ivec3 iris_entityInfoTCS[];"});
            ...
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat out ivec3 iris_entityInfoGS;", "flat in ivec3 iris_entityInfo" + (parameters.hasTesselation ? "TES" : "") + "[];"});
            tree.prependMainFunctionBody(t, "iris_entityInfoGS = iris_entityInfo" + (parameters.hasTesselation ? "TES" : "") + "[0];");
        } else if (parameters.type.glShaderType == ShaderType.FRAGMENT) {
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"flat in ivec3 iris_entityInfo;"});
            if (parameters.hasGeometry) {
