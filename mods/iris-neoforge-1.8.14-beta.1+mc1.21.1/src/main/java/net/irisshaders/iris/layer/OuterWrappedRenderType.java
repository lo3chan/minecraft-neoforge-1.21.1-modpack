...
BlendingStateHolder {
    ...
    public OuterWrappedRenderType(String name, RenderType wrapped, RenderStateShard extra) {
        super(name, wrapped.format(), wrapped.mode(), wrapped.bufferSize(), wrapped.affectsCrumbling(), OuterWrappedRenderType.shouldSortOnUpload(wrapped), () -> ((RenderType)wrapped).setupRenderState(), () -> ((RenderType)wrapped).clearRenderState());
        this.extra = extra;
