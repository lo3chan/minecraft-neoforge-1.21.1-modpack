...
public class ShadowRenderer {
    ...
    private static float getSkyAngle() {
        return ShadowRenderer.getLevel().getTimeOfDay(CapturedRenderingState.INSTANCE.getTickDelta());
    }
    ...
    private FrustumHolder createShadowFrustum(float renderMultiplier, FrustumHolder holder) {
        ...
        if (this.packCullingState == ShadowCullState.DEFAULT && this.packHasVoxelization || this.packCullingState == ShadowCullState.DISTANCE) {
            ...
            if (distance2 >= (double)(Minecraft.getInstance().options.getEffectiveRenderDistance() * 16) && !isReversed) {
    ...
    public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
        ...
        RenderSystem.viewport((int)0, (int)0, (int)client.getMainRenderTarget().width, (int)client.getMainRenderTarget().height);
        if (levelRenderer instanceof CullingDataCache) {
    ...
    public int renderBlockEntities(RenderBuffers bufferSource, PoseStack modelView, Camera camera, double cameraX, double cameraY, double cameraZ, float tickDelta, boolean hasEntityFrustum, boolean lightsOnly) {
        ShadowRenderer.getLevel().getProfiler().push("build blockentities");
        ...
        for (BlockEntity entity : visibleBlockEntities) {
            if (lightsOnly && entity.getBlockState().getLightEmission() == 0) continue;
            BlockPos pos = entity.getBlockPos();
            if (hasEntityFrustum && culler.isCulled(pos.getX() - 1, pos.getY() - 1, pos.getZ() - 1, pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1)) continue;
            modelView.pushPose();
            modelView.translate((double)pos.getX() - cameraX, (double)pos.getY() - cameraY, (double)pos.getZ() - cameraZ);
            Minecraft.getInstance().getBlockEntityRenderDispatcher().render(entity, tickDelta, modelView, (MultiBufferSource)this.buffers.bufferSource());
            modelView.popPose();
            ++shadowBlockEntities;
        }
        ShadowRenderer.getLevel().getProfiler().pop();
    ...
    private int renderEntities(LevelRendererAccessor levelRenderer, EntityRenderDispatcher dispatcher, MultiBufferSource.BufferSource bufferSource, PoseStack modelView, float tickDelta, Frustum frustum, double cameraX, double cameraY, double cameraZ) {
        ...
        for (Entity entity2 : ShadowRenderer.getLevel().entitiesForRendering()) {
            if (!dispatcher.shouldRender(entity2, frustum, cameraX, cameraY, cameraZ) || entity2.isSpectator()) continue;
        ...
        for (Entity entity2 : renderedEntities) {
            float realTickDelta = Minecraft.getInstance().level.tickRateManager().isEntityFrozen(entity2) ? tickDelta : CapturedRenderingState.INSTANCE.getRealTickDelta();
    ...
    private int renderPlayerEntity(LevelRendererAccessor levelRenderer, EntityRenderDispatcher dispatcher, MultiBufferSource.BufferSource bufferSource, PoseStack modelView, float tickDelta, Frustum frustum, double cameraX, double cameraY, double cameraZ) {
        ...
        if (!player.getPassengers().isEmpty()) {
            for (int i = 0; i < player.getPassengers().size(); ++i) {
                ...
                levelRenderer.invokeRenderEntity((Entity)player.getPassengers().get(i), cameraX, cameraY, cameraZ, realTickDelta, modelView, (MultiBufferSource)bufferSource);
                ++shadowEntities;
            }
        }
        if (player.getVehicle() != null) {
            ...
            levelRenderer.invokeRenderEntity(player.getVehicle(), cameraX, cameraY, cameraZ, realTickDelta, modelView, (MultiBufferSource)bufferSource);
            ++shadowEntities;
        }
    ...
    private String getEntitiesDebugString() {
        return this.shouldRenderEntities || this.shouldRenderPlayer ? this.renderedShadowEntities + "/" + Minecraft.getInstance().level.getEntityCount() : "disabled by pack";
    }
