...
public final class IdMapUniforms {
    ...
    private static class HeldItemSupplier {
        ...
        public void update() {
            ...
            ResourceLocation heldItemId = BuiltInRegistries.ITEM.getKey((Object)heldItem);
            this.intID = this.itemIdMap.applyAsInt((Object)new NamespacedId(heldItemId.getNamespace(), heldItemId.getPath()));
            IrisItemLightProvider lightProvider = (IrisItemLightProvider)heldItem;
            this.lightValue = lightProvider.getLightEmission((Player)Minecraft.getInstance().player, heldStack);
            if (this.applyOldHandLight) {
                lightProvider = this.applyOldHandLighting(player, lightProvider);
            }
            this.lightColor = lightProvider.getLightColor((Player)Minecraft.getInstance().player, heldStack);
        }
        ...
        private IrisItemLightProvider applyOldHandLighting(@NotNull LocalPlayer player, IrisItemLightProvider existing) {
            ...
            int newEmission = lightProvider.getLightEmission((Player)Minecraft.getInstance().player, offHandStack);
