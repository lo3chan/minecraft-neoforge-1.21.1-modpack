...
public class DHCompatInternal {
    ...
    public static boolean checkFrame() {
        ...
        if ((dhEnabled != (Boolean)DhApi.Delayed.configs.graphics().renderingEnabled().getValue() || guiScale != (Integer)Minecraft.getInstance().options.guiScale().get()) && Iris.isPackInUseQuick()) {
            guiScale = (Integer)Minecraft.getInstance().options.guiScale().get();
