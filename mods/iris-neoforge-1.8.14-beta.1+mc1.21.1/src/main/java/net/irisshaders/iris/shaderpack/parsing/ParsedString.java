...
public class ParsedString {
    ...
    public String takeNumber() {
        ...
        for (position = 0; position < this.text.length() && !(position + 1 < this.text.length() ? !Character.isDigit(this.text.charAt(position)) && !Character.isDigit(this.text.charAt(position + 1)) : !Character.isDigit(this.text.charAt(position))); ++position) {
        }
