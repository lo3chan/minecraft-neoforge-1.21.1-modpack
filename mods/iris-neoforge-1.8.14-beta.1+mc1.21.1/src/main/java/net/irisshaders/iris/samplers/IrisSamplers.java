...
public class IrisSamplers {
    ...
    public static void addLevelSamplers(SamplerHolder samplers, WorldRenderingPipeline pipeline, AbstractTexture whitePixel, boolean hasTexture, boolean hasLightmap, boolean hasOverlay) {
        if (hasTexture) {
            ...
            samplers.addDynamicSampler(() -> ((AbstractTexture)whitePixel).getId(), "tex", "texture", "gtexture", "gcolor", "colortex0");
        }
        if (hasLightmap) {
            samplers.addExternalSampler(2, "lightmap");
        } else {
            samplers.addDynamicSampler(() -> ((AbstractTexture)whitePixel).getId(), "lightmap");
        }
        if (hasOverlay) {
            samplers.addExternalSampler(1, "iris_overlay");
        } else {
            samplers.addDynamicSampler(() -> ((AbstractTexture)whitePixel).getId(), "iris_overlay");
        }
