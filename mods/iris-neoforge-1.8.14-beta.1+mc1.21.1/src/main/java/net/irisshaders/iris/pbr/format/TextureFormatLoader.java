...
public class TextureFormatLoader {
    public static final ResourceLocation LOCATION = ResourceLocation.withDefaultNamespace((String)"optifine/texture.properties");
    ...
    private static TextureFormat loadFormat(ResourceManager resourceManager) {
        Optional resource = resourceManager.getResource(LOCATION);
        if (!resource.isPresent()) return null;
        try (InputStream stream2222 = ((Resource)resource.get()).open();){
