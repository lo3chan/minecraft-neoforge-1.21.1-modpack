...
extends BaseOptionElementWidget<OptionMenuProfileElement> {
    private static final MutableComponent PROFILE_LABEL = Component.translatable((String)"options.iris.profile");
    private static final MutableComponent PROFILE_CUSTOM = Component.translatable((String)"options.iris.profile.custom").withStyle(ChatFormatting.YELLOW);
    ...
    public void init(ShaderPackScreen screen, NavigationController navigation) {
        ...
        this.profileLabel = (Component)profileName.map(name -> GuiUtil.translateOrDefault(Component.literal((String)name), "profile." + name, new Object[0])).orElse(PROFILE_CUSTOM);
    }

    @Override
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float tickDelta, boolean hovered) {
        this.updateRenderParams(this.bounds.width() - (Minecraft.getInstance().font.width((FormattedText)PROFILE_LABEL) + 16));
