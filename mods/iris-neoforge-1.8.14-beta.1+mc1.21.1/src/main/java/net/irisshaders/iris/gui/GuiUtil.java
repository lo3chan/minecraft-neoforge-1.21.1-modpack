...
public final class GuiUtil {
    public static final ResourceLocation IRIS_WIDGETS_TEX = ResourceLocation.fromNamespaceAndPath((String)"iris", (String)"textures/gui/widgets.png");
    private static final Component ELLIPSIS = Component.literal((String)"...");
    ...
    public static void drawButton(GuiGraphics guiGraphics, int x, int y, int width, int height, boolean hovered, boolean disabled) {
        ...
        guiGraphics.blit(IRIS_WIDGETS_TEX, x, y, 0.0f, (float)vOffset, halfWidth, halfHeight, 256, 256);
        guiGraphics.blit(IRIS_WIDGETS_TEX, x + halfWidth, y, (float)(200 - (width - halfWidth)), (float)vOffset, width - halfWidth, halfHeight, 256, 256);
        guiGraphics.blit(IRIS_WIDGETS_TEX, x, y + halfHeight, 0.0f, (float)(vOffset + (20 - (height - halfHeight))), halfWidth, height - halfHeight, 256, 256);
        guiGraphics.blit(IRIS_WIDGETS_TEX, x + halfWidth, y + halfHeight, (float)(200 - (width - halfWidth)), (float)(vOffset + (20 - (height - halfHeight))), width - halfWidth, height - halfHeight, 256, 256);
    }
    ...
    public static void drawTextPanel(Font font, GuiGraphics guiGraphics, Component text, int x, int y) {
        GuiUtil.drawPanel(guiGraphics, x, y, font.width((FormattedText)text) + 8, 16);
    ...
    public static MutableComponent shortenText(Font font, MutableComponent text, int width) {
        if (font.width((FormattedText)text) > width) {
            return Component.literal((String)font.plainSubstrByWidth(text.getString(), width - font.width((FormattedText)ELLIPSIS))).append(ELLIPSIS).setStyle(text.getStyle());
        }
        return text;
    }

    public static MutableComponent translateOrDefault(MutableComponent defaultText, String translationDesc, Object ... format) {
        if (I18n.exists((String)translationDesc)) {
            return Component.translatable((String)translationDesc, (Object[])format);
        }
        return defaultText;
    }

    public static void playButtonClickSound() {
        GuiUtil.client().getSoundManager().play((SoundInstance)SimpleSoundInstance.forUI((Holder)SoundEvents.UI_BUTTON_CLICK, (float)1.0f));
    }
    ...
    public static class Icon {
        ...
        public void draw(GuiGraphics guiGraphics, int x, int y) {
            RenderSystem.enableBlend();
            guiGraphics.blit(IRIS_WIDGETS_TEX, x, y, (float)this.u, (float)this.v, this.width, this.height, 256, 256);
        }
