...
implements Digraph<V> {
    ...
    private static <V> VertexMapFactory<V> getDefaultVertexMapFactory(Comparator<? super V> comparator) {
        return () -> {
            if (comparator == null) {
    ...
    private static <V> EdgeMapFactory<V> getDefaultEdgeMapFactory(Comparator<? super V> comparator) {
        return ignore -> {
