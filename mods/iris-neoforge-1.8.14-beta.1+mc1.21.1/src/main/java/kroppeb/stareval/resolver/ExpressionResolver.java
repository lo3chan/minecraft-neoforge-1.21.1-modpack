...
public class ExpressionResolver {
    ...
    private Expression resolveExpressionInternal(Type targetType, ExpressionElement expression, boolean allowNonImplicit, boolean allowImplicit) {
        ...
        ExpressionElement expressionElement = expression;
        int n = 0;
        switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{UnaryExpressionElement.class, BinaryExpressionElement.class, FunctionCall.class, AccessExpressionElement.class, NumberToken.class, IdToken.class}, (Object)expressionElement, n)) {
            case 0: {
                UnaryExpressionElement unaryExpressionElement = (UnaryExpressionElement)expressionElement;
                return this.resolveCallExpression(targetType, unaryExpressionElement.op().name(), Collections.singletonList(unaryExpressionElement.inner()), allowNonImplicit, allowImplicit);
            }
            case 1: {
                BinaryExpressionElement token = (BinaryExpressionElement)expressionElement;
                return this.resolveCallExpression(targetType, token.op().name(), Arrays.asList(token.left(), token.right()), allowNonImplicit, allowImplicit);
            }
            case 2: {
                FunctionCall token = (FunctionCall)expressionElement;
                return this.resolveCallExpression(targetType, token.id(), token.args(), allowNonImplicit, allowImplicit);
            }
            case 3: {
                AccessExpressionElement token = (AccessExpressionElement)expressionElement;
                return this.resolveCallExpression(targetType, "<access$" + token.index() + ">", Collections.singletonList(token.base()), allowNonImplicit, allowImplicit);
            }
            case 4: {
                NumberToken token = (NumberToken)expressionElement;
            ...
            case 5: {
                IdToken token = (IdToken)expressionElement;
    ...
    private ConstantExpression resolveNumber(String s) {
        return this.numbers.computeIfAbsent(s, str -> {
            try {
                ...
                return new ConstantExpression(this, Type.Int){
                ...
                try {
                    ...
                    return new ConstantExpression(this, Type.Float){
