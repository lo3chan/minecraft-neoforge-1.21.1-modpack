...
public class Tab<T extends AbstractFrame> {
    ...
    public static class Builder<T extends AbstractFrame> {
        ...
        public Tab<ScrollableFrame> from(Screen screen, ModOptions modOptions, Page page, Holder<Integer> verticalScrollBarOffset, OptionStateStore optionStateStore) {
            return new Tab<ScrollableFrame>(modOptions, page.name(), page, bounds -> {
                PageFrame pageFrame = PageFrame.builder().withDimension((LayoutBounds)bounds).withModOptions(modOptions).withPage(page).withScreen(screen).withOptionStateStore(optionStateStore).build();
                ScrollableFrame scrollableFrame = ScrollableFrame.builder().withDimension((LayoutBounds)bounds).withModOptions(modOptions).withScreen(screen).withFrame(pageFrame).withVerticalScrollBarOffset(verticalScrollBarOffset).build();
                pageFrame.setGroupToggleRebuildHandler(collapseKey -> {
                    scrollableFrame.rebuildContentFrame();
                    if (pageFrame.focusGroupHeader((ResourceLocation)collapseKey)) {
