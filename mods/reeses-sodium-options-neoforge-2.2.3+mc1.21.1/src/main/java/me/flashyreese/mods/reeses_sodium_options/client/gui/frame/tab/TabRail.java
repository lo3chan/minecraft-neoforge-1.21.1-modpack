...
import me.flashyreese.mods.reeses_sodium_options.client.gui.frame.tab.TabButtonWidget;
import me.flashyreese.mods.reeses_sodium_options.client.gui.frame.tab.TabGroup;
import me.flashyreese.mods.reeses_sodium_options.client.gui.frame.tab.TabGroupModel;
...
final class TabRail {
    ...
    private final List<BaseWidget> tabWidgets = new ArrayList<BaseWidget>();
    private final Map<String, TabHeaderWidget> tabHeaderWidgets = new HashMap<String, TabHeaderWidget>();
    private final Map<Tab<?>, TabButtonWidget> tabButtonWidgets = new HashMap();
    private LayoutBounds dim;
    private boolean canScroll;
    private ScrollBarWidget scrollBar;
    @Nullable
    private BaseWidget selectedTabWidget;
    ...
    void updateScrollState(LayoutBounds frameDim) {
        ...
        boolean bl = this.canScroll = contentHeight > frameDim.height();
        if (!this.canScroll) {
        ...
        this.scrollBar = new ScrollBarWidget(new LayoutBounds(this.dim.getLimitX() - 11, frameDim.y(), 10, frameDim.height()), ScrollBarWidget.ScrollDirection.VERTICAL, contentHeight, frameDim.height(), offset -> {
            this.scrollBarOffset.set((Integer)offset);
        ...
        for (TabGroup group : this.visibleGroups()) {
            ...
            if (!this.groupModel.shouldShowHeaders()) {
                for (Tab<?> tab : group.tabs()) {
                    LayoutBounds tabDim = LayoutBounds.relativeTo(this.dim, 0, offsetY, width, 18);
            ...
            int headerY = offsetY + (firstGroup ? 0 : 4);
            ...
            offsetY += headerHeight + (firstGroup ? 0 : 4);
            firstGroup = false;
            if (!showTabs) continue;
            for (Tab<?> tab : group.tabs()) {
                LayoutBounds tabDim = LayoutBounds.relativeTo(this.dim, 0, offsetY, width, 18);
                this.addTabButton(children, tab, tabDim, selectedTab, tabSelector);
                offsetY += 18;
    ...
    private List<TabGroup> visibleGroups() {
        ...
        HashSet<String> resultTabKeys = new HashSet<String>();
        for (SearchResultEntry result : this.optionStateStore.searchResults()) {
            resultTabKeys.add(result.tabKey());
        }
        ArrayList<TabGroup> groups = new ArrayList<TabGroup>();
        for (TabGroup group : this.groupModel.groups()) {
            List<Tab<?>> tabs = group.tabs().stream().filter(tab -> resultTabKeys.contains(tab.key())).toList();
    ...
    void scrollFocusedWidgetIntoView(GuiEventListener element, LayoutBounds frameDim) {
        ...
        block5: {
            block4: {
                if (!(element instanceof BaseWidget)) break block4;
                widget = (BaseWidget)element;
                if (this.canScroll) break block5;
            }
            return;
        }
    ...
    private void activateHeader(TabGroup group, Consumer<Tab<?>> tabSelector, Runnable rebuildFrame) {
        switch (this.groupModel.collapseMode()) {
            case SELECTED_GROUP: {
                ...
                tabSelector.accept((Tab)group.tabs().getFirst());
                break;
            }
            case ALL_EXPANDED: {
                tabSelector.accept((Tab)group.tabs().getFirst());
    ...
    private void addTabButton(List<GuiEventListener> children, Tab<?> tab, LayoutBounds tabDim, Optional<Tab<?>> selectedTab, Consumer<Tab<?>> tabSelector) {
        ...
        TabButtonWidget button = this.tabButtonWidgets.computeIfAbsent(tab, key -> new TabButtonWidget(tabDim, (Tab<?>)key, selected, () -> tabSelector.accept((Tab<?>)key)));
    ...
    private void addTabHeader(List<GuiEventListener> children, TabGroup group, LayoutBounds tabHeaderDim, Optional<Tab<?>> selectedTab, Consumer<Tab<?>> tabSelector, Runnable rebuildFrame) {
        ...
        TabHeaderWidget header = this.tabHeaderWidgets.computeIfAbsent(group.id(), unused -> new TabHeaderWidget(tabHeaderDim, group.modOptions(), () -> this.activateHeader(group, tabSelector, rebuildFrame), selected));
    ...
    private boolean contains(LayoutBounds widgetDim) {
        return widgetDim.x() >= this.dim.x() && widgetDim.getLimitX() <= this.dim.getLimitX() && widgetDim.height() <= this.dim.height();
    }
    ...
    private int calculateWidth(LayoutBounds frameDim) {
        int textWidth = this.tabs.stream().mapToInt(this::getTextWidth).max().orElse(0);
        int preferredWidth = textWidth + (this.canScroll ? 32 : 24);
        int maximumWidth = Math.max(120, Math.min(220, (int)((double)frameDim.width() * 0.35)));
        return Math.clamp((long)preferredWidth, (int)120, (int)maximumWidth);
    }

    private int getTextWidth(Tab<?> tab) {
        int tabTitleWidth = Minecraft.getInstance().font.width((FormattedText)tab.getTitle());
        if (!this.groupModel.shouldShowHeaders()) {
