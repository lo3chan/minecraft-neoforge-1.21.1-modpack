...
extends BaseWidget {
    ...
    TabButtonWidget(LayoutBounds dim, Tab<?> tab, boolean selected, Runnable action) {
        ...
        this.action = action;
        this.theme = ReeseSodiumOptionsConfig.config().isColorThemes() ? GuiThemes.fromSodium(tab.getModOptions().theme()) : GuiThemes.DEFAULT_BUTTON;
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        int textY = n - 9 / 2;
        this.drawString(guiGraphics, label, this.getX() + 10, textY, this.textColor());
        if (this.selected) {
            this.drawRect(guiGraphics, this.getX(), this.getY(), this.getX() + 2, this.getLimitY(), this.theme.theme);
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (this.isFocused() && TabButtonWidget.isSelectionKey(keyCode)) {
            this.doAction();
            return true;
        }
        return false;
    ...
    public void updateNarration(NarrationElementOutput builder) {
        builder.add(NarratedElementType.TITLE, (Component)Component.translatable((String)"gui.narrate.tab", (Object[])new Object[]{this.tab.getTitle()}));
        if (!this.selected) {
