...
NarratableEntry {
    ...
    protected boolean shouldRenderFocusBorder(boolean focused) {
        ...
        return switch (ReeseSodiumOptionsConfig.config().getFocusBorderMode()) {
            ...
            case ReeseSodiumOptionsConfig.FocusBorderMode.ALWAYS -> true;
            case ReeseSodiumOptionsConfig.FocusBorderMode.KEYBOARD -> BaseWidget.isKeyboardFocusVisible();
            case ReeseSodiumOptionsConfig.FocusBorderMode.NEVER -> false;
        };
    }
    ...
    public ComponentPath nextFocusPath(FocusNavigationEvent navigation) {
        return this.isFocused() ? null : ComponentPath.leaf((GuiEventListener)this);
    }
    ...
    protected void addButtonNarration(NarrationElementOutput builder, Component label) {
        builder.add(NarratedElementType.TITLE, (Component)Component.translatable((String)"gui.narrate.button", (Object[])new Object[]{label}));
    ...
    protected void addUsageNarration(NarrationElementOutput builder, String focusedKey, String hoveredKey) {
        if (this.isFocused()) {
            builder.add(NarratedElementType.USAGE, (Component)Component.translatable((String)focusedKey));
        } else if (this.isHovered()) {
            builder.add(NarratedElementType.USAGE, (Component)Component.translatable((String)hoveredKey));
        }
    }
    ...
    protected void drawBorder(GuiGraphics guiGraphics, int x1, int y1, int x2, int y2, int color) {
        BaseWidget.border(guiGraphics, x1, y1, x2, y2, color);
    }
