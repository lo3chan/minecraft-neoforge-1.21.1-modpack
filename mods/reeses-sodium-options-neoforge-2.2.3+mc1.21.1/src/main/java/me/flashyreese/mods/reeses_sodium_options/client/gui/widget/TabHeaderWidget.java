...
extends BaseWidget {
    ...
    private void drawScrollingString(GuiGraphics g, String text, int x, int y, int color, int availableWidth) {
        ...
        long t = nowMs % (cycleMs = 1000L + (travelMs = Math.max(1L, (long)Math.ceil((double)overflow / (speedPxPerMs = 0.01)))) + 1000L + travelMs);
        if (t < 1000L) {
            offset = 0.0;
        } else if (t < 1000L + travelMs) {
            long tt = t - 1000L;
            offset = Math.min((double)overflow, (double)tt * speedPxPerMs);
        } else if (t < 1000L + travelMs + 1000L) {
            offset = overflow;
        } else {
            long tt = t - (1000L + travelMs + 1000L);
            offset = Math.max(0.0, (double)overflow - (double)tt * speedPxPerMs);
        }
        this.applyScissor(g, x, y, availableWidth, lineHeight, () -> g.drawString(font, text, (int)((double)x - offset), y, color, false));
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int i, int j, float f) {
        ...
        this.applyScissor(guiGraphics, this.getX(), this.getY(), this.getWidth(), this.getHeight(), () -> {
            ...
            int xOffset = !ReeseSodiumOptionsConfig.config().isTabHeaderIcons() || this.modOptions.icon() == null ? 5 : IconRenderer.renderIconWithSpacing(guiGraphics, this.modOptions.icon(), primaryTextColor, this.modOptions.iconMonochrome(), this.getX(), this.getY(), this.getHeight(), 4);
            ...
            if (ReeseSodiumOptionsConfig.config().isTabHeaderVersionLabels()) {
                ...
                int centeredY = n + Math.ceilDiv((int)(n2 - 9), (int)2);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (this.action != null && this.isFocused() && TabHeaderWidget.isSelectionKey(keyCode)) {
            this.action.run();
            this.playClickSound();
            return true;
        }
    ...
    public void updateNarration(NarrationElementOutput builder) {
        ...
        MutableComponent label = Component.literal((String)this.modOptions.name());
        if (ReeseSodiumOptionsConfig.config().isTabHeaderVersionLabels()) {
            label = CommonComponents.optionNameValue((Component)label, (Component)Component.literal((String)this.modOptions.version()));
        }
        this.addButtonNarration(builder, (Component)label);
    }
