...
import me.flashyreese.mods.reeses_sodium_options.client.gui.frame.option.action.OptionActionButtonRenderer;
...
final class OptionActionButtonElement
...
ControlGuideProvider {
    ...
    OptionActionButtonElement(Supplier<LayoutBounds> rowBoundsSupplier, Supplier<@Nullable StatefulOption<?>> optionSupplier, IntSupplier buttonsFromRight, ResourceLocation icon, Component guideLabel, Function<StatefulOption<?>, Component> narrationLabelProvider, Predicate<StatefulOption<?>> visiblePredicate, Predicate<StatefulOption<?>> activePredicate, Consumer<StatefulOption<?>> action, Runnable clickSound, Runnable afterAction) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (!this.focused || !OptionActionButtonElement.isSelectionKey(keyCode)) {
            return false;
        }
        return this.performAction();
    }
    ...
    public ComponentPath nextFocusPath(FocusNavigationEvent navigation) {
        return this.isActive() && !this.focused ? ComponentPath.leaf((GuiEventListener)this) : null;
    }

    public boolean isMouseOver(double mouseX, double mouseY) {
        StatefulOption<?> option = this.optionSupplier.get();
        return option != null && this.visiblePredicate.test(option) && this.activePredicate.test(option) && this.getButtonDim(this.rowBoundsSupplier.get()).contains(mouseX, mouseY);
    }
    ...
    public void updateNarration(NarrationElementOutput builder) {
        ...
        builder.add(NarratedElementType.TITLE, (Component)Component.translatable((String)"gui.narrate.button", (Object[])new Object[]{this.narrationLabelProvider.apply(option)}));
        if (this.focused) {
            builder.add(NarratedElementType.USAGE, (Component)Component.translatable((String)"narration.button.usage.focused"));
