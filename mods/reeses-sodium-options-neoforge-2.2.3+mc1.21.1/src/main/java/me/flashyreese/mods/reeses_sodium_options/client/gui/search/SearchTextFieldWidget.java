...
import me.flashyreese.mods.reeses_sodium_options.client.gui.search.OptionSearch;
...
public class SearchTextFieldWidget
extends TextFieldWidget {
    ...
    private static final Component CLEAR_BUTTON_LABEL = Component.literal((String)"x");
    ...
    public SearchTextFieldWidget(LayoutBounds dim, List<ModOptions> modOptions, OptionsScreenUiState uiState, int tabDimHeight, Runnable refreshSearchResults) {
        super(dim, (Component)Component.translatable((String)"rso.search_bar_empty"));
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        int textX = buttonBounds.getCenterX() - this.getStringWidth((FormattedText)CLEAR_BUTTON_LABEL) / 2;
        int n = buttonBounds.getCenterY();
    ...
    protected boolean onSubmit(boolean reverse) {
        ...
        int targetIndex = lastIndex == null ? (reverse ? total - 1 : 0) : Math.floorMod(lastIndex + (reverse ? -1 : 1), total);
    ...
    private LayoutBounds clearButtonBounds() {
        int x = this.getLimitX() - 4 - 12;
        int y = this.getY() + (this.getHeight() - 12) / 2;
        return new LayoutBounds(x, y, 12, 12);
    }
}
