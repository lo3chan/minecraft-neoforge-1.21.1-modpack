...
public final class ScrollFrameLayout {
    ...
    public static ScrollFrameLayout create(LayoutBounds frameBounds, LayoutBounds contentBounds) {
        ...
        return new ScrollFrameLayout(new LayoutBounds(frameBounds.x(), frameBounds.y(), Math.max(0, viewportWidth), Math.max(0, viewportHeight)), Math.max(0, contentWidth), Math.max(0, contentHeight), canScrollHorizontal, canScrollVertical);
    }
    ...
    public int verticalScrollOffsetToInclude(LayoutBounds dim, int currentOffset) {
        return ScrollFrameLayout.scrollIntoViewOffset(this.viewport, dim, currentOffset);
    }
