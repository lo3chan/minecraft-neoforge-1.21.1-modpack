...
public class BlockAboveSolidProperty
extends BlocksProperty {
    protected BlockAboveSolidProperty(Properties properties, int propertyNum, String[] ids) throws RandomProperty.RandomPropertyException {
    ...
    protected BlockState[] getTestingBlocks(ETFEntityRenderState entity) {
        ...
        if (world.canSeeSky((BlockPos)mutablePos)) {
            return null;
        }
        int minBuildHeight = world.getMinBuildHeight();
        while (minBuildHeight <= mutablePos.getY() && !world.getBlockState((BlockPos)mutablePos).isSolidRender((BlockGetter)world, (BlockPos)mutablePos)) {
            if (!world.getBlockState((BlockPos)mutablePos).isAir() && world.canSeeSky(mutablePos.above())) {
                return null;
            }
