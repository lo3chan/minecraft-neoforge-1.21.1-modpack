...
extends TConfig {
    ...
    public boolean canDoCustomTextures() {
        ...
        if (key != null && this.entityRandomOverrides.containsKey(key)) {
            return this.entityRandomOverrides.getOrDefault(key, false);
        }
        return this.enableCustomTextures;
    }
    ...
    public boolean canDoEmissiveTextures() {
        ...
        if (key != null && this.entityEmissiveOverrides.containsKey(key)) {
            return this.entityEmissiveOverrides.getOrDefault(key, false);
        }
        return this.enableEmissiveTextures;
    }
    ...
    public int getLightOverride(Entity entity, float tickDelta, int light) {
        ...
        if (key != null && this.entityLightOverrides.containsKey(key)) {
            int lightETF = Mth.clamp((int)this.entityLightOverrides.get(key), (int)0, (int)15);
            BlockPos pos = BlockPos.containing((Position)entity.getLightProbePosition(tickDelta));
    ...
    public TConfigEntryCategory getGUIOptions() {
        ...
        }, true), new TConfigEntryEnumButton<SkinTransparencyMode>("config.entity_texture_features.transparent_skins.title", "config.entity_texture_features.transparent_skins.tooltip", () -> this.skinTransparencyMode, mode -> {
            this.skinTransparencyMode = mode;
        }, SkinTransparencyMode.ETF_SKINS_ONLY), new TConfigEntryBoolean("config.entity_texture_features.transparent_skins_extra.title", "config.entity_texture_features.transparent_skins_extra.tooltip", () -> this.skinTransparencyInExtraPixels, aBoolean -> {
            this.skinTransparencyInExtraPixels = aBoolean;
        }, true), new TConfigEntryBoolean("config.entity_texture_features.enable_enemy_team_players_skin_features.title", "config.entity_texture_features.enable_enemy_team_players_skin_features.tooltip", () -> this.enableEnemyTeamPlayersSkinFeatures, aBoolean -> {
        ...
        }, true), new TConfigEntryEnumButton<SettingsButtonLocation>("config.entity_texture_features.settings_button_loc.title", "config.entity_texture_features.settings_button_loc.tooltip", () -> this.configButtonLoc, settingsButtonLocation -> {
            this.configButtonLoc = settingsButtonLocation;
        }, SettingsButtonLocation.BOTTOM_RIGHT)), new TConfigEntryCategory("config.entity_texture_features.restrict_update_properties2").addAll(this.getPropertySettings()), this.getEntitySettings(), new TConfigEntryCategory("config.entity_features.optifine_settings", "config.entity_texture_features.optifine.desc").addAll(TConfigEntryText.fromLongOrMultilineTranslation("config.entity_texture_features.optifine.desc", 200, TConfigEntryText.TextAlignment.LEFT)).add(new TConfigEntryBoolean("config.entity_texture_features.optifine.random_skips.title", "config.entity_texture_features.optifine.random_skips.tooltip", () -> this.optifine_allowWeirdSkipsInTrueRandom, aBoolean -> {
            this.optifine_allowWeirdSkipsInTrueRandom = aBoolean;
        }, true), new TConfigEntryBoolean("config.entity_texture_features.optifine.prevent_base.title", "config.entity_texture_features.optifine.prevent_base.tooltip", () -> this.optifine_preventBaseTextureInOptifineDirectory, aBoolean -> {
            this.optifine_preventBaseTextureInOptifineDirectory = aBoolean;
        }, true)));
    }
    ...
    private List<TConfigEntry> getPropertySettings() {
        ...
        RandomProperties.forEachProperty(propertySettings -> {
            ...
            list.add(category);
            category.add(new TConfigEntryBoolean("config.entity_texture_features.restrict_update_properties.allow", "config.entity_texture_features.restrict_update_properties.allow.tooltip", () -> !this.propertiesDisabled.contains(id), aBoolean -> {
                if (aBoolean.booleanValue()) {
                    this.propertiesDisabled.remove(id);
                } else {
                    this.propertiesDisabled.add(id);
                }
            }, true), new TConfigEntryBoolean("config.entity_texture_features.restrict_update_properties.lock", "config.entity_texture_features.restrict_update_properties.lock.tooltip", () -> this.propertyInvertUpdatingOverrides.contains(id) != defaultNoUpdate, aBoolean -> {
    ...
    private TConfigEntry getPlayerSkinEditorButton() {
        ...
        return canLaunchSkinTool ? new TConfigEntryCustomScreenOpener("config.entity_texture_features.player_skin_editor.button.enabled", reasonText.toString(), () -> new ETFConfigScreenSkinTool(Minecraft.getInstance().screen), false) : new TConfigEntryCustomScreenOpener("config.entity_texture_features.player_skin_editor.button.disabled", reasonText.toString(), () -> new ETFConfigScreenSkinTool(Minecraft.getInstance().screen), false).setEnabled(false);
    }
    ...
    public static enum EmissiveRenderModes {
        ...
        public String toString() {
            return switch (this.ordinal()) {
                ...
                case 0 -> ETF.getTextFromTranslation("config.entity_texture_features.emissive_mode.dull").getString();
                case 1 -> ETF.getTextFromTranslation("config.entity_texture_features.emissive_mode.bright").getString();
            };
        }
    }
    ...
    public static enum SkinTransparencyMode {
        ...
        private SkinTransparencyMode(String key) {
            this.key = key;
        }
    ...
    extends HashMap<String, Boolean> {
        public void putNullable(String s, OverrideBooleanType v) {
            if (v == null) {
                ...
                this.put(s, v == OverrideBooleanType.TRUE);
            }
        }

        public OverrideBooleanType getNullable(String s) {
            if (this.getOrDefault(s, false).booleanValue()) {
