...
extends TConfigEntryValue<Integer> {
    ...
    extends AbstractSliderButton {
        ...
        private boolean isOff() {
            if (this.isMinOff && this.value == 0.0) {
                return true;
            }
            return this.isMaxOff && this.value == 1.0;
        }

        private void setValue(int intIndex) {
            this.value = (double)(Mth.clamp((int)intIndex, (int)this.min, (int)this.max) - this.min) / (double)this.difference;
            this.updateMessage();
        }

        protected void updateMessage() {
            this.snapValueToNearestIndex();
            this.setMessage(Component.nullToEmpty((String)(this.title + (this.getValueRoundedToIntBetweenMinMax() != ((Integer)TConfigEntryInt.this.getter.get()).intValue() ? "\u00a7a" : "") + String.valueOf(this.isOff() ? CommonComponents.OPTION_OFF.getString() : Integer.valueOf(this.getValueRoundedToIntBetweenMinMax())))));
        }
        ...
        private void snapValueToNearestIndex() {
            this.value = (double)((int)Math.round(this.value * (double)this.difference)) / (double)this.difference;
        }
        ...
        public int getValueRoundedToIntBetweenMinMax() {
            ...
            return (int)Math.round(this.value * (double)this.difference) + this.min;
        }
    }
}
