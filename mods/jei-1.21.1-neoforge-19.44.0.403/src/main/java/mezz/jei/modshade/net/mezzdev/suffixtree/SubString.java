...
implements CharSequence {
    ...
    public SubString(String string, int offset, int length) {
        ...
        if (offset + length > string.length()) {
            throw new IllegalArgumentException("offset (" + offset + ") plus length (" + length + ") must be less than or equal to the string's length (" + string.length() + ")");
        }
        this.string = string;
        this.offset = offset;
        this.length = length;
    ...
    public SubString subSequence(int start, int end) {
        ...
        return new SubString(this.string, this.offset + start, end - start);
    }
    ...
    public SubString extend(char newChar) {
        ...
        if (expectedChar != newChar) {
            throw new IllegalArgumentException("extend must be called with the next char. expected '" + expectedChar + "' but was given '" + newChar + "' instead.");
        }
    ...
    public boolean startsWith(SubString prefix, int lenToMatch) {
        ...
        if (this.string == prefix.string && this.offset == prefix.offset) {
            return true;
        }
        return this.string.regionMatches(this.offset, prefix.string, prefix.offset, lenToMatch);
    }
    ...
    public String debugString() {
        return this.getClass().getSimpleName() + ": \"" + String.valueOf(this) + "\"\nBacking string: \"" + this.string + "\"";
    }
}
