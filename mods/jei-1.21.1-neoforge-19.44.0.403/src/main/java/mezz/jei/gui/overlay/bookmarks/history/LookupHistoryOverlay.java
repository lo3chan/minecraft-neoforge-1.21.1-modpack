...
ILookupHistoryOverlay {
    ...
    public boolean isListDisplayed() {
        ...
        return this.clientConfig.lookupHistoryEnabled().getValue() != false && this.isDisplayedOnThisSide() && this.contents.hasRoom();
    }

    @Override
    public boolean isDisplayedOnThisSide() {
        return this.ownerDisplaySide.equals((Object)this.clientConfig.lookupHistoryDisplaySide().getValue());
    }
    ...
    public int getDisplayHeight() {
        return LookupHistoryOverlay.getDisplayHeight(this.clientConfig.maxLookupHistoryRows().getValue(), this.historyListConfig.drawBackground().getValue());
    }
    ...
    private void drawLine(GuiGraphics guiGraphics, ImmutableRect2i lineArea, int argbColor) {
        for (LineSegment segment : LookupHistoryOverlay.calculateLineSegments(lineArea, this.guiExclusionAreas)) {
            LookupHistoryOverlay.drawLineSegment(guiGraphics, segment.x1(), segment.x2(), lineArea.y(), lineArea.height(), argbColor);
        }
    }
    ...
    private static void drawLineSegment(GuiGraphics guiGraphics, int x1, int x2, int y, int height, int argbColor) {
        ...
        float floatInterval = (float)(availableWidth - 8) / (float)(dashCount - 1);
        for (int i = 0; i < dashCount; ++i) {
            float x = (float)x1 + (float)i * floatInterval;
            guiGraphics.fill((int)Mth.clamp((float)(x + 8.0f), (float)x1, (float)x2), y, (int)Mth.clamp((float)x, (float)x1, (float)x2), y + height, argbColor);
        }
    }
    ...
    static List<LineSegment> calculateLineSegments(ImmutableRect2i lineArea, Set<ImmutableRect2i> guiExclusionAreas) {
        ...
        List<LineSegment> blockedSegments = guiExclusionAreas.stream().filter(lineArea::intersects).map(exclusionArea -> new LineSegment(Math.max(lineArea.x(), exclusionArea.x()), Math.min(lineArea.x() + lineArea.width(), exclusionArea.x() + exclusionArea.width()))).filter(segment -> segment.x1() < segment.x2()).sorted(Comparator.comparingInt(LineSegment::x1)).toList();
    ...
    public void draw(Minecraft minecraft, GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListDisplayed()) {
            ...
            if (!this.historyListConfig.drawBackground().getValue().booleanValue()) {
    ...
    public void drawBackground(GuiGraphics guiGraphics) {
        if (this.isListDisplayed() && this.historyListConfig.drawBackground().getValue().booleanValue()) {
