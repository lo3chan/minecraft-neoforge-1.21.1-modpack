...
IClientToggleState.IEditModeListener {
    ...
    public IngredientFilter(IFilterTextSource filterTextSource, IClientConfig clientConfig, IIngredientFilterConfig config, IIngredientManager ingredientManager, Function<List<IListElementInfo<?>>, Comparator<IListElement<?>>> sortIndexUpdater, List<IListElementInfo<?>> ingredients, IModIdHelper modIdHelper, IIngredientVisibility ingredientVisibility, IColorHelper colorHelper, ISearchStorageBuilderFactory searchStorageBuilderFactory, IClientToggleState clientToggleState) {
        ...
        this.elementSearch = IngredientFilter.createElementSearch(clientConfig, this.elementPrefixParser, ingredients, ingredientManager);
    ...
    private static IElementSearch createElementSearch(IClientConfig clientConfig, ElementPrefixParser elementPrefixParser, List<IListElementInfo<?>> elementInfos, IIngredientManager ingredientManager) {
        if (clientConfig.lowMemorySlowSearchEnabled().getValue().booleanValue()) {
    ...
    public void rebuildItemFilter() {
        ...
        this.elementSearch = IngredientFilter.createElementSearch(this.clientConfig, this.elementPrefixParser, elementInfos, this.ingredientManager);
    ...
    private void updateDirtyState() {
        ...
        if (this.sortIndexesDirty) {
            List<IListElementInfo<?>> elementInfos = IngredientListElementFactory.rebuildList(this.ingredientManager, this.elementSearch.getAllIngredients(), this.modIdHelper);
            this.sortIndexUpdater.apply(elementInfos);
            this.sortIndexesDirty = false;
            this.invalidateCache();
        }
    ...
    public <V> void onIngredientsVisibilityChanged(Collection<ITypedIngredient<V>> ingredients, boolean visible) {
        ...
        for (ITypedIngredient<V> ingredient : ingredients) {
            ...
            IListElement<V> match = this.elementSearch.findElement(ingredient, ingredientHelper = this.ingredientManager.getIngredientHelper(ingredientType = ingredient.getType()));
            if (match == null || match.isVisible() == visible) continue;
            match.setVisible(visible);
            changed = true;
        }
        if (changed) {
    ...
    public <V> void onIngredientsAdded(IIngredientHelper<V> ingredientHelper, Collection<ITypedIngredient<V>> ingredients) {
        for (ITypedIngredient<V> value : ingredients) {
            ...
            if (matchingElement != null) {
                ...
                if (!DebugConfig.isDebugIngredientsEnabled()) continue;
    ...
    private SearchTokens parseSearchTokens(String filterText) {
        ...
        List<Token> tokens = this.searchTokenizer.tokenize(filterText);
