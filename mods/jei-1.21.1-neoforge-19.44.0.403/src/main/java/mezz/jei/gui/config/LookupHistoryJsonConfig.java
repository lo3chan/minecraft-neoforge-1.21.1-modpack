...
implements ILookupHistoryConfig {
    ...
    @Override
    public void save(IRecipeManager recipeManager, IIngredientManager ingredientManager, RegistryAccess registryAccess, ICodecHelper codecHelper, List<IBookmark> bookmarks, Codec<IBookmark> bookmarkCodec) {
        List<IBookmark> bookmarksSnapshot = List.copyOf(bookmarks);
        LookupHistoryJsonConfig.getPath(this.jeiConfigurationDir).ifPresent(path -> this.delayedSave.run(() -> this.save((Path)path, bookmarkCodec, registryAccess, (Collection<IBookmark>)bookmarksSnapshot)));
    }
    ...
    private void save(Path path, Codec<IBookmark> bookmarkCodec, RegistryAccess registryAccess, Collection<IBookmark> bookmarks) {
        ...
        try {
            JsonArrayFileHelper.write(path, 1, bookmarks, bookmarkCodec, registryOps, error -> LOGGER.error("Encountered an error when saving the lookup history config to file {}\n{}", (Object)path, error), (element, exception) -> LOGGER.error("Encountered an exception when saving the lookup history config to file {}\n{}", (Object)path, element, exception));
    ...
    @Override
    public List<IBookmark> load(IRecipeManager recipeManager, IIngredientManager ingredientManager, RegistryAccess registryAccess, ICodecHelper codecHelper, Codec<IBookmark> bookmarkCodec) {
        RegistryOps<JsonElement> registryOps = this.getRegistryOps(registryAccess);
