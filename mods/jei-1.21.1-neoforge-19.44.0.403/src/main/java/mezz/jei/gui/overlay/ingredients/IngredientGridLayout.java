...
public final class IngredientGridLayout {
    ...
    public static ImmutableSize2i calculateSize(IIngredientGridConfig config, ImmutableRect2i availableArea) {
        int columns = Math.min(availableArea.getWidth() / INGREDIENT_WIDTH, config.maxColumns().getValue());
        int rows = Math.min(availableArea.getHeight() / INGREDIENT_HEIGHT, config.maxRows().getValue());
    ...
    public static ImmutableRect2i calculateBounds(IIngredientGridConfig config, ImmutableRect2i availableArea) {
        ImmutableSize2i size = IngredientGridLayout.calculateSize(config, availableArea);
    ...
    public static int calculateAvailableSlotCount(ImmutableRect2i area, Set<ImmutableRect2i> exclusionAreas, @Nullable ImmutablePoint2i mouseExclusionPoint) {
        int blocked = 0;
        List<SlotLayout> slotLayouts = IngredientGridLayout.calculateSlots(area, exclusionAreas, mouseExclusionPoint, 0);
        for (SlotLayout slotLayout : slotLayouts) {
            if (!slotLayout.blocked()) continue;
            ++blocked;
        }
        return slotLayouts.size() - blocked;
    }

    public static List<SlotLayout> calculateSlots(ImmutableRect2i area, Set<ImmutableRect2i> exclusionAreas, @Nullable ImmutablePoint2i mouseExclusionPoint, int smoothScrollRowPixelOffset) {
        ArrayList<SlotLayout> slotLayouts = new ArrayList<SlotLayout>();
        int rowPixelOffset = Math.clamp((long)smoothScrollRowPixelOffset, (int)0, (int)(INGREDIENT_HEIGHT - 1));
        for (int y = area.getY() - rowPixelOffset; y < area.getY() + area.getHeight(); y += INGREDIENT_HEIGHT) {
            for (int x = area.getX(); x < area.getX() + area.getWidth(); x += INGREDIENT_WIDTH) {
                ...
                slotLayouts.add(new SlotLayout(slotArea, IngredientGridLayout.isSlotBlocked(slotArea, exclusionAreas, mouseExclusionPoint)));
            }
        }
        return slotLayouts;
    }

    private static boolean isSlotBlocked(ImmutableRect2i stackArea, Set<ImmutableRect2i> exclusionAreas, @Nullable ImmutablePoint2i mouseExclusionPoint) {
        return MathUtil.intersects(exclusionAreas, stackArea.expandBy(2)) || mouseExclusionPoint != null && stackArea.contains(mouseExclusionPoint);
    }
