...
implements IElementSearch {
    ...
    public Set<IListElement<?>> getSearchResults(ElementPrefixParser.TokenInfo tokenInfo) {
        ...
        return this.elementInfoList.stream().filter(elementInfo -> ElementSearchLowMem.matches(token, prefixInfo, elementInfo)).map(IListElementInfo::getElement).collect(Collectors.toSet());
    }
    ...
    @Override
    @Nullable
    public <T> IListElement<T> findElement(ITypedIngredient<T> typedIngredient, IIngredientHelper<T> ingredientHelper) {
        T ingredient = typedIngredient.getIngredient();
        IIngredientType<T> type = typedIngredient.getType();
        Function<ITypedIngredient<ITypedIngredient<T>>, Object> uidFunction = i -> ingredientHelper.getUid((ITypedIngredient)i, UidContext.Ingredient);
        ...
        ElementPrefixParser.TokenInfo tokenInfo = new ElementPrefixParser.TokenInfo(lowercaseDisplayName, this.noPrefix);
        PrefixInfo<IListElementInfo<?>, IListElement<?>> prefixInfo = tokenInfo.prefixInfo();
        for (IListElementInfo<?> elementInfo : this.elementInfoList) {
            IListElement<?> element;
            IListElement<T> match;
            if (!ElementSearchLowMem.matches(lowercaseDisplayName, prefixInfo, elementInfo) || (match = ElementSearchLowMem.checkForMatch(element = elementInfo.getElement(), type, ingredientUid, uidFunction)) == null) continue;
            return match;
        }
        return null;
    }
    ...
    private static <T> IListElement<T> checkForMatch(IListElement<?> element, IIngredientType<T> ingredientType, Object uid, Function<ITypedIngredient<T>, Object> uidFunction) {
        IListElement<T> cast = ElementSearchLowMem.optionalCast(element, ingredientType);
        if (cast == null) {
            return null;
        }
    ...
    public void logStatistics() {
        LOGGER.info("ElementSearchLowMem Element Count: {}", (Object)this.elementInfoList.size());
    }
}
