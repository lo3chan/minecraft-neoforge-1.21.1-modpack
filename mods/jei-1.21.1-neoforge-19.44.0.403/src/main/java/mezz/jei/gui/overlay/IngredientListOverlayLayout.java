...
class IngredientListOverlayLayout {
    ...
    IngredientListOverlayLayout() {
    }

    static Layout calculate(IGuiProperties guiProperties, boolean centerSearchBarEnabled, boolean lookupHistoryEnabled, boolean lookupHistoryDisplayedOnThisSide, int lookupHistoryDisplayHeight) {
        ImmutableRect2i displayArea = IngredientListOverlayLayout.createDisplayArea(guiProperties);
        boolean searchBarCentered = IngredientListOverlayLayout.isSearchBarCentered(centerSearchBarEnabled, guiProperties);
        ImmutableRect2i availableContentsArea = IngredientListOverlayLayout.getAvailableContentsArea(displayArea, searchBarCentered);
        ...
        if (lookupHistoryEnabled && lookupHistoryDisplayedOnThisSide && lookupHistoryDisplayHeight > 0) {
            ImmutableRect2i area = IngredientListOverlayLayout.getLookupHistoryArea(displayArea, searchBarCentered, lookupHistoryDisplayHeight);
            availableContentsArea = IngredientListOverlayLayout.cropBottomTo(availableContentsArea, area.y() - 4);
    ...
    private static ImmutableRect2i cropBottomTo(ImmutableRect2i area, int bottomY) {
        int cropAmount = IngredientListOverlayLayout.getBottom(area) - bottomY;
        if (cropAmount <= 0) {
            return area;
        }
    ...
    record Layout(IGuiProperties guiProperties, ImmutableRect2i displayArea, ImmutableRect2i availableContentsArea, Optional<ImmutableRect2i> lookupHistoryArea, boolean searchBarCentered) {
        ...
        private ImmutableRect2i getSearchAndConfigArea(boolean contentsHasRoom, ImmutableRect2i contentsArea) {
            ...
            if (this.searchBarCentered) {
                return insetDisplayArea.keepBottom(20).matchWidthAndX(new ImmutableRect2i(this.guiProperties.guiLeft(), this.guiProperties.guiTop(), this.guiProperties.guiXSize(), this.guiProperties.guiYSize()));
            }
            if (contentsHasRoom) {
                return insetDisplayArea.keepBottom(20).matchWidthAndX(contentsArea);
            }
