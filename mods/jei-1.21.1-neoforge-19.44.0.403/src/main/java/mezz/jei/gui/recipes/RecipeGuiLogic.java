...
implements IRecipeGuiLogic {
    ...
    public RecipeGuiLogic(IRecipeManager recipeManager, IIngredientManager ingredientManager, LookupHistory lookupHistory, IRecipeTransferManager recipeTransferManager, IRecipeLogicStateListener stateListener, IFocusFactory focusFactory, BookmarkFactory bookmarkFactory) {
    ...
    public boolean showRecipes(IFocusedRecipes<?> focusedRecipes, IFocusGroup focuses) {
        RecipeBookmark<?, ?> recipeBookmark = RecipeGuiLogic.createRecipeBookmark(this.recipeManager, this.ingredientManager, focusedRecipes, focuses);
        ...
        SingleCategoryLookupState state = new SingleCategoryLookupState(focusedRecipes, focuses);
        return this.setState(state, true);
    }

    @Nullable
    private static <T> RecipeBookmark<T, ?> createRecipeBookmark(IRecipeManager recipeManager, IIngredientManager ingredientManager, IFocusedRecipes<T> focusedRecipes, IFocusGroup focusGroup) {
        IRecipeCategory<T> recipeCategory = focusedRecipes.getRecipeCategory();
        List<T> recipes = focusedRecipes.getRecipes();
        if (recipes.size() != 1) {
            return null;
        }
        Object recipe = recipes.getFirst();
        return recipeManager.createRecipeLayoutDrawable(recipeCategory, recipe, focusGroup).map(drawable -> RecipeBookmark.create(drawable, ingredientManager)).orElse(null);
    }
    ...
    public boolean showAllRecipes() {
        ...
        ILookupState state = IngredientLookupState.create(this.recipeManager, this.focusFactory.getEmptyFocusGroup(), recipeCategories, this.recipeTransferManager);
    ...
    @Override
    public List<IRecipeLayoutWithButtons<?>> getVisibleRecipeLayoutsWithButtons(int availableHeight, int minRecipePadding, @Nullable AbstractContainerMenu container, BookmarkList bookmarkList, RecipesGui recipesGui) {
        int containerId;
        IRecipeCategory<?> recipeCategory = this.getSelectedRecipeCategory();
        IJeiClientConfigs jeiClientConfigs = Internal.getJeiClientConfigs();
        IClientConfig clientConfig = jeiClientConfigs.getClientConfig();
        Set<RecipeSorterStage> recipeSorterStages = RecipeSorterStage.getEnabled(clientConfig);
        int n = containerId = container == null ? -1 : container.containerId;
        if (!recipeSorterStages.equals(this.cachedSorterStages) || this.cachedRecipeLayoutsWithButtons == null || this.cachedRecipeCategory != recipeCategory || this.cachedContainerId != containerId) {
            IFocusedRecipes<?> focusedRecipes = this.state.getFocusedRecipes();
            this.cachedRecipeLayoutsWithButtons = IRecipeLayoutList.create(recipeSorterStages, container, focusedRecipes, this.state.getFocuses(), bookmarkList, this.recipeManager, recipesGui);
        ...
        int recipeHeight = this.cachedRecipeLayoutsWithButtons.findFirst().map(IRecipeLayoutWithButtons::getRecipeLayout).map(IRecipeLayoutDrawable::getRectWithBorder).map(Rect2i::getHeight).orElseGet(recipeCategory::getHeight);
