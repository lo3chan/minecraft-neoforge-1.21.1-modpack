...
implements IIngredientManager {
    ...
    private final List<IIngredientManager.IIngredientListener> listeners = new ArrayList<IIngredientManager.IIngredientListener>();
    ...
    @Override
    public <V> @Unmodifiable Collection<ITypedIngredient<V>> getAllTypedIngredients(IIngredientType<V> ingredientType) {
        ErrorUtil.checkNotNull(ingredientType, "ingredientType");
        return this.registeredIngredients.getIngredientInfo(ingredientType).getAllTypedIngredients();
    }
    ...
    public <V> void addIngredientsAtRuntime(IIngredientType<V> ingredientType, Collection<V> ingredients) {
        ...
        if (LOGGER.isDebugEnabled()) {
            String ingredientStrings = ingredients.stream().map(ingredientHelper::getResourceLocation).map(ResourceLocation::toString).collect(Collectors.joining(", ", "[", "]"));
            LOGGER.debug("Ingredients added at runtime: {}", (Object)ingredientStrings);
        }
        ArrayList validTypedIngredients = new ArrayList(ingredients.size());
        for (V ingredient : ingredients) {
            ...
            if (typedIngredient == null) {
                LOGGER.warn("Attempted to add an invalid ingredient at runtime: {}", (Object)ingredientHelper.getErrorInfo(ingredient));
                continue;
            }
