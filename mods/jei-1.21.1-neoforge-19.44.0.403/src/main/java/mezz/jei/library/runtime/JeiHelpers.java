...
implements IJeiHelpers {
    ...
    public <T> Optional<RecipeType<T>> getRecipeType(ResourceLocation uid, Class<? extends T> recipeClass) {
        return Optional.ofNullable(this.recipeCategories).flatMap(r -> r.stream().map(IRecipeCategory::getRecipeType).filter(t -> t.getUid().equals((Object)uid) && t.getRecipeClass().equals(recipeClass)).map(t -> {
            RecipeType cast = t;
            return cast;
        }).findFirst());
    }
    ...
    public Stream<RecipeType<?>> getAllRecipeTypes() {
        if (this.recipeCategories == null) {
            return Stream.of(new RecipeType[0]);
        }
