...
class Keys {
    static Key[] split(String key) {
        ArrayList<Key> splitKey = new ArrayList<Key>();
        ...
        for (int i = key.length() - 1; i > -1; --i) {
            ...
            if (Keys.isQuote(c) && (i == 0 || key.charAt(i - 1) != '\\')) {
                quoted = !quoted;
                indexable = false;
            }
            if (c != '.' || quoted) {
            ...
            splitKey.add(0, new Key(current.toString(), index, !splitKey.isEmpty() ? (Key)splitKey.get(0) : null));
            indexable = true;
            index = -1;
            current = new StringBuilder();
        }
    ...
    static class Key {
        ...
        Key(String name, int index, Key next) {
            this.name = name;
            this.index = index;
            this.path = next != null ? name + "." + next.path : name;
        }
    }
}
